...
const { OrganizationalUnit, OrganizationalUnitKpiSet, AllocationUnitResult, EmployeeKpiSet, Task, TaskTemplate } = Models;
...
const getAllocationResultUnitKpi = async (portal, currentUserUnitId) => {
  const result = await rabbitMq.gRPC(
    'kpiService.organizationalUnit.getAllocationResultUnitKpi',
    JSON.stringify({ portal, currentUserUnitId }),
    listRpcQueue.KPI_SERVICE
  );
