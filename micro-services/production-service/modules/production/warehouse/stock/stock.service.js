...
exports.importStocks = async (portal, data) => {
    if (data?.length) {
        const dataLength = data.length;
        for (let i = 0; i < dataLength; i++) {
            await Stock(connect(DB_CONNECTION, portal)).create({
                ...
                name: data[i].name,
                organizationalUnit: data[i].organizationalUnitValue,
                status: data[i].status,
                managementLocation: [{
                    role: '62a8092b5387be19102a8e18',
