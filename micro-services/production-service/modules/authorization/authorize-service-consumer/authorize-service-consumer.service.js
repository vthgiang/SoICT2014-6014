 ...
 */
const authorizeExternalServiceConsumer = async(
  portal,
  ...
  if (
    !internalServiceIdentity.data ||
    internalServiceIdentity.data.length == 0
  ) {
    return false;
  }
   ...
   */
  const isServiceConsumerSupportedByInternalService = (
    condition,
    consumerAttributes,
  ) => {
    if (condition.and) {
      return (condition.and).every((c) =>
        isServiceConsumerSupportedByInternalService(c, consumerAttributes),
      );
    }
  ...
  const externalPolicies =
    internalServiceIdentity.data[0].externalPolicies.filter((policy) =>
      isServiceConsumerSupportedByInternalService(
        policy.condition,
        externalServiceConsumer.attributes,
      ),
    );
  ...
  return externalPolicies.some(
    (policy) =>
      ...
      policy.actions.includes(action) && policy.enabled
  );
}

const authorizeInternalServiceIdentity = (
  ...
  return (
    ...
    internalPolicies.every((policy) => policy.effect === 'Allow')
  );
}
...
exports.authorizeServiceConsumer = async(portal, authorizerServiceAccount) => {
  ...
  const externalServiceConsumer =
    await ExternalServiceConsumerService.findAll(portal, {
      clientId,
      clientSecret,
    });
  ...
  if (externalServiceConsumer.data.length == 1) {
    const result = await authorizeExternalServiceConsumer(
      portal,
      externalServiceConsumer.data[0],
      resource,
      action,
    );
  ...
  if (data.length == 1) {
    const result = authorizeInternalServiceIdentity(
      data[0],
      resource,
      action,
    );
    ...
    LoggingService.create(
      portal,
      'AUTHORIZE_INTERNAL_SERVICE_IDENTITY',
      result ? 'SUCCESSFUL' : 'FAILED',
      data[0].name,
      result,
    );
