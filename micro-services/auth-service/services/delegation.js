...
const PolicyService = require('../services/super-admin-policy');
...
exports.createDelegation = async (portal, data, logs = []) => {
    ...
    const filterValidDelegationArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                const checkDelegationCreated = await Delegation(connect(getDbConnection(), portal)).findOne({ delegationName: array[i].delegationName }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    if (delArray && delArray.length !== 0) {
        for (let i = 0; i < delArray.length; i++) {
            let delegateRole = await Role(connect(getDbConnection(), portal)).findById({ _id: delArray[i].delegateRole });
            let delegatePrivileges = delArray[i].allPrivileges ? null : await Privilege(connect(getDbConnection(), portal)).find({ roleId: { $in: [delArray[i].delegateRole].concat(delegateRole.parents) }, resourceId: { $in: delArray[i].delegateLinks } })
            // console.log('delegatePrivileges', delegatePrivileges)
            let checkDelegationExist = await Delegation(connect(getDbConnection(), portal)).find({
            ...
            let checkUserHaveRole = await UserRole(connect(getDbConnection(), portal)).find({
            ...
            if (delArray[i].notCheck || (checkDelegationExist.length == 0 && checkUserHaveRole.length == 0)) {
                ...
                if (await this.checkDelegationPolicy(delArray[i].delegatePolicy, delArray[i].delegator, delArray[i].delegateRole, delArray[i].delegateLinks, delArray[i].delegatee, portal)) {
                    newDelegation = await Delegation(connect(getDbConnection(), portal)).create({
    ...
    let delegation = await Delegation(connect(getDbConnection(), portal)).findById({ _id: newDelegation._id }).populate([
...
exports.getNewlyCreateDelegation = async (id, data, portal) => {
    ...
    const checkDelegationCreated = await Delegation(connect(getDbConnection(), portal)).findOne({ delegationName: data.delegationName }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
...
exports.autoRevokeDelegation = async (delegation, portal) => {
    ...
    const job = schedule.scheduleJob('Revoke_' + delegation._id, date, async function () {
        ...
        await delegation.save();
        // await Delegation(connect(getDbConnection(), portal)).updateOne({ _id: delegation._id }, {
        //     logs: [
        //         ...delegation.logs,
        //         {
        //             createdAt: new Date(),
...
exports.assignDelegation = async (newDelegation, portal) => {
    ...
    // add delegationId to selected Privileges
    let delegatePrivileges = newDelegation.delegatePrivileges == null || newDelegation.delegatePrivileges.length == 0 ? null : await Privilege(connect(getDbConnection(), portal)).find({ _id: { $in: newDelegation.delegatePrivileges } })
    ...
    let newUserRole = await UserRole(connect(getDbConnection(), portal)).create({
...
exports.updateMissedDelegation = async (portal) => {
    ...
    const allDelegations = await Delegation(connect(getDbConnection(), portal)).find({ status: { $in: ['pending', 'activated'] } });
...
exports.checkDelegationPolicy = async (policyId, delegatorId, delegatedObjectId, delegateLinksIds, delegateeId, portal) => {
    ...
    let policy = await Policy(connect(getDbConnection(), portal)).findById({ _id: policyId });
    let delegator = await User(connect(getDbConnection(), portal)).findById({ _id: delegatorId })
    let delegatee = await User(connect(getDbConnection(), portal)).findById({ _id: delegateeId })
    let delegateRole = await Role(connect(getDbConnection(), portal)).findById({ _id: delegatedObjectId })
    let delegateTask = await Task(connect(getDbConnection(), portal)).findById({ _id: delegatedObjectId })
    ...
    if (delegateLinksIds) {
        let delegateLinks = await Link(connect(getDbConnection(), portal)).find({ _id: { $in: delegateLinksIds } })
...
//     let delegateRole = await Role(connect(getDbConnection(), portal)).findById({ _id: delegateRoleId })
//     let delegateRoleAttributes = delegateRole.attributes

//     let canDelegateRole = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_delegate_role" })
//     let canBeDelegated = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_be_delegated" })
//     let canReceiveRoleDelegation = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_receive_role_delegation" })
...
//     }
//     else {
//         let canDelegateChosenRole = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_delegate_role_" + delegateRole.name })
//         // let canBeDelegatedByUser = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_be_delegated_by_" + delegator.email })
//         let canReceiveChosenRole = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "can_receive_role_" + delegateRole.name })
...
//                 let delegateLinks = await Link(connect(getDbConnection(), portal)).find({ _id: { $in: delegateLinksIds } })
//                 let delegatableLinkForChosenRole = await Attribute(connect(getDbConnection(), portal)).findOne({ attributeName: "delegatable_link_for_" + delegateRole.name })
...
exports.getDelegations = async (portal, data) => {
    ...
    let totalList = await Delegation(connect(getDbConnection(), portal)).countDocuments(keySearch);
    let delegations = await Delegation(connect(getDbConnection(), portal)).find(keySearch)
        .populate([
            { path: 'delegateRole', select: '_id name' },
            {
...
exports.getOnlyDelegationName = async (portal, data) => {
    ...
    let totalList = await Delegation(connect(getDbConnection(), portal)).countDocuments(keySearch);
    let DelegationCollection = await Delegation(connect(getDbConnection(), portal)).find(keySearch, { delegationName: 1 })
        .skip((page - 1) * perPage)
        .limit(perPage);
...
exports.getDelegationById = async (portal, id) => {
    let delegation = await Delegation(connect(getDbConnection(), portal)).findById({ _id: id }).populate([
        { path: 'delegateRole', select: '_id name' },
        {
            path: 'delegateTask', select: '_id name taskActions logs timesheetLogs',
            populate: [
...
exports.deleteDelegations = async (portal, delegationIds) => {
    ...
    let delegations = await Delegation(connect(getDbConnection(), portal))
        .deleteMany({ _id: { $in: delegationIds.map(item => mongoose.Types.ObjectId(item)) } });
    // delegationIds.forEach(async delegationId => {
    //     await UserRole(connect(getDbConnection(), portal)).deleteOne({ delegation: delegationId });
    // })
    return delegations;
}
...
exports.revokeDelegation = async (portal, delegationIds, reason) => {
    ...
    delegationIds.forEach(async delegationId => {
        ...
        await UserRole(connect(getDbConnection(), portal)).deleteMany({ delegation: delegationId });
    })

    let delegation = await Delegation(connect(getDbConnection(), portal)).find({ _id: { $in: delegationIds.map(item => mongoose.Types.ObjectId(item)) } });
    let result = delegation[0];
    if (result.delegatePrivileges != null) {
        let privileges = await Privilege(connect(getDbConnection(), portal)).find({ _id: { $in: result.delegatePrivileges } })
    ...
    let newDelegation = await Delegation(connect(getDbConnection(), portal)).findOne({ _id: delegationIds[0] }).populate([
        { path: 'delegateRole', select: '_id name' },
        { path: 'delegateTask' },
        { path: 'delegatee', select: '_id name' },
        { path: 'delegatePolicy', select: '_id policyName' },
...
exports.rejectDelegation = async (portal, delegationId, reason) => {
    ...
    let delegation = await Delegation(connect(getDbConnection(), portal)).findOne({ _id: delegationId });
    ...
    let newDelegation = await Delegation(connect(getDbConnection(), portal)).findOne({ _id: delegationId }).populate([
        { path: 'delegateRole', select: '_id name' },
        {
            path: 'delegateTask', select: '_id name taskActions logs timesheetLogs',
            populate: [
...
exports.saveLog = async (portal, delegation, userId, content, category, time) => {
    ...
    let newDelegation = await Delegation(connect(getDbConnection(), portal)).findOne({ _id: delegation._id }).populate([
        { path: 'delegateRole', select: '_id name' },
        {
            path: 'delegateTask', select: '_id name taskActions logs timesheetLogs',
            populate: [
...
exports.createTaskDelegation = async (portal, data, logs = []) => {
    ...
    if (delArray && delArray.length !== 0) {
        for (let i = 0; i < delArray.length; i++) {
            ...
            let checkDelegateTask = await Task(connect(getDbConnection(), portal)).findOne({
            ...
            let task = await Task(connect(getDbConnection(), portal)).findOne({ _id: newDelegation.delegateTask })
            await Task(connect(getDbConnection(), portal)).updateOne({ _id: newDelegation.delegateTask }, {
                delegations: [
                    ...task.delegations,
                    newDelegation._id
                ]
...
exports.assignTaskDelegation = async (newDelegation, portal) => {
    ...
    let delegateTask = await Task(connect(getDbConnection(), portal)).findOne({ _id: newDelegation.delegateTask }).populate({
    ...
    delegateTaskRoles.forEach(async r => {
        if (r == 'responsible') {
            await Task(connect(getDbConnection(), portal)).updateOne({ _id: delegateTask._id }, {
    ...
    if (!delegateTask.informedEmployees.includes(delegator)) {
        ...
        await Delegation(connect(getDbConnection(), portal)).updateOne({ _id: newDelegation._id }, {
...
exports.revokeTaskDelegation = async (portal, delegationIds, reason) => {
    ...
    let delegateTask = await Task(connect(getDbConnection(), portal)).findOne({ _id: result.delegateTask }).populate({
        path: 'delegations', select: '_id status delegatorHasInformed'
    })
...
exports.deleteTaskDelegation = async (portal, delegationIds) => {
    ...
    let task = await Task(connect(getDbConnection(), portal)).findOne({ _id: result.delegateTask })

    await Task(connect(getDbConnection(), portal)).updateOne({ _id: task._id }, {
        delegations: task.delegations.filter(d => d.toString() !== result._id.toString())
    })
    if (!result.delegatorHasInformed) {
        await Delegation(connect(getDbConnection(), portal)).updateMany({ delegateTask: result.delegateTask, delegator: result.delegator }, {
            delegatorHasInformed: false,
        })
    }

    let delegationDelete = await Delegation(connect(getDbConnection(), portal))
...
exports.deleteTaskDelegationWhenTaskIsDeleted = async (portal, taskId) => {
    ...
    if (task.delegations) {
        ...
        await Delegation(connect(getDbConnection(), portal)).deleteMany({ delegateTask: task._id })
...
exports.handleDelegatorLosesRole = async (portal, users, roles) => {
    ...
    if (Array.isArray(users)) {
        ...
        let checkDelegationUser = await Delegation(connect(getDbConnection(), portal)).find({ delegator: { $nin: users }, delegateRole: roles, status: { $in: ['pending', 'activated'] } }).populate([
            { path: 'delegateRole', select: '_id name' },
            {
                path: 'delegateTask', select: '_id name taskActions logs timesheetLogs',
                populate: [
    ...
    if (Array.isArray(roles)) {
        ...
        let checkDelegationRole = await Delegation(connect(getDbConnection(), portal)).find({ delegator: users, delegateRole: { $nin: roles }, status: { $in: ['pending', 'activated'] } }).populate([
            { path: 'delegateRole', select: '_id name' },
            {
                path: 'delegateTask', select: '_id name taskActions logs timesheetLogs',
                populate: [
