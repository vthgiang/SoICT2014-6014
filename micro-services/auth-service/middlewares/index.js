...
const { links } = require('../../helpers/servicesPermission');
...
const uploadBackupFiles = (options) => {
 ...
 * @param {*} next
 */
const authTrueOwner = async (req, res, next) => {
    try {
        const token = req.header('utk'); //JWT nhận từ người dùng
 ...
 * @param {*} next
 */
const authAdminSuperAdmin = async (req, res, next) => {
    try {
        const token = req.header('utk'); //JWT nhận từ người dùng
 ...
 * @returns
 */
const rateLimitRequest = (windowMs = 60, maxRequest = 100, message) => { // mặc định trong vòng 1 tiếng chỉ cho phép tối đa 100 request tới api
