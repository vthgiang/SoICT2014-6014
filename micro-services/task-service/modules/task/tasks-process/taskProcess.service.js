...
exports.getAllXmlDiagram = async (portal, query) => {
  ...
  let taskProcesses = [];
  let roleId = allRole.map(function (el) {
  ...
  await ProcessTemplate(connect(DB_CONNECTION, portal)).populate(taskProcesses, {
    path: 'tasks.organizationalUnit tasks.collaboratedWithOrganizationalUnits',
  });
  await ProcessTemplate(connect(DB_CONNECTION, portal)).populate(taskProcesses, {
    path: 'tasks.fastest.task',
    populate: [{ path: 'responsibleEmployees' }],
  });
  await ProcessTemplate(connect(DB_CONNECTION, portal)).populate(taskProcesses, {
    path: 'tasks.responsibleEmployees tasks.accountableEmployees tasks.consultedEmployees tasks.informedEmployees tasks.confirmedByEmployees tasks.creator',
...
exports.createXmlDiagram = async (portal, body) => {
  ...
  for (const x in body.info) {
    ...
    body.info[x].taskActions = body.info[x].taskActions
      ? body.info[x].taskActions.map((item) => {
          return {
            name: item.name,
      ...
      : [];
    body.info[x].taskInformations = body.info[x].taskInformations
      ? body.info[x].taskInformations.map((item, key) => {
          return {
  ...
  data = await ProcessTemplate(connect(DB_CONNECTION, portal))
    .findById(data._id)
    .populate([
...
exports.createTaskByProcess = async (portal, processId, body) => {
  ...
  for (let i in data) {
    ...
    let collaboratedWithOrganizationalUnits = data[i].collaboratedWithOrganizationalUnits.map((item) => {
      return { organizationalUnit: item };
    });
    ...
    newTaskItem = await Task(connect(DB_CONNECTION, portal)).create({
      ...
      confirmedByEmployees: data[i].responsibleEmployees
        .concat(data[i].accountableEmployees)
        .concat(data[i].consultedEmployees)
        .includes(data[i].creator)
        ? data[i].creator
        : [],
    });
  ...
  for (let i in dataProcess) {
    ...
    if (item) {
      await TaskProcess(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
        dataProcess[i].process._id,
        { $set: { codeInProcess: dataProcess[i].code, processTemplate: processChild._id } },
        { new: true }
      );
      ...
      await ProcessTemplate(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
        processChild._id,
        { $inc: { numberOfUse: 1 }, $push: { listProcess: newTaskProcess1._id } },
        { new: true }
      );
  ...
  for (let x in data) {
    ...
    for (let i in data[x].followingTasks) {
      ...
      if (item) {
        ...
        item = await TaskProcess(connect(DB_CONNECTION, portal)).findOne({
          processParent: taskProcessId,
          codeInProcess: data[x].followingTasks[i].task,
        });
        if (item) {
          if (item.status === 'inprocess') {
            listFollowingTask.push({
    ...
    for (let i in data[x].preceedingTasks) {
      ...
      if (item) {
        ...
        item = await TaskProcess(connect(DB_CONNECTION, portal)).findOne({
          processParent: taskProcessId,
          codeInProcess: data[x].preceedingTasks[i].task,
        });
        if (item) {
          listPreceedingTask.push({
            task: item._id,
  ...
  for (let x in dataProcess) {
    ...
    for (let i in dataProcess[x].followingTasks) {
      ...
      if (item) {
        ...
        item = await TaskProcess(connect(DB_CONNECTION, portal)).findOne({
          processParent: taskProcessId,
          codeInProcess: dataProcess[x].followingTasks[i].task,
        });
        if (item) {
          if (item.status === 'inprocess') {
            listFollowingProcess.push({
    ...
    for (let i in dataProcess[x].preceedingTasks) {
      let item = await Task(connect(DB_CONNECTION, portal)).findOne({
        process: taskProcessId,
        codeInProcess: dataProcess[x].preceedingTasks[i].task,
      });
      if (item) {
  ...
  if (processId !== 'undefined') {
    await ProcessTemplate(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
      processId,
      { $inc: { numberOfUse: 1 }, $push: { listProcess: taskProcessId } },
      { new: true }
    );
  }
  let newProcess = await TaskProcess(connect(DB_CONNECTION, portal))
    .findByIdAndUpdate(taskProcessId, { $set: { tasks: listTask, processChilds: listProcess } }, { new: true })
    .populate([
  ...
  let myProcess = await ProcessTemplate(connect(DB_CONNECTION, portal))
    .find()
    .populate([
      { path: 'creator', select: 'name' },
      { path: 'viewer', select: 'name' },
...
exports.getAllTaskProcess = async (portal, query) => {
  ...
  let data = await TaskProcess(connect(DB_CONNECTION, portal))
    .find({
      processName: { $regex: name, $options: 'i' },
      status: { $ne: 'finished' },
      $or: [{ viewer: { $in: [userId] } }, { manager: { $in: [userId] } }, { viewer: { $in: [currentRole] } }, { manager: { $in: [currentRole] } }],
    })
...
exports.getAllTaskProcess = async (portal, query) => {
  ...
  let totalCount = await TaskProcess(connect(DB_CONNECTION, portal)).countDocuments({
    processName: { $regex: name, $options: 'i' },
    $or: [{ viewer: { $in: [userId] } }, { manager: { $in: [userId] } }],
  });
  let totalPages = Math.ceil(totalCount / noResultsPerPage);
  return {
    data: data,
...
exports.updateDiagram = async (portal, params, body) => {
  let diagram = await TaskProcess(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
    params.processId,
  ...
  if (officeHours.length !== 0) {
    ...
    officeHours.forEach((value) => {
      ...
      if (day2.getHours() * 60 + day2.getMinutes() < arrayAdministrativeStartTime[0] * 60 + parseInt(arrayAdministrativeStartTime[1])) {
        ...
        totalTime1 =
          totalTime1 + (arrayAdministrativeEndTime[0] * 60 + parseInt(arrayAdministrativeEndTime[1]) - day2.getHours() * 60 - day2.getMinutes()) / 60;
      }
      if (day1.getHours() * 60 + day1.getMinutes() < arrayAdministrativeStartTime[0] * 60 + parseInt(arrayAdministrativeStartTime[1])) {
        ...
        totalTime2 =
          totalTime2 +
          (day1.getHours() * 60 + day1.getMinutes() - arrayAdministrativeStartTime[0] * 60 - parseInt(arrayAdministrativeStartTime[1])) / 60;
      } else {
        totalTime2 =
          totalTime2 +
          (arrayAdministrativeEndTime[0] * 60 +
...
exports.editProcessInfo = async (portal, params, body) => {
  ...
  // if (body)
  if (body.start) {
    const item = await TaskProcess(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
      body.id,
      {
        $set: {
          status: 'inprocess',
    ...
    const item = await TaskProcess(connect(DB_CONNECTION, portal))
      .findByIdAndUpdate(
        body.id,
        {
          $set: {
    ...
    if (item.status === 'not_initialized') {
      ...
      for (let i in array) {
        ...
        let collaboratedWithOrganizationalUnits = array[i].collaboratedWithOrganizationalUnits.map((item) => {
          return { organizationalUnit: item };
        });
        let process = taskProcessId;
        newTaskItem = await Task(connect(DB_CONNECTION, portal)).create({
          ...
          confirmedByEmployees: array[i].responsibleEmployees
            .concat(array[i].accountableEmployees)
            .concat(array[i].consultedEmployees)
            .includes(array[i].creator)
            ? array[i].creator
            : [],
        });
      ...
      if (arrayProcess) {
        for (let i in arrayProcess) {
          ...
          if (item) {
            await TaskProcess(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
              arrayProcess[i].process._id,
              { $set: { codeInProcess: arrayProcess[i].code, processTemplate: processChild._id } },
              { new: true }
            );
            listProcess.push(item._id);
          } else {
      ...
      for (let x in array) {
        ...
        for (let i in array[x].followingTasks) {
          ...
          if (item) {
            ...
            item = await TaskProcess(connect(DB_CONNECTION, portal)).findOne({
              processParent: taskProcessId,
              codeInProcess: array[x].followingTasks[i].task,
            });
            if (item) {
              if (item.status === 'inprocess') {
                listFollowingTask.push({
        ...
        for (let i in array[x].preceedingTasks) {
          ...
          if (item) {
            ...
            item = await TaskProcess(connect(DB_CONNECTION, portal)).findOne({
              processParent: taskProcessId,
              codeInProcess: array[x].preceedingTasks[i].task,
            });
            if (item) {
              listPreceedingTask.push({
                task: item._id,
      ...
      if (arrayProcess) {
        for (let x in arrayProcess) {
          ...
          for (let i in arrayProcess[x].followingTasks) {
            ...
            let item = await Task(connect(DB_CONNECTION, portal)).findOne({
              process: taskProcessId,
              codeInProcess: arrayProcess[x].followingTasks[i].task,
            });
            if (item) {
              if (item.status === 'inprocess') {
                listFollowingProcess.push({
          ...
          for (let i in arrayProcess[x].preceedingTasks) {
            let item = await Task(connect(DB_CONNECTION, portal)).findOne({
              process: taskProcessId,
              codeInProcess: arrayProcess[x].preceedingTasks[i].task,
            });
            if (item) {
              listPreceedingProcess.push({
                task: item._id,
      ...
      array.forEach((data) => {
        ...
        Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
          data._id,
          {
            $set: {
              name: data.name,
    ...
    for (let k in data[i].tasks) {
      ...
      for (let j = 0; j < data[i].tasks[k].collaboratedWithOrganizationalUnits.length; j++) {
        ...
        if (data[i].tasks[k].collaboratedWithOrganizationalUnits[j].trim() !== 0) {
          let collaborateItem = await OrganizationalUnit(connect(DB_CONNECTION, portal)).findOne({
            name: data[i].tasks[k].collaboratedWithOrganizationalUnits[j].trim(),
          });
...
exports.getListUserProgressTask = async (currentRole, month, year, portal) => {
  const currentUnit = await OrganizationalUnit(connect(DB_CONNECTION, portal)).findOne({
    $or: [{ managers: currentRole }, { deputyManagers: currentRole }, { employees: currentRole }],
  });
  const startOfMonth = new Date(year, month - 1, 1);
  const endOfMonth = new Date(year, month, 0, 23, 59, 59, 999);

  const buildAggregationPipeline = (groupByField) => {
    const startOfMonth = new Date(year, month - 1, 1);
    const endOfMonth = new Date(year, month, 0, 23, 59, 59, 999);
    ...
    const pipeline = [
      {
        $match: {
          organizationalUnit: currentUnit._id,
          createdAt: {
            $gte: startOfMonth,
            $lte: endOfMonth,
          },
    ...
    if (groupByField === 'responsibleEmployeesDetails.name') {
      pipeline.push(
        {
          $lookup: {
            ...
            foreignField: '_id',
            as: 'responsibleEmployeesDetails',
          },
        },
        {
          $unwind: '$responsibleEmployeesDetails', // Deconstruct the responsibleEmployeesDetails array
        }
      );
    }
  ...
  // Build and execute the aggregation pipeline for grouping by responsible employee name
  const pipelineByEmployeeName = buildAggregationPipeline('responsibleEmployeesDetails.name');
  const tasksGroupedByEmployeeName = await Task(connect(DB_CONNECTION, portal)).aggregate(pipelineByEmployeeName);

  // Build and execute the aggregation pipeline for grouping by task name
  const pipelineByTaskName = buildAggregationPipeline('name');
  const tasksGroupedByTaskName = await Task(connect(DB_CONNECTION, portal)).aggregate(pipelineByTaskName);

  return { tasksGroupedByEmployeeName, tasksGroupedByTaskName };
};
