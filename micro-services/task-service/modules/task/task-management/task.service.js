  ...
  for (let i in taskId) {
    ...
    if (allowDelete) {
      if (tasks.taskTemplate !== null) {
        await TaskTemplate(connect(DB_CONNECTION, portal)).findByIdAndUpdate(tasks.taskTemplate, { $inc: { numberOfUse: -1 } }, { new: true });
      }
  ...
  if (userId) {
    let tsl = await Task(connect(DB_CONNECTION, portal)).aggregate([
      ...
      { $unwind: '$timesheetLogs' },
      {
        $replaceRoot: {
          newRoot: {
            $mergeObjects: [{ _id: '$_id', name: '$name', actionDescription: '$actionDescription', actionId: '$actionId' }, '$timesheetLogs'],
          },
        },
      },
      {
...
exports.getAllUserTimeSheetLog = async (portal, month, year, rowLimit, page, timeLimit, unitArray, sortType) => {
  ...
  listEmployee.docs = listEmployee.docs.filter((employee) => {
    return employee.totalDuration[1] + employee.totalDuration[2] + employee.totalDuration[3] >= 60 * 60 * 1000 * timeLimit && employee.active == true;
  });
                    ...
                    if (criteria === CRITERIA.COEFFICIENT) {
                      ...
                      if (
                        ...
                        dataSumPointAndCoefficient[task.organizationalUnit._id].sumAutomaticPointNotCoefficient =
                          dataSumPointAndCoefficient?.[task?.organizationalUnit?._id]?.sumAutomaticPointNotCoefficient + result?.automaticPoint;
                      ...
                      if (
                        ...
                        dataSumPointAndCoefficient[task.organizationalUnit._id].sumEmployeePointNotCoefficient =
                          dataSumPointAndCoefficient?.[task?.organizationalUnit._id]?.sumEmployeePointNotCoefficient + result?.employeePoint;
                      ...
                      if (
                        ...
                        dataSumPointAndCoefficient[task.organizationalUnit._id].sumApprovedPointNotCoefficient =
                          dataSumPointAndCoefficient?.[task?.organizationalUnit?._id]?.sumApprovedPointNotCoefficient + result?.approvedPoint;
