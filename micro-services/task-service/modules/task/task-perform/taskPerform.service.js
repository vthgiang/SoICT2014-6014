...
exports.getTaskById = async (portal, id, userId, thirdParty = false) => {
  let task = await Task(connect(DB_CONNECTION, portal))
    ...
    .populate([
      ...
      {
        path: 'responsibleEmployees accountableEmployees consultedEmployees informedEmployees confirmedByEmployees creator inactiveEmployees',
  ...
  const subTasks = await Task(connect(DB_CONNECTION, portal))
    .find({
      parent: id,
    })
    .sort('createdAt');
  ...
  if (!flag) {
    ...
    for (let i = 0; i < listRoleLength; i++) {
      let tr = await OrganizationalUnitService.getChildrenOfOrganizationalUnitsAsTree(portal, listRole[i]);
      ...
      if (!flag) {
        for (let j = 0; j < tree.length; j++) {
          if (tree[j].managers.indexOf(rol) !== -1) {
            let v = tree[j];
            let f = _checkManagers(v, task.organizationalUnit._id);
            ...
            if (!f) {
              ...
              for (let k = 0; k < task.collaboratedWithOrganizationalUnits.length; k++) {
                if (!f && task.collaboratedWithOrganizationalUnits[k] && task.collaboratedWithOrganizationalUnits[k].organizationalUnit) {
                  f = _checkManagers(v, task.collaboratedWithOrganizationalUnits[k].organizationalUnit._id);
                }
              }
            }
            if (f === 1) {
...
exports.getTaskTimesheetLogs = async (portal, params) => {
  let timesheetLogs = await Task(connect(DB_CONNECTION, portal))
    ...
    .populate([
      { path: 'timesheetLogs.creator', select: '_id name email avatar' },
      { path: 'timesheetLogs.delegator', select: '_id name' },
    ]);
...
exports.getActiveTimesheetLog = async (portal, query) => {
  ...
  if (timerStatus !== null) {
    timerStatus.timesheetLogs = timerStatus.timesheetLogs.find((element) => !element.stoppedAt && element.creator == query.userId);
...
exports.startTimesheetLog = async (portal, params, body, user) => {
  const now = new Date();
  const taskDelegate = await Task(connect(DB_CONNECTION, portal))
  ...
  if (timerStatus) throw ['timer_exist_another_task'];
  ...
  if (body.overrideTSLog === 'yes') {
    ...
    let newTotalHoursSpent = sumBy(result.timesheetLogs, function (o) {
      return o.duration;
    });
    ...
    result.timesheetLogs.reduce(function (res, value) {
      ...
      if (value.duration) res[value.creator].hoursSpent += value.duration;
      return res;
    }, {});
  ...
  timer.timesheetLogs = timer.timesheetLogs.find((element) => !element.stoppedAt && element.creator == body.creator);
...
exports.stopTimesheetLog = async (portal, params, body, user) => {
  ...
  if (body.type && body.type === 'cancel') {
    const cancelTimer = await Task(connect(DB_CONNECTION, portal))
      .findOneAndUpdate(
    ...
    if (body.addlogStartedAt && body.addlogStoppedAt) {
      ...
      timer = await Task(connect(DB_CONNECTION, portal))
        .findByIdAndUpdate(params.taskId, { $push: { timesheetLogs: addLogTime } }, { new: true })
      ...
      const getTime = await Task(connect(DB_CONNECTION, portal)).findOne(
      ...
      await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
        { _id: params.taskId, 'taskActions._id': body.taskActionStartTimer },
        {
          $push: {
...
exports.createCommentOfTaskAction = async (portal, params, body, files, user) => {
  let commentOfTaskAction = await Task(connect(DB_CONNECTION, portal))
    .findOneAndUpdate(
  ...
  if (userReceive && userReceive.length > 0) NotificationServices.createNotification(portal, user.company._id, data);
  return taskActions;
};
/**
...
exports.editCommentOfTaskAction = async (portal, params, body, files) => {
  const now = new Date();
  const action = await Task(connect(DB_CONNECTION, portal))
    .findOneAndUpdate(
      {
        _id: params.taskId,
...
exports.deleteCommentOfTaskAction = async (portal, params) => {
  ...
  let action = await Task(connect(DB_CONNECTION, portal))
    .findOneAndUpdate(
      {
        _id: params.taskId,
        'taskActions._id': params.actionId,
...
exports.createTaskComment = async (portal, params, body, files, user) => {
  ...
  const taskComment = await Task(connect(DB_CONNECTION, portal))
    .findByIdAndUpdate(
      params.taskId,
      {
        $push: {
  ...
  const resEmployees =
    taskComment.responsibleEmployees &&
    taskComment.responsibleEmployees.map((o) => {
      return { _id: o._id, email: o.email };
    });
  const accEmployees =
    taskComment.accountableEmployees &&
    taskComment.accountableEmployees.map((o) => {
      return { _id: o._id, email: o.email };
    });
  ...
  if (emailReceive && emailReceive.length > 0) {
    sendEmail(emailReceive, taskComment.name, '', emailContent);
  }
...
exports.deleteTaskComment = async (portal, params) => {
  ...
  const comment = await Task(connect(DB_CONNECTION, portal))
    .findOneAndUpdate(
      { _id: params.taskId, 'taskComments._id': params.commentId },
...
exports.evaluationAction = async (portal, params, body) => {
  ...
  if (danhgia.length === 0) {
    ...
    await Task(connect(DB_CONNECTION, portal)).updateOne(
      ...
      {
        arrayFilters: [{ 'elem.creator': body.creator, 'elem.role': body.role }, { 'item._id': params.actionId }],
      }
    );
  }
  ...
  for (let i = 0; i < taskActionLength; i++) {
    ...
    if (danhgia.length === 0) {
      ...
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        ...
        {
          arrayFilters: [{ 'elem.creator': userId, 'elem.role': body[i].role }, { 'item._id': body[i].actionId }],
        }
      );
    }
...
exports.addTaskLog = async (portal, taskId, body) => {
  ...
  await Task(connect(DB_CONNECTION, portal))
    .updateOne({ _id: taskId }, { $push: { logs: log } }, { new: true })
    .populate([
        ...
        if (temp?.length > 0) {
          temp.map((item) => {
            if (item?.value && item?.target === 'Point' && item?.value !== eva?.approvedPoint) {
              ...
              descriptionLog =
                descriptionLog +
                '<span>Phần trăm đóng góp mới của ' +
                eva?.employee?.name +
                ' với vai trò ' +
                eva?.role +
                ' là: ' +
...
async function checkEvaluations(portal, date, taskId, storeDate) {
  ...
  if (initTask.evaluations.length === 0) {
    ...
    let chk = initTask.evaluations.find((e) => monthOfParams === e.evaluatingMonth.getMonth() && yearOfParams === e.evaluatingMonth.getFullYear());
    if (!chk) {
  ...
  // TH2: Có evaluation nhưng chưa có tháng giống với evaluatingMonth => tạo mới
    ...
    evaluateId = taskV2.evaluations.find(
      (e) => monthOfParams === e.evaluatingMonth.getMonth() && yearOfParams === e.evaluatingMonth.getFullYear()
    )._id;
  }
...
exports.getIdEvaluationOfThisMonth = async (portal, taskId) => {
  ...
  if (initTask.evaluations.length === 0) {
    ...
    let chk = initTask.evaluations.find(
      (e) =>
        monthOfParams === e.evaluatingMonth && e.evaluatingMonth.getMonth() && yearOfParams === e.evaluatingMonth && e.evaluatingMonth.getFullYear()
    );
    if (!chk) {
      // có evaluate nhưng k có tháng này
      testCase = 'TH2';
...
exports.editTaskByAccountableEmployees = async (portal, data, taskId) => {
  ...
  let newCollab = [];
  let oldCollab = taskItem.collaboratedWithOrganizationalUnits.map((e) => {
    return e.organizationalUnit;
  });
  ...
  let deletedInfoItems = task.taskInformations
    .filter(
      (x) => listInfoTask?.length > 0 && listInfoTask.find((e) => String(e._id) === String(x._id)) === undefined // không tìm được phần tử nào có id giống với x._id, tức là x là deleted info
    )
    .map((el) => el._id);
  ...
  let managersOfDeletedCollabID = [],
    ...
    deletedCollabHtml,
    deletedCollabEmail,
    additionalCollabHtml,
    additionalCollabEmail;
  ...
  return {
    ...
    user: user,
    tasks: tasks,
    deletedCollabEmail: deletedCollabEmail,
    deletedCollabHtml: deletedCollabHtml,
    managersOfDeletedCollab: managersOfDeletedCollab.map((item) => item.userId && item.userId._id),

    additionalCollabEmail: additionalCollabEmail,
    additionalCollabHtml: additionalCollabHtml,
...
exports.editEmployeeCollaboratedWithOrganizationalUnits = async (portal, taskId, data) => {
  ...
  task = await Task(connect(DB_CONNECTION, portal)).findById(taskId).populate({ path: 'collaboratedWithOrganizationalUnits.organizationalUnit' });
  ...
  if (oldAccountableEmployees?.length > 0 && task?.accountableEmployees) {
    for (let i = task.accountableEmployees.length - 1; i >= 0; i--) {
      if (oldAccountableEmployees.includes(task.accountableEmployees[i].toString())) {
  ...
  task.accountableEmployees = task.accountableEmployees.concat(accountableEmployees);
  ...
  html =
    ...
    '<p>Người thực hiện</p> ' +
    `<ul>${newTask.responsibleEmployees
      .map((item) => {
        return `<li>${item.name} - ${item.email}</li>`;
      })
      .join('')}
    ...
    '<p>Người phê duyệt</p> ' +
    `<ul>${newTask.accountableEmployees
      .map((item) => {
        return `<li>${item.name} - ${item.email}</li>`;
      })
      .join('')}
...
exports.evaluateTaskByResponsibleEmployees = async (portal, data, taskId) => {
  let { user, unit, checkSave, progress, automaticPoint, employeePoint, role, evaluatingMonth, startDate, endDate, kpi, info, isProject } = data;
...
exports.evaluateTaskByAccountableEmployees = async (portal, data, taskId) => {
  let { unit, user, hasAccountable, checkSave, progress, evaluatingMonth, startDate, endDate, info, results, kpi } = data;
  ...
  let newTask = await Task(connect(DB_CONNECTION, portal))
    ...
    .populate([
      ...
      { path: 'collaboratedWithOrganizationalUnits.organizationalUnit' },
      {
        path: 'responsibleEmployees accountableEmployees consultedEmployees informedEmployees confirmedByEmployees creatorinactiveEmployees',
        select: 'name email _id active avatar',
      },
      {
        path: 'evaluations.results.employee',
  ...
  for (let i in timesheetLogs) {
    ...
    if (results) {
      for (let j = 0; j < results.length; j++) {
        if (results[j].employee && results[j].employee.toString() === employee.toString()) {
          check = false;
...
exports.deleteEvaluation = async (portal, params) => {
  ...
  await Task(connect(DB_CONNECTION, portal)).updateOne({ _id: taskId }, { $pull: { evaluations: { _id: evaluationId } } }, { $new: true });
  // let newTask = await Task(connect(DB_CONNECTION, portal)).findById(taskId);
  let newTask = await Task(connect(DB_CONNECTION, portal))
    .findById(taskId)
    .populate([
...
exports.confirmTask = async (portal, taskId, userId) => {
  let confirmedByEmployee = await Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(taskId, { $push: { confirmedByEmployees: userId } });
...
exports.requestAndApprovalCloseTask = async (portal, taskId, data) => {
  ...
  let task = await this.getTaskById(portal, taskId, userId);
  const requestStatusNumber = type === 'request' ? 1 : type === 'cancel_request' ? 2 : type === 'approval' ? 3 : 0;
...
exports.openTaskAgain = async (portal, taskId, data) => {
  ...
  let taskUpdate = await Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
    taskId,
    {
      status: 'inprocess',
      requestToCloseTask: {
...
filterEmployeeBeforeAndAfter = async (newEmloyees, oldEmployees) => {
  let data = await filterArrayBeforeAndAfter(
    newEmloyees.map((item) => item?._id),
    oldEmployees.map((item) => item?._id)
  );
  let newEmployee = '',
    oldEmployee = '';
...
exports.createDescriptionEditTaskLogs = async (portal, userId, newTask, oldTask) => {
  ...
  if (accountableEmployees) {
    ...
    if (data?.newEmployee && data?.newEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người phê duyệt mới: ' +
        data?.newEmployee?.slice(0, data?.newEmployee?.length - 2) +
        '.</span></br>';
    }
    if (data?.oldEmployee && data?.oldEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người phê duyệt đã xóa bỏ: ' +
        data?.oldEmployee?.slice(0, data?.oldEmployee?.length - 2) +
        '.</span></br>';
    }
  }
  if (responsibleEmployees) {
    ...
    if (data?.newEmployee && data?.newEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người thực hiện mới: ' +
        data?.newEmployee?.slice(0, data?.newEmployee?.length - 2) +
        '.</span></br>';
    }
    if (data?.oldEmployee && data?.oldEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người thực hiện đã xóa bỏ: ' +
        data?.oldEmployee?.slice(0, data?.oldEmployee?.length - 2) +
        '.</span></br>';
    }
  }
  if (consultedEmployees) {
    ...
    if (data?.newEmployee && data?.newEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người tư vấn mới: ' +
        data?.newEmployee?.slice(0, data?.newEmployee?.length - 2) +
        '.</span></br>';
    }
    if (data?.oldEmployee && data?.oldEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người tư vấn đã xóa bỏ: ' +
        data?.oldEmployee?.slice(0, data?.oldEmployee?.length - 2) +
        '.</span></br>';
    }
  }
  if (informedEmployees) {
    ...
    if (data?.newEmployee && data?.newEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người quan sát mới: ' +
        data?.newEmployee?.slice(0, data?.newEmployee?.length - 2) +
        '.</span></br>';
    }
    if (data?.oldEmployee && data?.oldEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên có vai trò người quan sát đã xóa bỏ: ' +
        data?.oldEmployee?.slice(0, data?.oldEmployee?.length - 2) +
        '.</span></br>';
    }
  }
  if (inactiveEmployees) {
    ...
    if (data?.newEmployee && data?.newEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Thêm nhân viên không còn tham gia công việc: ' +
        data?.newEmployee?.slice(0, data?.newEmployee?.length - 2) +
        '.</span></br>';
    }
    if (data?.oldEmployee && data?.oldEmployee !== '') {
      descriptionLog =
        descriptionLog +
        '<span>Nhân viên không còn tham gia công việc đã tham gia lại: ' +
        data?.oldEmployee?.slice(0, data?.oldEmployee?.length - 2) +
        '.</span></br>';
    }
  }
...
exports.editArchivedOfTask = async (portal, taskId) => {
  ...
  for (let i in taskId) {
    let t = await Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(taskId[i]);

    let isArchived = t.isArchived;
    if (t.status === 'finished' || t.status === 'delayed' || t.status === 'canceled') {
      await Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(taskId[i], { $set: { isArchived: !isArchived } }, { new: true });
...
exports.sortActions = async (portal, params, body) => {
  ...
  let i;
  await Task(connect(DB_CONNECTION, portal)).update({ _id: taskId }, { $set: { taskActions: [] } });
  await Task(connect(DB_CONNECTION, portal)).update(
    { _id: taskId },
    {
      $set: {
...
exports.evaluateTaskByResponsibleEmployeesProject = async (portal, data, taskId) => {
  let { userId, taskAutomaticPoint, automaticPoint, employeePoint, accountablePoint, checkSave, progress, info, actualResMemberCost } = data;
  // let startEval = new Date(startDate);
  ...
  const currentTask = await Task(connect(DB_CONNECTION, portal)).findOne({ _id: taskId });
    ...
    // Nếu chưa có userId này trong array đánh giá
      ...
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        ...
        },
        {
          $set: {
            'overallEvaluation.automaticPoint': taskAutomaticPoint,
            'overallEvaluation.responsibleEmployees': [
              currentOverallEvaluation.responsibleEmployees,
              {
                employee: userId,
                automaticPoint,
                employeePoint,
...
exports.evaluateTaskByAccountableEmployeesProject = async (portal, data, taskId) => {
  let { userId, taskAutomaticPoint, automaticPoint, employeePoint, checkSave, progress, actualAccMemberCost, actualCostTask, info } = data;
  // let startEval = new Date(startDate);
    ...
    // Nếu chưa có userId này trong array đánh giá
      ...
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        ...
        },
        {
          $set: {
            'overallEvaluation.automaticPoint': taskAutomaticPoint,
            'overallEvaluation.accountableEmployees': [
              currentOverallEvaluation.accountableEmployees,
              {
                employee: userId,
                automaticPoint,
                employeePoint,
...
exports.createSubmissionResults = async (portal, params, body, files) => {
  const newTask = await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
  ...
  const taskActions = await Task(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
    params.taskId,
    {
      $push: {
...
exports.approveTaskOutputs = async (portal, params, body) => {
  ...
  if (status === 'approved') {
    ...
    const newVersion = await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
      {
        _id: params.taskId,
  ...
  const taskIsChanged = await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
    {
      _id: params.taskId,
      'taskOutputs._id': mongoose.Types.ObjectId(params.taskOutputId),
    },
...
exports.editTaskOutputs = async (portal, params, body) => {
  for (item of body) {
    const newTaskEdit = await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
      {
        _id: params.taskId,
...
exports.deleteFileOfTaskOutput = async (portal, params) => {
  ...
  const newEditTask = await Task(connect(DB_CONNECTION, portal)).findOneAndUpdate(
    {
      _id: params.taskId,
      'taskOutputs._id': mongoose.Types.ObjectId(params.taskOutputId),
    },
