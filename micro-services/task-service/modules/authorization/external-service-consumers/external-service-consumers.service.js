...
exports.create = async (portal, createExternalServiceConsumerDto) => {
  ...
  const newRecord = await ExternalServiceConsumer(connect(DB_CONNECTION, portal)).create(newExternalServiceConsumer);
...
exports.findAll = async (portal, queryParams = {}) => {
  ...
  const externalServiceConsumers = await ExternalServiceConsumer(connect(DB_CONNECTION, portal)).find(query).skip(page).limit(perPage);

  const totalExternalServiceConsumers = await ExternalServiceConsumer(connect(DB_CONNECTION, portal)).countDocuments(query);
  const totalPages = perPage ? Math.ceil(totalExternalServiceConsumers / perPage) : 1;
...
exports.findOne = async (portal, id) => {
  ...
  const { name, description, clientCredential, attributes } = await ExternalServiceConsumer(connect(DB_CONNECTION, portal)).findById(id);
...
exports.update = async (portal, id, updateExternalServiceConsumerDto) => {
  ...
  const { name, description, clientCredential, attributes } = await ExternalServiceConsumer(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
    id,
    {
