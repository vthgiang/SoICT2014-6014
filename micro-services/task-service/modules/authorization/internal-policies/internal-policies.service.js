...
exports.create = async (portal, internalPolicyDto) => {
  const { name, description, effect, actions, resources, effectiveStartTime, effectiveEndTime } = internalPolicyDto;
  ...
  LoggingService.create(portal, LoggingType.CREATE_INTERNAL_POLICY, 'SUCCESSFUL', policy.name, policy);
...
exports.findAll = async (portal, queryParams = {}) => {
  ...
  const policies = await InternalPolicy(connect(DB_CONNECTION, portal)).find(query).skip(queryParams?.page).limit(queryParams?.perPage);

  const totalInternalPolicies = await InternalPolicy(connect(DB_CONNECTION, portal)).countDocuments(query);
  const totalPages = queryParams?.perPage ? Math.ceil(totalInternalPolicies / queryParams.perPage) : 1;
...
exports.findOne = async (portal, id) => {
  ...
  const { name, description, effect, actions, resources, effectiveStartTime, effectiveEndTime } = await InternalPolicy(
...
exports.update = async (portal, id, updateInternalPolicyDto) => {
  ...
  const { description, effect, actions, resources, effectiveStartTime, effectiveEndTime } = updateInternalPolicyDto;
  ...
  LoggingService.create(portal, LoggingType.UPDATE_INTERNAL_POLICY, 'SUCCESSFUL', policy.name, policy);
