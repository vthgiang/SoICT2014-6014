...
let connectOptions, systemDB, roleAbstract;
...
const init = async () => {
  ...
  systemDB = mongoose.createConnection(`mongodb://${process.env.DB_HOST}:${process.env.DB_PORT || '27017'}/${process.env.DB_NAME}`, connectOptions);
...
const getSystemAdminPage = async (data) => {
  ...
  let examples = await Link(systemDB)
    .find(keySearch)
    .skip((page - 1) * perPage)
    .limit(perPage);
...
const deleteSystemAdminPage = async (pageIds) => {
  init();
  let pages = await Link(systemDB).deleteMany({ _id: { $in: pageIds.map((item) => mongoose.Types.ObjectId(item)) } });
  await Privilege(systemDB).deleteMany({ resourceId: { $in: pageIds.map((item) => mongoose.Types.ObjectId(item)) } });
  return pages;
};
