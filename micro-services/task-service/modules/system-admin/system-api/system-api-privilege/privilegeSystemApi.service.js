...
const getPrivilegeApis = async (portal, data) => {
  ...
  if (role === 'system_admin') {
    ...
    totalPrivilegeApis = await PrivilegeApi(connect(DB_CONNECTION, process.env.DB_NAME)).countDocuments(keySearch);
    ...
    totalPrivilegeApis = await PrivilegeApi(connect(DB_CONNECTION, portal)).countDocuments(keySearch);
    totalPages = Math.ceil(totalPrivilegeApis / perPage);
  }
...
const createPrivilegeApi = async (data) => {
  ...
  let privilege = await PrivilegeApi(connect(DB_CONNECTION, process.env.DB_NAME)).findOne({
    email: email,
  ...
  let company = await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findById(companyId);
  ...
  let systemApis = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).find({
    _id: { $in: apis },
  });
  ...
  if (status === 3 && (role === ROLE.SYSTEM_ADMIN || role === ROLE.ADMIN)) {
    ...
    if (role === ROLE.SYSTEM_ADMIN) {
       ...
       */
      privilege = await PrivilegeApi(connect(DB_CONNECTION, process.env.DB_NAME)).create({
        email: email,
        description,
        apis: systemApis,
        company: companyId,
      ...
      await Company(connect(DB_CONNECTION, process.env.DB_NAME)).populate(privilege, { path: 'company' });

      /**
       * Thêm phân quyền vào db cty
       */
      await PrivilegeApi(connect(DB_CONNECTION, company.shortName)).create({
        email: email,
        apis: systemApis,
        company: companyId,
        status: 3,
      ...
      const user = await User(connect(DB_CONNECTION, company.shortName)).findOne({ email: email });
      ...
      privilege = await PrivilegeApi(connect(DB_CONNECTION, company.shortName)).create({
        email: email,
        apis: systemApis,
        company: companyId,
        status: 3,
...
const updateStatusPrivilegeApi = async (portal, data) => {
  ...
  let privilege = await PrivilegeApi(connect(DB_CONNECTION, portal)).findOne({
   ...
   */
  if (role !== 'system_admin') {
    const user = await User(connect(DB_CONNECTION, portal)).findById(privilege.creator);
  ...
  if (Number(status) === 0 || Number(status) === 2) {
    await PrivilegeApi(connect(DB_CONNECTION, portal)).updateOne(
      { _id: { $in: privilegeApiIds.map((item) => mongoose.Types.ObjectId(item)) } },
      { status: status }
    );
    ...
    for (let i = 0; i < privilegeApiIds?.length; i++) {
      ...
      const company = await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findById(privilege?.company);
...
const deletePrivilegeApis = async (portal, data) => {
  ...
  let privilegeApis = await PrivilegeApi(connect(DB_CONNECTION, portal)).deleteMany({
    _id: { $in: privilegeApiIds.map((item) => mongoose.Types.ObjectId(item)) },
  });
