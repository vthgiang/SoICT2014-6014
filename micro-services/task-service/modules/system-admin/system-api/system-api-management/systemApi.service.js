...
const getSystemApis = async (data) => {
  ...
  let totalSystemApis = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).countDocuments(keySearch);
...
const createSystemApi = async (data) => {
  ...
  let checkSystemApi = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).findOne({
    path: path,
    method: method,
  });
  ...
  if (checkSystemApi) {
    ...
    let systemApi = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).create({
      path: path,
      method: method,
  ...
  try {
    ...
    const getRoute = async (path, layer) => {
      if (layer.route) {
         ...
         */
        if (!appRoutes.find((el) => el.method === currentMethod && el.path === currentPath)) {
          appRoutes.push({
    ...
    for (let i = 0; i < appRoutes.length; i++) {
      const systemApiIndex = systemApis.findIndex((systemApi) => appRoutes[i].path === systemApi.path && appRoutes[i].method === systemApi.method);

      if (systemApiIndex >= 0) {
        if (
          appRoutes[i].description &&
          (!systemApis[systemApiIndex].description || appRoutes[i].description !== systemApis[systemApiIndex].description)
        ) {
...
const editSystemApi = async (systemApiId, data) => {
  ...
  let systemApi = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).updateOne(
  ...
  systemApi = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).findById(systemApiId);
...
const deleteSystemApi = async (systemApiId) => {
  let systemApi = await SystemApi(connect(DB_CONNECTION, process.env.DB_NAME)).deleteOne({ _id: mongoose.Types.ObjectId(systemApiId) });
