const { SystemComponent, SystemLink, Company, Link, Component, Role, Privilege } = require('../../../models');
...
exports.getAllSystemComponents = async (query) => {
  ...
  if (!page && !limit) {
    ...
    return await SystemComponent(connect(DB_CONNECTION, process.env.DB_NAME)).paginate(option, {
      page,
      limit,
      populate: [{ path: 'roles' }, { path: 'links' }],
    });
...
exports.createSystemComponent = async (name, description, links, roles) => {
  ...
  for (let i = 0; i < companyList.length; i++) {
    let companyLinks = await Link(connect(DB_CONNECTION, companyList[i].shortName)).find({
      url: { $in: systemLinks.map((link) => link.url) },
    });
    let companyComponent = await Component(connect(DB_CONNECTION, companyList[i].shortName)).create({
      name,
      description,
      links: companyLinks.map((link) => link._id),
    });
    for (let x = 0; x < companyLinks.length; x++) {
      let companyLink = await Link(connect(DB_CONNECTION, companyList[i].shortName)).findById(companyLinks[x]._id);
      if (companyLink) {
