...
exports.createCompany = async (data) => {
  if (data.shortName) {
    let company = await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findOne({ shortName: data.shortName });
    if (company) throw ['company_already_exist'];
  }
...
exports.initConfigBackup = async (companyShortName) => {
  return await Configuration(connect(DB_CONNECTION, process.env.DB_NAME)).create({
 ...
 */
exports.createCompanySuperAdminAccount = async (companyShortName, userEmail, companyId) => {
  let checkEmail = await User(connect(DB_CONNECTION, companyShortName)).findOne({ email: userEmail });
  if (checkEmail) throw ['email_exist'];
  let roleSuperAdmin = await Role(connect(DB_CONNECTION, companyShortName)).findOne({ name: Terms.ROOT_ROLES.SUPER_ADMIN.name });
  ...
  let companyUpdate = await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findOne({ shortName: companyShortName });
...
exports.createCompanyLinks = async (company, linkArr, roleArr, companyId) => {
  ...
  let allLinks = await SystemLink(connect(DB_CONNECTION, process.env.DB_NAME)).find().populate({ path: 'roles' });
...
exports.createCompanyComponents = async (company, linkArr, companyId) => {
  let systemLinks = await SystemLink(connect(DB_CONNECTION, process.env.DB_NAME)).find({ _id: { $in: linkArr } });
...
exports.editCompanySuperAdmin = async (company, superAdminEmail) => {
  ...
  if (oldSuperAdmin.email === superAdminEmail) {
    ...
    if (user === null) {
      let newUser = await this.createCompanySuperAdminAccount(com.shortName, superAdminEmail, com._id);
...
exports.editCompanyOrgInformation = async (shortName, organizationalUnitImage) => {
  return await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findOneAndUpdate(
    { shortName: shortName },
    {
...
exports.getCompanyInformation = async (shortName) => {
  return await Company(connect(DB_CONNECTION, process.env.DB_NAME)).findOne({ shortName: shortName });
};
