const { Delegation, Privilege, UserRole, Role, User, Link, Policy, Task } = require('../../models');
  ...
  const filterValidDelegationArray = async (array) => {
    ...
    if (array.length > 0) {
      for (let i = 0; i < array.length; i++) {
        const checkDelegationCreated = await Delegation(connect(DB_CONNECTION, portal))
          .findOne({ delegationName: array[i].delegationName })
          .collation({ locale: 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' });
    ...
    for (let i = 0; i < delArray.length; i++) {
      ...
      let delegatePrivileges = delArray[i].allPrivileges
        ...
        : await Privilege(connect(DB_CONNECTION, portal)).find({
            roleId: { $in: [delArray[i].delegateRole].concat(delegateRole.parents) },
...
exports.assignDelegation = async (newDelegation, portal) => {
  ...
  let delegatePrivileges =
    newDelegation.delegatePrivileges == null || newDelegation.delegatePrivileges.length == 0
      ? null
      : await Privilege(connect(DB_CONNECTION, portal)).find({ _id: { $in: newDelegation.delegatePrivileges } });
...
exports.updateMissedDelegation = async (portal) => {
  ...
  allDelegations.forEach(async (delegation) => {
    if (delegation.endDate != null && compareDate(delegation.endDate, new Date()) < 0) {
      ...
      await Delegation(connect(DB_CONNECTION, portal)).updateOne(
        { _id: delegation._id },
        {
          logs: [
            ...delegation.logs,
...
exports.checkDelegationPolicy = async (policyId, delegatorId, delegatedObjectId, delegateLinksIds, delegateeId, portal) => {
  ...
  if (delegateRole) {
    if (
      PolicyService.ruleCheck([delegateRole], policy.delegatedObject.delegatedObjectAttributes, policy.delegatedObject.delegatedObjectRule).length ==
      0
    ) {
  ...
  if (delegateTask) {
    if (
      PolicyService.ruleCheck([delegateTask], policy.delegatedObject.delegatedObjectAttributes, policy.delegatedObject.delegatedObjectRule).length ==
      0
    ) {
...
exports.deleteDelegations = async (portal, delegationIds) => {
  ...
  let delegations = await Delegation(connect(DB_CONNECTION, portal)).deleteMany({
...
exports.revokeDelegation = async (portal, delegationIds, reason) => {
  ...
  let delegation = await Delegation(connect(DB_CONNECTION, portal)).find({
    _id: { $in: delegationIds.map((item) => mongoose.Types.ObjectId(item)) },
  });
  ...
  let newDelegation = await Delegation(connect(DB_CONNECTION, portal))
...
exports.saveLog = async (portal, delegation, userId, content, category, time) => {
  if (
    ...
    (delegation.logs.length > 0 && (new Date().getTime() - new Date(delegation.logs[delegation.logs.length - 1].createdAt).getTime()) / 1000 > 5) ||
    category == 'logout'
  ) {
    console.log('alo');
    await Delegation(connect(DB_CONNECTION, portal)).updateOne(
...
exports.createTaskDelegation = async (portal, data, logs = []) => {
  ...
  if (delArray && delArray.length !== 0) {
    for (let i = 0; i < delArray.length; i++) {
      ...
      let checkDelegateTask = await Task(connect(DB_CONNECTION, portal))
        .findOne({
  ...
  delegateTaskRoles.forEach(async (r) => {
    if (r == 'responsible') {
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        ...
        {
          responsibleEmployees: [...delegateTask.responsibleEmployees.filter((e) => e.toString() !== delegator.toString()), delegatee],
        }
      );
    }
    if (r == 'accountable') {
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        { _id: delegateTask._id },
        {
          accountableEmployees: [...delegateTask.accountableEmployees.filter((e) => e.toString() !== delegator.toString()), delegatee],
        }
      );
    }
    if (r == 'consulted') {
      await Task(connect(DB_CONNECTION, portal)).updateOne(
        { _id: delegateTask._id },
        {
          consultedEmployees: [...delegateTask.consultedEmployees.filter((e) => e.toString() !== delegator.toString()), delegatee],
        }
      );
    }
    if (r == 'informed') {
...
exports.revokeTaskDelegation = async (portal, delegationIds, reason) => {
    ...
    delegateTaskRoles.forEach(async (r) => {
      if (r == 'responsible') {
        await Task(connect(DB_CONNECTION, portal)).updateOne(
          { _id: delegateTask._id },
          {
            responsibleEmployees: [...delegateTask.responsibleEmployees.filter((e) => e.toString() !== delegatee.toString()), delegator],
          }
        );
      }
      if (r == 'accountable') {
        await Task(connect(DB_CONNECTION, portal)).updateOne(
          { _id: delegateTask._id },
          {
            accountableEmployees: [...delegateTask.accountableEmployees.filter((e) => e.toString() !== delegatee.toString()), delegator],
          }
        );
      }
      if (r == 'consulted') {
        await Task(connect(DB_CONNECTION, portal)).updateOne(
          { _id: delegateTask._id },
          {
            consultedEmployees: [...delegateTask.consultedEmployees.filter((e) => e.toString() !== delegatee.toString()), delegator],
          }
        );
      }
      if (r == 'informed') {
        await Task(connect(DB_CONNECTION, portal)).updateOne(
          { _id: delegateTask._id },
          {
            informedEmployees: [...delegateTask.informedEmployees.filter((e) => e.toString() !== delegatee.toString())],
          }
        );
      }
    });
  ...
  if (
    delegateTask.delegations.map((d) => d.delegatorHasInformed).includes(false) &&
    delegateTask.delegations.filter((d) => d._id.toString() !== result._id.toString() && d.status == 'activated').length == 0
  ) {
    await Task(connect(DB_CONNECTION, portal)).updateOne(
      { _id: delegateTask._id },
      {
        informedEmployees: [...delegateTask.informedEmployees.filter((e) => e.toString() !== delegator.toString())],
      }
    );
  }
...
exports.deleteTaskDelegation = async (portal, delegationIds) => {
  ...
  let delegationDelete = await Delegation(connect(DB_CONNECTION, portal)).deleteOne({ _id: result._id });
...
exports.handleDelegatorLosesRole = async (portal, users, roles) => {
  ...
  if (Array.isArray(users)) {
    ...
    let checkDelegationUser = await Delegation(connect(DB_CONNECTION, portal))
      .find({ delegator: { $nin: users }, delegateRole: roles, status: { $in: ['pending', 'activated'] } })
      .populate([
        { path: 'delegateRole', select: '_id name' },
        {
          path: 'delegateTask',
  ...
  if (Array.isArray(roles)) {
    ...
    let checkDelegationRole = await Delegation(connect(DB_CONNECTION, portal))
      .find({ delegator: users, delegateRole: { $nin: roles }, status: { $in: ['pending', 'activated'] } })
      .populate([
        { path: 'delegateRole', select: '_id name' },
        {
          path: 'delegateTask',
...
exports.createServiceDelegation = async (portal, data, logs = []) => {
  ...
  const isServiceCanAccessResources = (service, apis) => {
    ...
    for (let i = 0; i < apis.length; i++) {
      ...
      if (internalPolicies.length > 0 && internalPolicies.every((policy) => policy.effect === 'Allow')) continue;
...
exports.deleteServiceDelegation = async (portal, delegationIds) => {
  ...
  for (let i = 0; i < delegationIds.length; i++) {
    ...
    let delegationDelete = await Delegation(connect(DB_CONNECTION, portal)).deleteOne({ _id: delegationId });
