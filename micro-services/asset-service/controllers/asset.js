...
let updateAssetInformationFromFile = async (req, res) => {
    try {
...
exports.getAssetGroupChart = async (req, res) => {
    ...
    try {
        ...
        const listAssetOfTypes = chartAssetChart.listType;
        //console.log("listAssetOfTypes",listAssetOfTypes)
        if (chartAssetChart) {
        ...
        let depreciationAsset, depreciationExpenseOfBuilding = 0, depreciationExpenseOfVehicle = 0,
            depreciationExpenseOfMachine = 0, depreciationExpenseOfOther = 0;
        ...
        // chia theo thể loại 
        let typeName = [], shortName = [], countAssetType = [], countAssetValue = [], countDepreciation = [],
            idAssetType = [], idAssetTypeTest = [];
        ...
        let listAssetTypeSortShow = listAssetTypeSort.map((value, index) => {
            return (value._doc)
        })
        ...
        if (req.query.time) {
            ...
            if (item1.name = 'purchase-date-data') {
        ...
        if (assetChart) {
            for (let i = 0; i < listMonth.length - 1; i++) {
                ...
                for (let j in listAssetOfTypes) {
                    countAssetcount[j] = 0;
                    valueAsset[j] = 0;
                    idAssetTypePurchase.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestPurchase.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                if (assetChart) {
                    assetChart.forEach(asset => {
                ...
                countType.push({
                    xType: category[i],
                    countAssetcount: countAssetcount,
                    valueAsset: valueAsset,
                    idAssetTypeTestPurchase: idAssetTypeTestPurchase,
                    idAssetTypePurchase: idAssetTypePurchase
                })
                countAsset.push(cnt);
                value.push(val);
        ...
        if (assetChart) {
            for (let i = 0; i < categoryYear.length; i++) {
                ...
                let countAssetcountYear = [], valueAssetYear = [], idAssetTypePurchaseYear = [],
                    idAssetTypeTestPurchaseYear = [];
                for (let j in listAssetOfTypes) {
                    countAssetcountYear[j] = 0;
                    valueAssetYear[j] = 0;
                    idAssetTypePurchaseYear.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestPurchaseYear.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                if (assetChart) {
                    assetChart.forEach(asset => {
                        for (let j in asset.assetType) {
                ...
                countTypeYear.push({
                    xType: categoryYear[i],
                    countAssetcountYear: countAssetcountYear,
                    valueAssetYear: valueAssetYear,
                    idAssetTypeTestPurchaseYear: idAssetTypeTestPurchaseYear,
                    idAssetTypePurchaseYear: idAssetTypePurchaseYear
                })
        ...
        result = {
            ...result,
            purchaseChartYear: countTypeYear
        }


        // Disposalchart
        let listMonthDisposal = [], valueDisposal = [], countAssetDisposal = [], categoryDisposal = [],
            arrDisposal = [];
        if (req.query.time) {
            let item1 = JSON.parse(req.query.time)
            if (item1.name = 'disposal-date-data') {
        ...
        let startDateDisposal = new Date(disposalDateAfter);
        let endDateDisposal = new Date(disposalDateBefore);

        let periodDisposal = Math.round((endDateDisposal - startDateDisposal) / 2592000000) + 1;

        let mDisposal = disposalDateAfter.slice(5, 7);
        let yDisposal = disposalDateAfter.slice(0, 4);
        ...
        let countTypeDisposal = [], valueTypeDisposal = []
        ...
        if (assetChart) {
            for (let i = 0; i < listMonthDisposal.length - 1; i++) {
                ...
                for (let j in listAssetOfTypes) {
                    countAssetcount[j] = 0;
                    valueAsset[j] = 0;
                    idAssetTypeDisposal.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestDisposal.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeDisposal.push({
                    xTypeDipsosal: categoryDisposal[i],
                    countAssetcount: countAssetcount,
                    valueAsset: valueAsset,
                    idAssetTypeTestDisposal: idAssetTypeTestDisposal,
                    idAssetTypeDisposal: idAssetTypeDisposal
                })
                countAssetDisposal.push(cnt);

                valueDisposal.push(val);
        ...
        result = {
            ...result,
            disposalChart: countTypeDisposal
        }


        // disposal chart year 

        let startDateYearDisposal = disposalDateAfter.slice(0, 4);
        let endDateYearDisposal = disposalDateBefore.slice(0, 4);

        let periodYearDisposal = endDateYearDisposal - startDateYearDisposal + 1;
        let valueYearDisposal = [], countAssetYearDisposal = [], categoryYearDisposal = [], arrYearDisposal = [];

        for (let i = 0; i < periodYearDisposal; i++) {
            categoryYearDisposal.push(parseInt(startDateYearDisposal) + i);
        }

        let countTypeYearDisposal = []

        if (assetChart) {
            for (let i = 0; i < categoryYearDisposal.length; i++) {
                let cntYear = 0, valYear = 0;
                let countAssetcountYear = [], valueAssetYear = [], idAssetTypeDisposalYear = [],
                    idAssetTypeTestDisposalYear = [];
                for (let j in listAssetOfTypes) {
                    countAssetcountYear[j] = 0;
                    valueAssetYear[j] = 0;
                    idAssetTypeDisposalYear.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestDisposalYear.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeYearDisposal.push({
                    xType: categoryYearDisposal[i],
                    countAssetcountYear: countAssetcountYear,
                    valueAssetYear: valueAssetYear,
                    idAssetTypeTestDisposalYear: idAssetTypeTestDisposalYear,
                    idAssetTypeDisposalYear: idAssetTypeDisposalYear
                })
                countAssetYearDisposal.push(cntYear);
                valueYearDisposal.push(valYear);

            }
        }
        result = {
            ...result,
            disposalChartYear: countTypeYearDisposal
        }
        ...
        if (req.query.time) {
            let item1 = JSON.parse(req.query.time)
            if (item1.name = 'incident-date-data') {
        ...
        let startDateIncident = new Date(incidentDateAfter);
        let endDateIncident = new Date(incidentDateBefore);

        let periodIncident = Math.round((endDateIncident - startDateIncident) / 2592000000) + 1;
        let listMonthIncident = [], countAssetIncident = [], categoryIncident = [], arrIncident = [];
        let mIncident = incidentDateAfter.slice(5, 7);
        let yIncident = incidentDateAfter.slice(0, 4);

        for (let i = 0; i <= periodIncident; i++) {
            if (mIncident > 12) {
                mIncident = 1;
                yIncident++;
            }
            if (mIncident < 10) {
                mIncident = '0' + mIncident;
            }
            categoryIncident.push([mIncident, yIncident].join('-'));
            listMonthIncident.push([yIncident, mIncident].join(','));
            mIncident++;

        }
        let countTypeIncident = []

        if (assetChart) {
            for (let i = 0; i < listMonthIncident.length - 1; i++) {
                let cnt = 0, val = 0;
                let minDate = new Date(listMonthIncident[i]).getTime();
                let maxDate = new Date(listMonthIncident[i + 1]).getTime();
                let countAssetcount = [], idAssetTypeIncident = [], idAssetTypeTestIncident = [];
                for (let j in listAssetOfTypes) {
                    countAssetcount[j] = 0;
                    idAssetTypeIncident.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestIncident.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeIncident.push({
                    xType: categoryIncident[i],
                    countAssetcount: countAssetcount,
                    idAssetTypeTestIncident: idAssetTypeTestIncident,
                    idAssetTypeIncident: idAssetTypeIncident
                })
        ...
        result = {
            ...result,
            incidentChart: countTypeIncident
        }
        // Incident chart year
        let startDateYearIncident = incidentDateAfter.slice(0, 4);
        let endDateYearIncident = incidentDateBefore.slice(0, 4);

        let periodYearIncident = endDateYearIncident - startDateYearIncident + 1;
        let valueYearIncident = [], countAssetYearIncident = [], categoryYearIncident = [], arrYearIncident = [];
        for (let i = 0; i < periodYearIncident; i++) {
            categoryYearIncident.push(parseInt(startDateYearIncident) + i);
        }
        let countTypeYearIncident = []

        if (assetChart) {
            for (let i = 0; i < categoryYearIncident.length; i++) {
                let cntYear = 0, valYear = 0;
                let countAssetcountYear = [], valueAssetYear = [], idAssetTypeIncidentYear = [],
                    idAssetTypeTestIncidentYear = [];
                for (let j in listAssetOfTypes) {
                    countAssetcountYear[j] = 0;
                    valueAssetYear[j] = 0;
                    idAssetTypeIncidentYear.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestIncidentYear.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeYearIncident.push({
                    xType: categoryYearIncident[i],
                    countAssetcountYear: countAssetcountYear,
                    idAssetTypeTestIncidentYear: idAssetTypeTestIncidentYear,
                    idAssetTypeIncidentYear: idAssetTypeIncidentYear
                })
        ...
        result = {
            ...result,
            incidentChartYear: countTypeYearIncident
        }
        // maintaince chart
        ...
        if (req.query.time) {
            let item1 = JSON.parse(req.query.time)
            if (item1.name == 'maintenance-date-data') {
        ...
        let startDateMaintenance = new Date(maintenanceDateAfter);
        let endDateMaintenance = new Date(maintenanceDateBefore);

        let periodMaintenance = Math.round((endDateMaintenance - startDateMaintenance) / 2592000000) + 1;
        let listMonthMaintenance = [], countAssetMaintenance = [], categoryMaintenance = [], arrMaintenance = [];
        let mMaintenance = maintenanceDateAfter.slice(5, 7);
        let yMaintenance = maintenanceDateAfter.slice(0, 4);

        for (let i = 0; i <= periodMaintenance; i++) {
            if (mMaintenance > 12) {
                mMaintenance = 1;
                yMaintenance++;
            }
            ...
            categoryMaintenance.push([mMaintenance, yMaintenance].join('-'));
            listMonthMaintenance.push([yMaintenance, mMaintenance].join(','));
            mMaintenance++;

        }
        let countTypeMaintenance = []

        if (assetChart) {
            for (let i = 0; i < listMonthMaintenance.length - 1; i++) {
                let cnt = 0, val = 0;
                let minDate = new Date(listMonthMaintenance[i]).getTime();
                let maxDate = new Date(listMonthMaintenance[i + 1]).getTime();
                let countAssetcount = [], valueAsset = [], idAssetTypeMaintenance = [], idAssetTypeTestMaintenance = [];
                for (let j in listAssetOfTypes) {
                    countAssetcount[j] = 0;
                    valueAsset[j] = 0;
                    idAssetTypeMaintenance.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestMaintenance.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeMaintenance.push({
                    xType: categoryMaintenance[i],
                    countAssetcount: countAssetcount,
                    valueAsset: valueAsset,
                    idAssetTypeTestMaintenance: idAssetTypeTestMaintenance,
                    idAssetTypeMaintenance: idAssetTypeMaintenance
                })
        ...
        result = {
            ...result,
            maintenanceChart: countTypeMaintenance
        }

        let startDateYearMaintenance = maintenanceDateAfter.slice(0, 4);
        let endDateYearMaintenance = maintenanceDateBefore.slice(0, 4);
        let periodYearMaintenance = endDateYearMaintenance - startDateYearMaintenance + 1;
        let valueYearMaintenance = [], countAssetYearMaintenance = [], categoryYearMaintenance = [],
            arrYearMaintenance = [];
        for (let i = 0; i < periodYearMaintenance; i++) {
            categoryYearMaintenance.push(parseInt(startDateYearMaintenance) + i);
        }
        let countTypeYearMaintenance = []

        if (assetChart) {
            for (let i = 0; i < categoryYearMaintenance.length; i++) {
                let cntYear = 0, valYear = 0;
                let countAssetcountYear = [], valueAssetYear = [], idAssetTypeMaintenanceYear = [],
                    idAssetTypeTestMaintenanceYear = [];
                for (let j in listAssetOfTypes) {
                    countAssetcountYear[j] = 0;
                    valueAssetYear[j] = 0;
                    idAssetTypeMaintenanceYear.push(listAssetOfTypes[j]._id)
                    idAssetTypeTestMaintenanceYear.push(JSON.stringify(listAssetOfTypes[j]._id))
                }
                ...
                countTypeYearMaintenance.push({
                    xType: categoryYearMaintenance[i],
                    countAssetcountYear: countAssetcountYear,
                    valueAssetYear: valueAssetYear,
                    idAssetTypeTestMaintenanceYear: idAssetTypeTestMaintenanceYear,
                    idAssetTypeMaintenanceYear: idAssetTypeMaintenanceYear
                })
        ...
        result = {
            ...result,
            maintenanceYearChart: countTypeYearMaintenance
        }
...
let calculateDepreciation = (depreciationType, cost, usefulLife, estimatedTotalProduction, unitsProducedDuringTheYears, startDepreciation) => {
...
exports.getAssetStatisticChart = async (req, res) => {
    try {
        ...
        let listAssetTypeSortShowStatistic = listAssetTypeSortStatistic.map((value, index) => {
            return (value._doc)
        })
        let numberOfReadyToUse = [], numberOfInUse = [], numberOfBroken = [], numberOfLost = [], numberOfDisposed = [],
            idAssetTypes = [], idAssetTypeTests = [];
        ...
        let lessThanOneHundred = [], oneHundred = [], twoHundred = [], fiveHundred = [], oneBillion = [],
            twoBillion = [], fiveBillion = [], tenBillion = [], idAssetTypeCost = [], idAssetTypeTestCost = [];
