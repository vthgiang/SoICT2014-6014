...
const { FIREBASE_ADMIN } = require('../helpers/firebaseHelper')
const { SOCKET_IO, CONNECTED_CLIENTS } = require('../helpers/sockerHelper');
...
exports.getAllManualNotifications = async (portal, creator) => {
    //id cua cong ty do
    return await ManualNotification(connect(getDbConnection(), portal))
        .find({
            creator,
        })
        .populate([
...
exports.paginateManualNotifications = async (portal, creator, data) => {
    ...
    return await ManualNotification(connect(getDbConnection(), portal)).paginate(
        info,
        {
            page: data.page,
            limit: data.limit,
...
exports.createManualNotification = async (portal, data, files) => {
    ...
    const notify = await ManualNotification(connect(getDbConnection(), portal)).create(data)
...
exports.getAllUsersInOrganizationalUnit = async (portal, departmentId) => {
    var department = await OrganizationalUnit(connect(getDbConnection(), portal))
...
exports.createNotification = async (
    ...
    const listUsers = await User(connect(getDbConnection(), portal)).find({
        _id: { $in: usersArr },
    });
...
exports.getAllNotifications = async (portal, user) => {
    return await Notification(connect(getDbConnection(), portal))
        .find({
            user,
        })
        .sort({
...
exports.paginateNotifications = async (portal, user, data) => {
    ...
    if (typeof data.readed === 'boolean') {
        info = Object.assign({ user }, data.content, { readed: false });
    } else {
        info = Object.assign({ user }, data.content);
    }

    return await Notification(connect(getDbConnection(), portal)).paginate(info, {
        page: data.page,
        limit: data.limit,
        sort: {
            createdAt: -1,
...
exports.changeNotificationStateToReaded = async (portal, user, id, readAll) => {
    ...
    if (!readAll) {
        ...
        notification = await Notification(connect(getDbConnection(), portal)).find({
            readed: false,
            user: user,
        });
        ...
        await Notification(connect(getDbConnection(), portal)).updateMany(
            { _id: listId },
            { $set: { readed: true } }
        );
    }
...
exports.deleteManualNotification = async (portal, notificationId) => {
    return await ManualNotification(connect(getDbConnection(), portal)).deleteOne({
        _id: notificationId,
    });
};
...
exports.deleteNotification = async (portal, notificationId) => {
    return await Notification(connect(getDbConnection(), portal)).deleteOne({
        _id: notificationId,
    });
};
