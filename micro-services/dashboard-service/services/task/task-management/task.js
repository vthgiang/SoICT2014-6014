...
const { Task, TaskTemplate, OrganizationalUnit, User, Company, UserRole, ProjectMilestone } = require('../../../models');
const overviewService = require('../../kpi/employee/management');
const UserService = require('../../super-admin/user')
const DelegationService = require('../../delegation')
...
exports.getTaskEvaluations = async (portal, data) => {
    ...
    let result = await Task(connect(getDbConnection(), portal)).aggregate(condition); // kết quả sau khi truy vấn mongodb
...
exports.getPaginatedTasks = async (portal, task) => {
    ...
    let keySearchSpecial = {}, keySearchDateTime = {}, keyTags = {};
    ...
    if (responsibleEmployees) {
        const responsible = await User(connect(getDbConnection(), portal)).find({
            $or: [
                {
                    name: {
    ...
    if (accountableEmployees) {
        const accountable = await User(connect(getDbConnection(), portal)).find({
            $or: [
                {
                    name: {
    ...
    if (creatorEmployees) {
        const creator = await User(connect(getDbConnection(), portal)).find({
            $or: [
                {
                    name: {
    ...
    if (getAll === 'true') {
        taskList = await Task(connect(getDbConnection(), portal)).find(optionQuery).sort({ 'createdAt': -1 });
    } else {
        taskList = await Task(connect(getDbConnection(), portal)).find(optionQuery).sort({ 'createdAt': -1 })
    ...
    let totalCount = await Task(connect(getDbConnection(), portal)).countDocuments(optionQuery);
...
exports.getPaginatedTasksThatUserHasResponsibleRole = async (portal, task) => {
    ...
    let keySearchSpecial = {}, keySearchDateTime = {};
    ...
    responsibleTasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
    ...
    var totalCount = await Task(connect(getDbConnection(), portal)).countDocuments({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getPaginatedTasksThatUserHasAccountableRole = async (portal, task) => {
    ...
    accountableTasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getPaginatedTasksThatUserHasConsultedRole = async (portal, task) => {
    ...
    consultedTasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getPaginatedTasksCreatedByUser = async (portal, task) => {
    ...
    creatorTasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getPaginatedTasksThatUserHasInformedRole = async (portal, task) => {
    ...
    informedTasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getPaginatedTasksByUser = async (portal, task, type = 'paginated_task_by_user') => {
    ...
    console.log(keyTags)
    tasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime,
    ...
    let totalCount = await Task(connect(getDbConnection(), portal)).countDocuments({
        $and: [
            keySearch,
            keySearchSpecial,
            keySearchDateTime
...
exports.getAllTaskOfOrganizationalUnitByMonth = async (portal, task) => { //nguyen
    ...
    organizationUnitTasks = await Task(connect(getDbConnection(), portal)).find(keySearch).sort({ 'createdAt': -1 })
        .populate({ path: 'organizationalUnit parent' })
        .populate({ path: 'creator', select: '_id name email avatar' })
        .populate({ path: 'responsibleEmployees', select: '_id name email avatar' })
    return {
...
exports.getAllTaskByPriorityOfOrganizationalUnit = async (portal, task) => {
    ...
    const data = await Task(connect(getDbConnection(), portal)).find({ ...keySearch, endDate: { $gte: new Date(date) } }) // lấy những việc còn thời hạn
        .populate({ path: 'organizationalUnit parent' })
        .populate({ path: 'creator', select: '_id name email avatar' })
        .populate({ path: 'responsibleEmployees', select: '_id name email avatar' })
        .lean();
    ...
    // lấy việc quá hạn
    const tasksExpire = await Task(connect(getDbConnection(), portal)).find(keySearch)
        .populate({ path: 'organizationalUnit parent' })
        .populate({ path: 'creator', select: '_id name email avatar' })
        .populate({ path: 'responsibleEmployees', select: '_id name email avatar' })
        .lean();
...
exports.sendEmailForCreateTask = async (portal, task) => {
    ...
    var res = await User(connect(getDbConnection(), portal)).find({ _id: { $in: resId } });
    // res = res.map(item => item.name);
    userIds = resId;
    var accId = task.accountableEmployees;  // lấy id người phê duyệt
    var acc = await User(connect(getDbConnection(), portal)).find({ _id: { $in: accId } });
    userIds.push(...accId);

    var conId = task.consultedEmployees;  // lấy id người tư vấn
    var con = await User(connect(getDbConnection(), portal)).find({ _id: { $in: conId } })
    userIds.push(...conId);

    var infId = task.informedEmployees;  // lấy id người quan sát
    var inf = await User(connect(getDbConnection(), portal)).find({ _id: { $in: infId } })
    ...
    for (let i = 0; i < task.collaboratedWithOrganizationalUnits.length; i++) {
        let unit = task.collaboratedWithOrganizationalUnits[i] && await OrganizationalUnit(connect(getDbConnection(), portal))
    ...
    managersOfOrganizationalUnitThatHasCollaborated = await UserRole(connect(getDbConnection(), portal))
        .find({
            roleId: { $in: managersOfOrganizationalUnitThatHasCollaboratedId }
        })
        .populate('userId')
    user = await User(connect(getDbConnection(), portal)).find({
        _id: { $in: userIds }
    })
...
exports.createTask = async (portal, task) => {
    ...
    if (mongoose.Types.ObjectId.isValid(task.parent)) {
        var parent = await Task(connect(getDbConnection(), portal)).findById(task.parent);
    ...
    if (task.taskTemplate) {
        taskTemplate = await TaskTemplate(connect(getDbConnection(), portal)).findById(task.taskTemplate);
    ...
    const newTask = await Task(connect(getDbConnection(), portal)).create({ //Tạo dữ liệu mẫu công việc
    ...
    if (newTask.taskTemplate !== null) {
        await TaskTemplate(connect(getDbConnection(), portal)).findByIdAndUpdate(
...
exports.createProjectTask = async (portal, task) => {
    ...
    var newTask = await Task(connect(getDbConnection(), portal)).create({ //Tạo dữ liệu mẫu công việc
        organizationalUnit: task.organizationalUnit,
        collaboratedWithOrganizationalUnits: task.collaboratedWithOrganizationalUnits,
        creator: task.creator, //id của người tạo
        name: task.name,
...
exports.deleteTask = async (portal, taskId, userId) => {
    ...
    for (let i in taskId) {
        let tasks = await Task(connect(getDbConnection(), portal)).findById(taskId[i]);
        ...
        if (allowDelete) {
            ...
            await Task(connect(getDbConnection(), portal)).findByIdAndDelete(taskId[i]); // xóa mẫu công việc theo id
...
exports.getSubTask = async (portal, taskId) => {
    var task = await Task(connect(getDbConnection(), portal)).find({
        parent: taskId
    }).sort('createdAt')
...
exports.getTasksByUser = async (portal, data) => {
    ...
    if (data.data == 'organizationUnit') {
        for (let i in data.organizationUnitId) {
            let organizationalUnit = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ _id: data.organizationUnitId[i] })
            let test = await Task(connect(getDbConnection(), portal)).find(
...
exports.getAllTasksThatHasEvaluation = async (portal, data) => {
    ...
    let tasks = await Task(connect(getDbConnection(), portal))
        .find({
            $and: [
...
exports.getAllTaskOfOrganizationalUnit = async (portal, roleId, organizationalUnitId, month) => {
    ...
    if (!organizationalUnitId) {
        organizationalUnit = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({
            $or: [
                { 'managers': roleId },
        ...
        organizationalUnit = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ '_id': organizationalUnitId });
    }

    if (organizationalUnit) {
        tasksOfOrganizationalUnit = await Task(connect(getDbConnection(), portal)).aggregate([
...
exports.sendEmailCheckTaskLastMonth = async () => {
    let company = await Company(connect(getDbConnection(), process.env.DB_NAME)).find({});
    ...
    for (let i in company) {
        ...
        let user = await User(connect(getDbConnection(), portal)).find({ company: company[i] });  // lay ra tat ca nguoi dung trong tung cong ty
        ...
        for (let j in userId) {
            ...
            // informedTasks = await Task(connect(getDbConnection(), portal)).find({informedEmployees: userId[j] , isArchived: false}).populate({ path: "organizationalUnit creator parent" });
...
exports.getTaskAnalyseOfUser = async (portal, userId, type, date) => {
    ...
    let keySearchDateTime = {}, keySearch = {};
    ...
    let tasks = await Task(connect(getDbConnection(), portal)).find({
        $and: [
            keySearch,
            keySearchDateTime
        ]
...
exports.getUserTimeSheet = async (portal, userId, month, year, requireActions) => {
    ...
    if (requireActions == 'true') {
        let tsl = await Task(connect(getDbConnection(), portal)).aggregate([
            {
                $match: {
...
exports.getAllUserTimeSheetLog = async (portal, month, year, rowLimit, page, timeLimit, unitArray, sortType) => {
    ...
    let listTask = await Task(connect(getDbConnection(), portal)).aggregate([
        {
            $match: {
...
exports.getTasksByProject = async (portal, data) => {
    ...
    if (preceedingTasks) {
        const predecessor = await Task(connect(getDbConnection(), portal)).find({
            name: {
     ...
     if (preceedingMilestones) {
        const predecessor = await ProjectMilestone(connect(getDbConnection(), portal)).find({
            name: {
    ...
    let totalList = await Task(connect(getDbConnection(), portal)).countDocuments(optionQuery);

    // Nếu calledId là 'get_all' thì bỏ qua page và perPage
    if (calledId === 'get_all') {
        tasks = await Task(connect(getDbConnection(), portal)).find(optionQuery).sort({ createdAt: -1 })
        ...
        tasks = await Task(connect(getDbConnection(), portal))
...
exports.checkImportTasks = async (data, portal, user) => {
    if (data?.length) {
        ...
        const taskCodeFilter = await Task(connect(getDbConnection(), portal)).find({ code: { $in: arrTaskCode } }).select('name parent level code');
        const parentCodeFilter = await Task(connect(getDbConnection(), portal)).find({ code: { $in: arrayParentCode } }).select('name parent level code');
...
exports.importTasks = async (dataConvert, portal, user) => {
    if (dataConvert?.length) {
        ...
        if (taskNotParentAndHasParentId?.length) {
            await Task(connect(getDbConnection(), portal)).insertMany(taskNotParentAndHasParentId);
        }
        ...
        if (task?.length) {
            ...
            for (let ts = 0; ts < taskLength; ts++) {
                const taskParent = await Task(connect(getDbConnection(), portal)).findOne({ code: task[ts].parent }).select('_id name parent');

                await Task(connect(getDbConnection(), portal)).create({
                    ...task[ts],
...
exports.checkImportUpdateTasks = async (data, portal, user) => {
    ...
    if (data?.length) {
        ...
        const allTask = await Task(connect(getDbConnection(), portal)).find({ code: { $in: arrTaskCode } }, {
            _id: 1,
            name: 1,
            code: 1,
        })
...
exports.importUpdateTasks = async (valueImport, portal, user) => {
    if (valueImport?.length) {
        ...
        for (let k = 0; k < importLength; k++) {
            ...
            await Task(connect(getDbConnection(), portal)).bulkWrite([
                {
                    updateOne: {
...
exports.importTaskActions = async (data, portal, user) => {
    if (data) {
        ...
        let getTaskInfo = [];
        if (arrayCode?.length) {
            getTaskInfo = await Task(connect(getDbConnection(), portal)).find({ code: { $in: arrayCode } }).select('code taskActions');
        }
        ...
        await Task(connect(getDbConnection(), portal)).bulkWrite(dataImport)
    }
...
exports.importTimeSheetLogs = async (data, portal, user) => {
    if (data) {
        ...
        let getTaskInfo = [];
        if (arrayCode?.length) {
            getTaskInfo = await Task(connect(getDbConnection(), portal)).find({ code: { $in: arrayCode } }).select('code timesheetLogs');
        }
...
exports.getOrganizationTaskDashboardChartData = async (query, portal, user) => {
    ...
    const organizationUnitTasks = await Task(connect(getDbConnection(), portal)).find(keySearch).sort({ 'createdAt': -1 })
        .populate({ path: 'organizationalUnit parent' })
        .populate({ path: 'creator', select: '_id name email avatar' })
        .populate({ path: 'responsibleEmployees', select: '_id name email avatar' })
...
exports.saveTaskAttributes = async (portal, taskId, data) => {
    ...
    await Task(connect(getDbConnection(), portal)).updateOne({ _id: taskId }, {
        attributes: dataAttr
    })

    let task = await Task(connect(getDbConnection(), portal)).findOne({ _id: taskId }).populate([
        { path: 'organizationalUnit parent' },
        { path: 'creator', select: '_id name email avatar' },
        { path: 'responsibleEmployees', select: '_id name email avatar' },
        { path: 'accountableEmployees', select: '_id name email avatar' },
...
exports.addTaskDelegation = async (portal, taskId, data) => {
    ...
    // let task = await Task(connect(getDbConnection(), portal)).findOne({ _id: taskId })
    // let taskDelegation =
    ...
    let updateTask = await Task(connect(getDbConnection(), portal)).findOne({ _id: taskId }).populate([
        { path: 'organizationalUnit parent' },
        { path: 'creator', select: '_id name email avatar' },
        { path: 'responsibleEmployees', select: '_id name email avatar' },
        { path: 'accountableEmployees', select: '_id name email avatar' },
...
exports.proposalPersonnel = async (portal, params, body) => {
    ...
    const organizationalUnits = await OrganizationalUnit(connect(getDbConnection(), portal)).find({
        _id: {
            $in: unitIds
        }
    })
