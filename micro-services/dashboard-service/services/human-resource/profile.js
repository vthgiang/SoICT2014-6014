...
exports.getAllPositionRolesAndOrganizationalUnitsOfUser = async (
    ...
    let roles = [],
        organizationalUnits = [];
    let user = await User(connect(getDbConnection(), portal)).findOne(
        {
    ...
    if (user !== null) {
        roles = await UserRole(connect(getDbConnection(), portal))
            .find({
                userId: user._id,
            })
            .populate([
...
exports.getEmployeeEmailsByOrganizationalUnitsAndPositions = async (
    ...
    let userRoles = await UserRole(connect(getDbConnection(), portal)).find({
        roleId: {
            $in: roles,
        },
    });
    ...
    let emailUsers = await User(connect(getDbConnection(), portal)).find(
        {
            _id: {
                $in: userId,
            },
...
exports.getEmployeeProfile = async (portal, email) => {
    let employees = await Employee(connect(getDbConnection(), portal)).find({
    ...
    if (employees.length === 0) {
        ...
        let salaries = await Salary(connect(getDbConnection(), portal)).find({
        ...
        let disciplines = await Discipline(connect(getDbConnection(), portal)).find(
            {
                employee: employees[0]._id,
            }
        );
        let courses = await Course(connect(getDbConnection(), portal)).find({
            results: {
                $elemMatch: {
                    employee: employees[0]._id,
                },
...
exports.updatePersonalInformation = async (portal, userId, data, avatar) => {
    ...
    let user = await User(connect(getDbConnection(), portal)).findById(userId);
    let employeeInfo = await Employee(connect(getDbConnection(), portal)).findOne(
        {
    ...
    await Employee(connect(getDbConnection(), portal)).findOneAndUpdate(
        {
    ...
    return await Employee(connect(getDbConnection(), portal)).find({
        _id: employeeInfo._id,
    });
};
...
exports.getEmployeeInforById = async (portal, id) => {
    return await Employee(connect(getDbConnection(), portal)).findById(id);
};
...
exports.getEmployeeInforByEmailInCompany = async (
    ...
    return await Employee(connect(getDbConnection(), portal)).findOne(
        {
            company: company,
...
exports.getEmployeeNumberExpiresContractInCurrentMonth = async (
    ...
    let results = await Employee(connect(getDbConnection(), portal)).find({
        company: company,
        status: {
            $in: [
                "active",
...
exports.searchEmployeeProfiles = async (portal, params, company) => {
    ...
    let listEmployees = await Employee(connect(getDbConnection(), portal))
        .find(keySearch, {
            field1: 1,
            employeeNumber: 1,
            emailInCompany: 1,
...
exports.createEmployee = async (portal, data, company, fileInfor) => {
    ...
    let createEmployee = await Employee(connect(getDbConnection(), portal)).create({
        avatar: avatar,
    ...
    if (data.disciplines !== undefined) {
        ...
        for (let x in disciplines) {
            await Discipline(connect(getDbConnection(), portal)).create({
    ...
    if (data.commendations !== undefined) {
        ...
        for (let x in commendations) {
            await Commendation(connect(getDbConnection(), portal)).create({
                employee: createEmployee._id,
                company: company,
...
exports.updateEmployeeInformation = async (
    ...
    let oldEmployee = await Employee(connect(getDbConnection(), portal)).findById(
        id
    );
    ...
    queryEditCreateDeleteDocumentInCollection = async (
        ...
        if (query.length !== 0) {
            await collection(connect(getDbConnection(), portal)).bulkWrite(query);
        }
    };
    ...
    if (employee.emailInCompany) {
        ...
        const user = await User(connect(getDbConnection(), portal)).findOne({
            email: employee.emailInCompany,
        });

        // Kiểm tra nhân viên được edit  có tài khoản người dùng hay chưa.
        const oldUser = await User(connect(getDbConnection(), portal))
            .findOne({
        ...
        if (
            ...
            await User(connect(getDbConnection(), portal)).findOneAndUpdate(
                {
                    email: oldMailInCompany,
                },
                {
...
exports.deleteEmployee = async (portal, id) => {
    ...
    await Discipline(connect(getDbConnection(), portal)).deleteMany({
        employee: id,
    });
    await Commendation(connect(getDbConnection(), portal)).deleteMany({
        employee: id,
    });
    await AnnualLeave(connect(getDbConnection(), portal)).deleteMany({
        employee: id,
    });
    await Salary(connect(getDbConnection(), portal)).deleteMany({
        employee: id,
    });
    await Timesheet(connect(getDbConnection(), portal)).deleteMany({
        employee: id,
    });
    if (employee?.avatar) {
        let deleteAvatar = `.${employee.avatar}`;
...
exports.checkEmployeeExisted = async (portal, employeeNumber, company) => {
    let employee = await Employee(connect(getDbConnection(), portal)).find(
        {
...
exports.createNotificationForEmployeesHaveBrithdayCurrent = async (portal) => {
    ...
    users = await User(connect(getDbConnection(), portal)).find(
        {
            email: {
                $in: emails,
            },
    ...
    for (let n in users) {
        ...
        let usersArr = await UserRole(connect(getDbConnection(), portal)).find(
            {
                roleId: {
                    $in: roles,
                },
    ...
    let result = await Notification(connect(getDbConnection(), portal)).insertMany(
        notifications
    );
};
...
exports.createNotificationEndOfContract = async (portal) => {
    ...
    for (let n in arrayTime) {
        ...
        let users = await User(connect(getDbConnection(), portal)).find(
            {
                email: {
                    $in: emails,
                },
    ...
    await Notification(connect(getDbConnection(), portal)).insertMany(
        notifications
    );
};
...
exports.importEmployeeInfor = async (portal, company, data) => {
    let employeeInfo = await Employee(connect(getDbConnection(), portal)).find(
        {
            company: company,
        },
        {
    ...
    if (rowError.length !== 0) {
        ...
        return await Employee(connect(getDbConnection(), portal)).insertMany(data);
    }
};
...
exports.importUpdateEmployeeInfor = async (portal, company, data) => {
    ...
    for (let i = 0; i < dataLength; i++) {
        ...
        if (
            ...
            const getUser = await User(connect(getDbConnection(), portal)).findOne({
    ...
    for (let i = 0; i < data?.length; i++) {
        if (data[i].updateFlag) {
            await Employee(connect(getDbConnection(), portal)).updateOne(
        ...
        if (data[i].updateUserFlag) {
            ...
            if (employee?.length)
                await User(connect(getDbConnection(), portal)).updateOne(
                    { email: employee[0].emailInCompany },
                    { email: data[i].emailInCompany }
                );
        }
...
exports.searchEmployeeForPackage = async (portal, params, companyId) => {
    ...
    listData = await Employee(connect(getDbConnection(), portal)).aggregate(
        keySearch
    );
...
exports.getEmployeeInforByListId = async (portal, listId, params) => {
    listEmployees = await Employee(connect(getDbConnection(), portal))
        .find({
            _id: {
                $in: listId,
            },
    ...
    totalList = await Employee(connect(getDbConnection(), portal)).countDocuments({
        _id: {
            $in: listId,
        },
    });
    ...
    for (let i = 0; i < listEmployees.length; i++) {
        ...
        let company = await Company(
            connect(getDbConnection(), process.env.DB_NAME)
        ).findById(idCompany);
        let contactPerson = await User(
            connect(getDbConnection(), company.shortName)
