...
exports.getAnnaulLeaveBeforAndAfterOneWeek =async (portal, organizationalUnits) =>{
    ...
    const annualLeave = await AnnualLeave(connect(getDbConnection(), portal)).find(keySearch)
    return annualLeave;
}
 ...
 */
exports.getNumberAnnaulLeave = async (portal, email, year) => {
    let employee = await Employee(connect(getDbConnection(), portal)).findOne({
    ...
    if (employee) {
        ...
        let annulLeaves = await AnnualLeave(connect(getDbConnection(), portal)).find({
        ...
        let listAnnualLeavesOfOneYear = await AnnualLeave(connect(getDbConnection(), portal)).find({
            employee: employee._id,
            startDate: {
                '$gt': firstDay,
                '$lte': lastDay
...
exports.getTotalAnnualLeave = async (portal, company, organizationalUnits, month) => {
    ...
    if (month !== undefined && month.length !== 0) {
        ...
        totalListOfYear = await AnnualLeave(connect(getDbConnection(), portal)).countDocuments({
            ...keySearch,
            startDate: {
                '$gt': firstDayOfYear,
                '$lte': lastDayOfYear
    ...
    let listAnnulLeave = await AnnualLeave(connect(getDbConnection(), portal)).find(keySearch);
    return {
 ...
 * @param {*} company : Id công ty
 */
exports.getAnnualLeaveByStartDateAndEndDate = async (portal, organizationalUnits, startDate, endDate) => {
    if (new Date(startDate).getTime() > new Date(endDate).getTime()) {
        ...
        if (organizationalUnits) {
            let listAnnualLeaveOfNumberMonth = await AnnualLeave(connect(getDbConnection(), portal)).find({
                status: 'approved',
                organizationalUnit: {
                    $in: organizationalUnits
                },
 ...
 */
 exports.getAnnualLeaveByStartDateAndEndDateUserOfOrganizationalUnits = async (portal,email, organizationalUnits, startDate, endDate) => {
    if (new Date(startDate).getTime() > new Date(endDate).getTime()) {
        return {
            arrMonth: [],
            listAnnualLeaveOfNumberMonth: [],
        ...
        if (organizationalUnits) {
            ...
            let employee = await Employee(connect(getDbConnection(), portal)).find(keySearchEmployee, {
                _id: 1
            });
            let listAnnualLeaveOfNumberMonth = await AnnualLeave(connect(getDbConnection(), portal)).find({
                status: 'approved',
...
const fetchNumberOfWaitForAppoval = async (portal, params) => {
    ...
    let arrayOfListAbsentLetter = await AnnualLeave(connect(getDbConnection(), portal)).aggregate([
        { $match: 
            keySearch 
        },
        { 
...
exports.searchAnnualLeaves = async (portal, params, company) => {
    ...
    let listAnnualLeaves = await AnnualLeave(connect(getDbConnection(), portal)).find(keySearch).populate({
            path: 'employee',
    ...
    let totalList = await AnnualLeave(connect(getDbConnection(), portal)).countDocuments(keySearch);
 ...
 */
exports.createAnnualLeave = async (portal, data) => {
    // Tạo mới thông tin nghỉ phép vào database
    let createAnnualLeave = await AnnualLeave(connect(getDbConnection(), portal)).create({
    ...
    let userReceiveds = [];
    const link = await Link(connect(getDbConnection(), portal)).find({ url: '/hr-annual-leave' });
    if (link.length) {
        const privilege = await Privilege(connect(getDbConnection(), portal)).find({ resourceId: link[0]._id });
        if (privilege.length) {
            let roleIds = [];
            for (let i in privilege) {
                roleIds.push(privilege[i].roleId);
            }
            const userRoles = await UserRole(connect(getDbConnection(), portal)).find({ roleId: { $in: roleIds } })
    ...
    const result =  await AnnualLeave(connect(getDbConnection(), portal)).findOne({
...
exports.deleteAnnualLeave = async (portal, id) => {
    return await AnnualLeave(connect(getDbConnection(), portal)).findOneAndDelete({
        _id: id
...
exports.updateAnnualLeave = async (portal, id, data) => {
    let annualLeave = await AnnualLeave(connect(getDbConnection(), portal)).findById(id);
    ...
    return await AnnualLeave(connect(getDbConnection(), portal)).findOne({
        _id: id
    }).populate([{
        path: 'employee',
        select: 'emailInCompany fullName employeeNumber'
 ...
 * @param {*} company : Id công ty
 */
exports.importAnnualLeave = async (portal, data) => {
    ...
    if (organizationalUnitId?.length) {
        ...
        for (let k = 0; k < organizationalUnitId?.length; k++) {
            ...
            let roles = [];
            let units = await OrganizationalUnit(connect(getDbConnection(), portal)).find({ '_id': organizationalUnitId[k] });
            ...
            users = await UserRole(connect(getDbConnection(), portal)).aggregate([
                {
...
console.log('6')
            await User(connect(getDbConnection(), portal)).populate(users, { path: 'userId', select: 'email' });

            let listMail = [];
            users?.length && users.forEach(x => listMail = [...listMail, x?.userId?.email]);

            // timf danh sach nhan vien thong qua danh sach email
            let listEmployeeInUnit = await Employee(connect(getDbConnection(), portal)).find({
                emailInCompany: {
                    $in: listMail
                }
            }, {
    ...
    if (rowError.length !== 0) {
        ...
        return await AnnualLeave(connect(getDbConnection(), portal)).insertMany(data);
    }
}

exports.getAnnualLeaveById = async (portal, id) => {
    return await AnnualLeave(connect(getDbConnection(), portal))
        .findById(id)
...
exports.requestToChangeAnnuaLeave = async (portal, data, id) => {
    ...
    let  result =  await AnnualLeave(connect(getDbConnection(), portal)).findByIdAndUpdate(id, {
        $set: {
    ...
    if (link.length) {
        ...
        if (privilege.length) {
            ...
            const userRoles = await UserRole(connect(getDbConnection(), portal)).find({ roleId: { $in: roleIds } }).populate({path: 'userId', select: 'name email'})
            if (userRoles && userRoles.length > 0) {
                for (let j in userRoles) {
