...
const { getDbConnection } = require('../../database/index')
...
exports.getRoles = async (portal, query) => {
    ...
    if (!page && !limit && !roleId) {
        return await Role(connect(getDbConnection(), portal))
            .find()
            .populate([
        ...
        const roles = await OrganizationalUnit(connect(getDbConnection(), portal))
            .findOne({
                $or: [
                    { 'managers': roleId },
        ...
        if (type === 'same') {
            ...
            let roleParents = await Role(connect(getDbConnection(), portal)).find({
                parents: roleId
            });
...
exports.createRole = async (portal, data) => {
    ...
    const checkRoleCreated = await Role(connect(getDbConnection(), portal)).findOne({ name: data.name }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    const roleTuTao = await RoleType(connect(getDbConnection(), portal))
        .findOne({ name: Terms.ROLE_TYPES.COMPANY_DEFINED });
    const role = await Role(connect(getDbConnection(), portal)).create({
        name: data.name.trim(),
...
exports.createRoleAttribute = async (portal, data) => {
    ...
    const roleAddAttribute = await Role(connect(getDbConnection(), portal)).find({
        _id: {
...
exports.createRootRole = async (portal, data) => {
    const rootRole = await RoleType(connect(getDbConnection(), portal))
        .findOne({ name: Terms.ROLE_TYPES.ROOT });
    const check = await Role(connect(getDbConnection(), portal))
        .findOne({ name: data.name });

    if (check !== null) throw ['role_name_exist'];

    const role = await Role(connect(getDbConnection(), portal))
        .create({
            name: data.name,
            type: rootRole._id,
            parents: data.parents
...
exports.createRolesForOrganizationalUnit = async (portal, data) => {
    const filterValidRoleArray = async (array) => {
        let resArray = [];
        if (array.length > 0) {
            const checkRoleValid = await Role(connect(getDbConnection(), portal)).findOne({ name: { $in: array } }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' });
    ...
    const employees = dataEmployee.length > 0 ?
        await Role(connect(getDbConnection(), portal)).insertMany(dataEmployee) : [];
    ...
    const deputyManagers = dataDeputyManager.length > 0 ?
        await Role(connect(getDbConnection(), portal)).insertMany(dataDeputyManager) : [];
    ...
    const managers = dataManager.length > 0 ?
        await Role(connect(getDbConnection(), portal)).insertMany(dataManager) : [];
...
exports.createRelationshipUserRole = async (portal, userId, roleId) => {
    const relationship = await UserRole(connect(getDbConnection(), portal))
        .create({
            userId,
            roleId
        });
...
exports.editRole = async (portal, id, data = {}) => {
    const role = await Role(connect(getDbConnection(), portal)).findById(id);
    const check = await Role(connect(getDbConnection(), portal)).findOne({ name: data.name }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
...
exports.editRelationshipUserRole = async (portal, roleId, userArr = []) => {
    const check = await Role(connect(getDbConnection(), portal)).findById(roleId);
    if (!check) throw ['role_not_found'];

    const checkDelegate = await UserRole(connect(getDbConnection(), portal)).find({ userId: { $in: userArr }, roleId: roleId, delegation: { $nin: [[], undefined] } })
    console.log('check', checkDelegate)
    if (checkDelegate.length > 0) { throw ['user_role_exist'] }

    const userRoleWithPolicies = await UserRole(connect(getDbConnection(), portal)).find({
        userId: { $in: userArr },
        roleId: roleId,
    ...
    await UserRole(connect(getDbConnection(), portal)).deleteMany({
        roleId: roleId,
    ...
    if (userArr.length > 0) {
        ...
        var relationship = await UserRole(connect(getDbConnection(), portal)).insertMany(user_role);
...
exports.deleteRole = async (portal, id) => {
    await Role(connect(getDbConnection(), portal)).deleteOne({ _id: id });
    const roles = await Role(connect(getDbConnection(), portal)).find({ parents: id });

    for (let i = 0; i < roles.length; i++) {
        const role = await Role(connect(getDbConnection(), portal)).findById(roles[i]._id);
        role.parents.splice(role.parents.indexOf(roles[i]._id), 1);
        await role.save();
    }
    await UserRole(connect(getDbConnection(), portal)).deleteMany({ roleId: id });
    await Privilege(connect(getDbConnection(), portal)).deleteMany({ roleId: id });
    const organD = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ managers: id });
    if (organD) {
        organD.managers.splice(organD.managers.indexOf(id));
        await organD.save();
    }

    const organV = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ deputyManagers: id });
    if (organV) {
        organV.deputyManagers.splice(organV.deputyManagers.indexOf(id));
        await organV.save();
    }

    const organE = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ employees: id });
    if (organE) {
...
exports.importRoles = async (portal, data) => {
    ...
    if (data?.length) {
        roleTuTao = await RoleType(connect(getDbConnection(), portal))
            .findOne({ name: Terms.ROLE_TYPES.COMPANY_DEFINED });

        const roles = await Role(connect(getDbConnection(), portal))
            .find({});
    ...
    if (rowError.length !== 0) {
        ...
        const role = await Role(connect(getDbConnection(), portal)).insertMany(dataConvert)
        let roleNames = [];
        if (role?.length) {
        ...
        const roleImports = await Role(connect(getDbConnection(), portal)).find({
            name: {
                $in: roleNames
            }
        }).populate([
