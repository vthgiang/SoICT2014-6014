...
exports.createPolicy = async (portal, data) => {
    ...
    const filterValidPolicyArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                const checkPolicyCreated = await Policy(connect(getDbConnection(), portal)).findOne({ policyName: array[i].policyName, category: 'Authorization' }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    const filterValidAttributeArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                // const attribute = await Attribute(connect(getDbConnection(), portal)).findOne({ _id: array[i].attributeId });
                if (array[i]) {
    ...
    if (polArray && polArray.length !== 0) {
        for (let i = 0; i < polArray.length; i++) {
            ...
            newPolicy = await Policy(connect(getDbConnection(), portal)).create({
    ...
    let policy = await Policy(connect(getDbConnection(), portal)).findById({ _id: newPolicy._id });
    return policy;
}
...
exports.createPolicyDelegation = async (portal, data) => {
    ...
    const filterValidPolicyArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                const checkPolicyCreated = await Policy(connect(getDbConnection(), portal)).findOne({ policyName: array[i].policyName, category: 'Delegation' }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
...
exports.getPolicies = async (portal, data) => {
    ...
    let totalList = await Policy(connect(getDbConnection(), portal)).countDocuments(keySearch);
    let policies = await Policy(connect(getDbConnection(), portal)).find(keySearch)
        .skip((page - 1) * perPage)
        .limit(perPage);

    if (!data.page && !data.perpage) {
        policies = await Policy(connect(getDbConnection(), portal)).find(keySearch)
    }
...
exports.satisfiedFunc = async (policy, portal) => {
    let policyId = policy;
    let polici = await Policy(connect(getDbConnection(), portal)).findOne({ _id: policyId })
    polici = polici.toObject();
    let currentPrivilegesHaveThisPolicy = await Privilege(connect(getDbConnection(), portal)).find({ policies: policyId }).populate({
        path: 'resourceId',
        select: '_id url description name'
    });
    let currentUserRoleHaveThisPolicy = await UserRole(connect(getDbConnection(), portal)).find({ policies: policyId }).populate([
...
exports.getOnlyPolicyName = async (portal, data) => {
    ...
    let totalList = await Policy(connect(getDbConnection(), portal)).countDocuments(keySearch);
    let PolicyCollection = await Policy(connect(getDbConnection(), portal)).find(keySearch, { policyName: 1 })
        .skip((page - 1) * perPage)
        .limit(perPage);
...
exports.getPolicyById = async (portal, id) => {
    let policy = await Policy(connect(getDbConnection(), portal)).findById({ _id: id });
    if (policy) {
        policy = policy.toObject();
        let currentPrivilegesHaveThisPolicy = await Privilege(connect(getDbConnection(), portal)).find({ policies: id }).populate({
            path: 'resourceId',
            select: '_id url description name'
        });
        let currentUserRoleHaveThisPolicy = await UserRole(connect(getDbConnection(), portal)).find({ policies: id }).populate([
            { path: 'roleId', select: '_id name' },
            { path: 'userId', select: '_id name email' },
        ]);
...
exports.editPolicy = async (portal, id, data) => {
    let oldPolicy = await Policy(connect(getDbConnection(), portal)).findById(id);
    if (!oldPolicy) {
        return -1;
    }
    const check = await Policy(connect(getDbConnection(), portal)).findOne({ policyName: data.policyName, category: 'Authorization' }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    await Policy(connect(getDbConnection(), portal)).updateOne({ _id: id }, {
        $set: {
    ...
    // Cach 2 de update
    // await Policy(connect(getDbConnection(), portal)).update({ _id: id }, { $set: data });
    let policy = await Policy(connect(getDbConnection(), portal)).findById({ _id: oldPolicy._id });
...
exports.editPolicyDelegation = async (portal, id, data) => {
    ...
    const check = await Policy(connect(getDbConnection(), portal)).findOne({ policyName: data.policyName, category: 'Delegation' }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
...
exports.deletePoliciesDelegation = async (portal, policyIds) => {
    const delegationsHavePolicy = await Delegation(connect(getDbConnection(), portal)).find({ delegatePolicy: { $in: policyIds } });
    ...
    if (delegationsHavePolicy.length > 0) {
        ...
        policies = await Policy(connect(getDbConnection(), portal))
...
exports.deletePolicies = async (portal, policyIds) => {
    let policies = await Policy(connect(getDbConnection(), portal))
        .deleteMany({ _id: { $in: policyIds.map(item => mongoose.Types.ObjectId(item)) } });
...
exports.cleanPolicy = async (portal, policyId) => {
    ...
    let currentPrivilegesHaveThisPolicy = await Privilege(connect(getDbConnection(), portal))
    ...
    if (currentPrivilegesHaveThisPolicy.length > 0) {
        ...
        currentPrivilegesHaveThisPolicy.forEach(async p => {
            if (p.policies.length == 1) {
                await Privilege(connect(getDbConnection(), portal)).deleteOne({ _id: p._id })
            }
            else {
    ...
    let currentUserRoleHaveThisPolicy = await UserRole(connect(getDbConnection(), portal)).find({ policies: policyId });
    if (currentUserRoleHaveThisPolicy.length > 0) {
        ...
        currentUserRoleHaveThisPolicy.forEach(async ur => {
            // await UserRole(connect(getDbConnection(), portal)).updateOne({ _id: ur._id }, {
            //     $set: {
            //         policies: ur.policies.splice(ur.policies.indexOf(policyId), 1)
            //     }
            // });
            // let urr = await UserRole(connect(getDbConnection(), portal)).find({ _id: ur.id })
...
exports.checkAllPolicies = async (portal) => {
    const allPolicies = await Policy(connect(getDbConnection(), portal)).find({ category: 'Authorization' });
...
exports.addPolicyToRelationship = async (portal, policyId) => {
    const policy = await Policy(connect(getDbConnection(), portal)).findById({ _id: policyId });
    const users = await User(connect(getDbConnection(), portal)).find().populate(
        {
            path: 'roles',
            populate: {
                path: 'roleId',
    ...
    const links = await Link(connect(getDbConnection(), portal)).find().populate([{ path: 'roles', populate: { path: 'roleId' } }, { path: 'components' }]);
    const components = await Component(connect(getDbConnection(), portal)).find().populate([{ path: 'roles', populate: { path: 'roleId' } }, { path: 'links' }]);
    ...
    satisfiedSubjects.forEach(async subject => {
        subject.roles.forEach(async role => {
            ...
            //     }
            // ]
            let userRole = await UserRole(connect(getDbConnection(), portal)).findOne({
                userId: subject.user,
                roleId: role
            });
    ...
    satisfiedLinks.forEach(async link => {
        satisfiedRoles.forEach(async role => {
            const existPrivilegeLink = await Privilege(connect(getDbConnection(), portal)).findOne({
                roleId: {
            ...
            if (!existPrivilegeLink) {
                newPrivilegeLink = await Privilege(connect(getDbConnection(), portal)).create({
                    roleId: role._id,
                    resourceId: link._id,
                    resourceType: 'Link',
                });
                const newPl = await Privilege(connect(getDbConnection(), portal)).findOne({
    ...
    satisfiedComponents.forEach(async component => {
        satisfiedRoles.forEach(async role => {
            const hasPrivilegeToComponentLink = await Privilege(connect(getDbConnection(), portal)).find({
                roleId: {
                    $in: [role._id].concat(role.parents),
                },
                resourceId: { $in: component.links }
            })
            const existPrivilegeComponent = await Privilege(connect(getDbConnection(), portal)).findOne({
                roleId: {
                    $in: [role._id].concat(role.parents),
                },
            ...
            if (hasPrivilegeToComponentLink && !existPrivilegeComponent) {
                newPrivilegeComponent = await Privilege(connect(getDbConnection(), portal)).create({
                    roleId: role._id,
                    resourceId: component._id,
                    resourceType: 'Component',
                });
                const newPc = await Privilege(connect(getDbConnection(), portal)).findOne({
