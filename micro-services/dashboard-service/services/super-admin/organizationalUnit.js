 ...
 * @id id công ty
 */
exports.getOrganizationalUnits = async (portal) => {
    return await OrganizationalUnit(connect(getDbConnection(), portal))
        .find() // { company: id }
        .populate([
            {
                path: 'managers',
 ...
 * @id id công ty
 */
exports.getOrganizationalUnitsAsTree = async (portal) => {
    const data = await OrganizationalUnit(connect(getDbConnection(), portal))
        .find() // { company: id }
        .populate([
            { path: 'managers' },
            { path: 'deputyManagers' },
...
exports.getChildrenOfOrganizationalUnitsAsTree = async (portal, role, organizationalUnitId = undefined) => {
    ...
    if (!organizationalUnitId) {
        ...
        organizationalUnit = await OrganizationalUnit(connect(getDbConnection(), portal)).findById(organizationalUnitId);
    }

    const data = await OrganizationalUnit(connect(getDbConnection(), portal)).find(); //{ company: id }
...
exports.getOrganizationalUnitByUserRole = async (portal, roleId) => {
    const department = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({
        $or: [
            { 'managers': roleId },
...
exports.getOrganizationalUnitsOfUser = async (portal, userId) => {
    const roles = await UserRole(connect(getDbConnection(), portal)).find({ userId });
    const newRoles = roles.map(role => role.roleId.toString());
    const departments = await OrganizationalUnit(connect(getDbConnection(), portal)).find({
        $or: [
...
exports.getOrganizationalUnitsOfUserByEmail = async (portal, email) => {
    let userId = await User(connect(getDbConnection(), portal)).findOne({ email: email }, { _id: 1 });
    if (userId) {
        const roles = await UserRole(connect(getDbConnection(), portal)).find({ userId: userId._id });
        const newRoles = roles.map(role => role.roleId.toString());
        const departments = await OrganizationalUnit(connect(getDbConnection(), portal)).find({
            $or: [
                { 'managers': { $in: newRoles } },
...
exports.getOrganizationalUnitsThatUserIsManager = async (portal, userId) => {
    const roles = await UserRole(connect(getDbConnection(), portal)).find({ 'userId': userId });
    const newRoles = roles.map(role => role.roleId);
    const departments = await OrganizationalUnit(connect(getDbConnection(), portal)).find({ 'managers': { $in: newRoles } });
    return departments;
}
...
exports.createOrganizationalUnit = async (portal, data, managerArr = [], deputyManagerArr = [], employeeArr = []) => {
    const checkDepartment = await OrganizationalUnit(connect(getDbConnection(), portal))
    ...
    const department = await OrganizationalUnit(connect(getDbConnection(), portal))
        .create({
            name: data.name.trim(),
...
exports.editOrganizationalUnit = async (portal, id, data) => {
    ...
    const checkDepartment = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ name: data.name }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    if (ObjectId.isValid(data.parent)) {
        const upOrg = await OrganizationalUnit(connect(getDbConnection(), portal)).findById(data.parent);
...
//     const roleChucDanh = await RoleType(connect(getDbConnection(), portal)).findOne({ name: Terms.ROLE_TYPES.POSITION });
//     const managerAb = await Role(connect(getDbConnection(), portal)).findOne({ name: Terms.ROOT_ROLES.MANAGER.name });
//     const deputyManagerAb = await Role(connect(getDbConnection(), portal)).findOne({ name: Terms.ROOT_ROLES.DEPUTY_MANAGER.name });
//     const employeeAb = await Role(connect(getDbConnection(), portal)).findOne({ name: Terms.ROOT_ROLES.EMPLOYEE.name });
//     const department = await OrganizationalUnit(connect(getDbConnection(), portal))
//         .findById(id)
//         .populate([
//             { path: 'managers' },
...
//         const updateRole = await Role(connect(getDbConnection(), portal)).findById(employees.editRoles[i]._id);
...
//     const newEmployees = await Role(connect(getDbConnection(), portal)).insertMany(newDataEmployees);
...
//         await Role(connect(getDbConnection(), portal)).updateOne({ _id: deputyManagers.editRoles[i]._id }, {
...
//     const newDeputyManagers = await Role(connect(getDbConnection(), portal)).insertMany(newDataDeputyManagers);
...
//         await Role(connect(getDbConnection(), portal)).updateOne({ _id: managers.editRoles[i]._id }, {
...
//     const newManagers = await Role(connect(getDbConnection(), portal)).insertMany(newDataManagers);
...
//     const departmentSave = await OrganizationalUnit(connect(getDbConnection(), portal)).findById(id);
...
exports.editRolesInOrganizationalUnit = async (portal, id, data) => {
    ...
    const validRole = async (inputRole) => {
        const checkRole = await Role(connect(getDbConnection(), portal)).findOne({ name: inputRole.name }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' })
    ...
    const filterValidRoleArray = async (array) => {
        let resArray = [];
        if (array.length > 0) {
            const checkRoleValid = await Role(connect(getDbConnection(), portal)).findOne({ name: { $in: array.map(role => role.name) } }).collation({ 'locale': 'vi', strength: 2, alternate: 'shifted', maxVariable: 'space' });
    ...
    const newEmployees = newDataEmployees.length > 0 ? await Role(connect(getDbConnection(), portal)).insertMany(newDataEmployees) : [];
    const employeeIdArr = [...newEmployees.map(em => em._id), ...department.employees.map(em => em._id)]; //id của tất cả các employee trong đơn vị dùng để kế thừa cho vicemanager

    //2.Chỉnh sửa phó đơn vị

    for (let i = 0; i < deputyManagers.editRoles.length; i++) {
        const updateRole = await Role(connect(getDbConnection(), portal)).findById(deputyManagers.editRoles[i]._id);
    ...
    const newDeputyManagers = newDataDeputyManagers.length > 0 ? await Role(connect(getDbConnection(), portal)).insertMany(newDataDeputyManagers) : [];
    const deputyManagerIdArr = [...newDeputyManagers.map(vice => vice._id), ...department.deputyManagers.map(vice => vice._id)]; //id của tất cả các deputyManager trong đơn vị dùng để kế thừa cho manager

    //3.Chỉnh sửa trưởng đơn vị

    for (let i = 0; i < managers.editRoles.length; i++) {
        const updateRole = await Role(connect(getDbConnection(), portal)).findById(managers.editRoles[i]._id);
    ...
    const newManagers = newDataManagers.length > 0 ? await Role(connect(getDbConnection(), portal)).insertMany(newDataManagers) : [];
    const managerIdArr = [...newManagers.map(manager => manager._id), ...department.managers.map(manager => manager._id)]; //id của tất cả các manager
...
exports.deleteOrganizationalUnit = async (portal, departmentId) => {
    const department = await OrganizationalUnit(connect(getDbConnection(), portal)).findById(departmentId);

    const roles = await Role(connect(getDbConnection(), portal)).find({
        _id: { $in: [...department.managers, ...department.deputyManagers, ...department.employees] }
    });

    const userroles = await UserRole(connect(getDbConnection(), portal)).find({
        roleId: { $in: roles.map(role => role._id) }
    });

    if (userroles.length === 0) {
        await Role(connect(getDbConnection(), portal)).deleteMany({
            _id: { $in: roles.map(role => role._id) }
        });

        if (department.parent) {
            await OrganizationalUnit(connect(getDbConnection(), portal)).updateMany({
                parent: department._id
            }, {
                $set: { parent: department.parent }
            });
        }
        return await OrganizationalUnit(connect(getDbConnection(), portal)).deleteOne({ _id: departmentId });
    } else {
...
exports.importOrganizationalUnits = async (portal, data) => {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].parent) {
            let parent = await OrganizationalUnit(connect(getDbConnection(), portal)).findOne({ name: data[i].parent });
            if (parent) {
