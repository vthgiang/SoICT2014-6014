            ...
            if (!req.thirdParty) {
                ...
                if (req.user) {
                    const user = await User(
                        connect(await getDbConnection()(), req.portal)
                 ...
                 */
                initModels(connect(await getDbConnection()(), req.portal), Models);
                ...
                if (crtp !== '/') {
                    ...
                    const role = await Role(connect(await getDbConnection()(), req.portal))
                    ...
                    const userrole = await UserRole(connect(await getDbConnection()(), req.portal)).findOne({ userId, roleId: role._id });
                    ...
                    if (role.name !== 'System Admin') {
                        const company = await Company(connect(await getDbConnection()(), process.env.DB_NAME)).findById(req.user.company._id);
                        if (!company.active) {
                    ...
                    if (!device) {
                        if (checkPage) {
                            const link = role.name !== 'System Admin' ?
                                await Link(connect(await getDbConnection()(), req.portal)).findOne({ url, deleteSoft: false }) :
                                await Link(connect(await getDbConnection()(), req.portal)).findOne({ url });

                            if (link === null) throw ['url_invalid'];
                            const roleArr = [role._id].concat(role.parents);
                            const privilege = await Privilege(connect(await getDbConnection()(), req.portal)).findOne({
                                resourceId: link._id,
                                resourceType: 'Link',
                                roleId: {
                                    $in: roleArr,
                            ...
                            if (userrole.delegation) {
                                ...
                                const delegation = await Delegation(connect(await getDbConnection()(), req.portal)).findOne({ _id: userrole.delegation, status: 'activated', delegateType: 'Role' })
                                ...
                                if (delegation) {
                                    ...
                                    if ((!delegation.logs || delegation.logs.length == 0) || (delegation.logs.length > 0 && ((new Date()).getTime() - (new Date(delegation.logs[delegation.logs.length - 1].createdAt)).getTime()) / 1000 > 5)) {
                                        ...
                                        await Delegation(connect(await getDbConnection()(), req.portal)).updateOne({ _id: delegation._id }, {
                                            logs: [
                                                ...delegation.logs,
                                                {
                                                    createdAt: new Date(),
                            ...
                            const delegationPending = await Delegation(connect(await getDbConnection()(), req.portal)).find({ delegatee: req.user._id, status: 'pending', delegateType: 'Role' })
                ...
                let systemApi = await SystemApi(connect(await getDbConnection()(), process.env.DB_NAME))
                    .findOne({
                ...
                let privilegeApi = await PrivilegeApi(connect(await getDbConnection()(), req.portal))
                    .findOne({
                        token,
                ...
                // let apiInCompany = await Company(connect(await getDbConnection()(), process.env.DB_NAME))
                //     .findOne({
                //         apis: {
                //             $in: [systemApi?._id]
                //         },
                ...
                req.user.company = await Company(connect(await getDbConnection()(), process.env.DB_NAME))
...
exports.authAdminSuperAdmin = async (req, res, next) => {
    try {
        ...
        initModels(connect(getDbConnection(), req.portal), Models);

        let ad = await Role(connect(getDbConnection(), portal)).find({
        ...
        let userrole = await UserRole(connect(getDbConnection(), portal)).find({
            userId,
