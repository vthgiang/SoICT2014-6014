...
const getDocuments = async (
  ...
  if (!(page || limit)) {
    ...
    allDocs.sort((a, b) => {
      let tmpA =
        a.versions.length && a.versions[a.versions.length - 1].issuingDate
          ? a.versions[a.versions.length - 1].issuingDate
          : new Date(1900, 1, 1, 0, 3, 3, 0);
      let tmpB =
        b.versions.length && b.versions[b.versions.length - 1].issuingDate
        ...
        let version = {
          ...
          file:
            data.numberFile[i] == 1 && data.files
              ...
              : '',
          scannedFileOfSignedDocument:
            data.numberFileScan[i] == 1 && data.scannedFileOfSignedDocument
...
const downloadAllFileOfDocument = async (query, portal) => {
  let { data } = query;
      ...
      if (fs.existsSync(parentPath) && data[i]?.versions?.length) {
        for (let j = 0; j < data[i].versions.length; j++) {
          ...
          if (data[i]?.versions[j]?.scannedFileOfSignedDocument) {
            ...
            const commandGetFileScan = `cp ${SERVER_DIR + fileScanPath
              } ${versionPath}`;
...
const importDocument = async (portal, data, company) => {
  for (let i in data) {
    let document = {
      name: data[i].name,
 ...
 */
const getDocumentCategories = async (portal, query, company) => {
  let page = query.page;
...
const importDocumentCategory = async (portal, data, company) => {
  for (let i in data) {
    description = data[i].description;
    let category = {
      name: data[i].name,
  ...
  switch (option) {
    case 'downloaded': //những tài liệu văn bản mà người dùng đã tải xuống
      ...
      let doc = allDocsDown.slice(
        (page - 1) * limit,
        page * limit > allDocsDown.length ? allDocsDown.length : page * limit
      );
    ...
    case 'latest': //những tài liệu văn bản mà người dùng chưa xem qua lần nào
      ...
      let docLast = allDocsLast.slice(
        (page - 1) * limit,
        page * limit > allDocsLast.length ? allDocsLast.length : page * limit
      );
...
const importDocumentDomain = async (portal, data, company) => {
  let results = [];
  for (let i in data) {
    description = data[i].description;
    let domain = {
...
const createDocumentArchive = async (portal, data, company) => {
  let query = {
    company,
    name: data.name,
    description: data.description,
...
const deleteManyDocumentArchive = async (array, portal, company) => {
  for (let i = 0; i < array.length; i++) {
...
const importDocumentArchive = async (portal, data, company) => {
  for (let i in data) {
    description = data[i].description;
    let archive = {
      name: data[i].name,
