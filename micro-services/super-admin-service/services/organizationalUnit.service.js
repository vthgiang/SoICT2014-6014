...
exports.getOrganizationalUnit = async (portal, id) => {
    return await OrganizationalUnit(connect(DB_CONNECTION, portal))
        ...
        .populate([
            ...
            {
                path: 'deputyManagers',
                populate: {path: 'users', populate: {path: 'userId', select: "_id name email avatar"}}
            },
    ...
    const newData = data.map(department => {
        return {
            ...
            managers: department.managers.map(manager => {
                return {_id: manager._id.toString(), name: manager.name}
            }),
            deputyManagers: department.deputyManagers.map(deputyManager => {
                return {_id: deputyManager._id.toString(), name: deputyManager.name}
            }),
            employees: department.employees.map(employee => {
                return {_id: employee._id.toString(), name: employee.name}
            }),
...
exports.editOrganizationalUnit = async (portal, id, data) => {
    ...
    const checkDepartment = await OrganizationalUnit(connect(DB_CONNECTION, portal)).findOne({name: data.name}).collation({
        "locale": "vi",
        strength: 2,
        alternate: "shifted",
        maxVariable: "space"
...
exports.editRolesInOrganizationalUnit = async (portal, id, data) => {
    ...
    const validRole = async (inputRole) => {
        const checkRole = await Role(connect(DB_CONNECTION, portal)).findOne({name: inputRole.name}).collation({
            "locale": "vi",
            strength: 2,
            alternate: "shifted",
            maxVariable: "space"
    ...
    const filterValidRoleArray = async (array) => {
        let resArray = [];
        if (array.length > 0) {
            const checkRoleValid = await Role(connect(DB_CONNECTION, portal)).findOne({name: {$in: array.map(role => role.name)}}).collation({
                "locale": "vi",
                strength: 2,
                alternate: "shifted",
                maxVariable: "space"
    ...
    for (let i = 0; i < deputyManagers.editRoles.length; i++) {
        ...
        if (updateRole.name.trim().toLowerCase().replace(/ /g, "") !== deputyManagers.editRoles[i].name.trim().toLowerCase().replace(/ /g, "")) {
            if (valid) {
                throw ['role_deputy_manager_exist'];
            }
        }
        await Role(connect(DB_CONNECTION, portal)).updateOne({_id: deputyManagers.editRoles[i]._id}, {
