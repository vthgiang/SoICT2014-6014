    ...
    const filterValidAttributeArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                const checkAttributeCreated = await Attribute(connect(DB_CONNECTION, portal)).findOne({attributeName: array[i].attributeName}).collation({
                    "locale": "vi",
                    strength: 2,
                    alternate: "shifted",
                    maxVariable: "space"
...
exports.editAttribute = async (portal, id, data) => {
    ...
    const check = await Attribute(connect(DB_CONNECTION, portal)).findOne({attributeName: data.attributeName}).collation({
        "locale": "vi",
        strength: 2,
        alternate: "shifted",
        maxVariable: "space"
