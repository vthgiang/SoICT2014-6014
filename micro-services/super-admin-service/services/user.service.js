...
const OrganizationalUnitService = require('../services/organizationalUnit.service');
...
exports.getAllEmployeeOfUnitByIds = async (portal, query) => {
    ...
    await User(connect(DB_CONNECTION, portal)).populate(employees, {
        path: "userId",
        select: "email _id name avatar active"
    });
...
exports.editRolesForUser = async (portal, userId, roleIdArr) => {
    const check = await UserRole(connect(DB_CONNECTION, portal)).find({
        userId: userId,
        roleId: {$in: roleIdArr},
