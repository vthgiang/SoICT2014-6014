    ...
    const filterValidPolicyArray = async (array) => {
        ...
        if (array.length > 0) {
            ...
            for (let i = 0; i < array.length; i++) {
                const checkPolicyCreated = await Policy(connect(DB_CONNECTION, portal)).findOne({
                    policyName: array[i].policyName,
...
exports.editPolicy = async (portal, id, data) => {
    ...
    const check = await Policy(connect(DB_CONNECTION, portal)).findOne({
...
exports.addPolicyToRelationship = async (portal, policyId) => {
    ...
    const links = await Link(connect(DB_CONNECTION, portal)).find().populate([{
        path: 'roles',
        populate: {path: 'roleId'}
    }, {path: 'components'}]);
    const components = await Component(connect(DB_CONNECTION, portal)).find().populate([{
        path: 'roles',
        populate: {path: 'roleId'}
    }, {path: 'links'}]);
