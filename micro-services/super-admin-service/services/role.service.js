...
exports.createRole = async (portal, data) => {
    ...
    const checkRoleCreated = await Role(connect(DB_CONNECTION, portal)).findOne({name: data.name}).collation({
        "locale": "vi",
        strength: 2,
        alternate: "shifted",
        maxVariable: "space"
...
exports.createRolesForOrganizationalUnit = async (portal, data) => {
    const filterValidRoleArray = async (array) => {
        let resArray = [];
        if (array.length > 0) {
            const checkRoleValid = await Role(connect(DB_CONNECTION, portal)).findOne({name: {$in: array}}).collation({
                "locale": "vi",
                strength: 2,
                alternate: "shifted",
                maxVariable: "space"
...
exports.editRole = async (portal, id, data = {}) => {
    ...
    const check = await Role(connect(DB_CONNECTION, portal)).findOne({name: data.name}).collation({
        "locale": "vi",
        strength: 2,
        alternate: "shifted",
        maxVariable: "space"
...
exports.editRelationshipUserRole = async (portal, roleId, userArr = []) => {
    ...
    const checkDelegate = await UserRole(connect(DB_CONNECTION, portal)).find({
        userId: {$in: userArr},
        roleId: roleId,
        delegation: {$nin: [[], undefined]}
    })
