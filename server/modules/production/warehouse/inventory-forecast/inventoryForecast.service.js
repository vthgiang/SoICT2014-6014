...
exports.saveForecasts = async (forecasts, portal) => {
    ...
    await InventoryForecast(dbConnection).insertMany(forecasts);
};
...
exports.getAllForecasts = async (query, portal) => {
    ...
    if (!page || !limit) {
        let allForecasts = await InventoryForecast(dbConnection)
            .find(option)
            .populate([
                {
                    path: "good",
...
exports.getForecastById = async (id, portal) => {
    const dbConnection = connect(DB_CONNECTION, portal);
    return await InventoryForecast(dbConnection).findById(id).populate([
        {
            path: "good",
            select: "code name"
        }
