const { KpiTemplate, Privilege, Role, UserRole, OrganizationalUnit, User } = require(`../../../models`);
...
exports.getAllKpiTemplates = async (portal, query) => {
    if (query.pageNumber === '1' && query.noResultsPerPage === '0') {
        ...
        let docs = await KpiTemplate(connect(DB_CONNECTION, portal)).find().populate([
    ...
    if (query.roleId) {
        ...
        return await KpiTemplate(connect(DB_CONNECTION, portal))
            .paginate(option, {
                page: pageNumber,
        ...
        return await KpiTemplate(connect(DB_CONNECTION, portal)).find();
    }
}
 ...
 */
exports.createKpiTemplate = async (portal, body, userId) => {
    //kiểm tra tên mẫu công việc đã tồn tại hay chưa ?
    let checkKpiTemplate = await KpiTemplate(connect(DB_CONNECTION, portal)).findOne({ name: body.name });
    ...
    for (let i in body.kpiInformations) {
        if (body.kpiInformations[i].description) {
            let str = body.kpiInformations[i].description;
            let vt = str.indexOf("&nbsp;");
            let st;
            while (vt >= 0) {
                if (vt == 0) {
            ...
            body.kpiInformations[i].description = str;
        }
    }
    //Tạo dữ liệu mẫu công việc
    var kpitemplate = await KpiTemplate(connect(DB_CONNECTION, portal)).create({
        ...
        kpiActions: body.kpiActions.map(item => {
            return {
                name: item.name,
        ...
        kpiInformations: body.kpiInformations.map((item, key) => {
            return {
...
exports.deleteKpiTemplate = async (portal, id) => {
    var template = await KpiTemplate(connect(DB_CONNECTION, portal)).findByIdAndDelete(id); // xóa mẫu công việc theo id
    var privileges = await Privilege(connect(DB_CONNECTION, portal)).deleteMany({
        resourceId: id, //id của kpi template
        resourceType: "KpiTemplate"
    });
...
exports.editKpiTemplate = async (portal, data, id) => {
    ...
    var kpiTemplate = await KpiTemplate(connect(DB_CONNECTION, portal)).findByIdAndUpdate(id,
        {
            $set: {
                name: data.name,
 ...
 * @param {*} id : id user
 */
exports.importKpiTemplate = async (portal, data, id) => {
    let results = [];
    for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].readByEmployees[0]) {
            for (let j = 0; j < data[i].readByEmployees.length; j++) {
                ...
                switch (data[i].readByEmployees[j]) {
                    case "Trưởng đơn vị": data[i].readByEmployees[j] = "Manager"; break;
                    case "Phó đơn vị": data[i].readByEmployees[j] = "Deputy Manager"; break;
                    case "Nhân viên đơn vị": data[i].readByEmployees[j] = "Employee"; break;
                    default: break;
                }
        ...
        for (let j = 0; j < data[i].kpiInformations.length; j++) {
            if (data[i].kpiInformations[j][0]) {
                // format thong tin "chi qua ly duoc dien"
                if (data[i].kpiInformations[j][3] === "true") {
                    data[i].kpiInformations[j]["filledByAccountableEmployeesOnly"] = "true";
                } else {
                    data[i].kpiInformations[j]["filledByAccountableEmployeesOnly"] = "false";
                }

                // formart thong tin kieu du lieu
                data[i].kpiInformations[j]["type"] = data[i].kpiInformations[j][2].toLowerCase();
                data[i].kpiInformations[j]["name"] = data[i].kpiInformations[j][0];
                data[i].kpiInformations[j]["description"] = data[i].kpiInformations[j][1];
                data[i].kpiInformations[j]["extra"] = "";
            } else {
                if (!data[i].kpiInformations[j][0]) {
                    data[i].kpiInformations[j] = [];
                    // break;
                }
                data[i].kpiInformations.splice(j, 1);
                j--;
            }
        }

        for (let j = 0; j < data[i].kpiActions.length; j++) {
            if (data[i].kpiActions[j][0]) {
                if (data[i].kpiActions[j][2] === "Bắt buộc" || data[i].kpiActions[j][2] === "true") {
                    data[i].kpiActions[j]["mandatory"] = true;
                } else {
                    data[i].kpiActions[j]["mandatory"] = false;
                }
                data[i].kpiActions[j]["name"] = data[i].kpiActions[j][0];
                data[i].kpiActions[j]["description"] = data[i].kpiActions[j][1];
            } else {
                if (!data[i].kpiActions[j][0]) {
                    data[i].kpiActions[j] = [];
                    // break;
                }
                data[i].kpiActions.splice(j, 1);
                j--;
            }
        }
        switch (data[i].priority) {
            case "Cao": data[i].priority = 3; break;
            case "Thấp": data[i].priority = 2; break;
            default: data[i].priority = 1;
        }
        ...
        let result = await this.createKpiTemplate(portal, data[i], id);
