...
class Pert {
    ...
    childs(task){
        return this.tasks.filter(t => (task.childList.includes(t.taskID)))
    }
    updateChildParent(){
    ...
    getCriticalPath(){
        // console.log(this.tasks.filter(t => t.slack==0).map(t1=> t1.taskId))
    ...
    getTotalProb() {
        ...
        // console.log(zScore)
        return {totalProb:ztable(zScore),totalDuration:totalDuration,criticalPath:criticalPath.map(task=> task.taskID)}
    }
}
// export default Pert;
module.exports = Pert