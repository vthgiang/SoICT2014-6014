...
exports.getEmployeeProfile = async (portal, email) => {
  let employees = await Employee(connect(DB_CONNECTION, portal))
    .find({
...
exports.getEmployees = async (
  ...
  if (allInfor === true) {
    ...
    let listAllEmployees = await Employee(connect(DB_CONNECTION, portal)).find(
      keySearch
    );
...
exports.searchEmployeeProfiles = async (portal, params, company) => {
  ...
  if (params?.certificates) {
    keySearch = {
      ...keySearch,
      certificates: {
        $elemMatch: { name: { $regex: params.certificates, $options: "i" } },
      },
    };
  }
...
exports.createEmployee = async (portal, data, company, fileInfor) => {
  ...
  let { degrees, certificates, career, major, contracts, files } = data;
...
exports.createNotificationEndOfContract = async (portal) => {
  ...
  for (let n in arrayTime) {
    ...
    for (let i in employees) {
      let infoCourses = await EmployeeCourse(connect(DB_CONNECTION, portal))
        .find(
          {
      ...
      let endDateCommitmentTimes = infoCourses.map((x) => {
        ...
        let endDateCommitmentTime = new Date(
          ...
          endDateCourse.getMonth() + Number(x.course.employeeCommitmentTime),
          ...
          `${
            employees[index].endDateCommitmentTime
              ? " Tuy nhiên bạn phải làm thêm đến ngày " +
                this.formatDate(employees[index].endDateCommitmentTime, false) +
  ...
  for (let i = 0; i < dataLength; i++) {
    ...
    if (data[i].employeeNumber)
      checkEmployeeNumber = employeeInfo.filter(
        (y) => y.employeeNumber.toString() === data[i].employeeNumber.toString()
      );
        ...
        if (user) {
          ...
          for (let k = 0; k < data[i]?.positionId?.length; k++) {
            if (data[i]?.positionId[k])
              await UserRole(connect(DB_CONNECTION, portal)).create({
                userId: user._id,
  ...
  if (rowError.length !== 0) {
    ...
    for (let x of importData) {
      ...
      editEmployee.socialInsuranceDetails =
        editEmployee.socialInsuranceDetails.concat(x.socialInsuranceDetails);
      editEmployee.save();
    }
    return data;
  }
...
exports.searchEmployeeForPackage = async (portal, params, companyId) => {
  ...
  if (params.careerPosition && params.sameExp) {
    keySearch = [
      ...keySearch,
      {
      ...
      {
        $match: {
          "careerPositions.careerPosition": mongoose.Types.ObjectId(
...
exports.getEmployeeByPackageId = async (portal, biddingPackageId, companyId) => {
  let biddingPackage = await BiddingPackage(connect(DB_CONNECTION, portal))
    .find({
      _id: {
  ...
  let numberEmployees = biddingPackage[0]?.keyPersonnelRequires.map(item => item.count)
  console.log("numberEmployees", numberEmployees)

  for (const require of biddingPackage[0]?.keyPersonnelRequires) {
    if (require) {
      let params = {
        careerPosition: require?.careerPosition ? String(require?.careerPosition) : NaN,
        professionalSkill: require?.professionalSkill ? String(require?.professionalSkill) : NaN,
        majors: require?.majors ? require?.majors?.map(item => String(item)) : NaN,
        certificates: require?.certificateRequirements?.certificates ? require?.certificateRequirements?.certificates?.map(item => String(item)) : NaN,
        certificatesCount: require?.certificateRequirements?.count ? require?.certificateRequirements?.count : 0,
        exp: require?.numberYearsOfExperience ? require?.numberYearsOfExperience : NaN,
        sameExp: require?.experienceWorkInCarreer ? require?.experienceWorkInCarreer : NaN,
        numblePackageWorkInCarreer: require?.numblePackageWorkInCarreer ? require?.numblePackageWorkInCarreer : NaN,
        page: 0,
        limit: 500,
      }
      // console.log("params", params)
      let employee = await this.searchEmployeeForPackage(portal, params, companyId);
  ...
  if (data.isComplete == 1) {
    ...
    for (const [index, item] of data.result.entries()) {
      ...
      listEmployees.push({
        careerPosition: biddingPackage[0]?.keyPersonnelRequires[index].careerPosition,
...
const findEmployee = async (keyPeople, otherPeople, result, resultIndex, number, index) => {
  ...
  if (result[index]) {
    ...
    for (let i = 0; i<=t; i++) {
      ...
      while (value < n - 1) {
        ...
        if (!otherPeople.includes(a[value])) {
          ...
          index = index + 1;
          let data = await findEmployee(keyPeople, otherPeople, result, resultIndex, number, index);
          return {
            result: data["result"],
    ...
    if ( key == 1 ) {
      let data = await findEmployee(keyPeople, otherPeople, result, resultIndex, number, index + 1);
      return {
        result: data["result"],
        isComplete: data["isComplete"]
      } 
    ...
    if ( key == 0 ) {
      ...
      let data = await findEmployee(keyPeople, oldOtherPeople, oldResult, oldResultIndex, number, index - 1);
      return {
        result: data["result"],
        isComplete: data["isComplete"]
      }
...
exports.getEmployeeInforByListId = async (portal, listId, companyId, params) => {
