...
exports.getEmployeeProfile = async (req, res) => {
    try {
        let inforEmployee;
        if (req.query.callAPIByUser === "true") {
            // Theo uerId
            let user = await UserService.getUser(req.portal, req.params.id);
            inforEmployee = await EmployeeService.getEmployeeProfile(
                req.portal,
                user.email
            );
        } else {
            // Theo employeeId
            let employee = await EmployeeService.getEmployeeInforById(
                req.portal,
                req.params.id
            );
            inforEmployee = await EmployeeService.getEmployeeProfile(
                req.portal,
                employee.emailInCompany
            );
        }
...
exports.updatePersonalInformation = async (req, res) => {
    try {
        ...
        let data = await EmployeeService.updatePersonalInformation(
            req.portal,
            req.params.userId,
            req.body,
            avatar
    ...
    try {
        ...
        if (req.query.exportData) {
            ...
            for (let i = 0; i < arrEmail.length; i++) {
                let employee = await EmployeeService.getEmployeeProfile(
                    req.portal,
                    arrEmail[i]
                );
                data = [...data, employee];
            ...
            data = await EmployeeService.getEmployeesByStartingAndLeaving(
                req.portal,
                req.query.organizationalUnits,
                req.query.startDate,
                req.query.endDate,
            ...
            let listId = await EmployeeService.searchEmployeeForPackage(
                req.portal,
                params,
                req.user.company
            );
            ...
            data = await EmployeeService.searchEmployeeProfiles(
                req.portal,
                params,
                req.user.company._id
            );
...
exports.createEmployee = async (req, res) => {
    try {
        ...
        let fileDegree = req.files ? req.files.fileDegree : undefined,
            ...
            fileExperience = req.files ? req.files.fileExperience : undefined,
            fileCareerPosition = req.files
                ? req.files.fileCareerPosition
                : undefined,
            file = req.files ? req.files.file : undefined,
            healthInsuranceAttachment = req.files
                ? req.files.healthInsuranceAttachment
                : undefined;
        let fileInfor = {
            fileDegree,
            fileCertificate,
        ...
        if (
            ...
            let checkMSNV = await EmployeeService.checkEmployeeExisted(
                req.portal,
            ...
            if (checkMSNV === true) {
                ...
                let checkEmail =
                    await EmployeeService.checkEmployeeCompanyEmailExisted(
                        req.portal,
                        emailInCompany
                    );
                if (checkEmail === true) {
                    ...
                    let data = await EmployeeService.createEmployee(
                        req.portal,
                        req.body,
                        req.user.company._id,
                        fileInfor
...
exports.updateEmployeeInformation = async (req, res) => {
    try {
        ...
        // Kiểm tra dữ liệu truyền vào
        if (
            req.body.employee.employeeNumber === undefined ||
            req.body.employee.employeeNumber.trim() === ""
        ) {
            throw ["employee_number_required"];
        } else if (
            req.body.employee.fullName === undefined ||
            req.body.employee.fullName.trim() === ""
        ) {
            throw ["full_name_required"];
        } else {
            let oldEmployee = await EmployeeService.getEmployeeInforById(
                req.portal,
                req.params.id
            );
            if (
                req.body.employee.employeeNumber !== oldEmployee.employeeNumber
            ) {
                // Kiểm tra sự tồn tại của mã nhân viên
                let checkMSNV = await EmployeeService.checkEmployeeExisted(
                    req.portal,
                    req.body.employee.employeeNumber,
                    req.user.company._id
                );
                if (checkMSNV === true) {
                    throw ["employee_number_have_exist"];
            ...
            if (
                req.body.employee.emailInCompany &&
                req.body.employee.emailInCompany !== oldEmployee.emailInCompany
            ) {
                // Kiểm tra sự tồn tại của email công ty nhân viên
                let checkEmail =
                    await EmployeeService.checkEmployeeCompanyEmailExisted(
            ...
            let data = await EmployeeService.updateEmployeeInformation(
                req.portal,
                req.params.id,
                req.body,
                fileInfor,
...
exports.deleteEmployee = async (req, res) => {
    try {
        let data = await EmployeeService.deleteEmployee(
            req.portal,
            req.params.id
        );
        if (req.query.emailInCompany)
            await UserService.deleteUserByEmail(
                req.portal,
                req.query.emailInCompany
            );
    ...
    try {
        ...
        if (req.body.importData) {
            req.body.importData = req.body.importData.map((o) => {
                if (!o.emailInCompany) {
                    o.emailInCompany =
                        `${o.employeeNumber}@autocreated.dxclan.com`.toLowerCase();
                }
                return o;
            });
        }
        ...
        if (req.body.importType === "Employee_Infor") {
            data = await EmployeeService.importEmployeeInfor(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "Update_Employee_Infor") {
            data = await EmployeeService.importUpdateEmployeeInfor(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "Experience") {
            data = await EmployeeService.importExperience(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "WorkProcess") {
            data = await EmployeeService.importWorkProcess(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "Degree") {
            data = await EmployeeService.importDegree(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "Certificate") {
            data = await EmployeeService.importCertificate(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "Contract") {
            data = await EmployeeService.importContract(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "SocialInsuranceDetails") {
            data = await EmployeeService.importSocialInsuranceDetails(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "File") {
            data = await EmployeeService.importFile(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (req.body.importType === "FamilyMembers") {
            data = await EmployeeService.importFamily(
                req.portal,
                req.user.company._id,
                req.body.importData
            );
        ...
        if (data?.errorStatus === true) {
            ...
            if (req.body.importType === "Employee_Infor") {
                ...
                newUsers = newUsers.filter(
                    (x) => !users.some((y) => y.email === x.emailInCompany)
                );
...
exports.createNotificationForEmployeesHaveBrithdayCurrent = async () => {
    ...
    companys = companys.map((x) => x.shortName);
    for (let n in companys) {
        await EmployeeService.createNotificationForEmployeesHaveBrithdayCurrent(
            companys[n]
        );
    }
};
...
exports.getEmployeesInCareerPosition = async (req, res) => {
    try {
        ...
        data = await EmployeeService.getEmployeeByPackageId(
            req.portal,
            params.packageId ? params.packageId : "1",
            req.user.company._id
        );
