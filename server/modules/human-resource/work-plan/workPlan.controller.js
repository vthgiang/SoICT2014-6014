...
exports.getAllWorkPlans = async (req, res) => {
    try {
        let data;
        if (req.query.year) {
            data = await WorkPlanService.getWorkPlansOfYear(req.user.company._id, req.query.year);
        } else {
            data = await WorkPlanService.getAllWorkPlans(req.user.company._id);
        }
        await LogInfo(req.user.email, 'GET_WORK_PLAN', req.user.company);
        res.status(200).json({
            success: true,
        ...
        await LogError(req.user.email, 'GET_WORK_PLAN', req.user.company);
        res.status(400).json({
            success: false,
...
exports.createWorkPlan = async (req, res) => {
    try {
        ...
        if (req.body.startDate.trim() === "") {
            await LogError(req.user.email, 'CREATE_WORK_PLAN', req.user.company);
            res.status(400).json({
                success: false,
            ...
            // Tạo mới thông tin lịch làm việc
            let result = await WorkPlanService.getAllWorkPlans(req.user.company._id);
            let checkData = this.checkForFuplicate(req.body, result.workPlans);
            if (checkData) {
                let data = await WorkPlanService.createWorkPlan(req.body, req.user.company._id);
                await LogInfo(req.user.email, 'CREATE_WORK_PLAN', req.user.company);
                res.status(200).json({
                    success: true,
...
exports.deleteWorkPlan = async (req, res) => {
    try {
        let data = await WorkPlanService.deleteWorkPlan(req.params.id, req.user.company._id);
        await LogInfo(req.user.email, 'DELETE_WORK_PLAN', req.user.company);
        res.status(200).json({
            success: true,
        ...
        await LogError(req.user.email, 'DELETE_WORK_PLAN', req.user.company);
        res.status(400).json({
            success: false,
...
exports.updateWorkPlan = async (req, res) => {
    try {
        if (req.body.maximumNumberOfLeaveDays) {
            let data = await WorkPlanService.updateNumberDateLeaveOfYear(req.body.maximumNumberOfLeaveDays, req.user.company._id);
            await LogInfo(req.user.email, 'EDIT_WORK_PLAN', req.user.company);
            res.status(200).json({
                success: true,
            ...
            if (req.body.startDate.trim() === "") {
                ...
                let checkData = this.checkForFuplicate(req.body, result.workPlans);
                if (checkData) {
                    let data = await WorkPlanService.updateWorkPlan(req.params.id, req.body, req.user.company._id);
                    await LogInfo(req.user.email, 'EDIT_WORK_PLAN', req.user.company);
                    res.status(200).json({
                        success: true,
                    ...
                    await LogError(req.user.email, 'EDIT_WORK_PLAN', req.user.company);
                    res.status(400).json({
                        success: false,
                        messages: ["work_plan_duplicate_required"],
                        content: {
...
exports.importWorkPlans = async (req, res) => {
    try {
        let data = await WorkPlanService.importWorkPlans(req.body, req.user.company._id);
        if (data.rowError !== undefined) {
            await LogError(req.user.email, 'IMPORT_WORK_PLAN', req.user.company);
            res.status(400).json({
                success: false,
            ...
            await LogInfo(req.user.email, 'IMPORT_WORK_PLAN', req.user.company);
            res.status(200).json({
                success: true,
