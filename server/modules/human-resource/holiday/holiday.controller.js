...
exports.getAllHolidays = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, messages: ["get_holiday_success"], content: data });
    } catch (error) {
        await LogError(req.user.email, 'GET_HOLIDAY', req.user.company);
        res.status(400).json({success: false, messages:["get_holiday_faile"], content: {error: error}});
    }
}
...
exports.createHoliday = async (req, res) => {
    try {
        if (req.body.startDate.trim()===""){
            ...
            await LogError(req.user.email, 'CREATE_HOLIDAY', req.user.company);
            res.status(400).json({ success: false, messages: ["reason_required"], content:{ inputData: req.body } });
        } else {
            ...
            res.status(200).json({ success: true, messages: ["create_holiday_success"], content: data });
        }
    } catch (error) {
        await LogError(req.user.email, 'CREATE_HOLIDAY', req.user.company);
        res.status(400).json({success: false, messages:["create_holiday_faile"], content: {error: error}});
    }
}
...
exports.deleteHoliday = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, messages: ["delete_holiday_success"], content: data });
    } catch (error) {
        await LogError(req.user.email, 'DELETE_HOLIDAY', req.user.company);
        res.status(400).json({success: false, messages:["delete_holiday_faile"], content: {error: error}});
    }
}
...
exports.updateHoliday = async (req, res) => {
    try {
        if (req.body.startDate.trim()===""){
            ...
            res.status(200).json({ success: true, messages: ["edit_holiday_success"], content: data });
        }
    } catch (error) {
        await LogError(req.user.email, 'EDIT_HOLIDAY', req.user.company);
        res.status(400).json({success: false, messages:["edit_holiday_faile"], content: {error: error}});
    }
}