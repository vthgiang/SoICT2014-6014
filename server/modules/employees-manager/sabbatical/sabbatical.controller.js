...
exports.get = async (req, res) => {
    try {
        ...
        res.status(400).json({ success: false, message: ["get_sabbatical_faile"], content: error });
    }
}
...
exports.create = async (req, res) => {
    try {
        if(req.body.startDate.trim()===""){
            ...
            res.status(400).json({ success: false, message: ["start_date_sabbatical_required"], content:{ inputDate: req.body } });
        } else if(req.body.endDate.trim()===""){
            await LogError(req.user.email, 'CREATE_SABBATICAL', req.user.company);
            res.status(400).json({ success: false, message: ["end_date_sabbatical_required"], content:{ inputDate: req.body } });
        } else if(req.body.reason.trim()===""){
            await LogError(req.user.email, 'CREATE_SABBATICAL', req.user.company);
            res.status(400).json({ success: false, message: ["reason_sabbatical_required"], content:{ inputDate: req.body } });
        } else if(req.body.status.trim()===""){
            await LogError(req.user.email, 'CREATE_SABBATICAL', req.user.company);
            res.status(400).json({ success: false, message: ["status_sabbatical_required"], content:{ inputDate: req.body } });
        } else {
            var newSabbatical = await SabbaticalService.create(req.body, req.user.company._id);
            if (newSabbatical === null) {
                await LogError(req.user.email, 'CREATE_SABBATICAL', req.user.company);
                res.status(404).json({ success: false, message: ["staff_code_not_find"], content:{ inputDate: req.body } });
            } else { 
                await LogInfo(req.user.email, 'CREATE_SABBATICAL', req.user.company);
                res.status(200).json({ success: true, message: ["create_sabbatical_success"], content: newSabbatical });
            }
        }
    } catch (error) {
        await LogError(req.user.email, 'CREATE_SABBATICAL', req.user.company);
        res.status(400).json({ success: false, message: "create_sabbatical_faile", content: { inputDate: req.body } });
    }
}
...
exports.delete = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: ["delete_sabbatical_success"], content: sabbaticalDelete });
    } catch (error) {
    ...
    try {
        if(req.body.startDate.trim()===""){
            ...
            if(sabbaticalUpdate===null){
                ...
                res.status(200).json({ success: true, message: ["edit_sabbatical_success"], content: sabbaticalUpdate });
            }
        }
    } catch (error) {
        await LogError(req.user.email, 'EDIT_SABBATICAL', req.user.company);
