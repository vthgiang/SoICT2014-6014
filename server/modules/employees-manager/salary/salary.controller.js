...
exports.get = async (req, res) => {
    try {
        ...
        await LogInfo(req.user.email, 'GET_SARALY', req.user.company._id );
        res.status(200).json({
            message: "success",
            content: ListSaraly
        });
    } catch (error) {
        await LogError(req.user.email, 'GET_SARALY', req.user.company._id );
        res.status(400).json({
            message: error
        });
    }
...
exports.create = async (req, res) => {
    try {
        ...
        await LogInfo(req.user.email, 'CREATE_SARALY', req.user.company._id );
        res.status(200).json({
            message: "success",
            content: saraly
        });
    } catch (error) {
        await LogError(req.user.email, 'CREATE_SARALY', req.user.company._id );
        res.status(400).json({
            message: error
        });
    }
...
exports.delete = async (req, res) => {
    try {
        ...
        await LogInfo(req.user.email, 'DELETE_SARALY', req.user.company._id );
        res.status(200).json({
            message: "success",
            content: saralyDelete
        });
    } catch (error) {
        await LogError(req.user.email, 'DELETE_SARALY', req.user.company._id );
        res.status(400).json({
            message: error
        });
    }
