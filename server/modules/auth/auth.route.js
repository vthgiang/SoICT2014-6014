...
router.get("/get-profile/:userId", authFunc(false), AuthController.getProfile);
router.patch("/profile/:userId/change-information", auth, authCUIP, uploadFile([{ name: 'avatar', path: '/avatars' }], 'single'), AuthController.changeInformation);
router.patch("/profile/:userId/change-password", auth, authCUIP, AuthController.changePassword);
...
router.delete("/profile/:userId/delete-password2", authFunc(false), AuthController.deletePassword2);
