...
//   const requesters = await Object(connect(DB_CONNECTION, portal))
//     .find(query)
//     .skip((page - 1) * perPage)
//     .limit(perPage);

//   const totalObjects = await Object(connect(DB_CONNECTION, portal)).countDocuments(
//     query,
//   );
//   const totalPages = queryParams?.perPage
//     ? Math.ceil(totalObjects / queryParams.perPage)
//     : 1;
...
exports.findAll = async (portal) => {
  const requesters = await Object(connect(DB_CONNECTION, portal)).find();
  ...
  return {
    data: requesters.map(x => ({
      ...
      attributes: x.attributes,
      type: x.type,
      relatedEntities: x.relatedEntities
    }))
  };
}

exports.findOne = async (portal, id) => {
  const existent = await Object(connect(DB_CONNECTION, portal)).exists({
    _id: id,
  });
  ...
  const requester = await Object(connect(DB_CONNECTION, portal)).findById(id);
...
exports.updateAttributes = async (portal, id, data) => {
  ...
  const requester = await Object(connect(DB_CONNECTION, portal)).findOneAndUpdate(
    { _id: id },
    { attributes: dataAttr },
    { new: true }
  );
...
exports.findByIds = async(portal, ids) => {
  const requesters = await Object(connect(DB_CONNECTION, portal)).find( {_id: {$in: ids} });
...
exports.getAccessibleResources = async(portal, requesterId, roleId = undefined) => {
  ...
  const requester = await Object(connect(DB_CONNECTION, portal)).findById(requesterId)
    .populate('refId');
