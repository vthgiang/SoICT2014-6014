...
//   const requesters = await Entity(connect(DB_CONNECTION, portal))
//     .find(query)
//     .skip((page - 1) * perPage)
//     .limit(perPage);

//   const totalEntities = await Entity(connect(DB_CONNECTION, portal)).countDocuments(
//     query,
//   );
//   const totalPages = queryParams?.perPage
//     ? Math.ceil(totalEntities / queryParams.perPage)
//     : 1;
...
exports.findAll = async (portal) => {
  const requesters = await Entity(connect(DB_CONNECTION, portal)).find();
...
exports.findOne = async (portal, id) => {
  const existent = await Entity(connect(DB_CONNECTION, portal)).exists({
    _id: id,
  });
  ...
  const requester = await Entity(connect(DB_CONNECTION, portal)).findById(id);
...
exports.updateAttributes = async (portal, id, data) => {
  ...
  const requester = await Entity(connect(DB_CONNECTION, portal)).findOneAndUpdate(
    { _id: id },
    { attributes: dataAttr },
    { new: true }
  );
...
exports.findByIds = async(portal, ids) => {
  const requesters = await Entity(connect(DB_CONNECTION, portal)).find( {_id: {$in: ids} });
...
exports.getAccessibleResources = async(portal, requesterId, roleId = undefined) => {
  ...
  const requester = await Entity(connect(DB_CONNECTION, portal)).findById(requesterId)
    .populate('refId');
