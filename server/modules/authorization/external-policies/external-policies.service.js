...
exports.create = async(portal, externalServiceIdentityDto) => {
  ...
  LoggingService.create(
    portal,
    LoggingType.CREATE_EXTERNAL_POLICY,
    'SUCCESSFUL',
    policy.name,
    policy,
  );
...
exports.findAll = async (portal, queryParams) => {
  ...
  const policies = await ExternalPolicy(connect(DB_CONNECTION, portal))
    .find(query)
    .skip(queryParams?.page)
    .limit(queryParams?.perPage);

  const totalExternalPolicies = await ExternalPolicy(connect(DB_CONNECTION, portal)).countDocuments(
    query,
  );
  const totalPages = queryParams?.perPage
    ? Math.ceil(totalExternalPolicies / queryParams.perPage)
    : 1;
...
exports.update = async (portal, id, updateExternalPolicyDto) => {
  ...
  const { description, actions, resources, enabled, condition } =
    updateExternalPolicyDto;
  ...
  LoggingService.create(
    portal,
    LoggingType.UPDATE_EXTERNAL_POLICY,
    'SUCCESSFUL',
    policy.name,
    policy,
  );
