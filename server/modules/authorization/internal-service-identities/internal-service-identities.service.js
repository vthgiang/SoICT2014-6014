...
const { InternalServiceIdentity, LoggingRecord } = require("../../../models");
...
exports.create = async (portal, internalServiceIdentityDto) => {
  if (
    internalServiceIdentityDto.name.length <= 0 ||
    internalServiceIdentityDto.apiPrefix.length <= 0
  ) {
  ...
  const {
    ...
    internalPolicies,
    externalPolicies,
  } = await InternalServiceIdentity(connect(DB_CONNECTION, portal)).create(
    newInternalServiceIdentity,
  );

  const internalPolicyPromises = [];
  const externalPolicyPromises = [];

  internalPolicyPromises.push(
    ...internalPolicies.map((policy) =>
      InternalPolicyService.findOne(portal, policy),
    ),
  );
  externalPolicyPromises.push(
    ...externalPolicies.map((policy) =>
      ExternalPolicyService.findOne(portal, policy),
    ),
  );
  ...
  LoggingService.create(
    portal,
    LoggingType.CREATE_INTERNAL_SERVICE_IDENTITY,
    'SUCCESSFUL',
    response.name,
    {
      id: response.id,
...
exports.findAll = async (portal, queryParams = {}) => {
  ...
  const internalServiceIdentities = await InternalServiceIdentity(connect(DB_CONNECTION, portal))
    .find(query)
    .skip(page)
    .limit(perPage);
  ...
  for (const identity of internalServiceIdentities) {
    const internalPolicyPromises = [];
    internalPolicyPromises.push(
      ...identity.internalPolicies.map((policy) =>
        InternalPolicyService.findOne(portal, policy),
      ),
    );
    ...
    const externalPolicyPromises = [];
    externalPolicyPromises.push(
      ...identity.externalPolicies.map((policy) =>
        ExternalPolicyService.findOne(portal, policy),
      ),
    );
  ...
  const totalInternalServiceIdentities =
    await InternalServiceIdentity(connect(DB_CONNECTION, portal)).countDocuments(query);
  const totalPages = perPage
    ? Math.ceil(totalInternalServiceIdentities / perPage)
    : 1;
...
exports.findOne = async (portal, id) => {
  ...
  const internalServiceIdentity =
    await InternalServiceIdentity(connect(DB_CONNECTION, portal)).findById(id);

  const { name, apiPrefix, description, clientCredential } =
  ...
  internalPolicyPromises.push(
    ...internalServiceIdentity.internalPolicies.map((policy) =>
      InternalPolicyService.findOne(portal, policy),
    ),
  );
  externalPolicyPromises.push(
    ...internalServiceIdentity.externalPolicies.map((policy) =>
      ExternalPolicyService.findOne(portal, policy),
    ),
  );
...
exports.update = async (portal, id, updateInternalServiceIdentityDto) => {
  ...
  const {
    ...
    internalPolicies,
    externalPolicies,
  } = await InternalServiceIdentity(connect(DB_CONNECTION, portal)).findByIdAndUpdate(
    id,
    {
  ...
  LoggingService.create(
    portal,
    LoggingType.UPDATE_INTERNAL_SERVICE_IDENTITY,
    'SUCCESSFUL',
    response.name,
    {
      id: response.id,
