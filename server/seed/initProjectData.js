...
const professionalSkill = ['intermediate_degree', 'colleges', 'university', 'master_degree', 'engineer', 'bachelor', 'phd', 'unavailable'];
...
const initHumanResourceForProjectData = async () => {
    ...
    const phongMaketing = await OrganizationalUnit(vnistDB).insertMany([
        {
            ...
            description: 'Phòng Marketing Công ty Cổ phần Công nghệ An toàn thông tin và Truyền thông Việt Nam',
    ...
    usersAlreadyInVNIST.forEach((x, index) => {
        let contractEndDate = new Date(
            `${index < 50 ? currentYear : currentYear - 1}-${months[Math.floor(Math.random() * 12)]}-${days[Math.floor(Math.random() * 19)]}`
        );
        staffsForUsersAlreadyInVNIST = [
            ...staffsForUsersAlreadyInVNIST,
            {
                ...
                leavingDate:
                    70 <= index && index <= 120
                        ? new Date(`${currentYear}-${months[Math.floor(Math.random() * 12)]}-${days[Math.floor(Math.random() * 19)]}`)
                        : null,
                ...
                healthInsuranceEndDate: new Date(`${currentYear}-${months[Math.floor(Math.random() * 12)]}-${days[Math.floor(Math.random() * 19)]}`),
                ...
                contracts: [
                    {
                        name: 'Thực tập',
                        contractType: `${index < 50 ? 'Thử việc' : index > 150 ? 'Hợp đồng ngắn hạn hạn' : 'Hợp đồng dài hạn'}`,
                        startDate: new Date(`${currentYear - 1}-${months[Math.floor(Math.random() * 12)]}-${days[Math.floor(Math.random() * 19)]}`),
            ...
            if (unit) {
                salaryForUsersAlreadyInVNIST = [
                    ...salaryForUsersAlreadyInVNIST,
                    {
                        ...
                        mainSalary: ((i + 1) % 19) * 7000000 + Math.floor(Math.random() * 20) * 1000000,
                        unit: 'VND',
                        bonus: [
                            {
                                nameBonus: 'Thưởng dự án',
                                number: (i % 19) * 1000000 + Math.floor(Math.random() * 20) * 1000000,
                            },
                        ],
                    },
                ];
            ...
            if (unit) {
                salaryForUsersProject = [
                    ...salaryForUsersProject,
                    {
                        ...
                        month: `${currentYear}-${months[Math.floor(Math.random() * 12)]}`,
                        organizationalUnit: unit._id,
                        mainSalary: ((i + 1) % 19) * 2000000 + Math.floor(Math.random() * 20) * 1000000,
                        unit: 'VND',
                        bonus: [
                            {
                                nameBonus: 'Thưởng dự án',
        ...
        if (moment(endDate).isSameOrBefore(moment())) {
            ...
            responsibleSalaryWithCost = responsibleSalary.map((resSalItem) => {
                ...
                const resActualCost = Math.floor(
                    tempActualCost - getRandomIntFromInterval(Math.floor(tempActualCost / 100), Math.floor(tempActualCost / 10))
                );
                return {
                    ...resSalItem,
            ...
            if (isFinished === 1) {
                ...
                const currentTask = {
                    ...
                    estimateOptimisticTime:
                        ...
                        (drugRNDProject.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS),
                };
                const dataTask = {
                    task: currentTask,
                    progress,
...
const getSalaryFromUserIdAndOrgId = (currentSalaries, currentEmployees, currentUsers, orgId, userId) => {
    ...
    const currentSalaryItem = currentSalaries.find(
        (salsItem) => String(salsItem.organizationalUnit) === String(orgId) && String(salsItem.employee) === String(currentEmployeeItem._id)
    );
        ...
        if (taskItem.preceedingTasks.length === 0 && (!taskItem.startDate || !taskItem.endDate)) {
            ...
            for (let preceedingItem of taskItem.preceedingTasks) {
                ...
                if (
                    ...
                    (!taskItem.startDate || moment(taskItem.startDate).isBefore(moment(currentPreceedingTaskItem.endDate)))
                ) {
...
const getEstimateMemberCostOfTask = (actorsWithSalary, estimateNormalTime, projectDetail, userId) => {
    ...
    if (currentEmployee) {
        estimateNormalMemberCost =
            ((Number(currentEmployee.salary) / getAmountOfWeekDaysInMonth(moment())) * Number(currentEmployee.weight / 100) * estimateNormalTime) /
...
const calcProjectTaskPoint = (data, getCalcPointsOnly = true) => {
    ...
    let actionsHasRating = task.taskActions.filter((item) => item.rating && item.rating !== -1);
    ...
    const taskQualityPoint =
        sumRatingOfAllActions === 0 ? 0 : [(sumRatingOfPassedActions / sumRatingOfAllActions) * 100] * (task?.taskWeight?.qualityWeight || 1 / 3);
...
const calcProjectMemberPoint = (data, getCalcPointsOnly = true) => {
    ...
    const memberQualityPoint =
        sumRatingOfAllActions === 0 ? 0 : [(sumRatingOfPassedActions / sumRatingOfAllActions) * 100] * (task?.memberWeight?.qualityWeight || 0.25);
    ...
    for (let timeSheetItem of timesheetLogs) {
        if (String(userId) === String(timeSheetItem.creator)) {
            ...
            sumTimeDistributionPoint +=
                timeSheetItem.autoStopped === 1 && isInWorkingTime && timeSheetItem.duration <= 12 * MILISECS_TO_HOURS ? 100 : 80;
        }
    }
    const memberTimedistributionPoint =
        (sumTimeDistributionPoint / (100 * userWithTimeSheetLogsCounter)) * 100 * (task?.memberWeight?.timedistributionWeight || 0.25);
