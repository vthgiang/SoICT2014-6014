    ...
    <section>
      ...
      <div className='box'>
        <div className='box-body'>
          <div className='row'>
            <ModalEditEmployeeKpi id={id} employeeKpi={employeeKpi} />
            ...
            {currentKPI ? (
              <div className='col-xs-12 col-sm-12'>
                <div style={{ marginLeft: '-10px', marginBottom: '10px' }}>
                    ...
                    <a
                      ...
                      onClick={
                        checkOrganizationalUnitKpi()
                          ? () => handleEditEmployeeKpiSet(currentKPI.status)
                  ...
                  {currentKPI.status === 0 ? (
                    <a
                      ...
                      onClick={
                        checkOrganizationalUnitKpi()
                          ? () => handleRequestApproveEmployeeKpiSet(currentKPI)
                          : () => handleStartOrganizationalUnitKpi()
                      }
                    >
                ...
                <div className='' style={{ marginBottom: '10px' }}>
                  <h4 style={{ display: 'inline', fontWeight: '600' }}>
                    ...
                    {formatDate(currentKPI.date)} (
                    {currentKPI.organizationalUnit.name
                      ? currentKPI.organizationalUnit.name
                      : translate(
                ...
                {editing ? (
                  ...
                  <div>
                    ...
                    <span style={{ fontWeight: '600' }}>
                      {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.approver')}:{' '}
                    </span>
                    <span>
                      {currentKPI.approver && currentKPI.approver.name
                        ? currentKPI.approver.name
                        : translate(
                            'kpi.employee.employee_kpi_set.create_employee_kpi_set.handle_populate_info_null.error_kpi_approver_null'
                          )}
                    </span>
                  </div>
                )}
                ...
                {!editing && (
                  <div>
                    <span>
                      {currentKPI.kpis.length} {translate('kpi.evaluation.employee_evaluation.target')} -{' '}
                      {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.weight_total')}{' '}
                    </span>
                    ...
                    {currentKPI.kpis.map((item) => parseInt(item.weight)).reduce((sum, number) => sum + number, 0) !== 100 ? (
                      <span className='text-danger' style={{ fontWeight: 'bold' }}>
                        {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.not_satisfied')}
                      </span>
                    ) : (
                      <span className='text-success' style={{ fontWeight: 'bold' }}>
                        {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.satisfied')}
                      </span>
                    )}
                    <span> - </span>
                ...
                <div>
                  {typeof currentKPI !== 'undefined' && currentKPI !== null && (
                    <h4 style={{ display: 'inline-block', fontWeight: '600', marginTop: '20px' }}>
                      {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.target_list')}
                    </h4>
                  )}
                  <SlimScroll
                    outerComponentId='kpi_table'
                    innerComponentId='kpipersonal_table'
                    innerComponentWidth='992px'
                    activate={state.fixTableWidth}
                  />
                  <div id='kpi_table'>
                    <table className='table table-bordered table-striped table-hover' id='kpipersonal_table'>
                      <thead>
                        <tr>
                          <th title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.no_')} style={{ width: '40px' }}>
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.no_')}
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.target_name')}
                            className='col-lg-3 col-sm-3'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.target_name')}
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.parents_target')}
                            className='col-lg-3 col-sm-3'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.parents_target')}
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.evaluation_criteria')}
                            className='col-lg-2 col-sm-2'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.evaluation_criteria')}
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.weight')}
                            className='col-lg-1 col-sm-1'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.weight')}
                          </th>
                          <th title='Chỉ tiêu' className='col-lg-1 col-sm-1'>
                            Chỉ tiêu
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.status')}
                            className='col-lg-1 col-sm-1'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.status')}
                          </th>
                          <th
                            title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.action')}
                            className='col-lg-1 col-sm-1'
                          >
                            {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.action')}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {typeof currentKPI === 'undefined' || currentKPI === null ? (
                          <tr>
                            <td colSpan={7}>
                              <center>
                                {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.not_initialize')} {formatDate(Date.now())}
                              </center>
                            </td>
                          </tr>
                        ) : (
                          currentKPI.kpis.map((item, index) =>
                            item ? (
                              <tr key={index + 1}>
                                ...
                                <td>
                                  <a
                                    ...
                                    onClick={
                                      checkOrganizationalUnitKpi()
                                        ? () => handleEditEmployeeKpi(currentKPI.status, item._id, item)
                                        : () => handleStartOrganizationalUnitKpi()
                                    }
                                  >
                                  ...
                                  {item.type !== 0 ? (
                                    ...
                                    <a
                                      ...
                                      onClick={
                                        checkOrganizationalUnitKpi()
                                          ? () => handleDeleteEmployeeKpi(currentKPI._id, currentKPI.status, item._id, item.status)
                                          : () => handleStartOrganizationalUnitKpi()
                                      }
                                    >
                              ...
                              translate(
                                'kpi.employee.employee_kpi_set.create_employee_kpi_set.handle_populate_info_null.error_kpi_targets_list_null'
                              )
                            )
                          )
                        )}
                ...
                >
                  <div className='col-xs-12 col-sm-12 col-md-6'>
                    <Comment
                      ...
                      deleteChildComment={(dataId, commentId, childCommentId) =>
                        props.deleteChildComment(dataId, commentId, childCommentId)
                      }
              ...
              currentKPILoading && (
                <div className='col-xs-12'>
                    ...
                    {checkOrganizationalUnitKpi() ? (
                      <span>
                        <a
                          className='btn btn-app'
                          data-toggle='modal'
                          data-target='#createEmployeeKpiSet'
                          data-backdrop='static'
                          data-keyboard='false'
                        >
                    ...
                    {checkOrganizationalUnitKpi() ? (
                      <span>
                        <a
                          className='btn btn-app'
                          data-toggle='modal'
                          data-target={`#copy-old-kpi-to-new-time-${parentKpi && parentKpi._id}`}
                          data-backdrop='static'
                          data-keyboard='false'
                        >
                  ...
                  <p>
                    {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.not_initialize')} {formatDate(month)}
                  </p>
                </div>
              )
            )}
