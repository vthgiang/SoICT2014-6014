...
function ResultsOfEmployeeKpiChart(props) {
  ...
  const setDataMultiLineChart = (listEmployeeKpiSet) => {
    ...
    if (listEmployeeKpiSet) {
      automaticPoint = [translate('kpi.evaluation.dashboard.auto_eva')].concat(
        listEmployeeKpiSet.map((x) => (x.automaticPoint ? x.automaticPoint : 0))
      )
      employeePoint = [translate('kpi.evaluation.dashboard.employee_eva')].concat(
        listEmployeeKpiSet.map((x) => (x.employeePoint ? x.employeePoint : 0))
      )
      approvedPoint = [translate('kpi.evaluation.dashboard.approver_eva')].concat(
        listEmployeeKpiSet.map((x) => (x.approvedPoint ? x.approvedPoint : 0))
      )
