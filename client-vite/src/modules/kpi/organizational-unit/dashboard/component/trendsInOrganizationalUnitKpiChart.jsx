  ...
  useEffect(() => {
    ...
    if (state.dataStatus === DATA_STATUS.QUERYING) {
      if (
        ...
        props.dashboardOrganizationalUnitKpi.employeeKpis &&
        props.dashboardOrganizationalUnitKpi.tasksOfOrganizationalUnit
      ) {
        setState({
          ...state,
    ...
    if (!listOrganizationalUnitKpi && listChildTarget) {
      ...
      if (listOrganizationalUnitKpi?.length > 0) {
        listOrganizationalUnitKpi.map((parent) => {
          ...
          let temporary = {}
          if (listChildTarget) {
            numberOfEmployeeKpi = listChildTarget
              .filter((item) => item._id === parent.name)
              .map((item) => {
