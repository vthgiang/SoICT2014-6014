...
function TrendsInChildrenOrganizationalUnitKpiChart(props) {
  ...
  useEffect(() => {
    ...
    if (state.dataStatus === DATA_STATUS.QUERYING) {
      if (
        ...
        props.dashboardOrganizationalUnitKpi.employeeKpisOfChildUnit &&
        props.dashboardOrganizationalUnitKpi.tasksOfChildrenOrganizationalUnit
      ) {
        setState({
          ...state,
          ...
          if (listChildTargetSameDeg) {
            if (index === 0) {
              listChildTargetSameParent[index] = listChildTargetSameDeg.map((item) => {
                return item.filter((kpi) => kpi._id === parent.name)
              })
            } else {
  ...
  const setExecutionTimeData = () => {
    ...
    executionTimes = Object.assign({ name: translate('kpi.organizational_unit.dashboard.trend_chart.execution_time') }, executionTimes)
  ...
  const setNumberOfTaskData = () => {
    ...
    numberOfTasks = Object.assign({ name: translate('kpi.organizational_unit.dashboard.trend_chart.amount_tasks') }, numberOfTasks)
  ...
  const setWeightData = () => {
    ...
    weight = Object.assign({ name: translate('kpi.organizational_unit.dashboard.trend_chart.weight') }, weight)
  ...
  const barChart = () => {
    ...
    let dataChart = [titleX, numberOfTasksArray, executionTimesArray, numberOfParticipantsArray, numberOfEmployeeKpisArray, weightArray]
    ...
    let chart = c3.generate({
          ...
          tick: {
            format: function (index) {
              let result = customAxisC3js(
                'trendsInChildrenUnitChart',
                titleX.filter((item, i) => i > 0),
                index
              )
              return result
            }
      ...
      tooltip: {
        format: {
          title: function (d) {
            if (titleX?.length > 1) return titleX[d + 1]
          }
        }
      }
    })
  ...
  return (
    <React.Fragment>
      {currentKpi ? (
        <div id='trendsInChildrenUnitChart' ref={refBarChart}>
          {' '}
        </div>
      ) : (
...
const connectedTrendsInChildrenOrganizationalUnitKpiChart = connect(
  mapState,
  actions
)(withTranslate(TrendsInChildrenOrganizationalUnitKpiChart))
