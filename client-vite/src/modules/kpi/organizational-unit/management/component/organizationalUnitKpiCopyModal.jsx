  ...
  const handleSubmit = () => {
    ...
    if (type === TYPE.DEFAULT) {
      ...
      let data = {
        ...
        approver: approver
          ? approver
          : approverDefault?.[1]?.value?.[0]?.value
            ? approverDefault?.[1]?.value?.[0]?.value
            : approverDefault?.[0]?.value?.[0]?.value,
        listKpiUnit: arrayKpiUnit?.filter((item) => listKpiUnit?.[item])
      }
  ...
  return (
    ...
    >
      ...
      <div className='form-group' style={{ marginLeft: '10px' }}>
        ...
        {organizationalUnitSelect?.length > 0 && type === TYPE.COPY_PARENT_KPI_TO_UNIT ? (
          <SelectBox
            ...
            style={{ width: '100%' }}
            items={organizationalUnitSelect.map((item) => {
      ...
      <div className='form-group' style={{ marginLeft: '10px' }}>
        ...
        <DatePicker id='new_date' value={formatDate(monthDefault)} onChange={handleNewDateChange} dateFormat='month-year' />
            ...
            {listKpi.kpis.map((item) => {
              return (
                <li key={item._id}>
                  <input
                    type='checkbox'
                    checked={listKpiUnit?.[item?._id]}
                    disabled={item?.type !== 0}
                    onChange={item?.type !== 0 ? null : () => handleKpiUnit(item?._id)}
                  ></input>
