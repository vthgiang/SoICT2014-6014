...
function KPIUnitManager(props) {
  ...
  const { startDate, endDate, status, errorOnDate, infosearch, organizationalUnit, defaultStartDate, defaultEndDate, currentRole } = state
      ...
      <div className='box'>
        {unitList && unitList.length !== 0 ? (
          <div className='box-body qlcv'>
            <ModalDetailKPI date={state.date} id={state.id} idkpiunit={state.idkpiunit} />
              ...
              <div className='form-group'>
                <label>{translate('kpi.organizational_unit.management.over_view.status')}</label>
                {
                  <SelectBox
                    ...
                    items={[
                      { value: -1, text: translate('kpi.organizational_unit.management.over_view.all_status') },
                      { value: 0, text: translate('kpi.organizational_unit.management.over_view.setting_up') },
                      { value: 1, text: translate('kpi.organizational_unit.management.over_view.activated') }
                    ]}
            ...
            <div className='form-inline'>
              <div className={`form-group ${!errorOnDate ? '' : 'has-error'}`}>
                ...
                <DatePicker id='start_date' value={defaultStartDate} onChange={handleStartDateChange} dateFormat='month-year' />
                <ErrorLabel content={errorOnDate} />
              </div>
              <div className='form-group'>
                <label>{translate('kpi.organizational_unit.management.over_view.end_date')}</label>
                <DatePicker id='end_date' value={defaultEndDate} onChange={handleEndDateChange} dateFormat='month-year' />
                <ErrorLabel content={errorOnDate} />
              </div>
              ...
              {exportData && (
                <ExportExcel id='export-unit-kpi-management-overview' exportData={exportData} style={{ margin: 0 }} buttonName={'Xuất'} />
              )}
            </div>
            ...
            <table id={tableId} className='table table-hover table-bordered'>
              <thead>
                <tr>
                  <th title={translate('kpi.organizational_unit.management.over_view.time')}>
                    {translate('kpi.organizational_unit.management.over_view.time')}
                  </th>
                  <th title={translate('task.task_management.col_organization')}>{translate('task.task_management.col_organization')}</th>
                  <th title={translate('kpi.organizational_unit.management.over_view.status')}>
                    {translate('kpi.organizational_unit.management.over_view.status')}
                  </th>
                  <th title={translate('kpi.organizational_unit.management.over_view.number_target')}>
                    {translate('kpi.organizational_unit.management.over_view.number_target')}
                  </th>
                  <th title={translate('kpi.organizational_unit.management.over_view.creator')}>
                    {translate('kpi.organizational_unit.management.over_view.creator')}
                  </th>
                  <th title={translate('kpi.evaluation.employee_evaluation.system_evaluate')}>
                    {translate('kpi.evaluation.employee_evaluation.system_evaluate')}
                  </th>
                  <th title={translate('kpi.evaluation.employee_evaluation.result_self_evaluate')}>
                    {translate('kpi.evaluation.employee_evaluation.result_self_evaluate')}
                  </th>
                  <th title={translate('kpi.evaluation.employee_evaluation.evaluation_management')}>
                    {translate('kpi.evaluation.employee_evaluation.evaluation_management')}
                  </th>
                  <th title={translate('kpi.organizational_unit.management.over_view.action')}>
                    {translate('kpi.organizational_unit.management.over_view.action')}
                  </th>
                </tr>
              </thead>
                            ...
                            <td>
                              ...
                              >
                                <i
                                  ...
                                  onClick={() => handleShowEdit(item._id, item.organizationalUnit._id, item.date)}
                                >
                                  view_list
                                </i>
                              </a>
                              ...
                              {checkPermisson(currentUnit && currentUnit[0] && currentUnit[0].managers) && (
                                <a
                                  href='#abc'
                                  onClick={() => showModalCopy(item._id)}
                                  className='copy'
                                  data-toggle='modal'
                                  data-backdrop='static'
                                  data-keyboard='false'
                                  title='Thiết lập kpi tháng mới từ kpi tháng này'
                                >
