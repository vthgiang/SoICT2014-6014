...
class CalendarUsage extends Component {
  ...
  shouldComponentUpdate = async (nextProps, nextState) => {
    ...
    if (nextProps.recommendDistribute && nextProps.recommendDistribute.listRecommendDistributesByAsset && this.dataStatus == 1) {
      if (this.dataStatus === 1) {
        ...
        for (let i in listRecommendDistributes) {
          ...
          if (
            (listRecommendDistributes[i].status == 'waiting_for_approval' || listRecommendDistributes[i].status == 'disapproved') &&
            !check.length
          ) {
            recommendDistribute = {
              id: listRecommendDistributes[i]._id,
              color:
                listRecommendDistributes[i].status == 'waiting_for_approval'
                  ? '#aaa'
                  : listRecommendDistributes[i].status == 'approved'
                    ? '#337ab7'
                    : '#dd4b39',
    ...
    if (nextState.createUsage && nextProps.assetsManager && nextProps.assetsManager.currentAsset) {
      ...
      let userlist = this.props.user.list,
        departmentlist = this.props.department.list
      ...
      if (newUsage.usedByUser && newUsage.usedByOrganizationalUnit) {
        let usedByUser = userlist.filter((item) => item._id === newUsage.usedByUser).pop()
          ? userlist.filter((item) => item._id === newUsage.usedByUser).pop().name
          : 'Chưa có đối tượng sử dụng'
        let usedByOrganizationalUnit = departmentlist.filter((item) => item._id === newUsage.usedByOrganizationalUnit).pop()
          ? departmentlist.filter((item) => item._id === newUsage.usedByOrganizationalUnit).pop().name
          : 'Chưa có đối tượng sử dụng'
        title = usedByUser + ', ' + usedByOrganizationalUnit
      } else if (newUsage.usedByUser && !newUsage.usedByOrganizationalUnit) {
        title = userlist.filter((item) => item._id === newUsage.usedByUser).pop()
          ? userlist.filter((item) => item._id === newUsage.usedByUser).pop().name
          : 'Chưa có đối tượng sử dụng'
      } else if (!newUsage.usedByUser && newUsage.usedByOrganizationalUnit) {
        title = departmentlist.filter((item) => item._id === newUsage.usedByOrganizationalUnit).pop()
          ? departmentlist.filter((item) => item._id === newUsage.usedByOrganizationalUnit).pop().name
          : 'Chưa có đối tượng sử dụng'
      }
  ...
  handleCreateUseRequest = async (data) => {
    ...
    if (data) {
      ...
      calendarApi.addEvent({
        ...
        title: userlist.filter((item) => item._id === data.proponent).pop()
          ? userlist.filter((item) => item._id === data.proponent).pop().name
          : 'Chưa có đối tượng sử dụng',
        color: '#aaa',
        start: startDate,
        end: endDate
  ...
  handleApprove = async (clickInfo) => {
    ...
    var { usageLogs } = this.state
    let list,
      dataRecommendDistribute,
      value = clickInfo.event.id
    ...
    if (dataRecommendDistribute) {
      ...
      await this.props.updateRecommendDistribute(dataRecommendDistribute[0]._id, {
      ...
      <>
        <div className='form-inline'>
          <div className='form-group'>
            ...
            >
              <i className='material-icons' id='display-event'>
                view_list
              </i>
            </a>
          </div>
          ...
          {eventInfo.event.borderColor != '#337ab7' &&
            (this.props.managedBy == this.state.userId || this.props.linkPage == 'management') && (
              <div className='form-group'>
                ...
                >
                  <i className='material-icons' id='approve-event'>
                ...
                >
                  <i className='material-icons' id='disapprove-event'>
                    block
                  </i>
                </a>
              </div>
          ...
          {(this.props.managedBy == this.state.userId ||
            this.props.linkPage == 'management' ||
      ...
      for (let i in nextProps.usageLogs) {
        ...
        if (!check.length) {
          ...
          if (nextProps.usageLogs[i].usedByUser && nextProps.usageLogs[i].usedByOrganizationalUnit) {
            let usedByUser = userlist.filter((item) => item._id === nextProps.usageLogs[i].usedByUser).pop()
              ? userlist.filter((item) => item._id === nextProps.usageLogs[i].usedByUser).pop().name
              : 'Chưa có đối tượng sử dụng'
            let usedByOrganizationalUnit = departmentlist
              .filter((item) => item._id === nextProps.usageLogs[i].usedByOrganizationalUnit)
              .pop()
              ? departmentlist.filter((item) => item._id === nextProps.usageLogs[i].usedByOrganizationalUnit).pop().name
              : 'Chưa có đối tượng sử dụng'
            title = usedByUser + ', ' + usedByOrganizationalUnit
          } else if (nextProps.usageLogs[i].usedByUser && !nextProps.usageLogs[i].usedByOrganizationalUnit) {
            title = userlist.filter((item) => item._id === nextProps.usageLogs[i].usedByUser).pop()
              ? userlist.filter((item) => item._id === nextProps.usageLogs[i].usedByUser).pop().name
              : 'Chưa có đối tượng sử dụng'
          } else if (!nextProps.usageLogs[i].usedByUser && nextProps.usageLogs[i].usedByOrganizationalUnit) {
            title = departmentlist.filter((item) => item._id === nextProps.usageLogs[i].usedByOrganizationalUnit).pop()
              ? departmentlist.filter((item) => item._id === nextProps.usageLogs[i].usedByOrganizationalUnit).pop().name
              : 'Chưa có đối tượng sử dụng'
          }
