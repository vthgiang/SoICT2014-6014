...
function TaskReportDetailForm(props) {
  ...
  const handleViewChart = async () => {
    ...
    if (listTaskReport) {
      let newData = {
        ...
        startDate: listTaskReport.startDate ? dayjs(listTaskReport.startDate.slice(0, 10)).format('DD-MM-YYYY') : '',
        endDate: listTaskReport.endDate ? dayjs(listTaskReport.endDate.slice(0, 10)).format('DD-MM-YYYY') : '',
  ...
  return (
    <DialogModal
      size='75'
      modalID='modal-detail-taskreport'
      isLoading={false}
      ...
      hasNote={false}
    >
      {idLineBarChart && idLineBarChart === `detail-lineBarChart-${taskReportId}` && barAndLineDataChartConvert && (
        <LineBarChartDetailForm
          id={idLineBarChart}
          barLineChartData={barAndLineDataChartConvert}
      ...
      <div className='row'>
        {idPieChart &&
          idPieChart === `detail-pieChart-${taskReportId}` &&
          pieChartDataConvert &&
          pieChartDataConvert.map((item, index) =>
      ...
      <div className='row row-equal-height'>
        <div className='col-xs-12 col-sm-12 col-md-6 col-lg-6'>
          <div className='description-box' style={{ height: '100%' }}>
            ...
            <div>
              ...
              <span>
                {listTaskReportById &&
                  (listTaskReportById.status === 0 ? 'Tất cả' : listTaskReportById.status === 1 ? 'Đã hoàn thành' : 'Đang thực hiện')}
              </span>
            </div>
            ...
            <div>
              ...
              <span>
                {listTaskReportById &&
                  (listTaskReportById.frequency === 'month' ? 'Tháng' : listTaskReportById.frequency === 'quarter' ? 'Quý' : 'Năm')}
              </span>
            </div>
            ...
            {listTaskReportById && listTaskReportById.responsibleEmployees && (
              <React.Fragment>
                ...
                <ul>
                  {listTaskReportById &&
                    listTaskReportById.responsibleEmployees.map((item, index) => {
            ...
            {listTaskReportById && listTaskReportById.accountableEmployees && (
              <React.Fragment>
                ...
                <ul>
                  {listTaskReportById &&
                    listTaskReportById.accountableEmployees.map((item, index) => {
                      return <li key={index}>{item.name}</li>
                    })}
                </ul>
              </React.Fragment>
            ...
            {listTaskReportById && listTaskReportById.readByEmployees && (
              <React.Fragment>
                ...
                <ul>
                  {listTaskReportById &&
                    listTaskReportById.readByEmployees.map((item, index) => {
                      return <li key={index}>{item.name}</li>
                    })}
                </ul>
              </React.Fragment>
            ...
            <React.Fragment>
              ...
              <ul>{listTaskReportById && listTaskReportById.dataForAxisXInChart.map((x, index) => <li key={index}>{x.name}</li>)}</ul>
            </React.Fragment>
            {/* </div> */}
          </div>
        </div>
        ...
        <div className='col-xs-12 col-sm-12 col-md-6 col-lg-6'>
          <div className='description-box ' style={{ height: '100%' }}>
            <h4>Điều kiện lọc theo các trường thông tin</h4>
            {listTaskReportById && (!listTaskReportById.configurations || listTaskReportById.configurations.length === 0) ? (
              <strong>Không có dữ liệu</strong>
            ) : (
              listTaskReportById &&
              listTaskReportById.configurations.map((item, index) => (
                <React.Fragment key={index}>
                  <strong>
                    {item.code} - {item.name} - {`Kiểu ${formatTypeInfo(item.type)}`}
                  </strong>
                  {
                    <React.Fragment>
                      <div>
