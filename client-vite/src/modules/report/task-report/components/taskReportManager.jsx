...
function TaskReportManager(props) {
  ...
  const checkPermissonCreator = (creator) => {
    ...
    if (creator) {
      if (Array.isArray(creator)) item = creator.map((o) => o._id)[0]
  ...
  let pageTotal = reports.totalList % limit === 0 ? parseInt(reports.totalList / limit) : parseInt(reports.totalList / limit + 1)
      ...
      <div className='box-body qlcv'>
        ...
        <div style={{ height: '40px' }}>
          <ButtonModal
            ...
            button_name={translate('report_manager.add_report')}
            title={translate('report_manager.add_report')}
          />
        </div>
        ...
        <div className='form-inline' style={{ marginBottom: '2px' }}>
          ...
          <div className='form-group'>
            ...
            <input
              ...
              type='text'
              onKeyUp={handleEnterLimitSetting}
              name='creator'
              onChange={handleChangeCreator}
              placeholder={translate('report_manager.search_by_creator')}
            />
          </div>
        </div>
        ...
        <div className='form-inline' style={{ marginBottom: 10 }}>
          <div className='form-group'>
            ...
            <input
              ...
              placeholder={translate('report_manager.search_by_name')}
            />
          </div>
        ...
        <DataTableSetting
          tableId={tableId}
          columnArr={[translate('report_manager.name'), translate('report_manager.description'), translate('report_manager.creator')]}
          setLimit={setLimit}
        />
        ...
        <table className='table table-hover table-striped table-bordered' id={tableId}>
          ...
          <tbody>
            {reports && reports.listTaskReport && reports.listTaskReport.length > 0
              ? reports.listTaskReport.map((item) => (
                  <tr key={item._id}>
                    <td>{item.name} </td>
                      ...
                      {(checkPermissonManager(item.organizationalUnit.managers) || checkPermissonCreator(item.creator)) && (
                        <React.Fragment>
                          <a
                            ...
                            style={{ width: '5px' }}
                            title={translate('report_manager.edit')}
                          >
                          ...
                          <a
                            ...
                            className='delete'
                            title={translate('report_manager.title_delete')}
                          >
