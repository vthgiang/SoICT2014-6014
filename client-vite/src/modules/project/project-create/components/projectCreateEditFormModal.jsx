...
function ProjectCreateEditFormModal(props) {
  ...
  const organizationalUnitKpiSets = createKpiUnit?.organizationalUnitKpiSets?.kpiUnitSets && createKpiUnit?.organizationalUnitKpiSets?.kpiUnitSets?.length > 0 ? createKpiUnit?.organizationalUnitKpiSets?.kpiUnitSets.map((item) => {
    return {
      ...item,
  ...
  useEffect(() => {
    ...
    if (actionType === PROJECT_ACTION_FORM.EDIT && projectEditId && projectEdit) {
      ...
      const {
        ...
        kpiTarget,
        usersInProject, projectManager, assets,
        proposals,
        tasks
      } = projectEdit
      ...
      if (tasks?.length) {
        tasksData = tasks.map((item) => {
          return {
            ...item,
            preceedingTasks: item?.preceedingTasks && item?.preceedingTasks?.length ? item?.preceedingTasks.map((preTask) => {
              return preTask?.link
            }) : []
          }
        })
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={actionType === PROJECT_ACTION_FORM.CREATE ? 'modal-create-project-new' : `modal-edit-project-new-${projectEdit?._id && projectEditId}`}
        isLoading={false}
        formID={actionType === PROJECT_ACTION_FORM.CREATE ? 'form-create-project-new' : `form-edit-project-new-${projectEdit?._id && projectEditId}`}
        title={actionType === PROJECT_ACTION_FORM.CREATE ? translate('project.add_title') : translate('project.edit_title')}
        func={save}
        // closeOnSave={false}
        afterClose={() => {
...
function mapState(state) {
  const { project, user, createKpiUnit, assetsManager, tag, capacity } = state
  return { project, user, createKpiUnit, assetsManager, tag, capacity }
}
const actions = {
  createProjectDispatch: ProjectActions.createProjectDispatch
}

const connectedExampleManagementTable = connect(mapState, actions)(withTranslate(ProjectCreateEditFormModal))
export { connectedExampleManagementTable as ProjectCreateEditFormModal }
