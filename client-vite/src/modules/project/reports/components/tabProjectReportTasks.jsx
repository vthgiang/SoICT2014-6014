...
import ModalListTasks from './modalListTasks'

const TabProjectReportTasks = (props) => {
  ...
  const getOnScheduleTasks = () => {
    ...
    return currentTasks.filter((taskItem, taskIndex) => {
      ...
      if (
        moment().isSameOrBefore(moment(taskItem.endDate)) &&
        diffFromStartToNow / diffFromStartToEstimateEnd <= Number(taskItem.progress) / 100
      )
        return taskItem
    })
  }
  ...
  const getOverdueScheduleTasks = () => {
    if (!currentTasks) return []
    return currentTasks.filter((taskItem, taskIndex) => {
      if (taskItem.progress < 100 && moment().isAfter(moment(taskItem.endDate))) return taskItem
    })
  }
    ...
    return (
      <div className='row col-md-4 col-sm-6 col-xs-6' style={{ marginBottom: 5 }}>
        <span className='task-name'>
          {label} <span style={{ fontWeight: 'bold' }}>({currentTask.length})</span>
        </span>
    ...
    <React.Fragment>
      <div>
        ...
        <div className='box-body qlcv'>
          <h4>
            <strong>Tổng quan công việc dự án</strong>
          </h4>
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslate(TabProjectReportTasks))
