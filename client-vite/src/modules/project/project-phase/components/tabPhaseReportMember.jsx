...
import {
  getActualMemberCostOfTask,
  getAmountOfWeekDaysInMonth,
  getCurrentProjectDetails,
  getEstimateMemberCostOfTask
...
const TabPhaseReportMember = (props) => {
  ...
  let responsibleEmployeesId = []
  for (let taskItem of currentTasks) {
    const responsibleEmployeesFlatten = taskItem.responsibleEmployees.map((resItem) => String(resItem.id))
    const accountableEmployeesFlatten = taskItem.accountableEmployees.map((accItem) => String(accItem.id))
    responsibleEmployeesId = [...responsibleEmployeesId, ...responsibleEmployeesFlatten, ...accountableEmployeesFlatten]
  }
  responsibleEmployeesId = new Set(responsibleEmployeesId)
  let responsibleEmployees = projectDetail?.responsibleEmployees?.filter((e) => responsibleEmployeesId?.has(e._id))

  const preprocessMembersData = () => {
    let result = []
    if (!currentTasks || !projectDetail) return []
    // Cần check xem có cần projectManager và creator không?
    for (let memberItem of responsibleEmployees) {
      ...
      const tasksWithMemberArr = currentTasks.filter((currentTaskItem) => {
        ...
        if (responsibleEmployeesFlatten.includes(String(memberItem.id)) || accountableEmployeesFlatten.includes(String(memberItem.id))) {
      ...
      let overdueTasks = [],
        behindScheduleTasks = [],
        behindBudgetTasks = []
      let totalTimeLogsMilliseconds = 0,
        totalActualCostForMember = 0,
        totalBudgetForMember = 0
      ...
      for (let tasksWithMemberItem of tasksWithMemberArr) {
        ...
        const resEvalItem = tasksWithMemberItem.overallEvaluation?.responsibleEmployees?.find(
          (resItem) => String(resItem.employee?._id) === String(memberItem.id)
        )
        const accEvalItem = tasksWithMemberItem.overallEvaluation?.accountableEmployees?.find(
          (accItem) => String(accItem.employee?._id) === String(memberItem.id)
        )
        ...
        // Push vào doingTasks
        if (
          moment().isSameOrAfter(moment(tasksWithMemberItem.startDate)) &&
          moment().isBefore(moment(tasksWithMemberItem.endDate)) &&
        ...
        if (
          moment().isSameOrBefore(moment(tasksWithMemberItem.endDate)) &&
          diffFromStartToNow / diffFromStartToEstimateEnd <= Number(tasksWithMemberItem.progress) / 100
        ) {
        ...
        if (
          (tasksWithMemberItem.progress < 100 && moment().isAfter(moment(tasksWithMemberItem.endDate))) ||
          (tasksWithMemberItem.progress === 100 &&
            moment().isAfter(moment(tasksWithMemberItem.endDate)) &&
            tasksWithMemberItem.status === 'inprocess')
        ) {
      ...
      result.push({
        ...
        totalTimeLogs:
          totalTimeLogsMilliseconds === 0 ? '00:00:00' : moment.duration(totalTimeLogsMilliseconds, 'milliseconds').format('HH:mm:ss'),
  ...
  const getMembersAlwaysBehindSchedule = (membersData) => {
    ...
    membersData.forEach((item) => {
      ...
      if (
        (behindScheduleTasks.length + overdueTasks.length) / totalTasks.length >= 0.5 &&
  ...
  const preprocessOverdueScheduleChartData = () => {
    let columns = [],
      categories = []
    let overdue = ['Công việc chậm tiến độ'],
      total = ['Tổng số công việc được giao']
    if (!membersData) {
      return {
        columns,
        categories
  ...
  const preprocessBehindBudgetChartData = () => {
    let columns = [],
      categories = []
    let behindBudget = ['Công việc vượt mức ngân sách'],
      total = ['Tổng số công việc được giao']
    if (!membersData) {
      return {
        columns,
      ...
      <div>
        {getCurrentProjectDetails(project, projectDetail?._id)?.projectType === 2 && (
          <div className='box-body qlcv'>
            <h4>
              <strong>Các thành viên điểm số cao (điểm trung bình lớn hơn hoặc bằng 85)</strong>
            </h4>
        ...
        <div className='box-body qlcv'>
          <h4>
            <strong>Các thành viên hay bị chậm tiến độ / quá hạn (tổng số trễ / tổng công việc lớn hơn hoặc bằng 1/2)</strong>
          </h4>
        ...
        {getCurrentProjectDetails(project, projectDetail?._id)?.projectType === 2 && (
          <div className='box-body qlcv'>
            <h4>
              <strong>Các thành viên hay để lãng phí chi phí (tổng số vi phạm / tổng công việc lớn hơn hoặc bằng 1/2)</strong>
            </h4>
        ...
        <div className='box-body qlcv'>
          <h4>
            <strong>Tổng quan thành viên</strong>
          </h4>
              ...
              {membersData &&
                ...
                membersData.map((memberItem, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ color: memberItem?.behindScheduleTasks.length === 0 ? 'black' : 'red' }}>
                      {memberItem?.behindScheduleTasks.length}
                    </td>
                    ...
                    {getCurrentProjectDetails(project, projectDetail?._id)?.projectType === 2 && (
                      <td style={{ color: memberItem?.behindBudgetTasks.length === 0 ? 'black' : 'red' }}>
                        {memberItem?.behindBudgetTasks.length}
                      </td>
                    )}
                    {getCurrentProjectDetails(project, projectDetail?._id)?.projectType === 2 && (
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslate(TabPhaseReportMember))
