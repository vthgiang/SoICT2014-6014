...
const MilestoneCreateForm = (props) => {
  ...
  const {
    ...
    errorOnName,
    errorOnDate,
    errorOnAccountableEmployees,
    errorOnResponsibleEmployees,
    preceedingMilestones
  } = newMilestone
  ...
  const handleChangePreceedingTask = (selected) => {
    ...
    let listMilestone = []
    listMilestone =
      newMilestone.preceedingMilestones && newMilestone.preceedingMilestones.length > 0
        ? currentProjectMilestone?.filter((milestoneItem) => newMilestone.preceedingMilestones.includes(milestoneItem._id))
        : []
  ...
  const handleChangePreceedingMilestone = (selected) => {
    ...
    let listTask =
      newMilestone.preceedingTasks && newMilestone.preceedingTasks.length > 0
        ? currentProjectTasks.filter((taskItem) => newMilestone.preceedingTasks.includes(taskItem._id))
        : []
  ...
  const isFormValidated = () => {
    if (
      !ValidationHelper.validateTaskName(translate, name, projectPhase?.milestones || []).status ||
      errorOnDate ||
      errorOnName ||
      errorOnAccountableEmployees ||
      errorOnResponsibleEmployees ||
            ...
            <fieldset className='scheduler-border'>
              ...
              <div className={'row'}>
                ...
                <div className={`col-lg-12 col-md-12 col-ms-12 col-xs-12 form-group ${errorOnName === undefined ? '' : 'has-error'}`}>
                  <label>
                    {translate('phase.milestone.name')}
                    <span className='text-red'>*</span>
                  </label>
                  <input
                    type='Name'
                    className='form-control'
                    placeholder={translate('phase.milestone.name')}
                    value={name}
              ...
              <div className='row form-group'>
                <div className={`col-lg-12 col-md-12 col-ms-12 col-xs-12 ${errorOnDate === undefined ? '' : 'has-error'}`}>
                  <label className='control-label'>
                    {translate('phase.milestone.date')}
                    <span className='text-red'>*</span>
                  </label>
                  <DatePicker id={`datepicker-milestone-1`} dateFormat='day-month-year' value={date} onChange={handleChangeMilestoneDate} />
                  <TimePicker id={`time-picker-milestone-1`} refs={`time-picker-milestone-1`} value={time} onChange={handleTimeChange} />
