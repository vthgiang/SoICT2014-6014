...
function AssetGantt(props) {
  ...
  useEffect(() => {
    if (gantt) {
      gantt.clearAll()
      gantt.init(`asset-proposal-schedule-gantt-${ganttId}`)
      if (ganttData) {
    ...
    let outer = window.$(`#asset-proposal-schedule-gantt-${ganttId}`)
  ...
  const activeSlimScroll = () => {
    if (ganttId) {
      window.$(`#asset-proposal-schedule-gantt-${ganttId}`).ready(function () {
  ...
  return (
    <React.Fragment>
      ...
      <div className='gantt-container' id={`asset-proposal-schedule-gantt-${ganttId}`} style={{ width: '100%', height: heightCalc }} />
    </React.Fragment>
  )
}
...
const actions = {}
const ganttConnected = connect(mapState, actions)(withTranslate(AssetGantt))
export { ganttConnected as AssetGantt }
