...
function EmployeeGantt(props) {
  ...
  useEffect(() => {
    if (gantt) {
      gantt.clearAll()
      gantt.init(`employee-proposal-schedule-gantt-${ganttId}`)
      if (ganttData) {
    ...
    let outer = window.$(`#employee-proposal-schedule-gantt-${ganttId}`)
    ...
    if (lenghHeight > window.innerHeight * 2 / 3) {
      outer.css({ 'max-height': `${window.innerHeight * 2 / 3}px` })
    }
    //console.log(outer[0]);
  ...
  const activeSlimScroll = () => {
    if (ganttId) {
      window.$(`#employee-proposal-schedule-gantt-${ganttId}`).ready(function () {
  ...
  return (
    <React.Fragment>
      ...
      <div className='gantt-container' id={`employee-proposal-schedule-gantt-${ganttId}`} style={{ width: '100%', height: heightCalc }} />
    </React.Fragment>
  )
}
...
const actions = {}
const ganttConnected = connect(mapState, actions)(withTranslate(EmployeeGantt))
export { ganttConnected as EmployeeGantt }
