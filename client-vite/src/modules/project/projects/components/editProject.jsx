...
const ProjectEditForm = (props) => {
  ...
  const isFormValidated = () => {
    if (
      !ValidationHelper.validateName(translate, projectName, 6, 255).status ||
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-edit-project-${projectEdit?._id && projectEditId}`}
        isLoading={false}
                    ...
                    {responsibleEmployeesWithUnit.list &&
                      ...
                      responsibleEmployeesWithUnit.list.map((item, index) => (
                        <tr key={index}>
                          ...
                          <td>
                            {item?.listUsers.map((userItem) => convertUserIdToUserName(listUsers, userItem.userId || userItem)).join(', ')}
                          </td>
                          {isTasksListEmpty && (
                            <td>
