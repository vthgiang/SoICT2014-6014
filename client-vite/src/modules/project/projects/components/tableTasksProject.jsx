...
const TableTasksProject = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      !tasks?.isLoading &&
      !user?.isLoading &&
      !performtasks?.isLoading &&
      !tasks?.isProjectPaginateLoading &&
      !projectPhase?.isPhaseLoading &&
      !projectPhase?.isMilestoneLoading
    ) {
      ...
      for (let n in currentTasks) {
        data[n] = {
          ...
          responsibleEmployees:
            currentTasks[n]?.responsibleEmployees?.length > 0 ? (
              <ToolTip dataTooltip={currentTasks[n]?.responsibleEmployees.map((o) => o.name)} />
            ) : null,
          accountableEmployees:
            currentTasks[n]?.accountableEmployees?.length > 0 ? (
        ...
        if (
          ...
          (currentTasks[n].accountableEmployees && currentTasks[n].accountableEmployees.find((e) => e._id === userId))
        ) {
      ...
      for (let m = phases.length - 1; m >= 0; m--) {
        phases[m] = {
          ...
          preceedingTasks: null,
          responsibleEmployees:
            phases[m]?.responsibleEmployees?.length > 0 ? (
              <ToolTip dataTooltip={phases[m]?.responsibleEmployees.map((o) => o.name)} />
            ) : null,
          accountableEmployees:
            phases[m]?.accountableEmployees?.length > 0 ? (
              <ToolTip dataTooltip={phases[m]?.accountableEmployees.map((o) => o.name)} />
            ) : null,
        ...
        if (
          (projectPhase?.phases[m]?.responsibleEmployees &&
            projectPhase?.phases[m]?.responsibleEmployees.filter((o) => o._id === userId).length > 0) ||
          (projectPhase?.phases[m]?.consultedEmployees &&
            projectPhase?.phases[m]?.consultedEmployees?.filter((o) => o._id === userId).length > 0)
        ) {
        ...
        if (
          ...
          (projectPhase?.phases[m]?.accountableEmployees &&
            projectPhase?.phases[m]?.accountableEmployees?.filter((o) => o._id === userId).length > 0) ||
          (projectPhase?.phases[m]?.creator && projectPhase?.phases[m]?.creator?._id === userId)
        ) {
      ...
      for (let k in milestones) {
        milestones[k] = {
          ...
          responsibleEmployees:
            milestones[k]?.responsibleEmployees?.length > 0 ? (
              <ToolTip dataTooltip={milestones[k]?.responsibleEmployees.map((o) => o.name)} />
            ) : null,
          accountableEmployees:
            milestones[k]?.accountableEmployees?.length > 0 ? (
              <ToolTip dataTooltip={milestones[k]?.accountableEmployees.map((o) => o.name)} />
            ) : null,
        ...
        if (
          checkIfAbleToCRUDProject({ project, user, currentProjectId }) ||
          (projectPhase?.milestones[k]?.accountableEmployees &&
            projectPhase.milestones[k]?.accountableEmployees?.filter((o) => o._id === userId).length > 0) ||
          (projectPhase?.milestones[k]?.creator && projectPhase?.milestones[k]?.creator?._id === userId)
        ) {
          milestones[k] = { ...milestones[k], action: ['view', 'edit', 'delete'] }
        } else if (
          (projectPhase?.milestones[k]?.responsibleEmployees &&
            projectPhase?.milestones[k]?.responsibleEmployees?.filter((o) => o._id === userId).length > 0) ||
          (projectPhase?.milestones[k]?.consultedEmployees &&
            projectPhase?.milestones[k]?.consultedEmployees?.filter((o) => o._id === userId).length > 0)
        ) {
    ...
    user?.isLoading,
    tasks?.isProjectPaginateLoading,
    JSON.stringify(props?.tasks?.tasksByProjectPaginate),
    JSON.stringify(props?.project?.data?.list),
    JSON.stringify(projectPhase?.phasesByProjectPaginate),
    JSON.stringify(tasks?.tasks),
    JSON.stringify(projectPhase?.milestonesByProjectPaginate)
  ])
  ...
  const processProjectMilestone = (taskList) => {
    ...
    if (!currentProjectMilestone || currentProjectMilestone?.length === 0 || taskList?.length === 0 || !taskList) result = []
      ...
      result = currentProjectMilestone?.filter((milestone) =>
        milestone.preceedingTasks.find((taskItem) => listTaskId.includes(taskItem.task))
      )
    return result
  }
  ...
  return (
    <React.Fragment>
      ...
      {currentTaskId && <ModalPerform units={units} id={currentTaskId} />}
          ...
          <div style={{ height: '40px', display: 'flex', justifyContent: 'space-between' }}>
            ...
            <div>
              <button
                ...
                onClick={() => {
                  window.$('#tasks-project-filter').slideToggle()
                }}
              >
          ...
          <div id='tasks-project-filter' className='form-inline' style={{ display: 'none' }}>
            ...
            <div className='form-group'>
              ...
              <input
                ...
                placeholder={translate('task.task_management.search_by_name')}
                name='name'
                onChange={(e) => handleChangePreceedingTasks(e)}
              />
            </div>
            ...
            <div className='form-group'>
              ...
              <input
                ...
                placeholder={translate('task.task_management.search_by_name')}
                name='name'
                onChange={(e) => handleChangePreceedingMilestones(e)}
              />
            </div>
            ...
            <div className='form-group'>
              ...
              <SelectMulti
                ...
                options={{
                  nonSelectedText: translate('project.task_management.select_type'),
                  allSelectedText: translate('project.task_management.select_all_type')
                }}
              ></SelectMulti>
            </div>
