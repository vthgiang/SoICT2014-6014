...
function InternalPolicy(props) {
        ...
        {listInternalPolicy?.length > 0 &&
          listInternalPolicy.map((policy, index) => (
            <tr key={policy?.id}>
              ...
              <td>
                {moment().isBetween(moment(policy?.effectiveStartTime), moment(policy?.effectiveEndTime)) ? 'Effective' : 'Ineffective'}
              </td>
              <td style={{ textAlign: 'center' }}>
                <a
                  className='edit text-green'
                  style={{ width: '5px' }}
                  title='Thông tin chi tiết chính sách'
                  onClick={() => handleView(policy)}
                >
                  <i className='material-icons'>visibility</i>
                </a>
                <a onClick={() => handleEdit(policy)} className='edit' title='Sửa thông tin chính sách'>
  ...
  return (
    <>
      ...
      <InternalPolicyViewModal internalPolicy={internalPolicyView} />
      <InternalPolicyEditModal id={internalPolicyEdit?.id} internalPolicy={internalPolicyEdit} />
      <div className='box'>
        <div className='box-body qlcv'>
          <div className='form-inline'>
            ...
            <button
              type='button'
              onClick={() => {
                window.$('#create-internal-policy-modal').modal('show')
              }}
