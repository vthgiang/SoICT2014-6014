      ...
      <div className='box-body'>
        <React.Fragment>
          ...
          <table className='table table-hover table-striped table-bordered'>
            ...
            <tbody>
              {systemLinks.listPaginate.length > 0 &&
                systemLinks.listPaginate.map((link) => (
                  <tr key={link._id}>
                    <td>{link.url}</td>
                    <td>{link.category}</td>
                    ...
                    <td>
                      {link.roles.map((role, index, arr) => {
                        if (index !== arr.length - 1) return <span key={role._id}>{role.name}, </span>
                        else return <span key={role._id}>{role.name}</span>
                      })}
                    </td>
                    <td>{link.components.map((component) => component.name)}</td>
                    <td style={{ textAlign: 'center' }}>
                      <a onClick={() => handleEdit(link)} className='edit' title={translate('system_admin.system_link.edit')}>
          ...
          {systemLinks.isLoading ? (
            ...
            systemLinks.listPaginate &&
            systemLinks.listPaginate.length === 0 && <div className='table-info-panel'>{translate('confirm.no_data')}</div>
          )}
          {/* PaginateBar */}
          <PaginateBar
            display={systemLinks.listPaginate.length}
            total={systemLinks.totalDocs}
            pageTotal={systemLinks.totalPages}
            currentPage={systemLinks.page}
            func={setPage}
          />
        </React.Fragment>
      </div>
