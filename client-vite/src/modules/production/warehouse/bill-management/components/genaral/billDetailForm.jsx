...
function BillDetailForm(props) {
  ...
  const getDataChart = () => {
    ...
    if (
      taskDashboardCharts &&
      taskDashboardCharts['gantt-chart'] &&
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        {quantityDetail && <QuantityLotDetailForm quantityDetail={quantityDetail} group={billDetail.group} />}
          ...
          <div className='row'>
            <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
              <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                ...
                <div className='form-group'>
                  ...
                  {billDetail ? (
                    <a style={{ color: translate(`manage_warehouse.bill_management.bill_color.${billDetail.status}`) }}>
                      {translate(`manage_warehouse.bill_management.bill_status.${billDetail.status}`)}
                    </a>
                  ) : (
                    []
                  )}
              ...
              <fieldset className='scheduler-border'>
                ...
                <table className='table table-bordered'>
                  ...
                  <tbody id={`good-edit-manage-by-archive`}>
                    {typeof billDetail.stockWorkAssignment !== 'undefined' &&
                      billDetail.stockWorkAssignment.length > 0 &&
                      billDetail.stockWorkAssignment.map((x, index) => (
                        <tr key={index}>
                          <td>{index + 1}</td>
                          <td>{x.nameField}</td>
                          <td>
                            {x.workAssignmentStaffs &&
                              x.workAssignmentStaffs.length > 0 &&
                              x.workAssignmentStaffs.map((y, index2) => <div key={index2}>{y.name}</div>)}
                          </td>
                  ...
                  <div className='form-inline' style={{ textAlign: 'center' }}>
                    <div className='form-group'>
                      ...
                      <label id='label-for-calendar'>
                        ...
                        {dataCalendar?.countAllTask?.intime}
                      </label>
                    </div>
                    <div className='form-group'>
                      <div id='delay'></div>
                      <label id='label-for-calendar'>
                        ...
                        {dataCalendar?.countAllTask?.delay}
                      </label>
                    </div>
                    <div className='form-group'>
                      ...
                      <label id='label-for-calendar'>
                        ...
                        {dataCalendar?.countAllTask?.notAchived}
                      </label>
                    </div>
                  </div>
                </div>
            ...
            <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
                ...
                <table className='table table-bordered'>
                  <thead>
                    <tr>
                      ...
                      <th title={translate('manage_warehouse.bill_management.code')}>
                      ...
                      {billDetail.group !== '3' && (
                        <th title={translate('manage_warehouse.bill_management.number_passed')}>
                          {translate('manage_warehouse.bill_management.number_passed')}
                        </th>
                      )}
                      {billDetail.group === '3' && (
                        <th title={translate('manage_warehouse.bill_management.quantity_issue')}>
                          {translate('manage_warehouse.bill_management.quantity_issue')}
                        </th>
                      )}
                      {billDetail.group === '3' && (
                        <th title={translate('manage_warehouse.bill_management.quantity_return')}>
                          {translate('manage_warehouse.bill_management.quantity_return')}
                        </th>
                      )}
                      {billDetail.group === '4' && (
                        <th title={translate('manage_warehouse.bill_management.real_quantity')}>
                          {translate('manage_warehouse.bill_management.real_quantity')}
                        </th>
                      )}
                      {billDetail.group === '4' && (
                        <th title={translate('manage_warehouse.bill_management.difference')}>
                          {translate('manage_warehouse.bill_management.difference')}
                        </th>
                      )}
                  ...
                  <tbody id={`good-edit-manage-by-archive`}>
                    {typeof billDetail.goods !== 'undefined' &&
                      billDetail.goods.length > 0 &&
                      billDetail.goods.map((x, index) => (
                        <tr key={index}>
                          ...
                          {billDetail.group !== '3' && (
                            <td>
                              {x.realQuantity}{' '}
                              <a href='#' onClick={() => handleShowDetailQuantity(x)}>
                                {' '}
                                (Chi tiết)
                              </a>
                            </td>
                          ...
                          <td>
                            {x.lots &&
                              x.lots.length > 0 &&
                              x.lots.map((lot, index) => (
                                <div key={index}>
                                  {lot.code && (
                                    <p>
                                      {lot.code}/{lot.quantity} {x.good.baseUnit}
                                    </p>
                                  )}
                                </div>
                              ))}
              ...
              <div className='pull-right' style={{ marginBottom: '10px' }}>
                <p type='button' className='btn btn-success' style={{ marginRight: '10px' }} onClick={handleViewVersion}>
                  {translate('manage_warehouse.bill_management.view_version')}
                </p>
