...
function StockRotateEditForm(props) {
  ...
  const getStatus = () => {
    ...
    if (oldStatus === '7') {
      statusArr = [{ value: '7', text: translate('manage_warehouse.bill_management.bill_status.7') }]
    }
    ...
    <React.Fragment>
      ...
      >
        ...
        <form id={`form-edit-bill-rotate`}>
          ...
          {status === '7' && (
            <div className='timeline-create' style={{ width: '0%' }}>
              <div className='timeline-progress'></div>
              <div className='timeline-items'>
                <div className={`timeline-item cancel`}>
                  <div className={`timeline-contain`}>{'Phiếu đã hủy'}</div>
                </div>
              </div>
            </div>
          )}
            ...
            <fieldset className='scheduler-border'>
              ...
              <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                ...
                <div className={`form-group ${!errorToStock ? '' : 'has-error'}`}>
                  <label>
                    {translate('manage_warehouse.bill_management.rotate_stock')}
                    <span className='text-red'> * </span>
                  </label>
                  <SelectBox
          ...
          <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
            <fieldset className='scheduler-border'>
              ...
              <div className='pull-right' style={{ marginBottom: '10px' }}>
                {good.good && (
                  <p type='button' className='btn btn-info' style={{ marginLeft: '10px' }} onClick={() => addQuantity()}>
                    {translate('manage_warehouse.inventory_management.select_lot')}
                  </p>
                )}
                {state.editInfo ? (
                  <React.Fragment>
                          ...
                          <td>
                            {x.lots.map((lot, index) => (
                              <div key={index}>
                                {lot.lot.code && (
                                  <p>
                                    {lot.lot.code}/{lot.quantity} {x.good.baseUnit}
                                  </p>
                                )}
                              </div>
                            ))}
