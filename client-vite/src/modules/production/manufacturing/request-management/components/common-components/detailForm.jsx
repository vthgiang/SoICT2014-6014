...
function DetailForm(props) {
  ...
  } else if (requestDetail && requestDetail.type == '2') timelineTextArr = timelineText.timelineTextReceipt()
  else if (requestDetail && requestDetail.type == '3') timelineTextArr = timelineText.timelineTextIssue()
  return (
    <React.Fragment>
      <DialogModal
      ...
      >
        {requestDetail ? (
          <form id={`form-detail-request`}>
            {requestDetail.status && (
              <div className='timeline-create'>
                <div
                  ...
                  style={{ width: parseInt(requestDetail.status) > 5 ? '100%' : ((parseInt(requestDetail.status) - 1) / 4) * 100 + '%' }}
                ></div>
            ...
            {requestDetail.status && requestDetail.type == '1' && (
              <div className='timeline-create'>
                <div className='timeline-progress' style={{ width: ((parseInt(requestDetail.status) - 6) / 3) * 100 + '%' }}></div>
                <div className='timeline-items'>
                  {timelineTextArr2 &&
                    timelineTextArr2.length > 0 &&
                    timelineTextArr2.map((item, index) => (
              ...
              <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
                <fieldset className='scheduler-border'>
                  ...
                  <table id='manufacturing-works-table' className='table table-striped table-bordered table-hover'>
                    ...
                    <tbody>
                      {requestDetail.goods && requestDetail.goods.length === 0 && (
                        <tr>
