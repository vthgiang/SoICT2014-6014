...
function getAllManufacturingPlans(query) {
  return (dispatch) => {
    ...
    manufacturingPlanServices
      .getAllManufacturingPlans(query)
      .then((res) => {
        dispatch({
...
function getAllApproversOfPlan(currentRole) {
  return (dispatch) => {
    ...
    manufacturingPlanServices
      .getAllApproversOfPlan(currentRole)
      .then((res) => {
        dispatch({
...
function handleEditManufacturingPlan(data, id) {
  return (dispatch) => {
    ...
    manufacturingPlanServices
      .handleEditManufacturingPlan(data, id)
      .then((res) => {
        dispatch({
...
function createAutomaticSchedule(data) {
  return (dispatch) => {
    dispatch({
      type: manufacturingPlanConstants.AUTOMATIC_SCHEDULE_REQUEST
    })
    manufacturingPlanServices
      .createAutomaticSchedule(data)
      .then((res) => {
        dispatch({
          type: manufacturingPlanConstants.AUTOMATIC_SCHEDULE_SUCCESS,
          payload: res.data.content
        })
      })
      .catch((error) => {
        dispatch({
          type: manufacturingPlanConstants.AUTOMATIC_SCHEDULE_FAILURE,
          error
        })
      })
  }
