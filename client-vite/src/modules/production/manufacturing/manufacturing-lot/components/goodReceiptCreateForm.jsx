...
function GoodReceiptCreateFrom(props) {
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-create-bill-issue-product`}
          ...
          <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
            <fieldset className='scheduler-border'>
              ...
              <div className='row'>
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  ...
                  <div className={`form-group ${!errorQuantity ? '' : 'has-error'}`}>
                    <label>
                      {translate('manage_warehouse.bill_management.number')} ({lot && lot.good.baseUnit})
              ...
              <div className='pull-right' style={{ marginBottom: '10px' }}>
                {state.editBill ? (
                  <React.Fragment>
                    <button className='btn btn-success' onClick={handleCancelEditBill} style={{ marginLeft: '10px' }}>
                      {translate('manufacturing.purchasing_request.cancel_editing_good')}
                    </button>
                    <button
                      className='btn btn-success'
                      disabled={!isValidateBill()}
                  ...
                  <button className='btn btn-success' style={{ marginLeft: '10px' }} disabled={!isValidateBill()} onClick={handleAddBill}>
                    {translate('manufacturing.purchasing_request.add_good')}
                  </button>
                )}
                <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearBill}>
              ...
              <table className='table'>
                ...
                <tbody>
                  {listBills && listBills.length == 0 ? (
                    <tr>
                      <td colSpan={8}>{translate('general.no_data')}</td>
                    </tr>
                  ) : (
                    listBills.map((bill, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <a href='#abc' className='edit' title={translate('general.edit')} onClick={() => handleEditBill(bill, index)}>
          ...
          <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
            {!isFormValidated() && (
              <div className='pull-left form-group has-error' style={{ marginBottom: '10px' }}>
                <label>
                  {translate('manage_warehouse.bill_management.choose_all_lot')}
                  <span className='attention'> * </span>
                </label>
              </div>
            )}
