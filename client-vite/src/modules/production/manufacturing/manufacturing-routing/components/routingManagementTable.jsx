...
const RoutingManagementTable = (props) => {
  ...
  const getWorksArr = () => {
    let worksArr = [{ value: '', text: `---${translate('manufacturing.routing.choose_works')}---` }]
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      await props.getAllManufacturingRoutings(data)
      ...
      <div className='box-body qlcv'>
        ...
        <div className='form-inline'>
          <div className='form-group'>
            ...
            <SelectBox id='select-works-filter' className='form-control select2' style={{ width: '100%' }} items={getWorksArr()} />
          </div>
          <div className='form-group'>
            <label className='form-control-static'>{translate('manufacturing.routing.created_at')}</label>
            <DatePicker id='created-created-date' disabled={false} />
          </div>
        </div>
        <div className='form-inline'>
          <div className='form-group'>
          ...
          <div className='form-group'>
            ...
            <button type='button' className='btn btn-success' title={translate('manufacturing.routing.search')}>
              {translate('manufacturing.quality.search')}
            </button>
          </div>
        </div>
        <table id={tableId} className='table table-striped table-bordered table-hover'>
          ...
          <tbody>
            {manufacturingRouting.listRoutings.length === 0 ? (
              <tr>
