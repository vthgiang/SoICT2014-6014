...
function CrmCareHomePage(props) {
  ...
  let pageTotal = cares.totalDocs % limit === 0 ? parseInt(cares.totalDocs / limit) : parseInt(cares.totalDocs / limit + 1)
      ...
      <div className='box-body qlcv'>
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            {unitMembers && (
              <SelectMulti
                id='multiSelectUnit-employees-care-home-page'
                defaultValue={''}
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            <SelectMulti
              id='multiSelectUnit-status-customercare-common-form'
              defaultValue={''}
              items={[
        ...
        <table className='table table-hover table-striped table-bordered' id='table-manage-crm-group' style={{ marginTop: '10px' }}>
          ...
          <tbody>
            {cares.list && cares.list.length > 0
              ? cares.list.map((o) => (
                  <tr key={o._id}>
                    <td>{o.name ? o.name : ''}</td>
                    ...
                    <td style={{ textAlign: 'left' }}>
                      <a className='text-green' title='Xem thông tin hoạt động CSKH' onClick={() => handleInfo(o._id)}>
                        <i className='material-icons'>visibility</i>
                      </a>
                      {o.status != 3 &&
                        o.status != 5 &&
                        (o.customerCareStaffs[0]._id == auth.user._id || getData.getRole(role) == 'manager') && (
                          <>
                            <a className='text-yellow' title='Chỉnh sửa hoạt động CSKH' onClick={() => handleEdit(o._id)}>
                              <i className='material-icons'>edit</i>
                            </a>
                            {/* Chỉ người thực hiện mới được báo hoàn thành */}
                            {o.customerCareStaffs[0]._id == auth.user._id && (
                              <a className='text-green' title='Hoàn thành hoạt động' onClick={() => handleComplete(o._id)}>
