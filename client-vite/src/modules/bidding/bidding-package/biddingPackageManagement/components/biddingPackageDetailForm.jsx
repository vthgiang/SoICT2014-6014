...
import { GeneralTab, KeyPeopleRequireTab, KeyPeople, Proposals } from '../../biddingPackageInfo/components/combinedContent'
...
const BiddingPackageDetailForm = (props) => {
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='75'
        modalID={`modal-detail-bidding-package${props?._id}`}
          ...
          {biddingPackageDetail && (
            <div className='nav-tabs-custom row' style={{ marginBottom: 0 }}>
              <ul className='nav nav-tabs'>
                ...
                <li>
                  <a title='Yêu cầu nhân sự chủ chốt' data-toggle='tab' href={`#view_contact${_id}`}>
                    Yêu cầu nhân sự chủ chốt
                  </a>
                </li>
                <li>
                  <a title='Danh sách nhân sự chủ chốt' data-toggle='tab' href={`#view_key_people_bidding_package${_id}`}>
                    Danh sách nhân sự chủ chốt
                  </a>
                </li>
                <li>
                  <a title='Hồ sơ đề xuất' data-toggle='tab' href={`#view_proppsals_bidding_package${_id}`}>
                    Hồ sơ đề xuất
                  </a>
                </li>
              </ul>
              <div className='tab-content'>
                {/* Thông tin chung */}
                <GeneralTab disableLinkCreateContract={true} _id={_id} id={`view_general${_id}`} biddingPackage={biddingPackageDetail} />
                {/* Yêu cầu nhân sự chủ chốt */}
                <KeyPeopleRequireTab
                  id={`view_contact${_id}`}
