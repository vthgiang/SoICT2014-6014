...
const CreateCPMTaskModal = (props) => {
  ...
  const handleSubmit = async () => {
    ...
    const newAffectedTasksList = [
      {
        ...
        new: {
          ...
          estimateNormalTime:
            Number(newTask.estimateNormalTime) * (projectDetail.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS),
          estimateOptimisticTime:
            Number(newTask.estimateOptimisticTime) * (projectDetail.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)
        }
      }
    ]
    ...
    const currentTask = {
      ...
      totalResWeight: newTask.totalResWeight,
      estimateNormalTime:
        Number(newTaskFormatted.estimateNormalTime) * (projectDetail.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS),
      estimateOptimisticTime:
        Number(newTaskFormatted.estimateOptimisticTime) * (projectDetail.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)
    }
  ...
  const isFormValidated = () => {
    const { newTask } = state
    return (
      !checkIfHasCommonItems(newTask.accountableEmployees, newTask.responsibleEmployees) &&
      newTask.name.trim().length > 0 &&
      newTask.estimateMaxCost.trim().length > 0 &&
