...
function ViewDecisionForImplement(props) {
  ...
  const [decision, setDecision] = useState(
    props.biddingContract?.decideToImplement ? props.biddingContract?.decideToImplement : initDecision
  )
    ...
    return (
      <>
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <div className='btn btn-success' onClick={() => generateDecision(state?.contract, listUsers)}>
            Tải file Quyết định giao triển khai hợp đồng
          </div>
        </div>
        <fieldset className='scheduler-border'>
          ...
          <div>
            ...
            {decision.projectManager?.map((userItem) => convertUserIdToUserName(listUsers, userItem)).join(', ')}
          </div>
          <div>
            <label>Thành viên dự án:</label>
            <ul>
              {decision.responsibleEmployeesWithUnit &&
                decision.responsibleEmployeesWithUnit.length > 0 &&
                decision.responsibleEmployeesWithUnit.map((item, index) => (
                  <li key={index}>
                    {' '}
                    {convertDepartmentIdToDepartmentName(user.usersInUnitsOfCompany, item?.unitId)}: &nbsp;
                    <span>{item?.listUsers.map((userItem) => convertUserIdToUserName(listUsers, userItem.userId)).join(', ')}</span>
                  </li>
                ))}
            </ul>
          </div>
