      ...
      <div className='box-body'>
        <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
          ...
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a href='#contract-bid' onClick={() => setSelectedTab('contract-bid')} data-toggle='tab'>
                Gói thầu đang thực hiện nhưng chưa có hợp đồng &nbsp;&nbsp;<small className='label label-danger'> {numOfBP} gói</small>
              </a>
            </li>
            <li>
              <a href={`#contract-prj`} onClick={() => setSelectedTab(`contract-prj`)} data-toggle='tab'>
                Hợp đồng chưa có dự án phân công thực hiện &nbsp;&nbsp;<small className='label label-danger'> {numOfCT} hợp đồng</small>
              </a>
            </li>
          </ul>
          <div className='tab-content'>
            <div className={selectedTab === 'contract-bid' ? 'active tab-pane' : 'tab-pane'} id='contract-bid'>
              ...
              <CreateBiddingContract id={currentRow ? currentRow._id : ''} />
              {numOfBP > 0 ? (
                biddingPackagesManager.listActiveBiddingPackage
                  .filter((x) => x?.hasContract === false)
                  .map((item, index) => {
                    return (
                      <section className='col-lg-12 col-md-12' key={`section-contact-bid-${index}`}>
                        <div style={{ borderBottom: '1px solid #eee' }}>
                          <div className='box-header with-border'>
                            <p
                              data-toggle='collapse'
                              data-target={`#contact-bid-${index}`}
                              aria-expanded='false'
                            ...
                            >
                              ...
                              <span
                                id={`arrow-down-contact-bid-${index}`}
                                className='material-icons'
                          ...
                          <div className='box-body collapse' data-toggle='collapse' id={`contact-bid-${index}`} style={{ lineHeight: 2 }}>
                            ...
                            <a href='' onClick={() => handleCreateContract(item)} className='seemore-task'>
                              Tạo hợp đồng cho gói thầu này
                            </a>
                          </div>
                        </div>
                      </section>
            ...
            <div className={selectedTab === 'contract-prj' ? 'active tab-pane' : 'tab-pane'} id='contract-prj'>
              ...
              {numOfCT > 0 ? (
                biddingContract.listBiddingContractStatistic
                  .filter((x) => x?.project === null || x?.project === undefined)
                  .map((item, index) => {
                    return (
                      <section className='col-lg-12 col-md-12' key={`section-contract-prj-${index}`}>
                        <div style={{ borderBottom: '1px solid #eee' }}>
                          <div className='box-header with-border'>
                            <p
                              data-toggle='collapse'
                              data-target={`#contract-prj-${index}`}
                              aria-expanded='false'
                              style={{ display: 'flex', alignItems: 'center', fontWeight: '600', cursor: 'pointer' }}
                              onClick={() => {
                            ...
                            >
                              <span
                                id={`arrow-up-contract-prj-${index}`}
                                className='material-icons'
                                style={{ fontWeight: 'bold', marginRight: '10px' }}
                              >
                                {`keyboard_arrow_up`}
                              ...
                              <span
                                id={`arrow-down-contract-prj-${index}`}
                                className='material-icons'
                                style={{ display: 'none', fontWeight: 'bold', marginRight: '10px' }}
                              >
                                {`keyboard_arrow_down`}
                          ...
                          <div className='box-body collapse' data-toggle='collapse' id={`contract-prj-${index}`} style={{ lineHeight: 2 }}>
                            ...
                            <div>
                              ...
                              <span>
                                {formatTimeOfEffection(item.unitOfTime, item.effectiveDate, item.endDate)} ({item.unitOfTime})
                              </span>
                            </div>
                            <div>
                              <strong>Giá trị hợp đồng: </strong>
                              <span>
                                {item.budget} ({item.currenceUnit})
                              </span>
                            </div>
                            <a href='' onClick={() => handleCreateProject(item)} className='seemore-task'>
                              Tạo dự án cho hợp đồng này
                            </a>
                          </div>
                        </div>
                      </section>
