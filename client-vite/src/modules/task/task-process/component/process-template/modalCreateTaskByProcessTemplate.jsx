...
import { EditProcessTemplateChild } from './editProcessTemplateChild'
...
function ModalCreateTaskByProcessTemplate(props) {
  ...
  const [modeler, setModeler] = useState(
    new BpmnModeler({
      additionalModules: [customModule, { zoomScroll: ['value', ''] }, { bendpoints: ['value', ''] }]
    })
  )
  ...
  const handleProcessTemplateDescProcessChild = async (value) => {
    let infoTemplates = state.infoTemplate
    infoTemplates[`${state.id}`].process.processDescription = value
    await setState((state) => {
      return {
        ...state,
        infoTemplate: infoTemplates
  ...
  const handleChangeManagerProcessChild = async (value) => {
    let infoTemplates = state.infoTemplate
    infoTemplates[`${state.id}`].process.manager = value
    await setState((state) => {
      return {
        ...state,
        infoTemplate: infoTemplates
  ...
  const handleChangeViewerProcessChild = async (value) => {
    let infoTemplates = state.infoTemplate
    infoTemplates[`${state.id}`].process.viewer = value
    await setState((state) => {
      return {
        ...state,
        infoTemplate: infoTemplates
  ...
  // Hàm lưu thông tin
  const save = async () => {
    let { info, infoTemplate, startDate, endDate, userId, processName, officeHours, processDescription, xmlDiagram, viewer, manager } =
      state
    ...
    for (let i in infoTemplate) {
      infoTemplate[i].process.startDate = infoTemplate[i].process.startDate ? infoTemplate[i].process.startDate : startDate
      infoTemplate[i].process.endDate = infoTemplate[i].process.endDate ? infoTemplate[i].process.endDate : endDate
    }
  ...
  if (listUserCompany && listUserCompany.length !== 0) {
    listItem = listUserCompany.map((item) => {
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='100'
        modalID={`modal-create-task-by-process-template`}
        isLoading={false}
        ...
        <div>
          <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
            <ul className='nav nav-tabs'>
              ...
              <li className='active'>
                <a href='#info-create-task' onClick={() => handleChangeContent('info')} data-toggle='tab'>
                  {translate('task.task_process.process_information')}
                </a>
              </li>
              {/* Nút tab quy trình - công việc */}
              <li>
                <a href='#process-create-task' onClick={() => handleChangeContent('process')} data-toggle='tab'>
              ...
              <div className={selected === 'info' ? 'active tab-pane' : 'tab-pane'} id='info-create-task'>
                    ...
                    <div className='row form-group'>
                      <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${errorOnStartDate === undefined ? '' : 'has-error'}`}>
                        <label className='control-label'>
                          {translate('task.task_management.start_date')} <span style={{ color: 'red' }}>*</span>
                        </label>
                        <DatePicker
                      ...
                      <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${errorOnEndDate === undefined ? '' : 'has-error'}`}>
                        <label className='control-label'>
                          {translate('task.task_management.end_date')} <span style={{ color: 'red' }}>*</span>
                        </label>
                        <DatePicker id={`datepicker2-process-${idProcess}`} value={endDate} onChange={handleChangeTaskEndDate} />
                ...
                <div className='form-group'>
                  <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                    <label>
                      Thông tin giờ làm việc hành chính<span className='text-red'>*</span>
                    </label>
                  </div>
                  <table id='project-table' className='table table-striped table-bordered table-hover'>
                    ...
                    <tbody>
                      {officeHours &&
                        ...
                        officeHours.map((item, index) => (
                          <tr key={index}>
                            ...
                            <td>{item.endTime}</td>
                            <td>
                              <a className='delete' title={translate('general.delete')} onClick={() => handleDeleteOfficeHours(index)}>
                      ...
                      <tr key={`add-time-input`}>
                        ...
                        <td>
                          <a className='save text-green' title={translate('general.save')} onClick={handleAddOfficeHours}>
                  ...
                  <div className={`right-content ${showInfo || showInfoProcess ? 'col-md-4' : undefined}`}>
                    ...
                    {showInfo && (
                      <div>
                        <div>
                          <h3>
                            {translate('task.task_process.create_task_with_template')} {taskName}
                          </h3>
                        </div>
