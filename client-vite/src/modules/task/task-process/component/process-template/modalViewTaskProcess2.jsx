...
function ModalViewTaskProcess2(props) {
  ...
  useEffect(() => {
    if (props.idProcess && props.idProcess != state.idProcess) {
      const info = {}
      const infoTask = props.data.tasks // TODO task list
      for (const i in infoTask) {
        info[`${infoTask[i].code}`] = infoTask[i]
      }
      const infoTemplate = {}
      const infoProcessTemplates = props.data.processTemplates // TODO task list
  ...
  return (
    <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
      <ul className='nav nav-tabs'>
        ...
        <li>
          <a href='#process-view' onClick={() => handleChangeContent('process')} data-toggle='tab'>
            {translate('task.task_process.task_process')}
          </a>
        </li>
        <li>
          <a href='#report-view' onClick={() => handleChangeContent('report')} data-toggle='tab'>
            Báo cáo
          </a>
        </li>
        <li>
          <a href='#report-human-view' onClick={() => handleChangeContent('human')} data-toggle='tab'>
            Báo cáo nhân sự
          </a>
        </li>
      </ul>
            ...
            <div className={`right-content ${showInfo || showInfoProcess ? 'col-md-4' : undefined}`}>
              {showInfo && (
                <div>
                  ...
                  <ViewTaskTemplate isProcess taskTemplate={info?.[`${id}`]} listUser={listUser} />
                </div>
              )}
              {showInfoProcess && (
                <div>
