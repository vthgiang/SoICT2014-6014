...
function ModalEditTaskByResponsibleEmployee(props) {
  ...
  const { task, taskName, taskDescription, kpi, taskProjectName, errorTaskName, errorTaskDescription, taskDescriptionDefault, tags } = state
  ...
  function getData(dateParam) {
    ...
    evaluations = task?.evaluations?.find(
      (e) => monthOfEval === new Date(e?.date)?.getMonth() && yearOfEval === new Date(e?.date)?.getFullYear()
    )
