    ...
    <React.Fragment>
      ...
      >
        {task.taskTemplate && (
          <div>
            <p>
              <strong>{translate('task.task_management.calc_formula')}: </strong>
              {task.formula}
            </p>
            <p>{translate('task.task_management.calc_where')}: </p>
            <ul style={{ lineHeight: 2.3 }}>
              {checkFormulaHasProgress && (
                <li>
                  progress - {translate('task.task_management.calc_progress')}:{' '}
                  {progressTask === undefined ? translate('task.task_management.calc_no_value') : `${progress}(%)`}
                </li>
              )}
              {checkFormulaHasAverageActionRating && (
                <li>
                  averageActionRating - {translate('task.task_management.calc_average_action_rating')}: {averageActionRating}{' '}
                  {!a && `(${avgActionNote})`}
                </li>
              )}
              {checkFormulaHasFailedAction && (
                <li>
                  numberOfFailedActions - {translate('task.task_management.calc_failed_action_rating')}: {numberOfFailedActions}
                </li>
              )}
              {checkFormulaHasPassedAction && (
                <li>
                  numberOfPassedActions - {translate('task.task_management.calc_passed_action_rating')}: {numberOfPassedActions}
                  {`${noteNotHasFailedAndPassedAction}`}
                </li>
              )}
              {checkFormulaHasTotalDays && (
                <li>
                  totalDays - {translate('task.task_management.calc_total_day')}: {totalDays} ({translate('task.task_management.calc_days')}
                  )
                </li>
              )}
              {checkFormulaHasDaysUsed && (
                <li>
                  daysUsed - {translate('task.task_management.calc_day_used')}: {daysUsed} ({translate('task.task_management.calc_days')})
                </li>
              )}
              {checkFormulaHasDaysOverdue && (
                <li>
                  daysOverdue - {translate('task.task_management.calc_overdue_date')}: {daysOverdue} (
                  {translate('task.task_management.calc_days')})
                </li>
              )}
              {taskInformations &&
                taskInformations.map((e, index) => {
                  if (e.type === 'number') {
                    return (
                      <li key={index}>
                        {e.code} - {e.name}:{' '}
                        {info[`${e.code}`] && (info[`${e.code}`].value || info[`${e.code}`].value === 0)
                          ? info[`${e.code}`].value
                          : translate('task.task_management.calc_no_value')}
                      </li>
                    )
                  }
                })}
            ...
            <p>
              <strong>{translate('task.task_management.calc_new_formula')}: </strong>
              {formula} = {result}
            </p>
          </div>
        )}
        {(task.taskTemplate === null || task.taskTemplate === undefined) && (
          <div>
            <p>
              <strong>{translate('task.task_management.calc_formula')}: </strong> {task.formula}{' '}
            </p>
            <p>{translate('task.task_management.calc_where')}: </p>
            <ul style={{ lineHeight: 2.3 }}>
            ...
            <p>
              <strong>{translate('task.task_management.calc_new_formula')}: </strong> {formula} = {result}
            </p>
