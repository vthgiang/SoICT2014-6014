        ...
        <fieldset className='scheduler-border'>
          ...
          {perform === 'evaluate' && !disabled && (
            <div className='pull-right' style={{ marginTop: -20 }}>
              <a style={{ cursor: 'pointer' }} onClick={props.updateInfo}>
                {translate('task.task_management.get_outside_info')}
              </a>
            </div>
          )}
          ...
          {perform !== 'evaluate' && !disabled && role === 'accountable' && (
            <div className='pull-right'>
              <a onClick={clickEditInfo} style={{ cursor: 'pointer', fontWeight: 'normal' }}>
                Chỉnh sửa thông tin
              </a>
            </div>
          )}
          {listInfo?.length > 0 &&
            listInfo.map((info, index) => {
              if (info?.type === 'text') {
                return (
                  <div
                    className={`form-group ${value.errorInfo && value.errorInfo[info.code] === undefined ? '' : 'has-error'}`}
                    key={index}
                  >
                    ...
                    <textarea
                      ...
                      value={
                        value.info[`${info.code}`] && checkNullUndefined(value.info[`${info.code}`].value)
                          ? value.info[`${info.code}`].value
                          : ''
                      }
                    />
                    ...
                    >
                      ...
                      <label className='radio-inline'>
                        <input
                          ...
                          checked={
                            value.info[`${info.code}`] &&
                            checkNullUndefined(value.info[`${info.code}`].value) &&
                            value.info[`${info.code}`].value === 'true'
                          }
                          disabled={disabled || (info.filledByAccountableEmployeesOnly && role !== 'accountable')}
                        />{' '}
                      ...
                      <label className='radio-inline'>
                        <input
                          ...
                          checked={
                            value.info[`${info.code}`] &&
                            checkNullUndefined(value.info[`${info.code}`].value) &&
                            value.info[`${info.code}`].value === 'false'
                          }
                          disabled={disabled || (info.filledByAccountableEmployeesOnly && role !== 'accountable')}
                        />{' '}
                  ...
                  return (
                    ...
                    >
                      ...
                      <SelectBox // id cố định nên chỉ render SelectBox khi items đã có dữ liệu
                        ...
                        value={
                          value.info[`${info.code}`] &&
                          checkNullUndefined(value.info[`${info.code}`].value[0]) &&
                          value.info[`${info.code}`].value[0]
                        }
                      />
                    </div>
                  )
          ...
          {perform === 'evaluate' && !disabled && (
            <label className={`form-group`}>
              <input
                ...
                disabled={disabled}
                name='checkSave'
                onChange={(e) => props.handleChangeSaveInfo(e)}
              />{' '}
