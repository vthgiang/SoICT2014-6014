...
import { DataTableSetting, DeleteNotification, PaginateBar, DatePicker, LazyLoadComponent } from "../../../../common-components";
...
import { DateDiff, isBetweenDate, roundProb, getRankingColors, isIntersection } from "../../riskHelper";
...
import { filteredArray } from './../../riskHelper';
function RiskDashboard(props) {
    ...
    useEffect(() => {
        if (props.user.organizationalUnitsOfUser) {
            let data = props.user.organizationalUnitsOfUser
            console.log(data)
            let currentRole = getStorage('currentRole')
            let currentDepartment  = data.find(item => item.employees.includes(currentRole)
    ...
    useEffect(() => {
        if (risk.lists.length != 0 && state.employees) {
            ...
            let employeesOfUnit = [getStorage('userId')].concat(state.employees.map(u => u.userId._id))
            // console.log(employeesOfUnit)
            let temp = risk.lists.filter(r => isIntersection(r.accountableEmployees.map(u => u._id), employeesOfUnit) &&
                isIntersection(r.responsibleEmployees.map(u => u._id), employeesOfUnit))
                console.log(temp)
            setState({
                ...state,
                lists: temp,
    ...
    useEffect(() => {
        if (lists) {
            ...
            // console.log(lists)
            let urgentRisk = lists.filter(r => r.ranking >= 9 && r.riskStatus != 'finished')
            console.log('urgentRisk', urgentRisk)
            let approvableRisk = lists.filter(r => r.riskStatus == "wait_for_approve")
            let finishedRisk = lists.filter(r => r.riskStatus == "finished")
            let inprocessRisk = lists.filter(r => r.riskStatus == "inprocess")
            let highImpactRisk = lists.filter(r => {
                let impact = r.impact ? r.impact : null
                // console.log(impact)
                let totalImpact = impact != null ? impact.health + impact.security + impact.enviroment : 0
                return (totalImpact > 9)
            })
            let highProbRisk = lists.filter(r => {
                // console.log(riskDistribution.lists)
                let riskDis = riskDistribution.lists.find(rd => rd.riskID == r.riskID)
                // console.log(riskDis)
                // if(riskDis==undefined )return false
                return riskDis?.prob > 0.5
            })
            let todayRisk = lists.filter(r => DateDiff.inDays(r.occurrenceDate, new Date()) == 0)
            setState({
                ...state,
                urgentRisk: urgentRisk,
                finishedRisk: finishedRisk,
                approvableRisk: approvableRisk,
                todayRisk: todayRisk,
                inprocessRisk: inprocessRisk,
                highImpactRisk: highImpactRisk,
                highProbRisk: highProbRisk
            })
        }
    }, [lists])
    ...
    const handleSearchData = () => {
        if (startMonth.length != 0 && endMonth.length != 0) {
            ...
            let riskListBetween = state.riskList.filter(risk => isBetweenDate(risk.occurrenceDate, start, end))
    ...
    const setStatisticalChartData = () => {
        ...
        let numMonthsBetween = DateDiff.inMonths(start, end)
        let months = []
        let data = []
        ...
        for (let i = 1; i <= numMonthsBetween; i++) {
            ...
            let monthStr = new Date(start)
            monthStr.setMonth(monthStr.getMonth() + i - 1)
            months.push(dayjs(monthStr).format('MM-YYYY'))
            let numOfRisk = lists.filter(risk => isBetweenDate(risk.occurrenceDate, monthStart, month)).length
            data.push(numOfRisk)
            monthStart = month
        }
        let x = []
    ...
    const setClassificationChartData = (type) => {
        ...
        if (type == RANKING) {
            let low = [translate('risk_dash.risk_matrix.minor')].concat(lists.filter(risk => risk.ranking <= 3).length)
            let medium = [translate('risk_dash.risk_matrix.moderate')].concat(lists.filter(risk => risk.ranking > 3 && risk.ranking <= 6).length)
            let high = [translate('risk_dash.risk_matrix.major')].concat(lists.filter(risk => risk.ranking > 6 && risk.ranking <= 9).length)
            ...
            let colors = getRankingColors()
            options.slices = colors.map(color => { return { color: color } })
            options.is3D = true
        ...
        if (type == PROB) {
            let low = [translate('risk_dash.low')].concat(lists.filter(r => {
                let riskDis = riskDistribution.lists.find(rd => rd.riskID == r.riskID)
                return riskDis?.prob <= 0.5
            }).length)
            let medium = [translate('risk_dash.medium')].concat(lists.filter(r => {
                let riskDis = riskDistribution.lists.find(rd => rd.riskID == r.riskID)
                return riskDis.prob > 0.5 && riskDis.prob <= 0.9
            }).length)
            let high = [translate('risk_dash.high')].concat(lists.filter(r => {
                let riskDis = riskDistribution.lists.find(rd => rd.riskID == r.riskID)
            ...
            rs.options = {
                ...
                is3D: true,
                slices: [getRiskColor(0.4), getRiskColor(0.5), getRiskColor(0.9)].map(c => { return { color: c } })
            }
            return rs
        }
        if (type == IMPACT) {
            let low = [translate('risk_dash.risk_matrix.low')].concat(lists.filter(risk => Math.max(risk.impact.security, risk.impact.health, risk.impact.enviroment) == 1).length)
            let medium = [translate('risk_dash.risk_matrix.medium')].concat(lists.filter(risk => Math.max(risk.impact.security, risk.impact.health, risk.impact.enviroment) == 2).length)
            let high = [translate('risk_dash.risk_matrix.high')].concat(lists.filter(risk => Math.max(risk.impact.security, risk.impact.health, risk.impact.enviroment) == 3).length)
            let veryHigh = [translate('risk_dash.risk_matrix.very_high')].concat(lists.filter(risk => Math.max(risk.impact.security, risk.impact.health, risk.impact.enviroment) == 4).length)
            let rs = []
            rs.data = [['Language', 'Speakers (in millions)'], low, medium, high, veryHigh]
            rs.options = {
                sliceVisibilityThreshold: 0,
                is3D: true,
                slices: getRankingColors().map(c => { return { color: c } })
            }
    ...
    useEffect(() => {
        ...
        if (lists.length != 0) {
            let lineData = setStatisticalChartData()
            const data = {
            ...
            // pie chart
            let rs = setClassificationChartData(type)
            setRatioChart({
                ...ratioChart,
                data: rs.data,
                options: rs.options
        ...
        <React.Fragment>
            <div className="box qlcv" style={{ textAlign: "center" }}>
                <div className="row" style={{textAlign:'center'}}>
                    <h3><strong>{state.currentDepartment&&state.currentDepartment.name.toUpperCase()}</strong></h3>
                        ...
                        <div className="qlcv box-body">
                            <div className="nav-tabs-custom" >
                                <ul className="general-tabs nav nav-tabs">
                                    <li className="active"><a className="general-task-type" href="#urgentRisk" data-toggle="tab" ><img style={{ width: '18px', height: '18px', marginRight: '5px' }} src={urgentIcon} alt="urgent" />{translate('risk_dash.urgency')} <span>{`(${state.urgentRisk ? state.urgentRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#approvableRisk" data-toggle="tab" ><img src={todoIcon} alt="todo" style={{ width: '20px', marginRight: '5px' }} />  {translate('risk_dash.need_approval')}<span>{`(${state.approvableRisk ? state.approvableRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#highProbRisk" data-toggle="tab" >{translate('risk_dash.high_probability')}<span>{`(${state.highProbRisk ? state.highProbRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#highImpactRisk" data-toggle="tab" >{translate('risk_dash.high_impact')}<span>{`(${state.highImpactRisk ? state.highImpactRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#todayRisk" data-toggle="tab" >{translate('risk_dash.occurred_today')}<span>{`(${state.todayRisk ? state.todayRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#finishedRisk" data-toggle="tab" >{translate('risk_dash.finished')}<span>{`(${state.finishedRisk ? state.finishedRisk.length : 0})`}</span></a></li>
                                    <li><a className="general-task-type" href="#inprocessRisk" data-toggle="tab" >{translate('risk_dash.processed')}<span>{`(${state.inprocessRisk ? state.inprocessRisk.length : 0})`}</span></a></li>
                                </ul>
                            </div>
                            <div className="tab-content" id="general-tasks-wraper" style={{ height: '300px' }}>
                                <div className="tab-pane active notifi-tab-pane StyleScrollDiv StyleScrollDiv-y" id="urgentRisk" style={{ height: '300px' }}>
                                    {state.urgentRisk.length != 0 && <RiskDetail
                                ...
                                <div className="tab-pane notifi-tab-pane StyleScrollDiv StyleScrollDiv-y" id="approvableRisk" style={{ height: '300px' }}>
                                    {state.approvableRisk.length != 0 && <RiskDetail
                                ...
                                <div className="tab-pane notifi-tab-pane StyleScrollDiv StyleScrollDiv-y" id="highProbRisk" style={{ height: '300px' }}>
                                    {state.highProbRisk.length != 0 && <RiskDetail
                                ...
                                <div className="tab-pane notifi-tab-pane StyleScrollDiv StyleScrollDiv-y" id="highImpactRisk" style={{ height: '300px' }}>
                                    {state.highImpactRisk.length != 0 && <RiskDetail
                                ...
                                <div className="tab-pane notifi-tab-pane StyleScrollDiv StyleScrollDiv-y" id="todayRisk" style={{ height: '300px' }}>
                                    {state.todayRisk.length != 0 && <RiskDetail
            ...
            <div className="row">
                <div className="col-xs-6">
                    <div className="box box-primary">
                        <div className="box-header with-border">
                            <div className="box-title">{translate('risk_dash.quantity')} {translate('task.task_management.lower_from')} {startMonthTitle} {translate('task.task_management.lower_to')} {endMonthTitle}</div>
                        </div>
                        {lineChartData != null &&
                            <Line
                                data={lineChartData.data}
                                options={lineChartData.option}
                            ></Line>}
                    </div>
                </div>
                <div className="col-xs-6">
                    <div className="box box-primary">
                        <div className="box-header with-border">
                            <div className="box-title">{translate('risk_dash.chart_title_1')} {translate('task.task_management.lower_from')} {startMonthTitle} {translate('task.task_management.lower_to')} {endMonthTitle}</div>
                        </div>
                        <div className="box-body">
                            ...
                            {ratioChart != null && ratioChart.data && <Chart
                                width={'600px'}
                                height={'300px'}
                                chartType="PieChart"
