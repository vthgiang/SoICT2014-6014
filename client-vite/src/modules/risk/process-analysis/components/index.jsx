...
function TaskPert(props) {
  ...
  const listOrganizationalUnit = department?.list
  useEffect(() => {
    if (notifications.associatedData?.length != 0) {
      // console.log('new NOti', notifications.associatedData)
      if (
        notifications.associatedData.dataType == 'realtime_close_task_process' ||
        notifications.associatedData.dataType == 'change_request'
      ) {
        props.updateProb()
  ...
  useEffect(() => {
    if (taskPert.processListData.length != 0) {
      setProcessTable(<TaskProcessManagement />)
      setState({
        ...state,
  ...
  const [widget, setWidget] = useState(<TaskWidgetGroup processData={processData} />)
  useEffect(() => {
    if (processData.length != 0) {
      console.log('loading change', processData)
      setWidget(<TaskWidgetGroup processData={processData} />)
    }
  }, [processData])
  ...
  return (
    <>
      {processData.length != 0 && <TaskWidgetGroup processData={processData} />}
      ...
      <div className='nav-tabs-custom'>
        ...
        <ul className='nav nav-tabs'>
          <li className='active'>
            <a title='Phân tích quy trình' data-toggle='tab' href='#analysis'>
              {translate('process_analysis.index.calculate_successfully')}
            </a>
          </li>
          <li>
            <a title='Quản lý yêu cầu thay đổi quy trình' data-toggle='tab' href='#request_change'>
              {translate('process_analysis.index.change_process_request')}
            </a>
          </li>
        </ul>
      </div>
              ...
              <div className='col-xs-12'>
                <div className='box box-primary'>
                  ...
                  <div className='box-body'>
                    ...
                    {taskPert.processListData.length != 0 && <TaskProcessManagement />}
                  </div>
                </div>
              </div>
            </div>
