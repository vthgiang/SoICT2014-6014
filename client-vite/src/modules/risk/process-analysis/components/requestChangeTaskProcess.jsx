...
function RequestChangeTaskProcess(props) {
  ...
  const { processName, processID, manager, reson, description, startDate, endDate, managerSendRequest } = state

  useEffect(() => {
    if (props.user.usersInUnitsOfCompany && props.user.usersInUnitsOfCompany.length != 0) {
      console.log(process.manager)
      const items = getEmployeeSelectBoxItems(props.user.usersInUnitsOfCompany)
      console.log(items)

      let managerItems = items.map((acc) => {
        const userInDepartment = acc.value
        for (const user of userInDepartment) {
          if (process.manager.map((u) => u._id).includes(user.value)) return user
        }
        return null
      })
  ...
  return (
    <DialogModal
      size={75}
      modalID='modal-request-change-task-process'
      isLoading={false}
          ...
          <div className='request-change-content'>
            <form>
              <div className='form-group'>
                <label htmlFor=''>
                  {translate('process_analysis.request_change_process.content')}
              ...
              <div className='form-group'>
                <label htmlFor=''>
                  {translate('process_analysis.request_change_process.reson')}
                  <span className='text-red'>*</span>
                </label>
                <QuillEditor
                  id='change-reson'
              ...
              <div className='form-group'>
                <label htmlFor=''>
                  {translate('process_analysis.request_change_process.manager')}
                  <span className='text-red'>*</span>
                </label>
