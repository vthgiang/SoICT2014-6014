...
function EditTransportRequestForm(props) {

    const { translate, bigModal, requestManagements, editRequest } = props;
    ...
    const { code, desiredTime, errorIntendReceiveTime, description, approver, errorApprover,
        errorStock, stock, worksValueError, worksValue, customer, optimalStock, listGoods } = state;
    ...
    useEffect(() => {
        if (editRequest && editRequest._id) {
            props.getDetailRequestById(editRequest?._id);
        }
    }, [editRequest]);
    ...
    useEffect(() => {
        if (requestManagements.currentRequest) {
            ...
            setState({
                ...state,
                desiredTime: currentRequestInfo.desiredTime,
    ...
    const save = () => {
        if (isFormValidated()) {
            ...
            props.editTransportRequest(editRequest._id, data);
        }
    }
    ...
    return (
        <React.Fragment>
            <DialogModal
                modalID="modal-edit-transportation-request"
                formID="form-edit-transportation-request"
                title="Thông tin đơn hàng"
                msg_success="Chỉnh sửa thành công"
                msg_failure="Chỉnh sửa thất bại"
                func={save}
                disableSubmit={!isFormValidated()}
                size={bigModal ? 75 : 50}
                maxWidth={500}
            >
                <form id="form-edit-transportation-request">
                    ...
                    <GoodComponentRequest
                        ...
                        listGoods={listGoods}
                        requestId={requestManagements?.currentRequest?._id}
                        selectBoxName={"edit-request-form"}
                    />
                    {/* Tim kiem kho hang phu hop */}
                    {
...
const mapDispatchToProps = {
    ...
    editTransportRequest: RequestActions.editTransportationRequest
}

export default connect(mapStateToProps, mapDispatchToProps)(withTranslate(EditTransportRequestForm));