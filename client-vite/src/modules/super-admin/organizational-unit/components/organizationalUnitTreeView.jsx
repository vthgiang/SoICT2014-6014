...
function DepartmentTreeView(props) {
    ...
    return (
      ...
      >
        ...
        <div
          ...
          id={`department-setting-${data.id}`}
        >
          <a
            href='#setting-organizationalUnit'
            className='edit text-green'
            onClick={() => handleCreateWithParent(data)}
  ...
  const displayTreeView = (data, translate) => {
    if (data) {
      if (!data.children) {
        return <li key={data.id}>{showNodeContent(data, translate)}</li>
      }
      ...
      <div className='form-inline'>
        <div className='dropdown pull-right'>
          <button
            ...
            title='Thêm'
          >
            {translate('manage_department.add_title')}
          </button>
          <ul className='dropdown-menu pull-right'>
            <li>
              <a
                href='#modal-create-department'
                title={translate('manage_department.add_title')}
                onClick={(event) => {
                  handleCreate(event)
                }}
            ...
            <li>
              ...
              >
                {translate('manage_department.import')}
              </a>
            </li>
          </ul>
        </div>
          ...
          {tree &&
            tree.map((tree, index) => (
              <div key={index} className='tf-tree example' style={{ textAlign: 'left', fontSize: state.zoom, marginTop: '50px' }}>
                <ul>{displayTreeView(tree, translate)}</ul>
              </div>
            ))}
        </div>
      </div>
                ...
                {organizationalUnitImagePreview && (
                  <>
                    <label style={{ marginBottom: 0 }} className='custom-file-input' htmlFor='files'>
                      <span className='material-icons icon_upload'>cloud_upload</span>
              ...
              <div style={{ height: '500px', textAlign: 'center' }}>
                {organizationalUnitImagePreview ? (
                  ...
                  <>
                    ...
                    <label className='upload_now' style={{ cursor: 'pointer' }} htmlFor='files'>
                      Cập nhật ngay
                    </label>
                  </>
                )}
