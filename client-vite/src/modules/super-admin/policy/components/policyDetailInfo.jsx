      ...
      >
        <div className='nav-tabs-custom' style={{ marginTop: '-15px' }}>
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a title={translate('manage_policy.policy_information')} data-toggle='tab' href={`#policy_information`}>
                {translate('manage_policy.policy_information')}
              </a>
            </li>
            <li>
              <a title={translate('manage_policy.satisfied_information')} data-toggle='tab' href={`#satisfied_information`}>
                {translate('manage_policy.satisfied_information')}
              </a>
            </li>
          </ul>
              ...
              <div className={`form-group`}>
                ...
                <table className='table table-hover table-bordered detail-policy-attribute-table not-sort'>
                  <thead>
                    <tr>
                      <th style={{ width: '20%' }}>
                        <label>{translate('manage_policy.attribute_owner_table')}</label>
                      </th>
                      <th style={{ width: '20%' }}>
                        <label>{translate('manage_policy.rule_table')}</label>
                      </th>
                      <th style={{ width: '20%' }}>
                        <label>{translate('manage_policy.attribute_name')}</label>
                      </th>
                      <th style={{ width: '20%' }}>
                        <label>{translate('manage_policy.attribute_value')}</label>
                      </th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div id='satisfied_information' className='tab-pane'>
              <div className={`form-group`}>
                ...
                <table className='table table-hover table-bordered detail-policy-attribute-table not-sort'>
                  ...
                  <tbody>
                    {!satisfiedSubjects || satisfiedSubjects.length <= 0
                      ? null
              ...
              <div className={`form-group`}>
                ...
                <table className='table table-hover table-bordered detail-policy-attribute-table not-sort'>
                  ...
                  <tbody>
                    {!satisfiedResources || satisfiedResources.length <= 0
                      ? null
                      : satisfiedResources.map((item, index) => {
                          return (
                            <tr key={index}>
                              ...
                              <td> {item.resourceType == 'Link' ? item.resourceId.url : item.resourceId.name}</td>
                              <td> {item.resourceType}</td>
                              <td> {item.resourceId.description}</td>
                            </tr>
                          )
                        })}
                  </tbody>
