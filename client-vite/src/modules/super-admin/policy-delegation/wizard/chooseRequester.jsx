...
export function ChooseRequester(props) {
  ...
  const isLoading = useSelector((x) => x.policyDelegation.isLoading)
  ...
  return (
    <DialogModal
      ...
      isLoading={isLoading}
      formID={`form-${id}`}
      title={translate(`manage_delegation_policy.wizard.${translation ?? 'requester'}.title`)}
      ...
      size={75}
      maxWidth={850}
      saveText={translate('manage_delegation_policy.wizard.next')}
    >
      {/* Chooose requesters */}
      <div className={`form-group delegation-wizard-row `}>
        <label>
          <div className='delegation-wizard-title'>
            {translate(`manage_delegation_policy.wizard.${translation ?? 'requester'}.choose`)}
            <span className='text-red'>*</span>
          </div>
          <div className='delegation-wizard-description'>{description}</div>
        </label>
        <SelectBox
          id={`select-${id}`}
      ...
      <div className={`form-group delegation-wizard-row `}>
        <label>
          <div className='delegation-wizard-title'>{translate(`manage_delegation_policy.wizard.${translation ?? 'requester'}.authorized`)}</div>
        </label>
        {/* Bảng dữ liệu */}
      ...
      <ProposedRequirement
        ...
        title={translate(`manage_delegation_policy.wizard.${translation ?? 'requester'}.proposed_requirement`)}
