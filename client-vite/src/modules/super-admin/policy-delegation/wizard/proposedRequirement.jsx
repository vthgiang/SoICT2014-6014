...
export function ProposedRequirement(props) {
  ...
  return (
    <div className='row'>
      <div className='form-group col-sm-12 delegation-wizard-row'>
        {state.emptyAttributesError && <div className='delegation-wizard-description text-red'>{state.emptyAttributesError}</div>}
        <label>
          <div className='delegation-wizard-title'>{title}</div>
          <div className='delegation-wizard-description'>{translate('manage_delegation_policy.wizard.requirement_description')}</div>
          <div className='delegation-wizard-description'>{translate('manage_delegation_policy.wizard.requirement_note')}</div>
        </label>
        {/* Select rule */}
        <div className='col-sm-5'>
          <label>
            <div className='delegation-wizard-subtitle'>{translate('manage_delegation_policy.add_rule')}</div>
            <div className='delegation-wizard-description'>{translate('manage_delegation_policy.wizard.rule_description')}</div>
          </label>
          <SelectBox
          ...
          />
          {state.errorRule && <div className='delegation-wizard-description text-red'>{state.errorRule}</div>}
        </div>
        <div className='col-sm-7'>
          <label>
            <div className='delegation-wizard-subtitle'>{translate('manage_delegation_policy.wizard.proposed_attributes')}</div>
          </label>
          <table className='table table-hover table-striped table-bordered'>
            <thead>
              <tr>
                <th>{translate('manage_delegation_policy.wizard.name')}</th>
                <th>{translate('manage_delegation_policy.wizard.value')}</th>
              </tr>
            </thead>
            <tbody>
