...
function getDetailedPolicyById(id) {
  return (dispatch) => {
    dispatch({
      type: policyConstants.GET_DETAILED_POLICY_BY_ID_REQUEST
    })
    policyServices
      .getDetailedPolicyById(id)
      .then((res) => {
        dispatch({
          type: policyConstants.GET_DETAILED_POLICY_BY_ID_SUCCESS,
          payload: res.data.content
        })
      })
      .catch((error) => {
        dispatch({
          type: policyConstants.GET_DETAILED_POLICY_BY_ID_FAILURE,
          error
        })
      })
  }
