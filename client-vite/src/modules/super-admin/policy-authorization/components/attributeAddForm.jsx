...
function AttributeAddForm(props) {
  ...
  //     let msg = undefined;
  //     const { translate } = props;
  //     let uniqueChosenAttribute = state.requesterRequirements.map(a => a.attributeId).filter((x, i, a) => a.indexOf(x) == i);

  //     if (uniqueChosenAttribute.length != state.requesterRequirements.length) {
  //         msg = translate('manage_authorization_policy.duplicate_attribute');
  //     }
  //     if (willUpdateState) {
  //         setState(state => {
  //             return {
    ...
    >
      ...
      <form id={`form-add-attribute-${props.id}`}>
        ...
        <div className={`form-group ${errorOnRuleField ? 'has-error' : ''}`}>
          ...
          <label>{translate('manage_authorization_policy.add_rule')}</label>
          <SelectBox
