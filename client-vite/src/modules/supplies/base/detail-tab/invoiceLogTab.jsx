      ...
      <div className='box-body'>
        <div className='row' style={{ paddingRight: '0px', paddingLeft: '0px' }}>
          ...
          <div className='col-md-12' style={{ paddingRight: '0px', paddingLeft: '0px', minWidth: '350px' }}>
            {logs && (
              <ShowMoreShowLess id={`invoiceLog${id}`} styleShowMoreLess={{ display: 'inline-block', marginBotton: 15 }}>
                {logs.map((item, index) => (
                  <div key={item._id} className={`item-box ${index > 3 ? 'hide-component' : ''}`}>
                    ...
                    <div>{item.description ? parse(item.description) : translate('supplies.general_information.none_description')}</div>
                  </div>
                ))}
              </ShowMoreShowLess>
            )}
