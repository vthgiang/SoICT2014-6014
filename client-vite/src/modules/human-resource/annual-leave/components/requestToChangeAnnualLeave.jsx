...
function RequestToChangeAnnualLeave(props) {
  ...
  const note = (
    <p className='text-left'>
      {' '}
      <span className='text-red'>Thông tin sẽ được gửi thông báo tới người quản lý</span>
    </p>
  )
  return (
    <DialogModal
      modalID={`modal-request-to-change-annualLeave-${id}`}
      isLoading={false}
      ...
      <form className='form-group' id='form-edit-sabbtical'>
        ...
        <div className='row'>
          ...
          <div className={`form-group col-sm-6 col-xs-12 ${errorOnStartDate && 'has-error'}`}>
            ...
            <DatePicker id={`edit_start_date${data._id}`} deleteValue={false} value={startDate} onChange={handleStartDateChange} />
            {type && <TimePicker id='edit_start_time' ref={editStartTime} value={startTime} onChange={handleStartTimeChange} />}
          ...
          <div className={`form-group col-sm-6 col-xs-12 ${errorOnEndDate && 'has-error'}`}>
            ...
            <DatePicker id={`edit_end_date${data._id}`} deleteValue={false} value={endDate} onChange={handleEndDateChange} />
            {type && <TimePicker id='edit_end_time' ref={editEndTime} value={endTime} onChange={handleEndTimeChange} />}
