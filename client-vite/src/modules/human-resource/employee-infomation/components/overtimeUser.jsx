...
function areEqual(prevProps, nextProps) {
  if (
    ...
    JSON.stringify(prevProps.timesheets.listTimesheetsByEmployeeIdAndTime) ===
      JSON.stringify(nextProps.timesheets.listTimesheetsByEmployeeIdAndTime) &&
    JSON.stringify(prevProps.annualLeave.listAnnualLeaveOfNumberMonth) ===
        ...
        <table className='table table-striped table-bordered table-hover' style={{ marginBottom: 0 }}>
          ...
          <tbody>
            {listAnnualLeaveOfNumberMonth &&
              listAnnualLeaveOfNumberMonth.length !== 0 &&
              listAnnualLeaveOfNumberMonth.map((x, index) => {
                let totalHours = x.totalHours
                  ? x.totalHours
                  : (Math.round((new Date(x.endDate).getTime() - new Date(x.startDate).getTime()) / (24 * 60 * 60 * 1000)) + 1) * 8
                return (
                  <tr key={index}>
                    <td>
                      <p>{formatDate2(x.startDate)}</p>
                      {x.startTime ? x.startTime : null}
                    </td>
                    <td>
                      <p>{formatDate2(x.endDate)}</p>
        ...
        <table className='table table-striped table-bordered table-hover' style={{ marginBottom: 0 }}>
          ...
          <tbody>
            {listTimesheetsByEmployeeIdAndTime &&
              listTimesheetsByEmployeeIdAndTime.length !== 0 &&
  ...
  if (listTimesheetsByEmployeeIdAndTime) {
    ...
    if (listTimesheetsByEmployeeIdAndTime.length !== 0) {
      timesheets.arrMonthById.forEach((x) => {
        ...
        let data = listTimesheetsByEmployeeIdAndTime.find(
          (x) => `${new Date(x.month).getFullYear()}-${new Date(x.month).getMonth()}` === month
        )
        if (data) {
        ...
        <div className='col-xs-12'>
          ...
          <div className='box box-primary'>
            <ul className='nav nav-tabs'>
              <li>
                <a>
                  {translate('human_resource.annual_leave.approved_letter')} :{' '}
                  {listAnnualLeaveOfNumberMonth ? listAnnualLeaveOfNumberMonth.length : 0}{' '}
                </a>
              </li>
              <li>
                <a>Số buổi nghỉ phép còn lại : {listAnnualLeaveOfNumberMonth ? 11 - listAnnualLeaveOfNumberMonth.length : 11}</a>
              </li>
              <li>
                <a>
                  {translate('human_resource.timesheets.total_timesheets')} : {totalHours}
                </a>
              </li>
              <li>
                <a>
                  {translate('human_resource.timesheets.total_over_time')} : {overTime}
                </a>
              </li>
              <li>
                <a>
                  {translate('human_resource.timesheets.total_hours_off')} : {hoursOff}
                </a>
              </li>
            </ul>
            <div className='box-header with-border'>
