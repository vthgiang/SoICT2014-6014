...
function SearchEmployeeByCareerPositionTab(props) {
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='75'
        modalID={`modal-change-employee-package${props?._id}`}
                        ...
                        {x.degrees?.length > 0 ? (
                          x.degrees?.map((e, key) => {
                            return (
                              <li key={key}>
                                {' '}
                                {formatDate(e?.year)} - {e?.name} - Loại: {translate(`human_resource.profile.${e?.degreeType}`)} - Chuyên
                                ngành: {e.major?.name} - Bậc:{' '}
                                {professionalSkillArr.filter((item) => item.value == e.degreeQualification).name}
                              </li>
                            )
                          })
                        ) : (
