...
$(document).ready(function () {
        ...
        toolbar: {
          ...
          onClick: function (event) {
            event.done(function () {
              ...
              if (
                aTargets.length === 1 &&
                ['upload', 'toggleVisualGrid', 'toggleAnalyzer', 'toggleOrderPickingGrid'].indexOf(aTargets[0]) === -1
              )
  ...
  parentLayout.get('main').toolbar.tooltip = 'top' // http://w2ui.com/web/docs/1.5/w2toolbar.tooltip
...
function fnShowMyWarehouseVisualizerDemo() {
  ...
  if (!appWarehouses[warehouseIndex].layoutData) {
    ...
    getData('layout')
      .then((response) => {
        // get layout warehouse from server
    ...
    function fnLoadWarehouseData(warehouseName, warehouseScene, layoutData) {
      getData('inventory-warehouse')
        .then((response) => {
          // get inventory warehouse from server
  ...
  function fnShowWarehouse(warehouseName, warehouseScene, inventoryData, layoutData) {
    ...
    var container =
      typeof parentLayout != 'undefined'
        ...
        : $(document.createElement('div')).appendTo($(document.body).css({ margin: '0px' })) //Layout Height Design Pattern: https://github.com/vitmalina/w2ui/issues/105
...
function fnHandleFileUploads(event) {
  ...
  var csvLayoutFile =
    files[0].name.toLowerCase().indexOf('layout') != -1 ? files[0] : files[1].name.toLowerCase().indexOf('layout') != -1 ? files[1] : null
  ...
  app.display = function (container, dataVisual) {
    ...
    var mdvLayoutProperties = {
      ...
      panels: [
        ...
        { type: 'right', size: '0%', resizable: true, style: pstyle } //Scale Control
      ] //panels
    }
...
function myDataVisualizer() {
  ...
  mdv.display = function (mdvLayout, dataVisual) {
    ...
    const table = $(document.createElement('table')).appendTo(predefinedScale)
    const tableRow = $(document.createElement('tr')).appendTo(table)
    const scaleTd = $(document.createElement('td')).appendTo(tableRow)
    const sliderTd = $(document.createElement('td')).appendTo(tableRow)
    ...
    function fnSearchIntersects(intersects, bSearch) {
      if (intersects == 'reset') {
        ...
        if (slotRow && slotRow[dataVisual.dataKey] == dataVisual.visualGrid.last.search && bSearch) {
    ...
    function fnBuildDataVisualizer(container, dataVisual) {
      ...
      function dataVisualizer(overrideProperties) {
        ...
        function fnBuildPerformanceStats(properties) {
          ...
          for (var i = 0; i < properties.stats.domElement.children.length; i++) {
        ...
        function fnBuildScene() {
          ...
          // RENDERER
          if (Detector.webgl) properties.renderer.obj = new THREE.WebGLRenderer({ antialias: true })
                ...
                if (intersects) {
                  ...
                  if (event.ctrlKey) {
                    ...
                    if (
                      ...
                      !$.isEmptyObject(dataVisual.getJoinByKey(dataRow[dataVisual.dataKey]).selectionLink)
                    ) {
            ...
            function fnHandleMouseMove(toolTip) {
              ...
              if (intersects) {
                ...
                if (dataRow) {
                  ...
                  var columnText =
                    fnIsPresetColor(column) || dataVisual.data[0][dataVisual.colorPrefix + column] != undefined ? columnText0 + text : text
                  var formattedValue =
                  ...
                  var text =
                    column == dataVisual.dataKey || column == dataVisual.visualKey
    ...
    function fnGetDataTypes(dataVisual, dataSource) {
      ...
      aColumns.forEach(function (column) {
        ...
        dataTypes[column].type =
          column.substring(0, dataVisual.urlPrefix.length) == dataVisual.urlPrefix
            ? 'url'
            : column.substring(0, dataVisual.datePrefix.length) == dataVisual.datePrefix
              ? 'date'
              : column.substring(0, dataVisual.dateTimePrefix.length) == dataVisual.dateTimePrefix
                ? 'dateTime'
                : column.substring(0, dataVisual.timePrefix.length) == dataVisual.timePrefix
                  ? 'time'
                  : dataTypes[column].type
      ...
      function fnGetGridProperties(dataVisual, visualizer, gridType) {
          ...
          columns.push(
            {
              ...
              render: function (record, rowIndex, columnIndex) {
                if (
                  dataVisual.selectionLinks.length == 0 &&
                  $.isEmptyObject(dataVisual.getJoinByKey(record[dataVisual.dataKey]).selectionLink)
                )
                ...
                if (dataVisual.selectionLinks.length > 0) {
                  dataVisual.selectionLinks.forEach(function (selection, i) {
                    ...
                    options +=
                      "<option value='" + selection.url + urlParms + "' >" + selection.urlText + ' (' + linkParms.join() + ')</option>'
                  }) //forEach
                } //if
                else {
        ...
        function fnGridColumnRender(record, row_index, column_index) {
          ...
          if (fieldValueType != 'text') {
            ...
            var minPixels = 50
            var maxPixels = 100
            var scale = d3.scaleLinear().domain([columnAttributes.minScale, columnAttributes.maxScale]).range([minPixels, maxPixels])
          ...
          $(document.createElement('span'))
            ...
            .text(
              predDefinedColor ? fieldText : fieldValueType != 'text' ? fnFloatFormat(dataVisual, fieldValueColumn, fieldText) : fieldText
            )
            .appendTo(div)
              ...
              switch (event.target) {
                case 'downloadDataButton':
                  if (!(document.documentMode || /Edge/.test(navigator.userAgent))) {
                    ...
                    if (anchor.length == 0) {
                      var anchor = $(document.createElement('a')).attr(
                        'download',
                        dataVisual.warehouseName + (dataVisual.warehouseName.indexOf('.csv') == -1 ? '.csv' : '')
                      )
                  ...
                  function fnFilterDownLoadData(data) {
                    var columns = Object.keys(data[0]).filter(
                      (column) => column == dataVisual.dataKey || !dataVisual.dataTypes[column].isVisualData
                    )
                ...
                case 'downloadGLTFButton': //Possible future feature
                  if (!(document.documentMode || /Edge/.test(navigator.userAgent))) {
                    ...
                    if (anchor.length == 0) {
                      var anchor = $(document.createElement('a')).attr('download', dataVisual.warehouseName + '.gltf')
                      $(event.originalEvent.currentTarget).append(anchor) //https://api.jquery.com/wrap/
                    } //if
        ...
        case 'preview':
          return {
            items: [
              ...
              //     type: 'color',
              //     id: 'colorGrid',
              //     color: new THREE.Color(visualizer.gridHelper.colorGrid).getHexString(),
              //     text: "Grid Color",
              ...
              //     type: 'color',
              //     id: 'colorCenter',
              //     color: new THREE.Color(visualizer.gridHelper.colorCenter).getHexString(),
    ...
    function fnGridSelectColumn(dataVisual) {
      ...
      for (var i = 0; i < dataVisual.activeGridColumn.grid.columns.length; i++) {
        dataVisual.activeGridColumn.grid.columns[i].style =
          i === columnIndex ? 'color: #000000 !important; background-color: #b6d5ff !important;' : ''
      } //for
    } //fnGridSelectColumn
...
function dataVisual() {
  ...
  this.joinDataToVisual = function (data, visual, dataKey, visualKey, colorPrefix, datePrefix, dateTimePrefix, timePrefix, urlPrefix) {
