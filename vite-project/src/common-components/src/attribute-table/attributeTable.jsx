...
function AttributeTable(props) {
  const [state, setState] = useState({
    ...
    noDescription: props.noDescription ? props.noDescription : false,
    attributeType: props.attributeType ? props.attributeType : ['Mixed', 'Authorization', 'Delegation']
  })
    ...
    <React.Fragment>
      ...
      <div className='form-group'>
        ...
        <table className='table table-hover table-striped table-bordered'>
          <thead>
            <tr>
              <th style={noDescription ? { width: '45%' } : { width: '30%' }}>
                <label>{translate(translation + '.attribute_name')}</label>
              </th>
              <th style={noDescription ? { width: '45%' } : { width: '30%' }}>
                <label>{translate(translation + '.attribute_value')}</label>
              </th>
              {noDescription ? null : (
                <th style={{ width: '30%' }}>
                  <label>{translate(translation + '.attribute_description')}</label>
                </th>
              )}
              <th style={{ width: '40px' }} className='text-center'>
                <a href='#add-attributes' className='text-green' onClick={handleAddAttributes}>
                      ...
                      <div className={`form-group ${parseInt(errorOnNameFieldPosition) === index && errorOnNameField ? 'has-error' : ''}`}>
                        <SelectBox
                          ...
                          items={attribute.lists
                            .filter((a) => attributeType.includes(a.type))
                            .map((attribute) => {
                              return { value: attribute ? attribute._id : null, text: attribute ? attribute.attributeName : '' }
                            })}
