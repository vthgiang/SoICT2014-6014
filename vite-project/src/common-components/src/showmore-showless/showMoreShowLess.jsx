...
class ShowMoreShowLess extends Component {
  ...
  addReadMoreText = () => {
    ...
    window.$(`#showmore-showless${id} > .text`).each(function () {
      if (window.$(this).find('.firstSec').length) return
      ...
      <React.Fragment>
        {isText ? (
          <span id={`showmore-showless${id}`} className={`add-read-more showless-content ${classContainer}`}>
            ...
            <span id={`span${id}`} style={styleShowMoreLess} className={classShowMoreLess}>
              <a id={`readMore${id}`} className='read-more' title={titleShowMore} onClick={this.handleShowMore}>
                {' '}
                ... {showMore}{' '}
              </a>
              <a
                id={`readLess${id}`}
                href={`#showmore-showless${id}`}
                className='read-less'
                title={titleShowLess}
          ...
          <div id={`showmore-showless${id}`} className={`add-read-more showless-content ${classContainer}`}>
            ...
            <span id={`span${id}`} style={styleShowMoreLess} className={classShowMoreLess}>
              <a id={`readMore${id}`} className='read-more' title={titleShowMore} onClick={this.handleShowMore}>
                {showMore} <i className='fa fa-angle-double-down'></i>
              </a>
              <a
                id={`readLess${id}`}
                href={`#showmore-showless${id}`}
                ...
                onClick={this.handleShowMore}
              >
                {showLess} <i className='fa fa-angle-double-up'></i>
              </a>
            </span>
          </div>
        )}
