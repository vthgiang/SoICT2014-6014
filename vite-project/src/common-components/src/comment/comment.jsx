      ...
      <>
        {comments && comments
          ? // Hiển thị bình luận của công việc
            comments.map((item) => {
              const listImage = item.files.map((elem) => (this.isImage(elem.name) ? elem.url : -1)).filter((url) => url !== -1)
              return (
                        ...
                        {item.creator?._id === currentUser && (
                          <div className='btn-group pull-right'>
                            ...
                            <ul className='dropdown-menu'>
                              <li>
                                <a style={{ cursor: 'pointer' }} onClick={() => this.handleEditComment(item)}>
                              ...
                              <li>
                                <a
                                  ...
                                  onClick={() => this.props.deleteComment(data._id, item._id, this.props.type)}
                                >
                        ...
                        {childrenComments && (
                          <li>
                            <a
                              ...
                              onClick={() => this.handleShowChildComment(item._id)}
                            >
                                ...
                                >
                                  <b>
                                    <i className='fa fa-paperclip' aria-hidden='true'>
                                      {translate('task.task_perform.attach_file')}({item.files && item.files.length})
                                    </i>
                                  </b>
                                </a>{' '}
                              </div>
                            ...
                            {showfile.some((obj) => obj === item._id) && (
                              <div style={{ cursor: 'pointer' }}>
                                {item.files.map((elem, index) => {
                                  return (
                                    <div key={elem._id} className='show-files-task'>
                  ...
                  {editComment === item._id && (
                    <div>
                      <ContentMaker
                        ...
                        onSubmit={(e) => {
                          this.editComment(e, item.description, item._id, data._id)
                        }}
                      />
                      {item.files.length > 0 && (
                              ...
                              return (
                                <div>
                                  ...
                                  <a
                                    ...
                                    onClick={() => {
                                      this.handleDeleteFile(file._id, file.name, '', item._id, data._id, 'comment')
                                    }}
                                  >
                    ...
                    <div className='comment-content-child'>
                      {item?.comments?.length > 0 &&
                        item.comments.map((child) => {
                          const listImage = child.files.map((elem) => (this.isImage(elem.name) ? elem.url : -1)).filter((url) => url !== -1)
                          return (
                            <div key={child._id}>
                                  ...
                                  <div className='content-level2'>
                                    ...
                                    {child.creator?._id === currentUser && (
                                      <div className='btn-group pull-right'>
                                        ...
                                        <ul className='dropdown-menu'>
                                          <li>
                                            <a style={{ cursor: 'pointer' }} onClick={() => this.handleEditChildComment(child)}>
                                              Sửa bình luận
                                            </a>
                                          </li>
                                          <li>
                                            <a
                                              style={{ cursor: 'pointer' }}
                                              onClick={() => this.props.deleteChildComment(data._id, item._id, child._id, this.props.type)}
                                            >
                                              Xóa bình luận
                                            </a>
                                          </li>
                                        ...
                                        <li style={{ display: 'inline-table' }}>
                                          <div>
                                            <a
                                              style={{ cursor: 'pointer' }}
                                              className='link-black text-sm'
                                              onClick={() => this.handleShowFile(child._id)}
                                            >
                                              <b>
                                                <i className='fa fa-paperclip' aria-hidden='true'>
                                                  {' '}
                                                  File đính kèm ({child.files && child.files.length})
                                                </i>
                                              </b>
                                            </a>
                                          </div>
                              ...
                              {editChildComment === child._id && (
                                <div>
                                  <ContentMaker
                                    ...
                                    onSubmit={(e) => {
                                      this.editChildComment(e, child.description, child._id, item._id, data._id)
                                    }}
                                  />
                                        ...
                                        return (
                                          <div key={file._id}>
                                            ...
                                            <a
                                              ...
                                              className='link-black text-sm btn-box-tool'
                                              onClick={() => {
                                                this.handleDeleteFile(file._id, file.name, child._id, item._id, data._id, 'childComment')
                                              }}
                                            >
                                              <i className='fa fa-times' />
                                            </a>
