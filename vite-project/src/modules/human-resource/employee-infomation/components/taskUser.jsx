    ...
    if (data.length !== 0) {
      result = data.map((x, index) => {
        ...
        return (
          <tr key={index}>
            <td>
              <a href={`/task?taskId=${x._id}`} target='_blank'>
                {x.name}
              </a>
            </td>
            ...
            <td>
              <a
                ...
                onClick={() => handleDelete(x._id, x.name)}
                className='delete'
                title={translate('task.task_management.action_delete')}
              >
  ...
  return (
    <div className='nav-tabs-custom'>
      <ul className='nav nav-tabs'>
        <li className='active'>
          <a href='#taskResponsible' data-toggle='tab' onClick={() => handleNavTabs(true, 0)}>
            Công việc thực hiện : {taskResponsible ? taskResponsible.totalCount : 0}
          </a>
        </li>
        <li>
          <a href='#taskAccountable' data-toggle='tab' onClick={() => handleNavTabs(true, 1)}>
            Công việc phê duyệt : {taskAccountable ? taskAccountable.totalCount : 0}
          </a>
        </li>
        <li>
          <a href='#taskConsulted' data-toggle='tab' onClick={() => handleNavTabs(true, 2)}>
            Công việc tư vấn : {taskConsulted ? taskConsulted.totalCount : 0}
          </a>
        </li>
        <li>
          <a href='#taskInformed' data-toggle='tab' onClick={() => handleNavTabs(true, 3)}>
            Công việc quan sát : {taskInformed ? taskInformed.totalCount : 0}
          </a>
        </li>
        <li>
          <a href='#taskCreator' data-toggle='tab' onClick={() => handleNavTabs(true, 4)}>
            Công việc đã tạo : {taskCreator ? taskCreator.tasks.length : 0}
          </a>
        </li>
      </ul>
      <div className='row'>
        <div className='col-xs-12'>
          ...
          <table className='table table-striped table-bordered table-hover box-body'>
            ...
            <tbody>
              {taskResponsible && status[0] === 1 && showData(taskResponsible.tasks)}

              {taskAccountable && status[1] === 1 && showData(taskAccountable.tasks)}

              {taskConsulted && status[2] === 1 && showData(taskConsulted.tasks)}

              {taskInformed && status[3] === 1 && showData(taskInformed.tasks)}

              {taskCreator && status[4] === 1 && showData(taskCreator.tasks)}
            </tbody>
          </table>
        </div>
      </div>
