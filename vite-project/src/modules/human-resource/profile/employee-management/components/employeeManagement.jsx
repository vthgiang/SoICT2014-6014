...
const EmployeeManagement = (props) => {
  ...
  const convertDataToExportData = (data) => {
    ...
    let workProcessSheet = [],
      experiencesSheet = [],
      degreesSheet = [],
      certificatesSheet = [],
      contractsSheet = [],
      socialInsuranceDetailsSheet = [],
      filesSheet = [],
      commendationsSheet = [],
      disciplinesSheet = [],
      salarysSheet = [],
  ...
  const handleShowEmployeesExpiresContract = (data) => {
    Swal.fire({
      html:
        `<h3 style="color: red"><div>Danh sách nhân viên hết hạn hợp đồng tháng ${dayjs().format('MM-YYYY')}</div> </h3>
  ...
  const handleShowEmployeesHaveBirthDate = (data) => {
    Swal.fire({
      html:
        `<h3 style="color: red"><div>Danh sách nhân viên có sinh nhật tháng ${dayjs().format('MM-YYYY')}</div> </h3>
            <div style="font-size: 1.3em; text-align: left; margin-top: 15px; line-height: 1.7">
           ` +
        `<ul>
            ${
      ...
      <div className='box-body qlcv'>
          ...
          <div className='col-lg-12 col-md-12 col-sm-12'>
            ...
            <div className='dropdown'>
              <button
                ...
                aria-expanded='true'
                title={translate('human_resource.profile.employee_management.add_employee_title')}
              >
                {translate('human_resource.profile.employee_management.add_employee')}
              </button>
              <ul className='dropdown-menu pull-right' style={{ marginTop: 0 }}>
                <li>
                  <a style={{ cursor: 'pointer' }} onClick={createEmployee}>
                    {translate('human_resource.profile.employee_management.add_by_hand')}
                  </a>
                </li>
                <li>
                  <a style={{ cursor: 'pointer' }} onClick={_importEmployee}>
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            <SelectMulti
              id={`multiSelectGender`}
              multiple='multiple'
              options={{
                nonSelectedText: translate('human_resource.profile.employee_management.no_gender'),
                allSelectedText: translate('human_resource.profile.employee_management.all_gender')
              }}
              items={[
                { value: 'male', text: translate('human_resource.profile.male') },
                { value: 'female', text: translate('human_resource.profile.female') }
              ]}
          ...
          {/* Tháng sinh */}
          <div className='form-group'>
            <label title={translate('human_resource.profile.employee_management.brithday_lable_title')} className='form-control-static'>
              {translate('human_resource.profile.employee_management.brithday_lable')}
            </label>
            <DatePicker id='month-birthdate' dateFormat='month-year' value='' onChange={handleBirthdateChange} />
          </div>
        </div>
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            <input
              ...
              name='typeOfContract'
              onChange={handleChange}
              placeholder={translate('human_resource.profile.employee_management.contract_type_title')}
            />
          </div>
          {/* Tháng hết hợp đồng lao động */}
          <div className='form-group'>
            <label title={translate('human_resource.profile.employee_management.contract_lable_title')} className='form-control-static'>
              {translate('human_resource.profile.employee_management.contract_lable')}
            </label>
            <DatePicker id='month-endDate-contract' dateFormat='month-year' value='' onChange={handleEndDateOfContractChange} />
          </div>
          ...
          <div className='form-group'>
            ...
            <SelectMulti
              id={`multiSelectProfessionalSkill`}
              multiple='multiple'
          ...
          <div className='col-md-6'>
            <div>
                ...
                <React.Fragment>
                  <span className='text-danger' style={{ fontWeight: 'bold' }}>
                    <a
                      ...
                      onClick={() => handleShowEmployeesExpiresContract(employeesManager?.expiresContract)}
                    >{` ${employeesManager?.expiresContract?.length} ${translate('human_resource.profile.employee_management.staff')}`}</a>
                  </span>
                ...
                <React.Fragment>
                  <span className='text-success' style={{ fontWeight: 'bold' }}>
                    <a
                      ...
                      onClick={() => handleShowEmployeesHaveBirthDate(employeesManager?.employeesHaveBirthdateInCurrentMonth)}
                    >{` ${employeesManager?.employeesHaveBirthdateInCurrentMonth?.length} ${translate('human_resource.profile.employee_management.staff')}`}</a>
                  </span>
              ...
              {(Number(employeesManager?.expiresContract?.length) > 0 ||
                Number(employeesManager?.employeesHaveBirthdateInCurrentMonth?.length)) > 0 && (
                <span>
                  &nbsp;{`${translate('human_resource.profile.employee_management.this_month')} (${formatDate(Date.now(), true)})`}
                </span>
              )}
            </div>
          </div>
            ...
            {listEmployees &&
              ...
              listEmployees.map((x, index) => (
                <tr key={index}>
                  ...
                  <td style={{ color: x.status === 'active' ? '#28A745' : x.status === 'active' ? '#dd4b39' : null }}>
                    {translate(`human_resource.profile.${x.status}`)}
                  </td>
                  <td>
                    <a
