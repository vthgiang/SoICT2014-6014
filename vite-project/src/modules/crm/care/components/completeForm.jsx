...
function CompleteForm(props) {
  ...
  const save = () => {
    ...
    if (newState.evaluation && !newState.evaluation.result)
      newState = { ...newState, evaluation: { ...careCompleteState.evaluation, result: 1 } }
        ...
        <form id={`modal-crm-care-complete${careCompleteId}`} className='col-md-12'>
          ...
          <div className=''>
            <div className='form-group unitSearch'>
              ...
              <SelectBox
                id={`SelectUnit-result-${careCompleteId}`}
                defaultValue={''}
                items={[
                  { value: '0', text: '---Chọn---' },
