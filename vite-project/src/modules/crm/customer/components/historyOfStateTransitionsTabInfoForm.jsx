                  ...
                  <div className='histories-timeline-item__content'>
                    ...
                    <p style={{ fontSize: '14px' }}>
                      {o.oldValue._id === o.newValue._id ? (
                        ...
                        <div>
                          <strong>{o.createdBy.name}</strong> đã chuyển trạng thái khách hàng từ <strong>{o.oldValue.name}</strong> thành{' '}
                          <strong>{o.newValue.name}</strong>
                        </div>
                      )}
                    </p>
                    {detailStatus[index] && (
                      <div key={index}>
                        <p style={{ fontSize: '14px' }}>
                          {' '}
                          <strong>Nội dung thay đổi : </strong>
                          {o.description}
                        </p>
                      </div>
                    )}
                    <p style={{ fontSize: '14px' }} className='pointer' onClick={() => handleSeeDetail(index)}>
                      {detailStatus[index] == false ? '>> xem chi tiết' : '<< đóng'}
                    </p>
