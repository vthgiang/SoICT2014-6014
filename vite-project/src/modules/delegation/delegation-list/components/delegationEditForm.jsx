...
function DelegationEditForm(props) {
          ...
          .filter(
            (link) =>
              ...
              link.roles
                ...
                .some((r) =>
                  role.list
                    .filter((role) => role._id == props.delegateRole._id.toString())[0]
                    .parents.map((p) => p._id)
                    .concat(props.delegateRole._id.toString())
                    .includes(r)
                )
          )
  ...
  const isFormValidated = () => {
    if (
      ...
      !validateDelegateRole(state.delegateRole, false) ||
  ...
  if (user && user.usersInUnitsOfCompany) {
    ...
    if (delegateRole != '') {
      let selectedRoleUnit = user.organizationalUnitsOfUser.find(
        (item) =>
  ...
  const validateDelegateRole = (value, willUpdateState) => {
    ...
    if (willUpdateState) {
      ...
      let selectedRoleAndParents = role.list
      ...
      let linksOfDelegateRole = link.list.filter(
        (link) =>
          link.roles.filter((role) => role.policies.length > 0).length == 0 &&
