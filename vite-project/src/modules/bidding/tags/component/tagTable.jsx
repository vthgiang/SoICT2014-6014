...
function TagTable(props) {
  ...
  let pageTotal = tag.totalList % state.limit === 0 ? parseInt(tag.totalList / state.limit) : parseInt(tag.totalList / state.limit + 1)
  ...
  return (
    <React.Fragment>
      <div className='box-body'>
        <div>
          <button className='btn btn-success pull-right' type='button' title='Add tag' onClick={handleAddTag}>
            Thêm mới
          </button>
        </div>
            ...
            <tr>
              ...
              <th style={{ width: '120px', textAlign: 'center' }}>
                ...
                <DataTableSetting
                  ...
                  columnArr={['Tên', 'Mô tả', 'Nhân sự phù hợp']}
                  setLimit={setLimit}
                  tableId={tableId}
                />
              </th>
          ...
          <tbody>
            {tag?.listTag?.map((tag) => (
              <tr
                key={`tagList${tag._id}`}
                style={
                  tagDuplicateName && tagDuplicateName.includes(tag.name.trim().toLowerCase().replaceAll(' ', ''))
              ...
              >
                ...
                <td style={{ textAlign: 'center' }}>
                  <a onClick={() => handleView(tag)} title='detail'>
                    <i className='material-icons'>view_list</i>
                  </a>
                  <a className='edit' title='edit' onClick={() => handleEdit(tag)}>
        ...
        {/* modal chỉnh sưa tag */}
        {<EditForm id={currentRow?._id ?? null} tagItem={currentRow ?? currentRow} />}
        {/* modal chỉnh sưa tag */}
        {<DetailForm id={currentRowView?._id ?? null} tagItem={currentRowView ?? currentRowView} />}
        {tag.isLoading ? (
