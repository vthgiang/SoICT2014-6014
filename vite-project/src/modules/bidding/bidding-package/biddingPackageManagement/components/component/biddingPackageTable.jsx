    ...
    <React.Fragment>
      ...
      <div style={{ display: 'flex', marginBottom: 6, float: 'right' }}>
        <a
          ...
          href='#modal-create-career-position'
          title='Add careerPosition'
          onClick={handleAddCareerPosition}
        >
          Thêm
        </a>
      </div>
          ...
          <tr>
            ...
            <th style={{ width: '120px', textAlign: 'center' }}>
              ...
              <DataTableSetting
                ...
                columnArr={['Tên', 'Code', 'Các tên khác tương đương', 'Mô tả']}
                setLimit={setLimit}
                tableId={tableId}
              />
            </th>
        ...
        <tbody>
          {career &&
            career?.listPosition?.map((careerPosition) => (
              <tr
                key={`careerPositionList${careerPosition._id}`}
                style={
                  careerPositionDuplicateName &&
                  careerPositionDuplicateName.includes(careerPosition.name.trim().toLowerCase().replaceAll(' ', ''))
              ...
              >
                ...
                <td style={{ textAlign: 'center' }}>
                  <a className='edit' href={`#${careerPosition._id}`} onClick={() => handleEdit(careerPosition)}>
                    <i className='material-icons'>edit</i>
                  </a>
                  <a className='delete' href={`#${careerPosition._id}`} onClick={() => handleDelete(careerPosition)}>
      ...
      {careerPosition?.isLoading ? (
        ...
        careerPosition?.listPaginate &&
        careerPosition?.listPaginate.length === 0 && <div className='table-info-panel'>{translate('confirm.no_data')}</div>
      )}
      {/* PaginateBar */}
      <PaginateBar
        display={careerPosition?.listPaginate?.length}
        total={careerPosition?.totalDocs}
        pageTotal={careerPosition?.totalPages}
        currentPage={careerPosition?.page}
        func={setPage}
      />
    </React.Fragment>
  )
