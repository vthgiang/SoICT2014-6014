    ...
    return (
      <div>
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          ...
          <div className='btn btn-success' onClick={() => generateContract(state)}>
            Tải file hợp đồng
          </div>
        </div>
        <div style={{ lineHeight: 2, marginLeft: '5px' }} className='row'>
          <div className='col-md-6'>
            ...
            <div>
              ...
              <span>
                {state.budget} ({state.currenceUnit})
              </span>
            </div>
          </div>
          ...
          <div className='col-md-6'>
            <div>
              <strong>Ngày ký kết: </strong>
              <span>{state.createdDate}</span>
            </div>
            <div>
              <strong>Ngày có kiệu lực: </strong>
              <span>{state.effectiveDate}</span>
            </div>
            <div>
              <strong>Thời hạn hợp đồng: </strong>
              <span>
                {state.timeOfEffection} ({state.unitOfTime})
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='col-md-6'>
          <fieldset className='scheduler-border'>
            ...
            <div>
              ...
              <span>{state.companyA}</span>
            </div>
            <div>
              <strong>Địa chỉ: </strong>
              <span>{state.addressA}</span>
            </div>
            <div>
              <strong>Email: </strong>
              <span>{state.emailA}</span>
            </div>
            <div>
              <strong>Điện thoại: </strong>
              <span>{state.phoneA}</span>
            </div>
            <div>
              <strong>Số tài khoản: </strong>
              <span>{state.bankAccountNumberA}</span>
            </div>
            <div>
              <strong>Ngân hàng: </strong>
              <span>{state.bankNameA}</span>
            </div>
            <div>
              <strong>Mã số thuế: </strong>
              <span>{state.taxCodeA}</span>
            </div>
            <div>
              <strong>Người đại diện: </strong>
              <span>{state.representativeNameA}</span>
            </div>
            <div>
              <strong>Chức vụ: </strong>
              <span>{state.representativeRoleA}</span>
            </div>
          </fieldset>
        </div>
        <div className='col-md-6'>
          <fieldset className='scheduler-border'>
            ...
            <div>
              <strong>Công ty: </strong>
              <span>{state.companyB}</span>
            </div>
            <div>
              <strong>Địa chỉ: </strong>
              <span>{state.addressB}</span>
            </div>
            <div>
              <strong>Email: </strong>
              <span>{state.emailB}</span>
            </div>
            <div>
              <strong>Điện thoại: </strong>
              <span>{state.phoneB}</span>
            </div>
            <div>
              <strong>Số tài khoản: </strong>
              <span>{state.bankAccountNumberB}</span>
            </div>
            <div>
              <strong>Ngân hàng: </strong>
              <span>{state.bankNameB}</span>
            </div>
            <div>
              <strong>Mã số thuế: </strong>
              <span>{state.taxCodeB}</span>
            </div>
            <div>
              <strong>Người đại diện: </strong>
              <span>{state.representativeNameB}</span>
            </div>
            <div>
              <strong>Chức vụ: </strong>
              <span>{state.representativeRoleB}</span>
            </div>
          </fieldset>
        </div>
      </div>
      ...
      <form id={`form-edit-bidding-contract-${id}`}>
        <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
          ...
          <ul className='nav nav-tabs'>
            ...
            <li className='active'>
              <a href='#general' onClick={() => handleChangeContent('general')} data-toggle='tab' title='Thông tin chung'>
                Thông tin chung
              </a>
            </li>
            ...
            <li>
              <a
                href={`#view-decision-${id}`}
                onClick={() => handleChangeContent(`#view-decision-${id}`)}
                data-toggle='tab'
                title='Thông tin quyết định giao triển khai hợp đồng'
              >
                Thông tin quyết định giao triển khai hợp đồng
              </a>
            </li>
            {/* Nút tab gói thầu*/}
            <li>
              <a
                href='#bidding-package'
                onClick={() => handleChangeContent('bidding-package')}
                data-toggle='tab'
            ...
            {/* Nút tab dự án */}
            <li>
              <a href='#project' onClick={() => handleChangeContent('project')} data-toggle='tab' title='Thông tin dự án'>
                Thông tin dự án
              </a>
            </li>
          </ul>
          <div className='tab-content'>
                    ...
                    return (
                      <div>
                        {!x.url ? (
                          ...
                          <a className='intable' style={{ cursor: 'pointer' }} onClick={(e) => requestDownloadFile(e, `.${x.url}`, x.name)}>
            ...
            <div className={state.selectedTab === 'project' ? 'active tab-pane' : 'tab-pane'} id='project'>
              ...
              {state.project ? (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    <div className='btn btn-primary' onClick={() => generateReportsZip(state)}>
                      Tải báo cáo theo đầu mục công việc
                    </div>
                  </div>
                  <DetailContent
            ...
            <div className={state.selectedTab === `#view-decision-${id}` ? 'active tab-pane' : 'tab-pane'} id={`#view-decision-${id}`}>
              <ViewDecisionForImplement id={`view-decision-${id}`} biddingContract={state} />
            </div>
          </div>
        </div>
      </form>
