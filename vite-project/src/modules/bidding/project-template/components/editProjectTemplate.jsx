...
const EditProjectTemplateForm = (props) => {
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-edit-project-template-${id}`}
        isLoading={false}
        ...
        <form id='form-create-project-template'>
          <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
            ...
            <ul className='nav nav-tabs'>
              ...
              <li className='active'>
                <a href={`#general-edit-${id}`} onClick={() => handleChangeContent(`general-edit-${id}`)} data-toggle='tab'>
                  Thông số và nhân sự mẫu dự án
                </a>
              </li>
              {/* Nút tab các bên tgia */}
              <li>
                <a href={`#tasks-edit-${id}`} onClick={() => handleChangeContent(`tasks-edit-${id}`)} data-toggle='tab'>
                  Thêm công việc vào mẫu dự án
                </a>
              </li>
            </ul>
