...
export const getParticipants = (listUsers, projectManager, responsibleEmployeesWithUnit, hasManagerAndCreator = false) => {
  let projectParticipants = []
  if (hasManagerAndCreator) {
    let formattedManagerArr = projectManager?.map((item) => {
      return {
        value: item,
    ...
    let formattedEmployeeArr = []
    let respEmployeesArr = []
    for (let unitItem of responsibleEmployeesWithUnit?.list) {
      for (let userItem of unitItem?.listUsers) {
        respEmployeesArr.push(userItem)
      }
    }
    for (let item of respEmployeesArr) {
      if (!projectManager.find((managerItem) => String(managerItem) === String(item))) {
        formattedEmployeeArr.push({
          text: convertUserIdToUserName(listUsers, item),
          value: item
        })
  ...
  for (let unitItem of responsibleEmployeesWithUnit?.list) {
    for (let userItemId of unitItem?.listUsers) {
      projectParticipants.push({
        text: convertUserIdToUserName(listUsers, userItemId),
        value: userItemId
      })
    }
  }
  ...
  for (let i = 0; i < newTasksList.length; i++) {
    if (!initTasksList[i]) {
      ...
      !(
        ...
        moment(newTasksList[i].startDate).isSame(moment(initTasksList[i].startDate)) &&
        moment(newTasksList[i].endDate).isSame(moment(initTasksList[i].endDate)) &&
      ...
      console.log(
        newTasksList[i].estimateNormalTime,
        initTasksList[i].estimateNormalTime,
        newTasksList[i].estimateNormalTime === initTasksList[i].estimateNormalTime
      )
      console.log(
        newTasksList[i].estimateOptimisticTime,
        initTasksList[i].estimateOptimisticTime,
        newTasksList[i].estimateOptimisticTime === initTasksList[i].estimateOptimisticTime
      )
      console.log(
        newTasksList[i].estimateNormalCost,
        initTasksList[i].estimateNormalCost,
        newTasksList[i].estimateNormalCost === initTasksList[i].estimateNormalCost
      )
      console.log(
        newTasksList[i].estimateMaxCost,
        initTasksList[i].estimateMaxCost,
        newTasksList[i].estimateMaxCost === initTasksList[i].estimateMaxCost
      )
      console.log(
        newTasksList[i].preceedingTasks,
        initTasksList[i].preceedingTasks,
        isArraysEqual(newTasksList[i].preceedingTasks, initTasksList[i].preceedingTasks)
      )
      console.log(
        newTasksList[i].startDate,
        initTasksList[i].startDate,
        moment(newTasksList[i].startDate).isSame(moment(initTasksList[i].startDate))
      )
      console.log(
        newTasksList[i].endDate,
        initTasksList[i].endDate,
        moment(newTasksList[i].endDate).isSame(moment(initTasksList[i].endDate))
      )
      affectedTasks.push({
...
export const renderProjectTemplateTypeText = (projectType) => {
