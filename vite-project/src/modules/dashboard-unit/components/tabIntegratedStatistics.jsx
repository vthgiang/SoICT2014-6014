  ...
  render() {
    ...
    if (taskUnits?.length) {
      for (let i in listEmployee) {
        ...
        taskUnits.forEach((x) => {
          if (x?.accountableEmployees) {
            x.accountableEmployees.forEach((k) => {
              if (typeof k === 'object') {
                if (k?._id === listEmployee?.[i]?.userId?._id) accountableTask = [...accountableTask, x._id]
              }
              if (typeof k === 'string') {
                if (k === listEmployee?.[i]?.userId?._id) accountableTask = [...accountableTask, x._id]
              }
            })
          }
          ...
          if (x?.responsibleEmployees) {
            x.responsibleEmployees.forEach((k) => {
              if (typeof k === 'object') {
                if (k?._id === listEmployee?.[i]?.userId?._id) responsibleTask = [...responsibleTask, x._id]
              }
              if (typeof k === 'string') {
                if (k === listEmployee?.[i]?.userId?._id) responsibleTask = [...responsibleTask, x._id]
              }
            })
          }
          ...
          if (x?.consultedEmployees) {
            x.consultedEmployees.forEach((k) => {
              if (typeof k === 'object') {
                if (k?._id === listEmployee?.[i]?.userId?._id) consultedTask = [...consultedTask, x._id]
              }
              if (typeof k === 'string') {
                if (k === listEmployee?.[i]?.userId?._id) consultedTask = [...consultedTask, x._id]
              }
            })
          }
          ...
          if (x?.informedEmployees) {
            x.informedEmployees.forEach((k) => {
              if (typeof k === 'object') {
                if (k?._id === listEmployee?.[i]?.userId?._id) informedTask = [...informedTask, x._id]
              }
              if (typeof k === 'string') {
                if (k === listEmployee?.[i]?.userId?._id) informedTask = [...informedTask, x._id]
              }
            })
          }
        })
        ...
        employeeTasks = [
          ...employeeTasks,
          { _id: listEmployee?.[i]?.userId?._id, name: listEmployee?.[i].userId?.name, totalTask: totalTask?.length }
        ]
      }
    }
    ...
    return (
      <div className='qlcv'>
          ...
          <div className='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                <div className='box-header with-border'>
                  <div className='box-title'>Tổng hợp nhân viên</div>
                </div>
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllEmployee} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                            ...
                            <tr key={index} style={{ width: 46 }}>
                              ...
                              <td>
                                <div className='clearfix'>
                                  {' '}
                                  <small className='pull-right'>{x.totalTask}</small>{' '}
                                </div>
                                <div className='progress xs'>
                                  <div
                                    style={{ width: `${(x.totalTask / maxTask).toFixed(2) * 100}%` }}
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllTasks} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
          ...
          <div className='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllCommendation} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllDiscipline} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
          ...
          <div className='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllHoursOff} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
            ...
            <div className='col-lg-6 col-md-6 col-sm-12 col-xs-12'>
              <div className='box box-solid'>
                ...
                <div className='box-footer text-center'>
                  <a style={{ cursor: 'pointer' }} onClick={this.viewAllOverTime} className='uppercase'>
                    Xem tất cả
                  </a>
                </div>
              </div>
        ...
        <ViewAllCommendation
          ...
          title={`Tổng hợp khen thưởng${month}`}
        />
        <ViewAllDiscipline
          dataDiscipline={employeeDashboardData.discipline?.totalLists?.length > 0 ? employeeDashboardData.discipline.totalLists : []}
          title={`Tổng hợp kỷ luật ${month}`}
        />
        {viewOverTime && <ViewAllOverTime dataView={employeeOvertime} title={`Tổng hợp tình hình tăng ca ${month}`} id={viewOverTime} />}
        {viewHoursOff && <ViewAllOverTime dataView={employeeHoursOff} title={`Tổng hợp tình hình nghỉ phép ${month}`} id={viewHoursOff} />}
      </div>
    )
  }
}
