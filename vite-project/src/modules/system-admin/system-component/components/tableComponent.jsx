  ...
  return (
    <React.Fragment>
        ...
        <thead>
          <tr>
            ...
            <th style={{ width: '120px' }}>
              ...
              <DataTableSetting columnName={translate('table.action')} hideColumn={false} setLimit={setLimit} />
            </th>
          </tr>
        </thead>
        <tbody>
          {systemComponents.listPaginate.length > 0 &&
            systemComponents.listPaginate.map((component) => (
              <tr key={component._id}>
                ...
                <td>
                  <ToolTip dataTooltip={component.links.map((link) => link.url)} />
                </td>
                <td>
                  {component.roles.map((role, i, arr) => {
                    if (i !== arr.length - 1) return <span key={role._id}>{role.name}, </span>
                    else return <span key={role._id}>{role.name}</span>
                  })}
                </td>
                <td style={{ textAlign: 'center' }}>
                  <a onClick={() => handleEdit(component)} className='edit' title={translate('system_admin.system_component.edit')}>
      ...
      {systemComponents.isLoading ? (
        ...
        systemComponents.listPaginate &&
        systemComponents.listPaginate.length === 0 && <div className='table-info-panel'>{translate('confirm.no_data')}</div>
      )}
      {/* PaginateBar */}
      <PaginateBar
        display={systemComponents.listPaginate.length}
        total={systemComponents.totalDocs}
        pageTotal={systemComponents.totalPages}
        currentPage={systemComponents.page}
        func={setPage}
      />
    </React.Fragment>
  )
