...
function PrivilegeApiCreateModal(props) {
  ...
  const checkAll = (e) => {
    ...
    if (checked) {
      if (listPaginateApi?.length > 0) listPaginateApi.map((api) => arr.push(api?._id))
      setState({
        ...state,
        apis: arr
      })
  ...
  const handleCheckboxCategory = (e, apiCategory) => {
    ...
    if (checked) {
      ...
      arr = arr.filter((apiId) => (apiCategoryId.findIndex((delApiId) => apiId === delApiId) >= 0 ? false : true))
    }
  ...
  const renderApiList = () => {
    return arrangedApisArr.map((apisCategory) => (
      <>
        <tr key={apisCategory}>
          <td>
            <input type='checkbox' onChange={(e) => handleCheckboxCategory(e, apisCategory)} />
          </td>
          <td
            colSpan='3'
            style={{
          ...
          >
            Path: <span>{apisCategory.category}</span>
          </td>
        </tr>
        ...
        {apisCategory.apis.map((api) => (
          <tr key={api._id}>
            <td>
              <input type='checkbox' value={api._id} onChange={handleCheckbox} checked={checkedCheckbox(api._id, apis)} />
            </td>
            <td>{api.path}</td>
            <td>{api.method}</td>
          </tr>
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID='privilege-system-api-modal'
        isLoading={false}
        size='75'
        ...
        >
          ...
          {!state.unlimitedExpirationTime && (
            <div className='row'>
              ...
              <div className='form-group col-xs-12 col-sm-6'>
                ...
                <DatePicker id={`datepicker-endDate`} value={state.endDate} onChange={handleEnterEndDate} />
              </div>
            </div>
          )}
          ...
          <div className='form-group'>
            ...
            <SelectBox
              ...
              style={{ width: '100%' }}
              items={
                listPaginateCompany?.length > 0
                  ? listPaginateCompany.map((item) => {
                      return {
                        value: item?._id,
                        text: item?.name
                      }
