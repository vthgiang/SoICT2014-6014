...
function QuoteCreatePayment(props) {
  ...
  useEffect(() => {
    ...
    // props.editCustomerPoint(props.customer, { point: 500 });
  ...
  const getDiscountsValueQuote = (amount) => {
    ...
    let totalCustomerPromotionsValue = customerPromotionsUsed.length
      ? Math.min(customerPromotionsUsed[0].promotionalValueMax, parseInt((amountAfterApplyTax * customerPromotionsUsed[0].value) / 100))
      : 0
      ...
      <fieldset className='scheduler-border' style={{ background: '#f5f5f5' }}>
              ...
              {listGoods &&
                ...
                listGoods.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      {item.amountAfterDiscount && item.amountAfterTax ? formatCurrency(item.amountAfterTax - item.amountAfterDiscount) : 0}
        ...
        <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12 shopping-discount'>
          ...
          <div className='shopping-apply-discounts'>
            ...
            <div className='shopping-apply-discounts-tag'>
              <div>
                {discountsOfOrderValue.length ? `Đã chọn ${discountsOfOrderValue.length} mã giảm giá` : 'Hãy kiểm tra và chọn mã giảm giá'}
              </div>
            </div>
            {/* <a>Chọn khuyến mãi</a> */}
        ...
        <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12 shopping-payment'>
          ...
          <div className='shopping-payment-button'>
            <button style={{ cursor: 'pointer' }} className={`${enableFormSubmit ? '' : 'disable-onclick-prevent'}`} onClick={saveQuote}>
              Lưu báo giá
            </button>
          </div>
        </div>
