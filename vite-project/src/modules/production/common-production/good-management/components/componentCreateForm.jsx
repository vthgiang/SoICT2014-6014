...
function ComponentCreateForm(props) {
  ...
  const isMaterialsValidated = () => {
    let result = validateQuantity(state.material.quantity, false) && validateGood(state.material.good, false)
    return result
  }
    ...
    <fieldset className='scheduler-border'>
      ...
      <div className={`form-group ${!errorOnMaterialQuantity ? '' : 'has-error'}`}>
        ...
        <div>
          <input
            ...
            placeholder={translate('manage_warehouse.good_management.quantity')}
      ...
      <div className='pull-right' style={{ marginBottom: '10px' }}>
        {state.editInfo ? (
          ...
          <button className='btn btn-success' style={{ marginLeft: '10px' }} disabled={!isMaterialsValidated()} onClick={handleAddMaterial}>
      ...
      <table className='table table-bordered'>
        ...
        <tbody id={`material-create-${id - type}`}>
          {typeof listMaterial === 'undefined' || listMaterial.length === 0 ? (
            <tr>
              <td colSpan={3}>
            ...
            listMaterial.map((x, index) => (
              <tr key={index}>
                ...
                <td>{x.quantity}</td>
                <td>
                  <a href='#abc' className='edit' title={translate('general.edit')} onClick={() => handleEditMaterial(x, index)}>
                    <i className='material-icons'></i>
                  </a>
                  <a href='#abc' className='delete' title={translate('general.delete')} onClick={() => handleDeleteMaterial(index)}>
                    <i className='material-icons'></i>
                  </a>
                </td>
              </tr>
