...
class WorkScheduleComponent extends Component {
  ...
  handleCheckBoxChange = async (index1, index2) => {
    ...
    listWorkSchedulesOfMill[index].turns[index1][index2] =
      listWorkSchedulesOfMill[index].turns[index1][index2] === null ? this.props.command : null
  ...
  render() {
    ...
    const currentWorkSchedule = listWorkSchedulesOfMill
      ? listWorkSchedulesOfMill.filter((x) => formatYearMonth(x.month) === month)[0]
      : undefined
      ...
      <div className='box box-primary' style={{ border: '1px solid black' }}>
        <div className='box-header with-border'>
          ...
          <div className='box-title'>
            {translate('manufacturing.plan.work_schedule')} {manufacturingMillName} {translate('manufacturing.plan.month_lower_case')}{' '}
            {month}
          </div>
          ...
          <div className='box-content' style={{ marginTop: '1rem' }}>
            <table className='plan-table-custom plan-table-custom-bordered not-sort'>
              <thead>
                <tr>
                  <th rowSpan={2} style={{ backgroundColor: '#F0F0F0', width: '10%' }}>
                    {translate('manufacturing.work_schedule.work_turns')}
                  </th>
                  <th colSpan={allDaysOfMonth.length} style={{ backgroundColor: '#3C8DBC', color: '#FFFFFF' }}>
                    {translate('manufacturing.plan.month_upper_case')} {month}
                  </th>
                </tr>
                <tr>
              ...
              <tbody>
                {currentWorkSchedule && currentWorkSchedule.turns ? (
                    ...
                    <tr key={index1}>
                      ...
                      {turn.map((command, index2) => {
                        ...
                        return (
                          <td key={index2}>
                            <div className='plan-checkbox-custom'>
                              <input type='checkbox' onChange={() => this.handleCheckBoxChange(index1, index2)} />
                            </div>
                          </td>
                        )
                      })}
                  ...
                  <tr>
                    <td colSpan={allDaysOfMonth.length + 1}>{translate('general.no_data')}</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {currentWorkSchedule && (
            <div className='pull-right' style={{ marginBottom: '10px', marginTop: '10px' }}>
              {!(formatYearMonth(listWorkSchedulesOfMill[0].month) === month) && (
                <button
                  className='btn'
                  style={{ marginLeft: '10px', backgroundColor: '#3C8DBC', color: '#FFF' }}
                  onClick={this.handlePreMonth}
                >{`< ${translate('manufacturing.plan.prev')}`}</button>
              )}
              {listWorkSchedulesOfMill.length > 1 &&
                !(formatYearMonth(listWorkSchedulesOfMill[listWorkSchedulesOfMill.length - 1].month) === month) && (
                  <button
                    className='btn'
                    style={{ marginLeft: '10px', backgroundColor: '#3C8DBC', color: '#FFF' }}
                    onClick={this.handleNextMonth}
                  >{`${translate('manufacturing.plan.next')} >`}</button>
                )}
            </div>
          )}
