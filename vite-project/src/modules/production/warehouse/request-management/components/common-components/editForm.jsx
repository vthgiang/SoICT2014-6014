        ...
        <form id={`form-edit-request-${requestId}`}>
          ...
          <fieldset className='scheduler-border'>
            ...
            <div className='form-group'>
              ...
              <p style={{ float: 'right' }} type='button' onClick={handleChooseTypeOfSelectStock} className='btn btn-primary'>
                {!isAutoSelectStock ? 'Chọn kho tự động' : 'Chọn kho thủ công'}
              </p>
            </div>
            {!isAutoSelectStock && <div></div>}
            {isAutoSelectStock && <div>{'Tự động chỗ này'}</div>}
          </fieldset>
        </form>
      </DialogModal>
    </React.Fragment>
