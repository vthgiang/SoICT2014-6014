...
function AddLotComponent(props) {
  ...
  const handleLotsChange = (lots, data, arrayId) => {
    ...
    if (isPassQualityControl) dataGoods[state.indexInfo].lots = lots
    else dataGoods[state.indexInfo].unpassed_quality_control_lots = lots
    setState({
      ...state,
      listGood: dataGoods
    })
  ...
  const {
    ...
    initialData,
    errorOnStatus,
    isPassQualityControl
  } = state
  return (
    <React.Fragment>
      <QuantityLotGoodReceipt
        group={group}
        good={good}
        stock={fromStock}
        type={type}
        ...
        isPassQualityControl={isPassQualityControl}
        onDataChange={handleLotsChange}
      />
            ...
            <table className='table'>
              <thead>
                <tr>
                  ...
                  <th title={translate('manage_warehouse.bill_management.action')}>
                    {translate('manage_warehouse.bill_management.action')}
                  </th>
                </tr>
              </thead>
                ...
                {typeof listGood === 'undefined' || listGood.length === 0 ? (
                  ...
                  listGood.map((x, index) => (
                    <tr key={index}>
                      ...
                      {checkLots(x.unpassed_quality_control_lots, x.quantity - x.realQuantity) ? (
                        <td>
                          {x.unpassed_quality_control_lots.map((lot, index) => (
                            <div key={index}>
                              {lot.code && (
                                <p>
                                  {lot.code}/{lot.quantity} {x.good.baseUnit}
                      ...
                      <td>
                        <a
                          ...
                          title={'Đánh lô hàng đạt kiểm định'}
                          onClick={() => addLotPassQualityControl(x, index)}
                        >
                        ...
                        <a
                          href='#abc'
                          className='text-blue'
                          title={'Đánh lô hàng không đạt kiểm định'}
                          onClick={() => addLotUnPassQualityControl(x, index)}
                        >
