...
const isTimeZoneDateSmaller = (date1, date2) => {
...
const getVehicleCarrierUsable = (date, listPlan, listCarriers, listVehicles) => {
  // date timezone
  let usedVehicles = []
  let usedCarriers = []
...
export const generatePlanFastestMove = (listRequirement, listPlan, allVehicles, allCarriers, inDay, startDate) => {
  ...
  if (
    !(listRequirement && allVehicles && allCarriers && listRequirement.length !== 0 && allVehicles.length !== 0 && allCarriers.length !== 0)
  ) {
    return
  }
  saveArr = []
...
let allVehicles = [
  {
    ...
    asset: {
      assetType: ['60950b874fd0700d10bfbd8e'],
      readByRoles: ['60950b854fd0700d10bfbaf0', '609512a133fa651a2874f1fd'],
      cost: 0,
      usefulLife: 0,
