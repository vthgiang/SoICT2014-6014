        ...
        <div style={{ flexDirection: 'row', display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
          <div className='box-tools' style={{ marginLeft: 10 }}>
            <div className='btn-group'>
              <button type='button' className={`btn btn-xs ${isTableType ? 'btn-danger' : 'active'}`} onClick={() => setIsTableType(true)}>
                Bảng
              </button>
              <button
                type='button'
                className={`btn btn-xs ${isTableType ? 'active' : 'btn-danger'}`}
                onClick={() => {
          ...
          <div style={{ flexDirection: 'row', display: 'flex' }}>
            ...
            {/* Button thêm mới */}
            {projectDetail?.projectType === 1 && <TaskAddModal onHandleReRender={onHandleReRender} projectId={projectDetail._id} />}
                ...
                <div className='dropdown'>
                  ...
                  {checkIfAbleToCRUDProject({ project, user, currentProjectId, isInsideProject: true }) ? (
                    <ul className='dropdown-menu pull-right'>
                      <li>
                        <a href='#' title={translate('project.task_management.add_task')} onClick={handleOpenCreateProjectTask}>
                          {translate('project.task_management.add_task')}
                        </a>
                      </li>
                      <li>
                        <a href='#' title={translate('project.task_management.add_phase')} onClick={handleOpenCreateProjectPhase}>
                          {translate('project.task_management.add_phase')}
                        </a>
                      </li>
                      <li>
                        <a href='#' title={translate('project.task_management.add_milestone')} onClick={handleOpenCreateProjectMilestone}>
                          {translate('project.task_management.add_milestone')}
                        </a>
                      </li>
                    </ul>
                  ) : (
