...
const TabEvalSelf = (props) => {
  ...
  const handleSelfData = (listTaskData) => {
    ...
    let selfData
    const selfItem =
      projectDetail.responsibleEmployees.find((projectEmpItem) => String(projectEmpItem._id) === userId) ||
    ...
    const tasksWithMemberArr = listTaskData.filter((listTaskItem) => {
      ...
      if (responsibleEmployeesFlatten.includes(String(userId)) || accountableEmployeesFlatten.includes(String(userId))) {
        return listTaskItem
      }
    })
    if (tasksWithMemberArr.length === 0) return undefined
      ...
      for (let tasksWithMemberItem of tasksWithMemberArr) {
        ...
        const currentMemberEvalRes = tasksWithMemberItem?.overallEvaluation?.responsibleEmployees.find(
          (resEval) => String(resEval.employee) === String(userId)
        )
        const currentMemberEvalAcc = tasksWithMemberItem?.overallEvaluation?.accountableEmployees.find(
          (accEval) => String(accEval.employee) === String(userId)
        )
  ...
  return (
    <React.Fragment>
      <div className='box-body qlcv'>
        <h4>
          <strong>Điểm số thành viên {processedSelfData?.name} trong tháng</strong>
        </h4>
                  ...
                  return (
                    <tbody key={`self-summary-${memberTaskItem._id}`}>
                      <tr key={`${userId}-${memberTaskIndex}-0`}>
                        <td>
                          <strong>{memberTaskIndex === 0 ? processedSelfData?.name : ''}</strong>
                        </td>
