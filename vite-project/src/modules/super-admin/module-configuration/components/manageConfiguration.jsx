  ...
  return (
    <div className='nav-tabs-custom box-body'>
      <ul className='nav nav-tabs'>
        <li className='active'>
          <a
            onClick={() => handleChangeContent('human_resource_config')}
            data-toggle='tab'
            href='#human_resource_config'
          >{`Quản lý nhân sự`}</a>
        </li>
        <li className=''>
          <a onClick={() => handleChangeContent('bidding_config')} data-toggle='tab' href='#bidding_config'>{`Quản lý đấu thầu`}</a>
        </li>
      </ul>
      <div className='tab-content'>
        <div className={selectedTab === 'human_resource_config' ? 'active tab-pane' : 'tab-pane'}>
          <HumanResourceConfiguration id='human_resource_config' setDataReq={setHumanResourceConfig} />
        </div>
        <div className={selectedTab === 'bidding_config' ? 'active tab-pane' : 'tab-pane'}>
          <BiddingConfiguration id='bidding_config' setDataReq={setBiddingConfig} />
        </div>
      </div>
      <div className=' col-md-12'>
        <button type='button' className='btn pull-right btn-success' onClick={() => save()}>
