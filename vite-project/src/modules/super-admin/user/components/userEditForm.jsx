...
function UserEditForm(props) {
  ...
  const checkSuperAdmin = (roleArr) => {
    const superAdmin = props.role.list.find((obj) => {
  ...
  const save = () => {
    const keys_to_keep = ['attributeId', 'value', 'description']
    if (isFormValidated()) {
  ...
  return (
    ...
    >
      ...
      <form id='form-edit-user'>
          ...
          {checkSuperAdmin(userRoles) ? ( // Là super admin của công ty
            <>
              <div className='form-group col-sm-3'>
                ...
                <select className='form-control' style={{ width: '100%' }} value={userActive} disabled>
                  {status.map((result) => (
                    <option key={result.id} value={result.value}>
                      {translate(`manage_user.${result.name}`)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <>
              <div className='form-group col-sm-3'>
                ...
                <select className='form-control' style={{ width: '100%' }} value={userActive} onChange={handleUserActiveChange}>
                  {status.map((result) => (
                    <option key={result.id} value={result.value}>
                      {translate(`manage_user.${result.name}`)}
                    </option>
        ...
        {checkSuperAdmin(userRoles) && auth.user._id !== userId ? null : (
