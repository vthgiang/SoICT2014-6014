...
function InformationForm(props) {
  ...
  const handleAddInformation = (event) => {
    ...
    if (!taskInformations) taskInformations = []

    const newTaskInformations = [...taskInformations, information]
  ...
  const isInfoFormValidated = () => {
    let result =
      ...
      (state.information.type !== INFO_TYPE.SET ||
        (state.information.type === INFO_TYPE.SET && validateInfoSetOfValues(state.information.extra, false)))
    return result
  }
  ...
  return (
    ...
    <fieldset className='scheduler-border'>
      ...
      <div className={`form-group ${state.information.errorOnName === undefined ? '' : 'has-error'}`}>
        ...
        <div>
          <input
            ...
            placeholder={translate('task_template.infor_name')}
      ...
      <div className={`form-group ${state.information.errorOnDescription === undefined ? '' : 'has-error'}`}>
        <label className='control-label' htmlFor='inputDescriptionInfo'>
      ...
      <div className='form-group'>
        ...
        {props.isEdit === true && state.warning && (
          <p style={{ color: '#FF6A00' }}>Đổi kiểu dữ liệu sẽ xóa dữ liệu ở tháng hiện tại và đánh giá tháng hiện tại</p>
        )}
        <div style={{ width: '100%' }}>
      ...
      {state.information.type === INFO_TYPE.SET ? (
        <div className={`form-group ${state.information.errorOnSetOfValues === undefined ? '' : 'has-error'}`}>
          ...
          <textarea
            ...
            onChange={handleChangeInfoSetOfValues}
            placeholder={`Nhập tập giá trị, mỗi giá trị một dòng`}
          />
      ...
      <div className='form-group'>
        ...
        <label className='control-label'>
          ...
          <input
            type='checkbox'
            className=''
            checked={information.filledByAccountableEmployeesOnly}
            onChange={handleChangeInfoFilledByAccountableEmployeesOnly}
          />
        </label>
      </div>
      ...
      <div className='pull-right' style={{ marginBottom: '10px' }}>
        {state.editInfo ? (
          <React.Fragment>
            <button className='btn btn-success' onClick={handleCancelEditInformation} style={{ marginLeft: '10px' }}>
              {translate('task_template.cancel_editing')}
            </button>
            <button
              className='btn btn-success'
              disabled={!isInfoFormValidated()}
              onClick={handleSaveEditedInformation}
        ...
        <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearInformation}>
      ...
      <table className='table table-bordered'>
        <thead>
          <tr>
            <th style={{ width: '50px' }} className='col-fixed'>
              {translate('task_template.code')}
            </th>
        ...
        {typeof taskInformations === 'undefined' || taskInformations.length === 0 ? (
          <tr>
            <td colSpan={5}>
          ...
          <ReactSortable
            ...
            tag='tbody'
            id='actions'
            list={taskInformations}
            setList={(newState) => {
              props.onDataChange(newState)
              setState({ ...state, taskInformations: newState })
            }}
          >
            {taskInformations.map((item, index) => (
              <tr key={`${state.keyPrefix}_${index}`}>
                ...
                <td>
                  <a href='#abc' className='edit' title={translate('general.edit')} onClick={() => handleEditInformation(item, index)}>
                    <i className='material-icons'></i>
                  </a>
                  <a href='#abc' className='delete' title={translate('general.delete')} onClick={() => handleDeleteInformation(index)}>
                    <i className='material-icons'></i>
                  </a>
                </td>
              </tr>
