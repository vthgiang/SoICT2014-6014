    ...
    <React.Fragment>
      <li>
        <a
          ...
          className='link-black text-sm edit-time'
          onClick={() => window.$(`#modal-edit-date-created-action${props.data._id}`).modal('show')}
        >
          {<DateTimeConverter dateTime={props.data.createdAt} />} <i className='fa fa-pencil'></i>
        </a>
      </li>
      <DialogModal
        size='20'
      ...
      >
        <div className={`form-group ${!errorOnDate ? '' : 'has-error'}`}>
          <DatePicker id={`change-date-created-action${id}`} onChange={handleChangeDateAction} value={dateCreatedAt} />
          <ErrorLabel content={errorOnDate} />
        </div>
        <div className={`form-group ${!errorOnTime ? '' : 'has-error'}`}>
          <TimePicker id={`change-time-created-action${id}`} value={timeCreatedAt} onChange={handleChangeTimeAction} />
