...
function OutgoingDataTab(props) {
  ...
  if (props.isOutgoingData && props.taskId !== state.taskId) {
    let isOutputInformation = {}
    let isOutputDocument = {}
    let isOutputTaskOutput = {}
      ...
      {task1 && (
        <>
          <div className='description-box outgoing-content'>
            ...
            {
              /** Danh sách thông tin */
              task1.taskInformations && task1.taskInformations.length !== 0 ? (
            ...
            {
              /** Danh sách tài liệu */
              task1.documents && task1.documents.length !== 0 ? (
                      ...
                      document.files.map((file, index) => (
                        <div key={index}>
                          {isImage(file.name) ? (
                            ...
                            <a style={{ cursor: 'pointer', marginTop: '2px' }} onClick={(e) => requestDownloadFile(e, file.url, file.name)}>
                              {' '}
                              {file.name}{' '}
                            </a>
                          )}
                ...
                <div>
                  <label>
                    <input
                      ...
                      name={item.tilte}
                      onClick={() => {
                        handleCheckBoxOutputTaskOutput(item)
                      }}
            ...
            <div style={{ marginTop: 20 }}>
              <button
                ...
                disabled={documents.length === 0 && informations.length === 0 && taskOutputs.length === 0}
              >
                {translate('task.task_process.save')}
              </button>
            </div>
          </div>
          {/* <TaskOutputsTab /> */}
