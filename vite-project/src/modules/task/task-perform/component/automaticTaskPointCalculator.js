...
String.prototype.replaceAll = function (
    ...
    while (intIndexOfMatch != -1) {
        // có thể bị lặp vô hạn vì có trường hợp thay x = x
...
function calcAutoPoint(data) {
    ...
    let actions = taskActions.filter(
        (item) =>
            item.rating !== -1 &&
    ...
    if (!task.formula) {
        if (task.taskTemplate === null || task.taskTemplate === undefined) {
            // Công việc không theo mẫu
        ...
        let formula;
        if (!task.taskTemplate && !task.process) {
            // Công việc không theo mẫu và ko theo quy trình
            // automaticPoint = a ? autoHasActionInfo : autoDependOnDay;
...
function calcProjectTaskPoint(data, getCalcPointsOnly = true) {
    ...
    const { timesheetLogs, estimateNormalCost, startDate, endDate, actorsWithSalary, responsibleEmployees, estimateAssetCost, accountableEmployees } =
        task;
    /***************** Yếu tố tiến độ **********************/
...
function calcTaskEVMPoint(data) {
    ...
    const diffFromStartToNow =
        getDurationWithoutSatSun(startDate, moment().format(), 'milliseconds') < 0
            ? 0
            : getDurationWithoutSatSun(startDate, moment().format(), 'milliseconds');
    ...
    const realDuration =
        task?.status === 'finished' && actualEndDate
            ? getDurationWithoutSatSun(startDate, actualEndDate, 'milliseconds') /
...
function calcMemberStatisticEvalPoint(data) {
    ...
    // Estimate duration
    const estDuration =
        ((currentMemberWithSalary?.weight / 100) * estimateNormalTime) / (projectDetail?.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS);
    // Estimate cost
    ...
    const realDuration =
        task?.status === 'finished' && actualEndDate
            ? (getDurationWithoutSatSun(startDate, actualEndDate, 'milliseconds') /
                  (projectDetail?.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)) *
              (currentMemberWithSalary?.weight / 100)
            : undefined;
    // Real cost
