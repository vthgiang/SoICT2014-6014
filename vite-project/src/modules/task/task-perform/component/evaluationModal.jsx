...
function EvaluationModal(props) {
  ...
  return (
    <DialogModal modalID={`task-evaluation-modal-${id}-`} title={title} hasSaveButton={false} size={100}>
        ...
        <div className='box box-solid' style={{ border: '1px solid #ecf0f6', borderBottom: 'none' }}>
          <div className='box-header with-border'>
            <h3 className='box-title' style={{ fontWeight: 800 }}>
              {translate('task.task_management.eval_list')}
            </h3>
          </div>
          <div className='box-body no-padding'>
            <ul className='nav nav-pills nav-stacked'>
              ...
              {task?.evaluations &&
                ...
                task?.evaluations.map((item, index) => (
                ...
                !disableAddItem ? (
                  <li className={content === 'new' ? 'active' : undefined}>
                    <a style={{ cursor: 'pointer' }} onClick={() => handleAddEval()}>
                      {translate('task.task_management.add_eval_of_this_month')}&nbsp;&nbsp;&nbsp;&nbsp;
                      <i style={{ color: 'green' }} className='fa fa-plus-square'></i>
                      &nbsp;
                    </a>
                  </li>
                ) : (
      ...
      <div className='col-xs-12 col-sm-8 qlcv'>
        {content !== undefined && role === 'responsible' && hasAccountable === true && (
          <EvaluateByResponsibleEmployee
            ...
            date={
              evaluation
                ? formatDate(evaluation.evaluatingMonth)
                : checkEvaluationOfMonth
                  ? dateParam[content]
                  : dateParam[content]
                    ? dateParam[content]
                    : TODAY
