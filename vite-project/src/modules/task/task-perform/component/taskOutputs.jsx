...
const checkRoleAccountable = (userId, accountable) => {
  ...
  check = accountableEmployee || null
...
function TaskOutputsTab(props) {
  ...
  const onCommentFilesChange = (files, taskOutputId) => {
    const { newCommentOfTaskOutput } = state
  ...
  const submitComment = async (taskOutputId, taskId) => {
    ...
    if (taskOutputId) {
      ...
      newCommentOfTaskOutput[`${taskOutputId}`]?.files &&
        newCommentOfTaskOutput[`${taskOutputId}`].files.forEach((x) => {
          data.append('files', x)
        })
  ...
  const handleApprove = (status, taskOutputId) => {
    if (status === 'waiting_for_approval') {
      Swal.fire({
        ...
        if (res.value) {
          props.approveTaskOutputs(performtasks.task._id, taskOutputId, {
    ...
    <div>
      ...
      <div>
        ...
        {performtasks.task.taskOutputs.map((taskOutput, index) => {
          return (
            <ShowMoreShowLess id={`historyLog${taskOutput._id}`} styleShowMoreLess={{ display: 'inline-block', marginBotton: 15 }}>
              {/* phê duyệt */}
              <div key={taskOutput._id} className={`item-box ${index > 3 ? 'hide-component' : 'block'}`}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex' }}>
                    {taskOutput.status === 'approved' && (
                      <i
                    ...
                    <h4
                      ...
                      style={{
                        fontSize: '16px',
                        marginLeft: taskOutput.status !== 'approved' ? '15px' : 0
                      }}
                    >
                      {taskOutput.title}
                    </h4>
                  ...
                  <div
                    onClick={() => {
                      if (showPanels.includes(taskOutput._id)) {
                        const newShowPanels = showPanels.filter((panels) => panels !== taskOutput._id)
                        setShowPanels(newShowPanels)
                      } else {
                        const newShowPanels = [...showPanels, taskOutput._id]
                        setShowPanels(newShowPanels)
                      }
                    }}
                  >
                    <i className={`fa ${showPanels.includes(taskOutput._id) ? 'fa-angle-up' : 'fa-angle-down'}`} />
                  </div>
                </div>
                <div
                  style={{
                    display: `${showPanels.includes(taskOutput._id) ? '' : 'none'}`
                  }}
                >
                  <div className='description'>
                    <div>
                      <strong>Kiểu dữ liệu:</strong> {formatTypeInfo(taskOutput.type)}
                    </div>
                    <div>
                    ...
                    >
                      <div>
                        <span style={{ fontWeight: 600 }}>
                          {taskOutput.submissionResults?.description
                            ? `Kết quả giao nộp lần ${taskOutput.status === 'approved' ? taskOutput.versions.length : taskOutput.versions.length + 1}`
                            : 'Chưa giao nộp kết quả'}
                        </span>
                        {role === 'responsible' && taskOutput.status === 'rejected' && (
                          <span className='text-sm' style={{ marginLeft: '3px' }}>
                            (Bị từ chối)
                          </span>
                        )}
                        {role === 'accountable' && getActionAccountable(idUser, taskOutput.accountableEmployees) == 'approve' && (
                          <span className='text-sm' style={{ marginLeft: '3px' }}>
                            (Đã phê duyệt)
                          </span>
                        )}
                        {role === 'accountable' && getActionAccountable(idUser, taskOutput.accountableEmployees) == 'reject' && (
                          <span className='text-sm' style={{ marginLeft: '3px' }}>
                            (Đã từ chối)
                          </span>
                        )}
                        {taskOutput.status === 'inprogess' && role === 'responsible' && (
                          <a
                            ...
                            onClick={() => {
                              handleApprove('waiting_for_approval', taskOutput._id)
                            }}
                          >
                            Yêu cầu phê duyệt
                          </a>
                      ...
                      <div style={{ display: 'flex' }}>
                        {role === 'accountable' &&
                          ...
                          (taskOutput.status === 'waiting_for_approval' ||
                            taskOutput.status === 'rejected' ||
                            taskOutput.status === 'approved') && (
                            <div style={{ display: 'flex' }}>
                              {getActionAccountable(idUser, taskOutput.accountableEmployees) !== 'approve' && (
                                <a
                                  style={{ cursor: 'pointer' }}
                                  onClick={() => {
                                    handleApprove('approve', taskOutput._id)
                                  }}
                                >
                                  <i className='fa fa-check' aria-hidden='true' /> Phê duyệt
                                </a>
                              )}
                              {getActionAccountable(idUser, taskOutput.accountableEmployees) !== 'reject' && (
                                <a
                                  style={{
                                    cursor: 'pointer',
                                    paddingLeft: '15px'
                                  ...
                                  onClick={() => {
                                    handleApprove('reject', taskOutput._id)
                                  }}
                                >
                                  <i className='fa fa-times' aria-hidden='true' /> Từ chối
                                </a>
                              )}
                            </div>
                          )}
                        {taskOutput.versions?.length > 0 && (
                          <a
                            style={{
                              cursor: 'pointer',
                              marginRight: '3px',
                          ...
                          >
                            <i className='fa fa-history' aria-hidden='true' /> Lịch sử giao nộp
                          </a>
                        )}
                        {role === 'responsible' &&
                          (taskOutput.status == 'inprogess' ||
                            taskOutput.status == 'waiting_for_approval' ||
                            taskOutput.status == 'rejected') && (
                            <>
                              <a
                                ...
                                onClick={() => handleEditAction(taskOutput)}
                              >
                              ...
                              <a
                                ...
                                onClick={() => {
                                  props.deleteSubmissionResults(performtasks.task._id, taskOutput._id)
                                }}
                              >
                                <i className='material-icons' id='delete-event'>
                                  
                                </i>
                              </a>
                            </>
                    ...
                    {role === 'responsible' && !taskOutput?.submissionResults?.description && (
                      <div>
                        <ContentMaker
                          idQuill={`add-action-${taskOutput._id}`}
                          imageDropAndPasteQuill={false}
                          inputCssClass='text-input-task-output'
                          controlCssClass='tool-task-output row'
                          ...
                          onSubmit={(e) => {
                            submitAction(taskOutput._id, performtasks.task._id, performtasks.task.taskActions.length, taskOutput.type)
                          }}
                        />
                      </div>
                    )}
                            ...
                            editAction !== taskOutput._id && (
                              <>
                                ...
                                >
                                  ...
                                  >
                                    <DateTimeConverter dateTime={taskOutput.submissionResults.createdAt} />
                                  </li>
                                  <li>
                                    <a
                                      style={{ cursor: 'pointer' }}
                                      ...
                                      onClick={() => {
                                        if (showAccountables.includes(taskOutput._id)) {
                                          const newShowAccountable = showAccountables.filter((item) => item !== taskOutput._id)
                                          setShowAccountables(newShowAccountable)
                                        } else {
                                          const newShowAccountable = [...showAccountables, taskOutput._id]
                                          setShowAccountables(newShowAccountable)
                                        }
                                      }}
                                    >
                                  ...
                                  <li>
                                    <a
                                      style={{ cursor: 'pointer' }}
                                      className='text-sm'
                                      onClick={() => handleShowFile(taskOutput.submissionResults._id)}
                                    >
                                      <i className='fa fa-paperclip' aria-hidden='true' /> Tập tin đính kèm (
                                      {taskOutput.submissionResults.files && taskOutput.submissionResults.files.length})
                                    </a>
                                  </li>
                                  <li>
                                    <a style={{ cursor: 'pointer' }} className='text-sm' onClick={() => handleShowComment(taskOutput._id)}>
                                      <i className='fa fa-comments-o margin-r-2' aria-hidden='true' /> Trao đổi (
                                      {taskOutput.comments && taskOutput.comments.length})
                                    </a>
                                  </li>
                                </ul>
                                  ...
                                  >
                                    <ul className='list-inline tool-level1'>
                                      {taskOutput.submissionResults.files.map((elem, index) => {
                                        const listImage = taskOutput.submissionResults.files
                                          ?.map((elem) => (isImage(elem.name) ? elem.url : -1))
                                          .filter((url) => url !== -1)
                                        return (
                                          <li key={index}>
                                            {isImage(elem.name) ? (
                                ...
                                {showAccountables.includes(taskOutput._id) &&
                              ...
                              <ContentMaker
                                ...
                                onSubmit={(e) => {
                                  handleSaveEditAction(
                                    ...
                                    taskOutput.submissionResults.description,
                                    task._id,
                                    taskOutput.submissionResults._id
                                  )
                                }}
                              />
                                  ...
                                  {editComment === child._id && (
                                    <div>
                                      <ContentMaker
                                        ...
                                        onSubmit={(e) => {
                                          handleSaveEditComment(e, task._id, taskOutput._id, child._id, child.description)
                                        }}
                                      />
