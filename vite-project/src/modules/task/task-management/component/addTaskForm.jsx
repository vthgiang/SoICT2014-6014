...
function AddTaskForm(props) {
  ...
  useEffect(() => {
    if (props.isProcess) {
      ...
      setState({
        ...
        newTask: {
          organizationalUnit:
            info && info.organizationalUnit
              ? typeof info.organizationalUnit === 'object'
                ? info.organizationalUnit._id
                : info.organizationalUnit
              : '', //props.department?.tree[0]?.id
          collaboratedWithOrganizationalUnits:
    ...
    if (props.task) {
      setState({
        ...
        newTask: {
          ...
          collaboratedWithOrganizationalUnits: props.task?.collaboratedWithOrganizationalUnits?.map((e) => {
            return { organizationalUnit: e.organizationalUnit._id }
          }),
  ...
  useEffect(() => {
    if (props.isProcess !== true) {
      ...
      if (newTask.organizationalUnit === '' && department.list.length !== 0 && department.isLoading === false) {
        ...
        let defaultUnit = department.list?.find(
          (item) =>
        ...
        if (!defaultUnit && department.list.length > 0) {
          // Khi không tìm được default unit, mặc định chọn là đơn vị đầu tiên
  ...
  const handleChangeCollaboratedWithOrganizationalUnits = (value) => {
    setState({
      ...
      newTask: {
        ...state.newTask,
        collaboratedWithOrganizationalUnits: value.map((item) => {
          return { organizationalUnit: item, isAssigned: false }
        })
      }
    })
  }
    ...
    if (value === '') {
      ...
      setState({
        ...
        newTask: {
          // update lại name,description và reset các selection phía sau
          ...state.newTask,
          collaboratedWithOrganizationalUnits: taskTemplate.collaboratedWithOrganizationalUnits.map((item) => {
            return { organizationalUnit: item._id, isAssigned: false }
          }),
  ...
  return (
    <React.Fragment>
      ...
      {newTask.organizationalUnit && (
        <ModalProposalPresonnel
          newTask={newTask}
          ...
          <fieldset className='scheduler-border'>
            ...
            <div className={'row'}>
              ...
              <div className='col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group'>
                <label className='control-label'>
                  {translate('task.task_management.unit_manage_task')}
            ...
            <div className='row form-group'>
              <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${newTask.errorOnStartDate === undefined ? '' : 'has-error'}`}>
                <label className='control-label'>
                  {translate('task.task_management.start_date')}
                  <span className='text-red'>*</span>
                </label>
                <DatePicker
              ...
              <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${newTask.errorOnEndDate === undefined ? '' : 'has-error'}`}>
                <label className='control-label'>
                  {translate('task.task_management.end_date')}
                  <span className='text-red'>*</span>
                </label>
                <DatePicker id={`datepicker2-${id}-${props.id}`} value={newTask.endDate} onChange={handleChangeTaskEndDate} />
                <TimePicker
        ...
        <div className={`${isProcess ? 'col-lg-12' : 'col-sm-6'}`}>
          ...
          <fieldset className='scheduler-border'>
            ...
            {listTaskTemplate && (
              <div className='form-group '>
                <label className='control-label'>
                  {translate('task.task_management.add_template')} ({listDepartment.find((e) => e._id === newTask.organizationalUnit)?.name}
                  )
                </label>
            ...
            {props.parentTask || props.task ? ( // modal tạo subtask và modal copytask
              ...
              <div className='form-group'>
                <label>
                  {translate('task.task_management.add_parent_task')}
                  <ToolTip type={'icon_tooltip'} dataTooltip={[translate('task.task_management.search_task_by_typing')]} />
                </label>
                <SelectBox
              ...
              <div className='form-group'>
                ...
                <SelectBox
                  ...
                  items={listDepartment
                    .filter((item) => newTask && item._id !== newTask.organizationalUnit)
                    .map((x) => {
            ...
            {!isProcess && (
              <div className='form-group'>
                ...
                <TreeSelect
                  ...
                  mode='radioSelect'
                  data={
                    props.projectIdFromDetailProject
                      ? project?.data?.list?.filter((projectItem) => projectItem._id === props.projectIdFromDetailProject)
                  ...
                  action={
                    checkCurrentRoleIsManager && checkCurrentRoleIsManager.length > 0
                      ? () => {
            ...
            <div className='form-group'>
              <label>Tags</label>
              <InputTags id={`tagsinput-${id}`} onChange={handleTaskTags} value={newTask?.tags} />
            </div>
          </fieldset>
        </div>
        <div className={`${isProcess ? 'col-lg-12' : 'col-sm-6'}`}>
          <ExpectedResults onChange={handleChangeTaskOutputs} id={props.id} quillId={props.quillId} />
        </div>
      </div>
    </React.Fragment>
  )
