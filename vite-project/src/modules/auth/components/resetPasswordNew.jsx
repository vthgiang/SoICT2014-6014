...
function ResetPasswordNew(props) {
  ...
  let token
  if (props?.location?.search) token = qs.parse(props.location.search, { ignoreQueryPrefix: true }).token
  ...
  useEffect(() => {
    if (token) props.checkLinkValid({ token })
  }, [])
  ...
  const resetPassword = () => {
    ...
    if (new_password !== confirm) {
      toast.error(<ServerResponseAlert type='error' title='general.error' content={['auth.validator.confirm_password_invalid']} />, {
  ...
  const verifyCaptchar = (response) => {
    if (response) {
      ...
      toast.success(<ServerResponseAlert type='success' title='general.success' content={['Verify successful']} />, {
        containerId: 'toast-notification'
      })
    }
  }
  const handleForgotPassword = () => {
    ...
    if (!state.isVerified) {
      toast.error(<ServerResponseAlert type='error' title='general.error' content={['Please verify you are not a robot']} />, {
        containerId: 'toast-notification'
      })
      return false
    }
  ...
  const isFormValidated = () => {
    ...
    if (!ValidationHelper.validateEmpty(translate, portal).status || !ValidationHelper.validateEmail(translate, email).status) return false
    return true
  }
      ...
      <div className='dx-login-form'>
        <div className='dx-login-line' style={{ fontSize: '14px', marginBottom: '10px' }}>
          ...
          <input
            ...
            placeholder={'Mã xác thực được gửi trong email'}
          />
        </div>
        <div className='dx-login-line' style={{ fontSize: '14px', marginBottom: '10px' }}>
      ...
      <div className='login-box-body'>
        <div className='alert alert-success alert-dismissible'>
          <p>
            ...
            {translate(`auth.reset_password_success`)}
          </p>
        </div>
        <div className='row'>
          <div className='col-xs-12'>
            <a className='btn btn-primary pull-right' href='/login'>
      ...
      <div className='dx-login-form'>
        <div className={`dx-login-line ${!forgotPwd.errorOnPortal ? '' : 'has-error'}`} style={{ fontSize: '14px', marginBottom: '10px' }}>
          ...
          <input
            ...
            className='form-control'
            name='portal'
            value={forgotPwd.portal ? forgotPwd.portal : ''}
            onChange={handleChangePortal}
            placeholder='Portal công ty'
          />
        ...
        <div className={`dx-login-line ${!forgotPwd.errorOnEmail ? '' : 'has-error'}`} style={{ fontSize: '14px', marginBottom: '10px' }}>
          ...
          <input
            type='text'
            className='form-control'
            value={forgotPwd.email ? forgotPwd.email : ''}
        ...
        <div className='dx-login-line' style={{ fontSize: '14px', marginBottom: '10px' }}>
          ...
          <input
            ...
            placeholder={'Nhập mật khẩu cấp 2 nếu có'}
          />
        </div>
        ...
        <ReCAPTCHA sitekey='6Ld94SMbAAAAANJYglZD8tCajp126HJLOIUiYV8H' onChange={verifyCaptchar} />
        ...
        <div className='row'>
          <div className='col-xs-12'>
            ...
            <button className='btn btn-primary pull-right' onClick={handleForgotPassword} disabled={!isFormValidated()}>
              {translate(`general.accept`)}
            </button>
          </div>
        </div>
    ...
    return (
      <React.Fragment>
        <div className='modal-body'>
          <div className='alert alert-success alert-dismissible'>
            <p>
              Mã xác thực đã được gửi đến <strong>{forgotPwd.email}</strong>
            </p>
          </div>
        </div>
    ...
    return (
      <div className='dx-login-page'>
        <div className='dx-login-box'>
          ...
          {!url_reset_password_valid && forgot_password === false && renderErrorUrlInvalid()}
