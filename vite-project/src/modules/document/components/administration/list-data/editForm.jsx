...
function EditForm(props) {
  ...
  const isValidateForm = () => {
    return (
      validateName(documentName, false) &&
  ...
  function save() {
    ...
    const roleList = role.list.map((role) => {
      return { value: role._id, text: role.name }
    })
    const userList = user.list.map((user) => {
      return { value: user._id, text: `${user.name} - ${user.email}` }
    })
    ...
    if (
      !compareArray(
        documentUserCanView,
        props.documentUserCanView.map((x) => x._id)
      )
    ) {
  ...
  const addNewVersion = (id) => {
    const { documentVersionName, documentIssuingDate, documentEffectiveDate, documentExpiredDate, documentFile, documentFileScan } = state
  ...
  let tmpMap = {}
  let relationshipDocs = documents.administration.relationshipDocs.paginate.map((doc) => {
    tmpMap[doc._id] = true
        ...
        <form id='form-edit-document'>
          <div className='nav-tabs-custom'>
            <ul className='nav nav-tabs'>
              <li className='active'>
                <a href='#doc-edit-info' data-toggle='tab'>
                  {translate('document.infomation_docs')}
                </a>
              </li>
              <li>
                <a href='#doc-edit-sub-info' data-toggle='tab'>
                  {translate('document.relationship_role_store')}
                </a>
              </li>
            </ul>
                ...
                <div className='row'>
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    ...
                    <div className={`form-group ${!errorOfficialNumber ? '' : 'has-error'}`}>
                      <label>
                        {translate('document.doc_version.official_number')}
                        <span className='text-red'>*</span>
                      </label>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    ...
                    <div className='form-group'>
                      ...
                      <TreeSelect data={list} value={documentDomains} type='1' handleChange={handleDomains} mode='hierarchical' />
                    </div>
                    <div className='form-group'>
                      ...
                      <textarea
                        ...
                        value={documentDescription ? documentDescription : ''}
                      />
                    </div>
                  </div>
                </div>
                <div className='row'>
                  <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
                    <ButtonModal
                      ...
                      button_name={translate('general.add')}
                    ...
                    >
                      <React.Fragment>
                        ...
                        <div className='form-group'>
                          ...
                          <DatePicker id={`document-edit-version-issuing-date-${documentId}`} onChange={handleIssuingDate} />
                        </div>
                        <div className='form-group'>
                          <label>{translate('document.doc_version.effective_date')}</label>
                          <DatePicker id={`document-edit-version-effective-date-${documentId}`} onChange={handleEffectiveDate} />
                        </div>
                        <div className='form-group'>
                          <label>{translate('document.doc_version.expired_date')}</label>
                          <DatePicker id={`document-edit-version-expired-date-${documentId}`} onChange={handleExpiredDate} />
                        </div>
                      </React.Fragment>
                    </DialogModal>
                    <table className='table table-hover table-striped table-bordered' id='table-document-version'>
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: '80px', textAlign: 'center' }}>{translate('general.action')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {documentVersions !== undefined && documentVersions.length > 0 ? (
                            ...
                            return (
                              <tr key={i}>
                                ...
                                <td>
                                  ...
                                  <a
                                    className='text-red'
                                    title={translate('document.delete')}
                                    onClick={() => deleteDocumentVersion(documentId, version._id, version.versionName)}
                                  >
                          ...
                          <tr>
                            <td colSpan={7}>{translate('document.no_version')}</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    <div className='form-group'>
                      ...
                      <textarea
                        ...
                        style={{ height: 107 }}
                        className='form-control'
                        onChange={handleRelationshipDescription}
                        value={documentRelationshipDescription}
                      />
                    </div>
                    <div className='form-group'>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    <div className='form-group'>
                      ...
                      <SelectBox // id cố định nên chỉ render SelectBox khi items đã có dữ liệu
                        ...
                        className='form-control select2'
                        style={{ width: '100%' }}
                        items={department.list.map((organ) => {
                    ...
                    <div className='form-group'>
                      ...
                      <TreeSelect data={archives} value={documentArchives} handleChange={handleArchives} mode='hierarchical' />
                      {path && path.length ? path.map((y, index) => <div key={index}>{y}</div>) : null}
                    </div>
                  </div>
                </div>
              </div>
