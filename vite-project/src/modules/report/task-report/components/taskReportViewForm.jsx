  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        {modal && modal === 'view' && barAndLineDataChartConvert && (
          <LineBarChartViewForm
            id='viewLineBarChart'
        ...
        <div className='form-inline'>
          <button id='exportButton' className='btn btn-sm btn-success ' style={{ marginBottom: '10px' }}>
            <span className='fa fa-file-excel-o'></span> Export to Excel
          </button>
        </div>
          ...
          <div className='col-md-12'>
            <div className='box'>
              <div className='box-header'>
                <div className='box-tools pull-right'>
                  <button
                    ...
                    onClick={() => state.collapseChart}
                    data-toggle='collapse'
                    data-target='#showInfoTask'
                  >
                ...
                >
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: 'center' }}>Điểm hệ thống tự tính</th>
                      <th style={{ textAlign: 'center' }}>Ngày bắt đầu</th>
                      <th style={{ textAlign: 'center' }}>Ngày kết thúc</th>
                      <th style={{ textAlign: 'center' }}>Ngày đánh giá</th>
                      <th style={{ textAlign: 'center' }}>Mức độ ưu tiên</th>
                      <th style={{ textAlign: 'center' }}>Trạng thái công việc</th>
                      {headTable && headTable.map((x, key) => <th key={key}>{x}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {listTaskEvaluations &&
                      listTaskEvaluations.map((item, key) => {
                        ...
                        if (result) {
                          result = result[0]
                          point = result && result.automaticPoint ? result.automaticPoint : 0
                        }
                        ...
                        return (
                          <tr key={key}>
                            ...
                            <td className='text-center'>{getNameResponsibleEmployees.join(', ')}</td>
                            <td className='text-center'>{getNameAccountableEmployees.join(', ')}</td>
                            ...
                            <td className='text-center'>{item.priority === 1 ? 'Thấp' : item.priority === 2 ? 'Trung bình' : 'Cao'}</td>
              ...
              {reports.isLoading ? (
                ...
                listTaskEvaluations &&
                listTaskEvaluations.length === 0 && (
                  <div className='table-info-panel' style={{ width: '100%' }}>
