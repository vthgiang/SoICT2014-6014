...
function TaskReportEditForm(props) {
  ...
  const handleClickTransferRightList = () => {
    ...
    setState({
      ...
      editingReport: {
        ...editingReport,
        listDataChart: checkId ? listDataChart : listDataChart,
  ...
  const handleClickTransferLeftList = () => {
    ...
    setState({
      ...
      editingReport: {
        ...editingReport,
        dataForAxisXInChart: checkId ? dataForAxisXInChart : dataForAxisXInChart,
  ...
  const { editingReport, nameErrorEditForm, descriptionErrorEditForm, startDateErrorEditForm, readByEmployeeErrorCreateForm } = state
  ...
  if (user.roledepartments) {
    ...
    for (let x in listRole.managers) listRoles[x] = listRole.managers[x]
    for (let x in listRole.deputyManagers) listRoles = [...listRoles, listRole.deputyManagers[x]]
    for (let x in listRole.employees) listRoles = [...listRoles, listRole.employees[x]]
  }
      ...
      >
            ...
            <div className={`form-group `}>
              ...
              {units && (
                <select
                  ...
                  onChange={handleReportOrganizationalUnitChangeEditForm}
                >
                  {units.map((x) => {
                    return (
        ...
        <div className='row'>
          <div className='col-md-6'>
            ...
            {/* Tên báo cáo */}
            {listTaskReportById && (
              <div className={`form-group ${!nameErrorEditForm ? '' : 'has-error'}`}>
                <label>
          ...
          <div className='col-md-6'>
            {/* Mô tả báo cáo */}
            {listTaskReportById && (
              <div className={`form-group ${!descriptionErrorEditForm ? '' : 'has-error'}`}>
                ...
                <textarea
                  ...
                  onChange={handleDesTaskReportChangeEditForm}
                />
        ...
        <div className='row'>
          <div className={`col-md-6 ${!startDateErrorEditForm ? '' : 'has-error'}`}>
            {/* Thống kê từ ngày */}
            {
              <div className='form-group'>
                <label>
                  Thống kê từ ngày <span className='text-red'>*</span>
                </label>
                <DatePicker
                  id='start-date'
                ...
                <table className='table table-hover table-striped table-bordered' id='report_manager'>
                  ...
                  <tbody>
                    {editingReport && editingReport.taskInformations ? (
                      editingReport.taskInformations.map((item, index) => (
                        <tr key={index}>
                          <td>{item.code}</td>
                          <td>{item.name}</td>
                          ...
                          {editingReport && editingReport.taskInformations && (
                            <td>
                              <input
                                ...
                                type='text'
                                value={item.filter}
                                onChange={(e) => handleEditFilter(index, e)}
                              />
                            </td>
                          )}
                          <td>
                              ...
                              <div className='checkbox' style={{ paddingLeft: '20%' }}>
                                <label>
                                  <input
                                    ...
                                    onChange={(e) => handleEditShowInReport(index, e)}
                                  />
                                </label>
                              </div>
                            ) : (
                          ...
                          <td>
                            {item.type === 'number' ? (
                              <input
                                ...
                                type='text'
                                value={item.newName}
                                onChange={(e) => handleEditNewName(index, e)}
                              />
                            ) : (
                              ''
                            )}
                      ...
                      <div className='border'>
                        ...
                        <div className='box-body box-size'>
                          <div className='listItem-left'>
                            {editingReport &&
                              editingReport.listDataChart &&
                              editingReport.listDataChart.map((x, index) => (
                    ...
                    <div className='col-md-5 '>
                      <div className='border'>
                        ...
                        <div className='box-body box-size'>
                          <div className='listItem-left'>
                            {editingReport &&
                              editingReport.dataForAxisXInChart &&
                              editingReport.dataForAxisXInChart.map((x, index) => (
                                <div className='item' key={index}>
                                  <input
                                    className='checkbox-input'
                                    type='checkbox'
