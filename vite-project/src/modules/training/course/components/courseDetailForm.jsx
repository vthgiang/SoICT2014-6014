      ...
      >
        <form className='form-group' id={`form-view-course${_id}`}>
          <div className='form-group'>
            ...
            {lecturer && (
              <span>
                {translate('training.course.with_lecturer')} "{lecturer}"
              </span>
            )}
          </div>
          <div className='form-group'>
            <span>
              {translate('training.course.offered_by')} "{offeredBy}" - {translate('training.course.table.education_program')} "
              {educationProgram.name}"
            </span>
          </div>
          <div className='form-group'>
            <span>
              {translate('training.course.belong_type')} "{translate(`training.course.type.${type}`)}"{' '}
              {translate('training.course.with_cost')} "{formater.format(cost)} {unit}" {translate('training.course.commitment_time')} "
              {employeeCommitmentTime} {translate('training.course.month')}"{' '}
            </span>
          </div>
          <div className='form-group'>
            <span>
              {total} {translate('training.course.staff')} {translate('training.course.attend')}
            </span>
            <span className='text-success' style={{ fontWeight: 'bold' }}>
              &nbsp;- {passNumber} {translate('training.course.result.pass')}&nbsp;
            </span>
            <span className='text-danger' style={{ fontWeight: 'bold' }}>
              &nbsp;- {failedNumber} {translate('training.course.result.failed')}&nbsp;
            </span>
          </div>
          <div className='form-group'>
