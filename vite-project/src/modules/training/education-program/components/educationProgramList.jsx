...
class ListEducation extends Component {
  ...
  render() {
    ...
    let listEducations = [],
      listPosition = [{ value: '', text: translate('human_resource.not_unit'), disabled: true }]
    ...
    if (organizationalUnit !== null) {
      ...
      organizationalUnit.forEach((u) => {
        list.forEach((x) => {
          if (x._id === u) {
            let roleManagers = x.managers.map((y) => {
              return { _id: y._id, name: y.name }
            })
            let roleDeputyManagers = x.deputyManagers.map((y) => {
              return { _id: y._id, name: y.name }
            })
            let roleEmployees = x.employees.map((y) => {
              return { _id: y._id, name: y.name }
            })
    ...
    let pageTotal = education.totalList % limit === 0 ? parseInt(education.totalList / limit) : parseInt(education.totalList / limit + 1)
      ...
      <div className='box'>
        <div className='box-body qlcv'>
          ...
          <div className='form-inline'>
            ...
            <div className='form-group'>
              ...
              <input
                ...
                placeholder={translate('training.education_program.table.program_code')}
                autoComplete='off'
              />
            </div>
            ...
            <div className='form-group'>
              ...
              <input
                ...
                name='name'
                onChange={this.handleChange}
                placeholder={translate('training.education_program.table.program_name')}
                autoComplete='off'
              />
            </div>
          </div>
          ...
          <table id='education-table' className='table table-striped table-bordered table-hover'>
            <thead>
              <tr>
                <th title={translate('training.education_program.education_program_code')}>
                  {translate('training.education_program.table.program_code')}
                </th>
                <th title={translate('training.education_program.education_program_name')}>
              ...
              {listEducations &&
                ...
                listEducations.map((x, index) => (
                  <tr key={index}>
                    ...
                    <td>{x.name}</td>
                    <td>
                      {typeof x.applyForOrganizationalUnits === 'undefined' || x.applyForOrganizationalUnits.length === 0
                        ? ''
                        : x.applyForOrganizationalUnits.map((y, indexs) => {
                            return (
                              <p style={{ margin: 0 }} key={indexs}>
                                {' '}
                                - {y.name}{' '}
                              </p>
                            )
                    ...
                    <td>
                      {typeof x.applyForPositions === 'undefined' || x.applyForPositions.length === 0
                        ? ''
