...
function EmployeeKpiEvaluateModal(props) {
  ...
  const { taskId, content, contentName, perPage, points, tasks, taskImportanceDetail, listKpiId, detailKpiId } = state
  ...
  const convertDataToExportData = (dataTask, currentKpiName, employeeName) => {
    ...
    let tableName =
      'Danh sách công việc ứng với KPI ' + (currentKpiName ? currentKpiName : '') + ' của ' + (employeeName ? employeeName : '')
    if (dataTask) {
        ...
        <ul className='nav nav-tabs'>
          <li className='active'>
            <a href='#overview' data-toggle='tab'>
              {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.overview')}
            </a>
          </li>
          <li>
            <a href='#evaluate' data-toggle='tab' onClick={() => handleEvaluateTab()}>
              {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set.evaluation')}
            </a>
          </li>
        </ul>
          ...
          <div className={'tab-pane row row-equal-height'} id='evaluate'>
            <div className='col-xs-12 col-sm-4'>
              <div className='form-group'>
                <ToolTip type='text_tooltip' dataTooltip={[` ${translate('kpi.evaluation.employee_evaluation.cal_all_kpis_title')}`]}>
              ...
              <div className='box box-solid' style={{ border: '1px solid #ecf0f6', borderBottom: 'none' }}>
                <div className='box-header with-border'>
                  <h3 className='box-title' style={{ fontWeight: 800 }}>
                    {translate('kpi.evaluation.employee_evaluation.KPI_list')}
                  </h3>
                </div>
                      ...
                      list.map((item, index) => (
                        <li key={index} className={content === item._id ? 'active' : undefined}>
                          <a
                            ...
                            onClick={() => handleChangeContent(item._id, employeeKpiSet.creator._id, item.type, item.name)}
                          >
                            {item.name}
            ...
            <div className='col-xs-12 col-sm-8 qlcv'>
              <div className='form-inline pull-right'>
                {currentKpi && (
                  <ToolTip
                    type='text_tooltip'
                    dataTooltip={[` ${translate('kpi.evaluation.employee_evaluation.refresh')} ${currentKpi ? currentKpi.name : ''}`]}
                  >
              ...
              {list &&
                list.map((item) => {
                  if (item._id === content)
                    return (
                      <div id={detailKpiId} key={item._id}>
                        ...
                        <div style={{ lineHeight: 2 }}>
                          ...
                          <div>
                            <label>
                              {translate('task.task_management.eval_of')}
                              {` (${translate('kpi.evaluation.dashboard.auto_point')} - ${translate('kpi.evaluation.dashboard.employee_point')} - ${translate('kpi.evaluation.dashboard.approve_point')})`}
                              :
                            </label>
                            <span>
                              {' '}
                              {item.automaticPoint ? item.automaticPoint : translate('kpi.evaluation.employee_evaluation.not_avaiable')}
                            </span>
                            <span>
                              {' '}
                              - {item.employeePoint ? item.employeePoint : translate('kpi.evaluation.employee_evaluation.not_avaiable')}
                            </span>
                            <span>
                              {' '}
                              - {item.approvedPoint ? item.approvedPoint : translate('kpi.evaluation.employee_evaluation.not_avaiable')}
                            </span>
                          </div>
                          <div>
                            <label>{translate('kpi.evaluation.employee_evaluation.weekly_point')}:</label>
                            <a style={{ cursor: 'pointer' }} onClick={() => showWeeklyPoint(translate, item?.weeklyEvaluations)}>
                              {' '}
                        ...
                        <div>
                          <table id={tableId} className='table table-hover table-bordered  table-striped mb-0'>
                            <thead>
                              <tr>
                                <th title='STT' style={{ width: '50px' }} className='col-fixed'>
                                  Stt
                                </th>
                            ...
                            <tbody>
                              {kpimembers.tasks &&
                                Array.isArray(kpimembers.tasks) &&
                                kpimembers.tasks?.length > 0 &&
                                kpimembers.tasks.map((itemTask, index) => (
                                  <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>
                                      <a style={{ cursor: 'pointer' }} onClick={() => handleClickTaskName(itemTask.taskId)}>
                                        {itemTask.name}
                                      </a>
                                      <a target='_blank' href={`/task?taskId=${itemTask.taskId}`}>
                                        <i className='fa fa-arrow-circle-right'></i>
                                      </a>
                                    </td>
                                    <td>
                                      {formatDate(itemTask.startDateTask)}
                                      <br /> <i className='fa fa-angle-double-down'></i>
                                      <br /> {formatDate(itemTask.endDateTask)}
                                    </td>
                                    <td>
                                      {itemTask.startDate ? formatDate(itemTask.startDate) : ''}
                                      <br /> <i className='fa fa-angle-double-down'></i>
                                      <br /> {itemTask.endDate ? formatDate(itemTask.endDate) : ''}
                                    </td>
                                    <td>{formatTaskStatus(translate, itemTask.status)}</td>
                                    <td>{itemTask.results.contribution ? itemTask.results.contribution : 0}%</td>
                                    <td
                                      title={`${translate('kpi.evaluation.dashboard.auto_point')} - ${translate('kpi.evaluation.dashboard.employee_point')} - ${translate('kpi.evaluation.dashboard.approve_point')}`}
                                    >
                                      {itemTask.results.automaticPoint +
                                        '-' +
                                        itemTask.results.employeePoint +
                                        '-' +
                                        itemTask.results.approvedPoint}
                                    </td>
                                    <td>
                                      {points && tasks && (
                                        <React.Fragment>
                                          ...
                                          <div>
                                            {translate('kpi.evaluation.employee_evaluation.old_value')}:{' '}
                                            {itemTask.results.taskImportanceLevel}
                                          </div>
                                          <div>
                                            <a href='#modal-taskimportance-auto' onClick={() => showDetailTaskImportanceCal(itemTask)}>
                                              {translate('kpi.evaluation.employee_evaluation.auto_value')}:{' '}
                                              {itemTask.taskImportanceLevelCal}
                                            </a>
                                          </div>
                                        </React.Fragment>
                                      )}
                        ...
                        {taskImportanceDetail && <TaskDialog task={taskImportanceDetail} />}
                        {taskId && <ModalPerform id={taskId} />}
                      </div>
                    )
                  return true
                })}
