      ...
      <div className='row'>
        <div className='col-md-6'>
          ...
          <div className={`form-group ${state.validate.name === undefined ? '' : 'has-error'}`}>
            <label className='control-label'>
              Tên mục tiêu <span style={{ color: 'red' }}>*</span>
            </label>
          ...
          <div className={`form-group ${state.validate.weight === undefined ? '' : 'has-error'}`}>
            ...
            <input
              ...
              value={kpi.weight}
              onChange={handleChangeKpiWeight}
            />
          ...
          <div className={`form-group ${state.validate.criteria === undefined ? '' : 'has-error'}`}>
            <label className='control-label'>
              Tiêu chí đánh giá <span style={{ color: 'red' }}>*</span>
            </label>
            <input
              type='text'
              className='form-control'
              ...
              value={kpi.criteria}
              onChange={handleChangeKpiCriteria}
            />
        ...
        <div className='col-md-6'>
          ...
          <div className='pull-right' style={{ marginBottom: '10px' }}>
            {state.editKpi ? (
              <React.Fragment>
                <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleCancelEditKpi}>
                  Hủy thay đổi
                </button>
                <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleSaveEditedKpi}>
                  Lưu
                </button>
              </React.Fragment>
            ) : (
              <button disabled={!isValidated()} className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleAddKpi}>
                Thêm
              </button>
            )}
            <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearKpi}>
              Xóa trắng
            </button>
          </div>
        </div>
      ...
      <table className='table table-hover table-striped table-bordered'>
        <thead>
          <tr>
            ...
            <th style={{ width: '60px' }} title='Hành động'>
              Hành động
            </th>
          </tr>
        </thead>

        {typeof kpis === 'undefined' || kpis.length === 0 ? (
          <tr>
            <td colSpan={5}>
