...
function DetailOfEmployeeKpiDialogModal(props) {
  const [state, setState] = useState({})

  const { translate } = props
  const { listEmployeeKpi } = props
    ...
    <React.Fragment>
      ...
      >
        <table className='table table-bordered table-striped table-hover'>
          <thead>
            <tr>
              <th title={translate('kpi.organizational_unit.create_organizational_unit_kpi_set.no_')} style={{ width: '40px' }}>
                {translate('kpi.organizational_unit.create_organizational_unit_kpi_set.no_')}
              </th>
              <th title={translate('kpi.organizational_unit.create_organizational_unit_kpi_set.target_name')}>
                {translate('kpi.organizational_unit.create_organizational_unit_kpi_set.target_name')}
              </th>
              ...
              <th title={translate('kpi.organizational_unit.create_organizational_unit_kpi_set.evaluation_criteria')}>
              ...
              <th title={translate('kpi.organizational_unit.create_organizational_unit_kpi_set.weight')} style={{ textAlign: 'left' }}>
            ...
            {listEmployeeKpi && listEmployeeKpi.length !== 0 ? (
              listEmployeeKpi.map((item, index) => (
                <tr key={item._id}>
                  ...
                  <td title={item.weight} style={{ textAlign: 'left' }}>
                    {item.weight}
                  </td>
                </tr>
              ))
