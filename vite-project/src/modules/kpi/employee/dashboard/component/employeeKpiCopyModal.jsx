...
function ModalCopyEmployeeKpiSet(props) {
  ...
  const handleSubmit = async (event, oldemployeeKpiSet) => {
    ...
    if (employeeKpiSet.organizationalUnit && employeeKpiSet.time) {
        ...
        <div className='modal-content'>
          <div className='modal-header'>
            ...
            <button
              ...
              className='close'
              data-dismiss='modal'
              onClick={() => handleCloseModal(employeeKpiSet._id)}
              aria-hidden='true'
            >
              ×
            </button>
            <h3 className='modal-title'>
              {translate('kpi.evaluation.organizational_unit.management.copy_modal.create')}+' '+ {formatDate(employeeKpiSet.time)}
            </h3>
          </div>
          <div className='modal-body'>
            <div className='form-group'>
              ...
              <label className='col-sm-5'>
                {translate('kpi.evaluation.organizational_unit.management.copy_modal.organizational_unit')}:
              </label>
              <label className='col-sm-8' style={{ fontWeight: '400', marginLeft: '-14.5%' }}>
                {employeeKpiSet && employeeKpiSet.organizationalUnit.name}
              </label>
            </div>
            <div className='form-group'>
            ...
            <div className='form-group'>
              ...
              <ul>
                {typeof employeeKpiSet !== 'undefined' &&
                  employeeKpiSet.kpis.length !== 0 &&
          ...
          <div className='modal-footer'>
            <button className='btn btn-success' onClick={(event) => handleSubmit(event, employeeKpiSet)}>
              {translate('kpi.evaluation.organizational_unit.management.copy_modal.setting')}
            </button>
            <button type='cancel' className='btn btn-primary' onClick={() => handleCloseModal(employeeKpiSet._id)}>
              {translate('kpi.evaluation.organizational_unit.management.copy_modal.cancel')}
            </button>
          </div>
        </div>
      </div>
