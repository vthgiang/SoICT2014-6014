...
function ModalEditEmployeeKpi(props) {
  ...
  if (!currentOrganizationalUnitKPI) {
    ...
    items = currentOrganizationalUnitKPI.kpis.map((x) => {
      //type !==0 thì đc. cái này để loại những mục tiêu mặc định?
  ...
  return (
    <React.Fragment>
      <DialogModal
        ...
        msg_failure={translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.failure')}
        func={handleEditTargetEmployeeKpi}
        disableSubmit={!isFormValidated()}
      >
        <form
          id='formEditTargetEmployeeKpi'
          onSubmit={() => handleEditTargetEmployeeKpi(translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.success'))}
        >
          {/**Tên của mục tiêu */}
          <div className={`form-group ${errorOnName === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.name')}
          ...
          {/**Mục tiêu cha */}
          {createKpiUnit.currentKPI !== null && items.length !== 0 && (
            <div className='form-group'>
              <label>
                {translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.parents')}
                <span className='text-red'>*</span>
              </label>
              <SelectBox
          ...
          <div className={`form-group ${errorOnCriteria === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.evaluation_criteria')}
              <span className='text-red'>*</span>
            </label>
            <QuillEditor
          ...
          <div className={`form-group ${errorOnWeight === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.edit_employee_kpi_modal.weight')}
              <span className='text-red'>*</span>
            </label>
