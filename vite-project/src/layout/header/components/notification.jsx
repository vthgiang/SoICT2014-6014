...
function Notification(props) {
  ...
  let notifyTaskUnRead = [],
    notifyAssetUnRead = [],
    notifyKPIUnRead = [],
    notifyProductionUnRead = [],
    notifyDefault = [],
    notifyProcessUnRead = []
    ...
    <React.Fragment>
      ...
      <li className='dropdown mega-dropdown notifications-menu'>
        <a
          ...
          onClick={() => checkTabPaneScroll(idTabPaneActive)}
        >
        ...
        <ul className='dropdown-menu notify-dropdown'>
          <div className='notification-header'>
            <p style={{ margin: 0 }}>
              <strong className='text-red'>{notify?.length ? notify.filter((notification) => !notification.readed).length : 0}</strong>{' '}
              {translate('notification.news')}
            </p>
            <i
              className='fa fa-cog config-notification-icon'
              data-toggle='collapse'
              data-target={`#setting-notification`}
              aria-expanded='false'
              aria-hidden='true'
            ></i>
          </div>
          <div className='collapse popup-noti-setting' data-toggle='collapse' id={`setting-notification`}>
            <p className='close-noti-setting' data-toggle='collapse' data-target={`#setting-notification`}>
            ...
            <div className='notification-on-off'>
              ...
              <div className='example'>
                <button
                  type='button'
                  className={`btn btn-sm btn-toggle ${sound ? 'active' : ''}`}
                  data-toggle='button'
                  aria-pressed='true'
                  autoComplete='off'
                  onClick={handleOnOffSound}
                >
          ...
          <div className='nav-tabs-custom'>
            <ul className='notify-tabs nav nav-tabs'>
              <li className='active'>
                <a
                  ...
                  href='#allNotificationDefault'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationDefault')}
                >{`Chung (${notifyDefault.length})`}</a>
              </li>
              <li>
                <a
                  className='notify-action'
                  href='#allNotificationOfTask'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationOfTask')}
                >{`Công việc (${notifyTaskUnRead.length})`}</a>
              </li>
              <li>
                <a
                  className='notify-action'
                  href='#allNotificationOfAsset'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationOfAsset')}
                >{`Tài sản (${notifyAssetUnRead.length})`}</a>
              </li>
              <li>
                <a
                  className='notify-action'
                  href='#allNotificationOfKPI'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationOfKPI')}
                >{`KPI (${notifyKPIUnRead.length})`}</a>
              </li>
              <li>
                <a
                  className='notify-action'
                  href='#allNotificationOfProduction'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationOfProduction')}
                >{`Sản phẩm (${notifyProductionUnRead.length})`}</a>
              </li>
              <li>
                <a
                  className='notify-action'
                  href='#allNotificationOfProcess'
                  data-toggle='tab'
                  onClick={() => checkTabPaneScroll('allNotificationOfProcess')}
                >{`Quy trình (${notifyProcessUnRead.length})`}</a>
              </li>
            </ul>
            ...
            <div className='tab-content'>
              <div className='tab-pane active notifi-tab-pane' id='allNotificationDefault'>
                {notifyDefault.length > 0 ? (
                  notifyDefault.map((notification, index) => {
                    return (
                      <div className='notify-wrapper' data-toggle='modal' key={index} onClick={() => showInfoNotification(notification)}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          {notification.level === 'info' ? (
                            ...
                            <i
                              ...
                              style={{
                                color: `${checkPriority(notification.associatedDataObject && notification.associatedDataObject.value)}`,
                                fontSize: '15px'
                              }}
                            />
                          ...
                          <span className='notify-title'>
                            {notification.associatedDataObject && notification.associatedDataObject.description
                              ? parse(notification.associatedDataObject.description)
                              : notification.title}
                            <DateTimeConverter
                              dateTime={notification.createdAt}
                              style={{ display: 'block', fontSize: '12px', color: '#d47979' }}
                            />
                          </span>
                        </div>
                      </div>
              ...
              <div className='tab-pane notifi-tab-pane' id='allNotificationOfTask'>
                {notifyTaskUnRead.length > 0 ? (
                  notifyTaskUnRead.map((notification, index) => {
                    return (
                      <div className='notify-wrapper' key={index} onClick={() => showInfoNotification(notification)}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          {notification.level === 'info' ? (
                            <i className='fa fa-info-circle text-blue' />
                          ) : notification.level === 'general' ? (
              ...
              <div className='tab-pane notifi-tab-pane' id='allNotificationOfProduction'>
                {notifyProductionUnRead.length > 0 ? (
                  notifyProductionUnRead.map((notification, index) => {
                    return (
                      <div className='notify-wrapper' key={index} onClick={() => showInfoNotification(notification)}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          {notification.level === 'info' ? (
              ...
              <div className='tab-pane notifi-tab-pane' id='allNotificationOfProcess'>
                {notifyProcessUnRead.length > 0 ? (
                  notifyProcessUnRead.map((notification, index) => {
                    return (
                      <div className='notify-wrapper' key={index} onClick={() => showInfoNotification(notification)}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          {notification.level === 'info' ? (
          ...
          <SlimScroll outerComponentId={idTabPaneActive} maxHeight={300} verticalScroll={true} activate={true} />
          <li className='footer'>
            <Link to='/notifications'>{translate('notification.see_all')}</Link>
          </li>
        </ul>
      </li>
    </React.Fragment>
