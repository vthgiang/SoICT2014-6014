  ...
  convertStringToObject = (data, configData) => {
    ...
    try {
      ...
      if (data[data.length - 1] === ',') {
        // xoá dấu "," nếu tồn tại ở cuối chuỗi để chuyển đổi dc về dạng string
    ...
    return (
      <React.Fragment>
        <button
          ...
          data-target={`#confic_import_file-${id}`}
          className='pull-right'
          style={{ border: 'none', background: 'none', paddingLeft: '5px' }}
        >
        ...
        <div id={`confic_import_file-${id}`} className='box box-solid box-default collapse col-sm-12 col-xs-12' style={{ padding: 0 }}>
          <div className='box-header with-border'>
            ...
            <div className='box-tools pull-right'>
              <button type='button' className='btn btn-box-tool' data-toggle='collapse' data-target={`#confic_import_file-${id}`}>
          ...
          <div className='box-body row'>
            ...
            <div className='form-group col-sm-12 col-xs-12'>
              <label>{translate('common_component.import_excel.user_config')}</label>
              <br />
              <span>{translate('common_component.import_excel.config')}</span>
              <span className='text-success' style={{ fontWeight: 'bold' }}>
                &nbsp;{configData.sheets.value.length > 1 ? configData.sheets.value.join(', ') : configData.sheets.value}
              </span>
              ...
              {scrollTable && (
                <SlimScroll
                  outerComponentId={`croll-table-${id}`}
                  innerComponentId={`importConfig-${id}`}
