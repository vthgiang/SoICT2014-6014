...
export const showWeeklyPoint = (translate, weeklyEvaluate) => {
  ...
  if (weeklyEvaluate?.length > 0) {
    weeklyEvaluate.map((item) => {
      weeklyPointHtml =
        weeklyPointHtml +
        `<li>${formatTitleWeeklyEvaluate(translate, item?.title)}: <strong>${item?.automaticPoint}</strong> - <strong>${item?.employeePoint}</strong> - <strong>${item?.approvedPoint}</strong></li>`
    })
  } else {
