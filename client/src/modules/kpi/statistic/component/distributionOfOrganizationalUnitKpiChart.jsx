...
function DistributionOfOrganizationalUnitKpiChart(props) {
  ...
  const setOrganizationalUnitImportance = () => {
    let currentKpi,
      ...
      objectOrganizationalUnitImportance = {}
  ...
  const setDataMultiChart = (organizationalUnitKPI, dataTreeUnitKpi, maxDeg) => {
              ...
              if (!objectChidrenKpiWeight[kpi?.id]) {
                ...
                if (objectChidrenKpiWeight[kpi?.id].length !== 0) {
                  ...
                  objectChidrenKpiWeight[kpi?.id].map((childrenKpi) => {
                    let importance = []
                    importance = kpi?.listEmployeeKpi?.[0]?.organizationalUnitImportances?.filter(
                      (item) => item?.organizationalUnit === childrenKpi?.organizationalUnitId
                    )
    ...
    return {
      xs: xs,
      dataChart: [dataChartDefault, dataChartAnalysisChidrenUnit, dataChartAnalysisTreeUnit]
    }
  }
  ...
  const multiLineChart = (organizationalUnitKPI, dataTreeUnitKpi, maxDeg) => {
    ...
    let chart = c3.generate({
          ...
          tick: {
            format: function (index) {
              let result = customAxisC3js(
                'chartOrganizationalUnitKpiRef',
                xs.filter((item, i) => i > 0),
                index
              )
              return result
            }
      ...
      tooltip: {
        format: {
          title: function (d) {
            if (xs && xs.length > 1) return xs[d + 1]
          }
        }
      }
    })
