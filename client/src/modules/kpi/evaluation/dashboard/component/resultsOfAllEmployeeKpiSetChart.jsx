...
function ResultsOfAllEmployeeKpiSetChart(props) {
  ...
  useEffect(() => {
    ...
    const employeeKpiSetsInOrganizationalUnitByMonthPaginate =
      employeeKpiSetsInOrganizationalUnitByMonth && employeeKpiSetsInOrganizationalUnitByMonth.slice(startIndex, endIndex + 1)
    ...
    for (let i = 0; i <= period; i++) {
      dateAxisX = [...dateAxisX, dayjs(startMonth).add(i, 'month').format('MM-YYYY')]
    }
    ...
    if (employeeKpiSetsInOrganizationalUnitByMonthPaginate) {
      for (let i = 0; i < employeeKpiSetsInOrganizationalUnitByMonthPaginate.length; i++) {
        let data = filterAndSetDataPoint(
          dateAxisX,
          employeeKpiSetsInOrganizationalUnitByMonthPaginate[i]._id,
          employeeKpiSetsInOrganizationalUnitByMonthPaginate[i].employeeKpi
        )
        point = [...point, data]
      }
    }
    ...
    JSON.stringify(props?.createEmployeeKpiSet?.employeeKpiSetsInOrganizationalUnitByMonth),
    state.kindOfPoint,
    state.infosearch.startMonth,
    state.infosearch.endMonth,
    state.page
  ])
    ...
    <div className='box box-solid'>
      <div className='box-header with-border'>
        <div className='box-title'>
          ...
          {formatString(state.startMonth)}
          <i className='fa fa-fw fa-caret-right'></i>
          {formatString(state.endMonth)}
        </div>
        {state.exportData && props?.dashboardPage === 'KPI' && (
          <ExportExcel
            type='link'
            id='export-all-employee-kpi-evaluate-result-dashboard'
          ...
          <>
            <section id={'resultsOfAllEmployeeKpiSet'} className='c3-chart-container'>
              ...
              <CustomLegendC3js
                ...
                title={
                  employeeKpiSetsInOrganizationalUnitByMonth &&
                  `${translate('general.list_employee')} (${employeeKpiSetsInOrganizationalUnitByMonth.length})`
                }
                dataChartLegend={state.dataChart && state.dataChart.map((item) => item[0])}
              />
            </section>
            <PaginateBar display={display} total={totalList} pageTotal={pageTotal} currentPage={currentPage} func={handlePagination} />
          </>
        ) : (
          employeeKpiSetsInOrganizationalUnitByMonth && <section>{translate('kpi.organizational_unit.dashboard.no_data')}</section>
        )}
      </div>
    </div>
  )
