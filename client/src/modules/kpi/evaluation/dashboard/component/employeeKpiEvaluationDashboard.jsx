...
function EmployeeKpiEvaluationDashboard(props) {
  ...
  let employeeKpiSets,
    lastMonthEmployeeKpiSets,
    currentMonthEmployeeKpiSets,
    settingUpKpi,
    awaitingApprovalKpi,
    activatedKpi,
    totalKpi,
  ...
  if (kpimembers.kpimembers) {
    ...
    let employeePoint = kpiApproved
      .map((item) => {
        return { label: formatDate(item.date), y: item.employeePoint }
      })
      .reverse()

    let approvedPoint = kpiApproved
      .map((item) => {
                ...
                <div className='info-box-content'>
                  ...
                  >
                    <span
                      className='info-box-number'
                      style={{ fontSize: '20px' }}
                    >{`${awaitingApprovalKpi ? awaitingApprovalKpi : 0}`}</span>
                  </Link>
                </div>
              </div>
            </div>
