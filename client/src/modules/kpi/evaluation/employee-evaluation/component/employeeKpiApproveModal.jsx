...
import { Comment, DataTableSetting, DatePicker, DialogModal, ErrorLabel, SlimScroll } from '../../../../../common-components/index'
...
function EmployeeKpiApproveModal(props) {
  ...
  const [state, setState] = useState({
    ...
    kpiSameTime: undefined
  })
  const { errorOnDate, date, compare, edit, defaultDate, employeeKpiSetId, type, tableBodyKpiId, tableBodyKpiCompareId, kpiSameTime } =
    state
  let kpimember, kpimembercmp, month, totalWeight, totalWeight2
  ...
  useEffect(() => {
    ...
    const { kpimembers: kpis, currentKPI } = kpimembers

    if (kpis) {
      const kpiSameTime = kpis.find(
        (kpi) =>
          new Date(kpi.date).getFullYear() === new Date(currentKPI?.date).getFullYear() &&
          new Date(kpi.date).getMonth() === new Date(currentKPI?.date).getMonth() &&
          kpi.type !== currentKPI?.type &&
          kpi.creator.id === currentKPI.creator.id
      )
      setState({
        ...state,
        kpiSameTime
  ...
  const handleEditStatusTarget = (event, kpi, status, listTarget) => {
    ...
    if (listTarget) {
      let totalWeight
      if (listTarget) {
        totalWeight = listTarget
          .filter((item) => item.status === 1 || item._id === kpi?._id)
  ...
  totalWeight =
    kpimember &&
    kpimember.kpis &&
    kpimember.kpis.length !== 0 &&
    kpimember.kpis.map((item) => parseInt(item.weight)).reduce((sum, number) => sum + number, 0)

  totalWeight2 =
    kpiSameTime && kpiSameTime.kpis && kpiSameTime.kpis.map((item) => parseInt(item.weight)).reduce((sum, number) => sum + number, 0)

  console.log(285, kpimember)
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-approve-KPI-member`}
        title={
          employeeKpiSet?.creator?.name &&
          `KPI ${employeeKpiSet.creator.name}, ${translate('kpi.evaluation.employee_evaluation.month')} ${formatMonth(employeeKpiSet?.date)}`
        }
        hasSaveButton={false}
        size={100}
      >
        <ModalEditEmployeeKpi id={edit?._id} employeeKpi={edit} />
        <EmployeeKpiSetLogsModal employeeKpiSetId={employeeKpiSetId} type={type} />
        <div className='qlcv'>
          <div className='form-inline pull-right'>
            {compare ? (
              <button className=' btn btn-primary' onClick={() => handleCompare()}>
                {translate('kpi.evaluation.employee_evaluation.end_compare')}
              </button>
            ) : (
              <button className=' btn btn-primary' onClick={() => kpimember && kpimember.creator && handleCompare(kpimember.creator._id)}>
                {translate('kpi.evaluation.employee_evaluation.compare')}
              </button>
            )}
            <button className=' btn btn-primary' onClick={() => kpimember && showEmployeeKPISetLogs(kpimember._id)}>
              {translate('kpi.evaluation.employee_evaluation.show_logs')}
            </button>
            {kpimember?.status !== 2 && kpiSameTime?.status !== 2 && (
              <button
                className=' btn btn-success'
                onClick={() => kpimember && handleApproveKPI(kpimember._id, kpimember.kpis)}
              >{`Phê duyệt KPI ${kpimember?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'}`}</button>
            )}
            {kpiSameTime && kpimember?.status !== 2 && kpiSameTime?.status !== 2 && (
              <button
                className=' btn btn-success'
                onClick={() => kpiSameTime && handleApproveKPI(kpiSameTime._id, kpiSameTime.kpis)}
              >{`Phê duyệt KPI ${kpiSameTime?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'}`}</button>
            )}
          </div>
          <br />
            ...
            <div>
              <div className='form-inline'>
                <div className={`form-group ${errorOnDate === undefined ? '' : 'has-error'}`}>
                  <label style={{ minWidth: '160px', marginLeft: '-40px' }}>
                    {translate('kpi.evaluation.employee_evaluation.choose_month_cmp')}
                  </label>
                  <DatePicker id='create_date' dateFormat='month-year' value={defaultDate} onChange={handleDateChange} />
                  <ErrorLabel content={errorOnDate} />
                </div>
                <div className='form-group'>
                  <button className='btn btn-success' onClick={() => searchKPIMemberByMonth(kpimember && kpimember.creator._id)}>
              ...
              <div id={tableBodyKpiCompareId}>
                <table className='table table-bordered table-striped table-hover'>
                  <thead>
                    <tr key='approve'>
                      <th title='STT' className='col-fixed' style={{ width: 50 }}>
                        STT
                      </th>
                  ...
                  <tbody style={{ height: '200px' }}>
                    {kpimembercmp ? (
                      ...
                      <tr>
                        <td colSpan={6}>{translate('kpi.evaluation.employee_evaluation.data_not_found')}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        {/* Danh sách mục tiêu kpi 1 */}
        {
          <div>
            {/* Label cảnh báo */}
            <label>{`KPI ${kpimember?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'} tháng ${formatMonth(employeeKpiSet?.date)} (${totalWeight ?? 0}/100)`}</label>
            {/* Danh sách KPI */}
            <DataTableSetting
            ...
            <div id={tableBodyKpiId}>
              <table id={tableId} className='table table-bordered table-hover'>
                <thead>
                  <tr>
                    <th className='col-fixed' style={{ width: 50 }}>
                  ...
                  {kpimember &&
                    kpimember.kpis.map((item, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          {item?.approvedPoint !== null && item?.approvedPoint >= 0
                            ? item.approvedPoint
                            : translate('kpi.evaluation.employee_evaluation.not_evaluated_yet')}
                        </td>
                        <td>
                          <a
                            className='edit'
                          ...
                          <a
                            ...
                            title={translate('kpi.evaluation.employee_evaluation.pass')}
                            onClick={(event) => kpimember && handleEditStatusTarget(event, item, 1, kpimember.kpis)}
                          >
                          ...
                          <a
                            style={{ cursor: 'pointer' }}
                            className='delete'
                            title={translate('kpi.evaluation.employee_evaluation.fail')}
                            onClick={(event) => handleEditStatusTarget(event, item, 0)}
                          >
        ...
        {/* Danh sách mục tiêu kpi 2*/}
        {kpiSameTime && (
          <div>
            <label>{`KPI ${kpiSameTime?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'} tháng ${formatMonth(employeeKpiSet?.date)} (${totalWeight2 ?? 0}/100)`}</label>
            {/* Danh sách KPI */}
            <DataTableSetting
              className='pull-right'
              tableId={'table-kpi-employee-auto'}
              columnArr={[
                'STT',
                'Tên mục tiêu',
                'Mục tiêu đơn vị',
            ...
            />
            <div id={tableBodyKpiId}>
              <table id={'table-kpi-employee-auto'} className='table table-bordered table-hover'>
                <thead>
                  <tr>
                    <th className='col-fixed' style={{ width: 50 }}>
                      {translate('kpi.evaluation.employee_evaluation.index')}
                ...
                <tbody>
                  {kpiSameTime.kpis.map((item, index) => (
                    <tr key={index}>
                      <td>{index + 1}</td>
                      <td>{item ? item.name : ''}</td>
                      <td>{item.parent ? item.parent.name : ''}</td>
