...
function EmployeeKpiApproveModal(props) {
    ...
    const { errorOnDate, date, compare, edit, defaultDate, employeeKpiSetId, type, tableBodyKpiId, tableBodyKpiCompareId, kpiSameTime } = state;
    ...
    useEffect(() => {
        ...
        if (kpis) {
            const kpiSameTime = kpis.find(kpi =>
    ...
    totalWeight2 = kpiSameTime && kpiSameTime.kpis
        && kpiSameTime.kpis.map(item => parseInt(item.weight)).reduce((sum, number) => sum + number, 0);
            ...
            >
                ...
                <div className="qlcv">
                    <div className="form-inline pull-right">
                        ...
                        {
                            kpimember?.status !== 2 && kpiSameTime?.status !== 2 && <button className=" btn btn-success" onClick={() => kpimember && handleApproveKPI(kpimember._id, kpimember.kpis)}>{`Phê duyệt KPI ${kpimember?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'}`}</button>
                        }
                        {
                            kpiSameTime && kpimember?.status !== 2 && kpiSameTime?.status !== 2 && <button className=" btn btn-success" onClick={() => kpiSameTime && handleApproveKPI(kpiSameTime._id, kpiSameTime.kpis)}>{`Phê duyệt KPI ${kpiSameTime?.type === 'auto' ? 'tạo tự động' : 'tạo thủ công'}`}</button>
                        }
                    </div>
                    <br />
