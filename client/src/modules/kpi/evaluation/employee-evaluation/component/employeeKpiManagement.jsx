...
function EmployeeKpiManagement(props) {
  ...
  const convertDataToExportTotalData = (kpimembers, managerKpiUnit) => {
    ...
    if (listTasks && listKpis && listKpiUnit && unitName) {
      ...
      for (let i = 0; i < listKpiUnit.length; i++) {
        ...
        let employeeNumber =
          parseInt(listKpiUnit[i].organizationalUnit.managers.length) +
          parseInt(listKpiUnit[i].organizationalUnit.deputyManagers.length) +
          parseInt(listKpiUnit[i].organizationalUnit.employees.length)
              ...
              return {
                moreInform: [
                    ...
                    : '',
                  {
                    title:
                      'Báo cáo ' +
                      (item?.names?.[index] ? item.names[index] : '') +
                      ' của ' +
                      (item?.organizationalUnits?.[index] ? item.organizationalUnits[index] : '') +
                      ' ' +
                      item?.sheetName,
                    value: [
  ...
  if (user?.error?.employees?.length > 0) {
    approverSelectBox = user.error.employees
      .filter((employee) => {
        ...
        <div className='box-body'>
          ...
          <div className='qlcv'>
              ...
              <div className='form-group'>
                ...
                {organizationalUnitSelectBox && organizationalUnitSelectBox.length !== 0 && (
                  <SelectMulti
                    ...
                    style={{ width: '100%' }}
                    items={organizationalUnitSelectBox.map((item) => {
            ...
            <div className='form-inline' style={{ marginBottom: '5px' }}>
              <div className='form-group'>
                ...
                <DatePicker id='start_date' value={startDateDefault} onChange={handleStartDateChange} dateFormat='month-year' />
              </div>
              <div className='form-group'>
                <label>{translate('kpi.evaluation.employee_evaluation.to')}</label>
                <DatePicker id='end_date' value={endDateDefault} onChange={handleEndDateChange} dateFormat='month-year' />
              </div>
              <div className='form-group'>
              ...
              <div className='dropdown pull-right' style={{ marginTop: '5px' }}>
                ...
                <ul className='dropdown-menu pull-right' style={{ marginTop: 0 }}>
                  <li>
                    {exportData && (
                      <ExportExcel
                        id='export-employee-kpi-evaluation-management'
                        type='link'
                        buttonName='Báo cáo chung'
                  ...
                  <li>
                    {kpimember && (
                      <ExportExcel
                        id='export-total-employee-kpi-evaluation-management'
                        type='link'
                        buttonName='Báo cáo tổng hợp'
                        onClick={() => handleExportTotalData(kpimember)}
                      />
                    )}
                  </li>
                </ul>
          ...
          <div id='tree-table-container' style={{ marginTop: '30px' }}>
            <table id={tableId} className='table table-hover table-bordered' style={{ marginBottom: '0px' }}>
              <thead>
                <tr>
                  <th title='STT' style={{ width: '40px' }} className='col-fixed not-sort'>
                    STT
                  </th>
                  <th title='Tên nhân viên'>{translate('kpi.evaluation.employee_evaluation.name')}</th>
                  <th title={translate('kpi.employee.employee_kpi_set.create_employee_kpi_set_modal.approver')}>
                    {translate('kpi.employee.employee_kpi_set.create_employee_kpi_set_modal.approver')}
                  </th>
                  ...
                  <th title={translate('kpi.evaluation.employee_evaluation.weekly_point')} style={{ textAlign: 'center' }}>
                    {translate('kpi.evaluation.employee_evaluation.weekly_point')}
                  </th>
                  <th title='Phê duyệt' style={{ textAlign: 'center' }}>
                    {translate('kpi.evaluation.employee_evaluation.approve')}
                  </th>
                  ...
                  ? kpimember.map((kpi, index) => (
                      <tr key={index}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <a style={{ cursor: 'pointer' }} onClick={() => showWeeklyPoint(translate, kpi?.weeklyEvaluations)}>
                            {' '}
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <a
                            data-target={`#modal-approve-KPI-member`}
                            onClick={() => handleShowApproveModal(kpi)}
                            data-toggle='modal'
                            className='approve'
                            title={translate('kpi.evaluation.employee_evaluation.approve_this_kpi')}
                          >
                        ...
                        <td>
                          <a
                            data-target={`#employee-kpi-evaluation-modal`}
                            onClick={() => showEvaluateModal(kpi)}
                            data-toggle='modal'
                            className='copy'
                            title={translate('kpi.evaluation.employee_evaluation.evaluate_this_kpi')}
                          >
