...
const StatisticsKpiUnits = (props) => {
  ...
  const showNodeContent = (translate, data, kindOfPoint) => {
    ...
    return (
      <div
        className={`tf-nc ${!data.parent_id && 'bg bg-primary'}`}
        style={{
          minWidth: '120px',
          border: 'none',
          padding: '9px 12px',
      ...
      >
        <div>
          ...
          <div className='progress-weigth-kpi' style={{ backgroundColor: data.parent_id ? '#8a8a8a' : '#676767' }}>
            <div
              className='fillmult'
              data-width={`${data.kpi.approvedPoint}%`}
              style={{ width: `${data.kpi.approvedPoint}%`, backgroundColor: data.kpi.approvedPoint < 50 ? '#dc0000' : 'rgb(76 179 99)' }}
            ></div>
            <a
              title={titleShow}
              className='perc'
            >{`${!pointShow && pointShow !== 0 ? translate('kpi.evaluation.employee_evaluation.not_avaiable') : pointShow + '/100'}`}</a>
          </div>
          <p className='kpi-number-target'>
            <span style={{ marginRight: '5px' }}>số mục tiêu:</span> <span style={{ fontWeight: 'bold' }}>{data.kpi?.kpis?.length}</span>
          </p>
        </div>
      </div>
    )
  ...
  const displayTreeView = (translate, data, kindOfPoint) => {
    if (data) {
      if (!data.children) {
        return <li key={data.id}>{showNodeContent(translate, data, kindOfPoint)}</li>
      }

      return (
        <li key={data.id}>
          {showNodeContent(translate, data, kindOfPoint)}
          <ul>{data.children.map((tag) => displayTreeView(translate, tag, kindOfPoint))}</ul>
        </li>
      )
    } else {
      return null
    ...
    <div className='box-body'>
      <section className='box-body' style={{ textAlign: 'right' }}>
        ...
        {checkHasComponent('refresh-kpi-unit-in-dashboard') && (
          <ToolTip type='text_tooltip' dataTooltip={[` ${translate('kpi.evaluation.employee_evaluation.refresh_all_kpis')}`]}>
            <i
              className='material-icons'
              onClick={() => handleRefreshKpiUnits()}
              style={{ fontSize: '22px', verticalAlign: 'top', color: '#06c', cursor: 'pointer', marginLeft: '10px' }}
            >
              refresh
            </i>
          </ToolTip>
      ...
      {managerKpiUnit?.isLoading ? (
        ...
        treeKpiUnits.map((tree, index) => (
          <div
            key={index}
            className='tf-tree example'
            style={{ display: 'flex', justifyContent: 'flex-start', marginTop: '20px', paddingTop: '10px', paddingBottom: '10px' }}
          >
            <ul>{displayTreeView(translate, tree, kindOfPoint)}</ul>
          </div>
        ))
      ) : (
