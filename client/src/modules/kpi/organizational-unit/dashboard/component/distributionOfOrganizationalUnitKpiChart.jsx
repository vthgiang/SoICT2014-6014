...
function DistributionOfOrganizationalUnitKpiChart(props) {
  ...
  const pieChart = (kindOfChart) => {
    ...
    let dataLineChart = [['x'], [translate('kpi.organizational_unit.dashboard.trend_chart.weight')]]
    ...
    let chart = c3.generate({
      ...
      tooltip: {
        format: {
          title: function (d) {
            if (tooltip?.length > 0) return tooltip?.[d]
          }
        }
      },
        ...
        <section id={'distributionOfUnit'} className='c3-chart-container'>
          <section style={{ textAlign: 'right' }}>
            <section className='btn-group'>
              <button
                ...
                onClick={() => handleSelectKindOfChart(KIND_OF_CHART.LINE)}
              >
                {translate('kpi.organizational_unit.dashboard.line_chart')}
              </button>
              <button
                type='button'
                className={`btn btn-xs ${kindOfChart === KIND_OF_CHART.PIE ? 'btn-danger' : null}`}
                onClick={() => handleSelectKindOfChart(KIND_OF_CHART.PIE)}
              >
                {translate('kpi.organizational_unit.dashboard.pie_chart')}
              </button>
            </section>
          </section>
          <div ref={refPieChart} id={'abc'}>
            {' '}
          </div>
          {kindOfChart === KIND_OF_CHART.PIE && (
            <CustomLegendC3js
              ...
              title={
                state.dataPieChart &&
                `${translate('kpi.evaluation.employee_evaluation.KPI_list')} (${currentKpi.kpis && currentKpi.kpis.length})`
              }
...
const connectedDistributionOfOrganizationalUnitKpiChart = connect(
  mapState,
  actions
)(withTranslate(DistributionOfOrganizationalUnitKpiChart))
