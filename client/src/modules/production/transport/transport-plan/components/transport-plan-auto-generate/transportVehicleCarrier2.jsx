    ...
    <React.Fragment>
      ...
      <div className='form-inline'>
        <div className='form-group'>
          ...
          <DatePicker id={`start_date_23`} value={formatDate(date.startDate)} onChange={handleStartDateChange} disabled={false} />
        </div>
        <div className='form-group'>
          <label className='form-control-static'>Đến ngày: </label>
          <DatePicker id={`end_date_23`} value={formatDate(date.endDate)} onChange={handleEndDateChange} disabled={false} />
        </div>
      </div>
      <TransportPlanDetailInfo currentTransportPlan={planChosenSeen} idPlan={idPlan} />
        ...
        <table className={'tableTest table-bordered table-hover not-sort'}>
          <thead>
            ...
            <tr className='word-no-break'>
              ...
              <th>{'Tên xe'}</th>
              <th>{'Chi tiết'}</th>
              {listDay && listDay.length !== 0 && listDay.map((item, index) => <th key={index + 'day'}>{formatDate(item)}</th>)}
            </tr>
          </thead>
          <tbody className='transport-special-row'>
            {listAllVehicles &&
              listAllVehicles.length !== 0 &&
              listAllVehicles.map(
                (vehicle, index1) =>
                  vehicle && (
                      ...
                      {listDay &&
                        ...
                        listDay.map((day, index2) => (
                          <td className='tooltip-checkbox-transport' key={index2}>
                            <span
                              className={'icon ' + getStatusTickBox(formatDate(day), vehicle._id)}
                              title={'KH'}
                            ...
                            {getCurrentPlan(formatDate(day), vehicle._id) && (
                              <span className='tooltiptext'>
                                ...
                                >
                                  {getCurrentPlan(formatDate(day), vehicle._id)?.code}
                                </a>
                              </span>
                            )}
                          </td>
            ...
            <tr key='page-nav-vehicle'>
              <td colSpan={listDay ? listDay.length + 4 : 4}>{}</td>
            </tr>
          </tbody>
          ...
          <thead>
            ...
            <tr className='word-no-break'>
              ...
              {listDay && listDay.length !== 0 && listDay.map((item, index) => <th key={index + 'day2'}>{formatDate(item)}</th>)}
            </tr>
          </thead>
          <tbody className='transport-special-row'>
            {listAllCarriers &&
              listAllCarriers.length !== 0 &&
              listAllCarriers.map(
                (carrier, index1) =>
                  carrier && (
                    <tr key={carrier._id} className='word-no-break'>
                      ...
                      {listDay &&
                        ...
                        listDay.map((day, index2) => (
                          <td className='tooltip-checkbox-transport' key={index2}>
                            <span
                              className={'icon ' + getStatusTickBox(formatDate(day), carrier._id)}
                              title={'KH'}
                              style={{ cursor: 'default' }}
                            ></span>
                            {getCurrentPlan(formatDate(day), carrier._id) && (
                              <span className='tooltiptext'>
                                ...
                                >
                                  {getCurrentPlan(formatDate(day), carrier._id)?.code}
                                </a>
                              </span>
                            )}
                          </td>
