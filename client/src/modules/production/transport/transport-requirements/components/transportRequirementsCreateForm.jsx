...
function TransportRequirementsCreateForm(props) {
  ...
  const isFormValidated = () => {
    if (
      state.value &&
      requirementsForm.code &&
      requirementsForm.info?.customer1AddressTransport &&
      requirementsForm.info?.customer2AddressTransport &&
      requirementsForm.goods &&
      requirementsForm.goods.length !== 0 &&
      requirementsForm.approver &&
      requirementsForm.department
    ) {
      return true
    }
    return false
  ...
  const save = async () => {
    ...
    if (requirementsForm.info?.customer1AddressTransport && !fromLat && !fromLng) {
      await getGeocode(requirementsForm.info.customer1AddressTransport).then((value) => {
        fromLat = value.lat
        fromLng = value.lng
      })
    }
    if (requirementsForm.info?.customer2AddressTransport && !toLat && !toLng) {
      await getGeocode(requirementsForm.info.customer2AddressTransport).then((value) => {
        toLat = value.lat
        toLng = value.lng
      })
    }
  ...
  useEffect(() => {
    if (bills && bills.listBills) {
      let lists = bills.listBills.filter((r) => String(r.type) === String(state.billType) && String(r.group) === String(state.billGroup))
      setListBills(lists)
    }
  }, [bills])
  ...
  return (
    <React.Fragment>
      ...
      >
        <form
          id='form-create-transport-requirements'
        ...
        >
              ...
              <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12' style={{ padding: '0px' }}>
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  <div className={`form-group`}>
                    <label>Mã yêu cầu vận chuyển</label>
                    <input type='text' className='form-control' disabled={true} value={requirementsForm.code} />
                  </div>
                </div>
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  <div className={`form-group`}>
              ...
              <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12' style={{ padding: '0px' }}>
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  <div className={`form-group`}>
                    ...
                    {billFromStockModules && (
                      <input type='text' className='form-control' disabled={true} value={getTypeRequirement(state.value)} />
                    )}
                  </div>
                </div>
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  ...
                  {billFromStockModules && (
                    <div className={`form-group`}>
                      ...
                      <input type='text' className='form-control' disabled={true} value={currentBill?.bill?.code} />
                    </div>
                  )}
                </div>
              </div>
          ...
          {state.value === '2' && <TransportReturn currentBill={currentBill?.bill} callBackGeneralInfo={callBackGeneralInfo} />}
          {state.value === '3' && (
          ...
          {state.value === '4' && <TransportMaterial />}
          {state.value === '5' && <TransportNewOne callBackGeneralInfo={callBackGeneralInfo} />}
          <TransportGoods goods={goodsTransport} callBackState={callBackGoodsInfo} />
          <TransportTime callBackState={callBackTimeInfo} />
        </form>
      </DialogModal>
    </React.Fragment>
  )
