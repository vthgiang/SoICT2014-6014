...
class PurchaseDetailForm extends Component {
  ...
  render() {
    ...
    return (
      ...
      <DialogModal
          modalID="modal-detail-material-purchase-order" isLoading={false}
      ...
      >
        <div className="row row-equal-height" style={{ marginTop: -25 }}>
          <div className={"col-xs-12 col-sm-12 col-md-6 col-lg-6"} style={{ padding: 10 }}>
            ...
            <h4>Thông tin chung đơn mua NVL</h4>
              ...
              {data.returnRule ? 
              (<div className="form-group">
                <strong>{"Quy tắc đổi trả"}:&emsp; </strong>
                {data.returnRule}
              </div>) : ''}

              {data.serviceLevelAgreement ? 
                (<div className="form-group">
                <strong>{"Cam kết chất lượng"}:&emsp; </strong>
                {data.serviceLevelAgreement}
                </div>) : ''}
          ...
          <div className={"col-xs-12 col-sm-12 col-md-6 col-lg-6"} style={{ padding: 10 }}>
            <div className="description-box" style={{ height: "100%" }}>
              ...
              <div className="form-group">
                <strong>{"Chịu trách nhiệm mua hàng"}:&emsp; </strong>
                {data.responsible}
              </div>
              <div className="form-group">
                <strong>{"Những người phê duyệt"}:&emsp; </strong>
                {data.approvers.length ? data.approvers.map((user)=>(<div style={{display: 'flex'}}>
                    <div style={{width: '50%'}}><b> <i>Tên: </i></b> {user.name}</div>
                    <div style={{width: '50%'}}><b><i>Chức vụ: </i></b> {user.role}</div>
                ...
                {data.payments.length ? (
                  ...
                  <legend className="scheduler-border">{'Thông tin thanh toán'}</legend>
                    <table className="table table-bordered">
                      <thead>
                        <tr>
                          <th title={'Người thanh toán'}>Người thanh toán</th>
