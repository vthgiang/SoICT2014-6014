...
function CreateDiscountsForOrder(props) {
    ...
    const handleDiscountChange = (e) => {
        ...
        let { discountprops } = props;
        let { id, checked } = e.target;

        if (checked === true) {
            let discountValue = getDiscountValue(id);
            discountprops.push(discountValue);
            handleDiscountsChange(discountprops);
        } else {
            let filterDiscountprops = discountprops.filter((element) => element._id !== id.split("-")[0]); //lọc phần tử có id ra khỏi state
            for (let key in discountsChecked) {
                ...
                if (discountsChecked[`${key}`] === false) {
                    filterDiscountprops = filterDiscountprops.filter((element) => element._id !== key.split("-")[0]);
                }
            }
            handleDiscountsChange(filterDiscountprops);
        }
