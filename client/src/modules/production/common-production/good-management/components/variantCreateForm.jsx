    ...
    return (
        ...
        <fieldset className="scheduler-border">
            ...
            <div className={`form-group`}>
                ...
                <div>
                    {(typeof variants['variantOptions'] === 'undefined' || variants['variantOptions'].length === 0) ? <p>Chưa có tùy chọn nào được thêm</p> : ''}
                    <p type="button" className="btn btn-success" onClick={() => addVariantOptions()}>Thêm mới</p>
                </div>
            </div>
            <table className="table table-bordered">
                ...
                <tbody id={`variant_option`}>
                    {
                        (typeof variants === 'undefined' || variants.length === 0) ? <tr><td colSpan={4}><center>{translate('task_template.no_data')}</center></td></tr> :
                            variants[0].variantOptions.map((item, index) =>
                                <tr key={index}>
                                    <td>{index + 1}</td>
                                    <td>{Object.entries(item).map(([key, value]) => ({ key, value }))[0].value}</td>
                                    <td>{Object.entries(item).map(([key, value]) => ({ key, value }))[1].value.map((variantOptionValue, i) =>
                                        <div key={i}>
                                            {variantOptionValue} &nbsp;
                                        </div>)}</td>
                                </tr>
            ...
            <table id={`good-table`} className="table table-striped table-bordered table-hover" style={{ marginTop: "15px" }}>
                ...
                <tbody>
                    {(typeof variants === '' || variants === 0) ? <tr><td colSpan={13}><center>{translate('task_template.no_data')}</center></td></tr> :
                        variants.map((x, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{x.variantName}</td>
                                <td>{x.sku}</td>
                                <td><input value={x.defaultPrice ? x.defaultPrice : (productDefaultPrice ? productDefaultPrice : '')} type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'defaultPrice', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'salePrice', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'msrp', index)} /></td>
                                <td><input value={x.weight ? x.weight : (productWeight ? productWeight : '')} type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'weight', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'width', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'height', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'depth', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'cost', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'upcEan', index)} /></td>
                                <td><input type="text" className="form-control" onChange={(e) => handleVariantFieldChange(e, 'mpn', index)} /></td>
                            </tr>
                        ))
                    }
                </tbody>
