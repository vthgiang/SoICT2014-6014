...
function PriceCreateForm(props) {
  ...
  return (
    <fieldset className='scheduler-border'>
      ...
      <div className='col-xs-12 col-sm-6 '>
        <div className={`form-group`}>
          ...
          <input
            ...
            value={productDefaultPrice ? productDefaultPrice : ''}
            onChange={(e) => handleInputFieldChange(e, 'defaultPrice')}
          />
        </div>
      </div>
      <div className='col-xs-12 col-sm-6 '>
        <div className={`form-group`}>
          <label>{'Mã số thuế nhà cung cấp'}</label>
      ...
      <div className='col-xs-12 col-sm-6 '>
        <div className={`form-group`}>
          <label>{'Giá bán lẻ đề xuất'}</label>
      ...
      <div className='pull-right' style={{ marginBottom: '10px' }}>
        {state.editInfo ? (
          <React.Fragment>
            <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleCancelEditDiscountTier}>
              {translate('task_template.cancel_editing')}
            </button>
            <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleSaveEditDiscountTier}>
              {translate('task_template.save')}
            </button>
          </React.Fragment>
        ) : (
          <button className='btn btn-success' style={{ marginLeft: '10px' }} disabled={!isUnitsValidated()} onClick={handleAddDiscountTier}>
            {translate('task_template.add')}
          </button>
        )}
        <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearDiscountTier}>
      ...
      <table className='table table-bordered'>
        ...
        <tbody id={`unit-create-good`}>
          {typeof listDiscountTiers === 'undefined' || listDiscountTiers.length === 0 ? (
            <tr>
              <td colSpan={4}>
            ...
            listDiscountTiers.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <a href='#abc' className='edit' title={translate('general.edit')} onClick={() => handleEditDiscountTier(item, index)}>
                    <i className='material-icons'></i>
                  </a>
                  <a href='#abc' className='delete' title={translate('general.delete')} onClick={() => handleDeleteDiscountTier(index)}>
                    <i className='material-icons'></i>
                  </a>
                </td>
              </tr>
