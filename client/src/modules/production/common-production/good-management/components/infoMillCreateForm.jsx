...
function InfoMillCreateForm(props) {
    ...
    const isMillValidated = () => {
        const { manufacturingMill } = state
        if (validateManufacturingMillChange(manufacturingMill.manufacturingMill._id, false)
    ...
    return (
        ...
        <fieldset className="scheduler-border">
            ...
            <div className={`form-group ${!errorOnProductivity ? "" : "has-error"}`}>
                ...
                <div>
                    <input type="number" className="form-control" placeholder={translate('manage_warehouse.good_management.productivity')} value={manufacturingMill.productivity} onChange={handleProductivityChange} />
                </div>
            ...
            <div className={`form-group ${!errorOnPersonNumber ? "" : "has-error"}`}>
                ...
                <div>
                    <input type="number" className="form-control" placeholder={translate('manage_warehouse.good_management.person_number')} value={manufacturingMill.personNumber} onChange={handlePersonNumberChange} />
                </div>
            ...
            <div className="pull-right" style={{ marginBottom: "10px" }}>
                {state.editInfo ?
                    <React.Fragment>
                        <button className="btn btn-success" onClick={handleCancelEditMill} style={{ marginLeft: "10px" }}>{translate('task_template.cancel_editing')}</button>
                        <button className="btn btn-success" disabled={!isMillValidated()} onClick={handleSaveEditMill} style={{ marginLeft: "10px" }}>{translate('task_template.save')}</button>
                    </React.Fragment> :
                    <button className="btn btn-success" style={{ marginLeft: "10px" }} disabled={!isMillValidated()} onClick={handleAddMill}>{translate('task_template.add')}</button>
                }
                <button className="btn btn-primary" style={{ marginLeft: "10px" }} onClick={handleClearMill}>{translate('task_template.delete')}</button>
            </div>
                        ...
                        (typeof listManufacturingMills === 'undefined' || listManufacturingMills.length === 0) ? <tr><td colSpan={6}><center>{translate('task_template.no_data')}</center></td></tr> :
                            listManufacturingMills.map((x, index) =>
                                <tr key={index}>
                                    ...
                                    <td>
                                        <a href="#abc" className="edit" title={translate('general.edit')} onClick={() => handleEditMill(x, index)}><i className="material-icons"></i></a>
                                        <a href="#abc" className="delete" title={translate('general.delete')} onClick={() => handleDeleteMill(index)}><i className="material-icons"></i></a>
                                    </td>
                                </tr>
                            )
                    }
