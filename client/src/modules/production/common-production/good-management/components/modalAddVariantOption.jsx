    ...
    <React.Fragment>
      ...
      >
        <div className='form-group'>
          <p type='button' className='btn btn-success' onClick={handleAddvariantOption}>
            Thêm mới
          </p>
        </div>
        <table className='table'>
          ...
          <tbody>
            {!variantOption || variantOption.length === 0 ? (
              <tr>
                <td colSpan={10}>
                  ...
                  <tr key={index}>
                    <td>
                      ...
                      <a onClick={() => delete_function(index)}>
                        <p className='text-red'>Xóa thuộc tính</p>
                      </a>
                    </td>

                    <td>
                      {x.variantOptionValue && x.variantOptionValue.length ? (
                        x.variantOptionValue.map((y, i) => {
                          return (
                            <div
                              key={i}
                              className={`form-group ${parseInt(errorOnValuePosition) === i && errorOnValue ? 'has-error' : ''}`}
                              style={{ display: 'flex' }}
                            >
                              <input
                                className='form-control'
                                ...
                                name='value'
                                style={{ width: '100%' }}
                                onChange={(e) => handleChangeVariantOptionValue(e, i, index)}
                              />
                              <a className='delete' title='Delete' data-toggle='tooltip' onClick={() => deleteVariantOptionValue(i, index)}>
                      ...
                      <a style={{ textAlign: 'left' }} onClick={() => handleAddVariantOptionValue(index)}>
                        <p className='text-green'>Thêm giá trị mới</p>
                      </a>
                    </td>
                  </tr>
                )
