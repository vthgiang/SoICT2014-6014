      ...
      <div className='form-inline'>
        <div className='form-group'>
          ...
          <SelectMulti
            ...
            multiple='multiple'
            options={{
              nonSelectedText: translate('manage_warehouse.bin_location_management.choose_status'),
              allSelectedText: translate('manage_warehouse.bin_location_management.all_type')
            }}
      ...
      <div className='form-inline'>
        <div className='form-group'>
          ...
          <input
            ...
            placeholder={translate('manage_warehouse.bin_location_management.code')}
            autoComplete='off'
          />
        </div>
        <div className='form-group'>
          ...
          <button
            ...
            title={translate('manage_warehouse.bin_location_management.search')}
          ...
          {typeof listPaginate !== undefined &&
            ...
            listPaginate.map((x, index) => (
              <tr key={index}>
                {/* <td>{index + 1}</td> */}
                <td>{x.path}</td>
                <td style={{ color: translate(`manage_warehouse.bin_location_management.${x.status}.color`) }}>
                  {translate(`manage_warehouse.bin_location_management.${x.status}.status`)}
                </td>
                <td>
                  {x.capacity ? x.capacity : 0} {x.unit}
                </td>
                <td>
                ...
                <td>
                  {x.enableGoods &&
                    x.enableGoods.length > 0 &&
                    (handleEnableGood(x.enableGoods) !== null
                      ? handleEnableGood(x.enableGoods).map((x, i) => {
                          return (
                            <p key={i}>
                              {x.good.name} ({x.contained} {x.good.baseUnit})
                            </p>
                          )
                        })
                      : translate('manage_warehouse.bin_location_management.empty_stock'))}
                  {handleEnableGood(x.enableGoods) !== null && handleEnableGood(x.enableGoods).length == 2 && (
