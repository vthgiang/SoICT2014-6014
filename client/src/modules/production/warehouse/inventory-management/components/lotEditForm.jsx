...
function LotEditForm(props) {
    ...
    const isBinLocationsValidated = () => {
        let result =
            validateBin(state.binLocation.binLocation, false) &&
            validateQuantity(state.binLocation.quantity, false)
        return result;
    }
                        ...
                        <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <fieldset className="scheduler-border">
                                ...
                                <div className={`form-group ${!errorQuantity ? "" : "has-error"}`}>
                                    ...
                                    <div>
                                        <input type="number" className="form-control" value={binLocation.quantity} placeholder={translate('manage_warehouse.good_management.max_quantity')} onChange={handleQuantityChange} />
                                    </div>
                                ...
                                <div className="pull-right" style={{ marginBottom: "10px" }}>
                                    {state.editInfo ?
                                        <React.Fragment>
                                            <button className="btn btn-success" onClick={handleCancelEditBinLocation} style={{ marginLeft: "10px" }}>{translate('task_template.cancel_editing')}</button>
                                            <button className="btn btn-success" disabled={!isBinLocationsValidated()} onClick={handleSaveEditBinLocation} style={{ marginLeft: "10px" }}>{translate('task_template.save')}</button>
                                        </React.Fragment> :
                                        <button className="btn btn-success" style={{ marginLeft: "10px" }} disabled={!isBinLocationsValidated()} onClick={handleAddBinLocation}>{translate('task_template.add')}</button>
                                    }
                                    <button className="btn btn-primary" style={{ marginLeft: "10px" }} onClick={handleClearBinLocation}>{translate('task_template.delete')}</button>
                                </div>
                                ...
                                <table className="table table-bordered">
                                    ...
                                    <tbody id={`good-manage-by-stock`}>
                                        {(typeof state.binLocations === 'undefined' || state.binLocations.length === 0) ? <tr><td colSpan={3}><center>{translate('task_template.no_data')}</center></td></tr> :
                                            state.binLocations.map((x, index) =>
                                                <tr key={index}>
                                                    <td>{x.binLocation.path}</td>
                                                    <td>{x.quantity}</td>
                                                    <td>
                                                        <a href="#abc" className="edit" title={translate('general.edit')} onClick={() => handleEditBinLocation(x, index)}><i className="material-icons"></i></a>
                                                        <a href="#abc" className="delete" title={translate('general.delete')} onClick={() => handleDeleteBinLocation(index)}><i className="material-icons"></i></a>
                                                    </td>
                                                </tr>
                                            )
                                        }
