...
class TreeTable extends Component {
    ...
    showActionColumn = (data, id) => {
        let { titleAction, lots } = this.props;
        switch (data) {
            case "edit":
                ...
                <tbody id="taskTable" className="task-table">
                    {   this.dataTreetable(column, data).length > 0 ?
                        this.dataTreetable(column, data).map((rows, index) => (
                            rows.parent === null ? <tr style={{backgroundColor: 'rgba(221, 221, 221, 0.867)'}} key={index} data-id={rows._id} data-parent={rows.parent} data-level={rows.level}>
                                {
