        ...
        <div className='box-body qlcv'>
          <div className='form-inline'>
            <div className='form-group'>
              ...
              <SelectMulti
                ...
                multiple='multiple'
                options={{
                  nonSelectedText: translate('manage_warehouse.inventory_management.total_stock'),
                  allSelectedText: translate('manage_warehouse.inventory_management.total_stock')
                }}
                className='form-control select2'
                style={{ width: '100%' }}
                items={stocks.listStocks.map((x, index) => {
          ...
          <div className='form-inline'>
            <div className='form-group'>
              ...
              <input
                ...
                placeholder={translate('manage_warehouse.inventory_management.good_code')}
                autoComplete='off'
              />
            </div>
            <div className='form-group'>
              <label className='form-control-static'>{translate('manage_warehouse.inventory_management.date')}</label>
              <DatePicker id='expiration-date' value={expirationDate} onChange={handleExpirationDateChange} />
            </div>
            <div className='form-group'>
              <button
                type='button'
                className='btn btn-success'
                title={translate('manage_warehouse.inventory_management.search')}
                onClick={handleSubmitSearch}
              >
                {translate('manage_warehouse.inventory_management.search')}
              </button>
            </div>
          </div>
          <div className='box-body'>
            <ul className='todo-list'>
              <li>
                <span className='text'>
                  <a href='/good-management'>Tổng số {translate(`manage_warehouse.inventory_management.${type}`)} trong kho</a>
                </span>
                <span className='label label-info' style={{ fontSize: '11px' }}>
                  {totalGoodsByType} {translate(`manage_warehouse.inventory_management.${type}`)}
                </span>
              </li>
              <li>
                <span className='text'>
                  <a href='#'>Tổng số lô hàng của {translate(`manage_warehouse.inventory_management.${type}`)}</a>
                </span>
                <span className='label label-warning' style={{ fontSize: '11px' }}>
                  {totalLot} {translate('manage_warehouse.inventory_management.lots')}
                </span>
              </li>
              {good ? (
                <li>
                  <span className='text'>
                    Số lượng tồn kho của {goodName.text}{' '}
                    <a href='#' onClick={() => handleShowDetailInfo(goodName.value)}>
                      (xem thẻ kho)
                    </a>
                  </span>
                  <span className='label label-success' style={{ fontSize: '11px' }}>
                    {quantityTotal} {goodName.baseUnit}
                  </span>
                </li>
              ) : (
                []
          ...
          {lotDetailId && <InventoryDetailForm id={lotDetailId} stock={stock} quantity={quantityTotal} />}
          <LotDetailForm />

          {lotId && <LotEditForm id={lotId} lot={lots.lotDetail} />}
                ...
                listPaginate.map((x, index) => (
                  <tr key={index}>
                    ...
                    {checkQuantity(x.stocks) ? (
                      <td className='tooltip-inventory'>
                        <span style={{ color: 'red' }}>
                          {(stock && stock.length > 0) || stocks.listStocks.length > 0 ? totaQuantity(x.stocks) : x.quantity}
                        </span>
                        <span className='tooltiptext'>
                          <p style={{ color: 'white' }}>{translate('manage_warehouse.inventory_management.text')}</p>
                        </span>
                      </td>
                    ) : (
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <a onClick={() => handleShowDetailLot(x._id)}>
                        <i className='material-icons'>view_list</i>
                      </a>
                      <a onClick={() => handleEdit(x._id)} href={`#${x._id}`} className='text-yellow'>
