...
function GoodIssueWorkFlowModal(props) {
  ...
  const handleAddBinLocation = () => {
    const { binLocations } = props
    const { listPaginate } = binLocations
    let { binLocation, listGoods, currentLot, dataLots } = state
  ...
  const { translate } = props
  const { dataLots, code, status, stockInfo, errorOnStatus, errorBinLocation, errorQuantity, binLocation, currentIndex } = state
              ...
              <div className='row'>
                ...
                <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                  ...
                  <div className='form-group'>
                    ...
                    {stockInfo.status && (
                      <span style={{ color: translate(`manage_warehouse.bin_location_management.${stockInfo.status}.color`) }}>
                        {translate(`manage_warehouse.bin_location_management.${stockInfo.status}.status`)}
                      </span>
                    )}
                  </div>
                  <div className='form-group'>
                      ...
                      {state.editBinLocation ? (
                        <React.Fragment>
                          <button
                            ...
                            disabled={!isBinLocationValidated()}
                            onClick={handleSaveEditBinLocation}
                            style={{ marginLeft: '10px' }}
                          >
                            {'Lưu chỉnh sửa'}
                          </button>
                          <button className='btn btn-danger' onClick={handleCancelEditBinLocation} style={{ marginLeft: '10px' }}>
                            {'Hủy chỉnh sửa'}
                          </button>
                        </React.Fragment>
                      ) : (
                            ...
                            <tr key={index}>
                                    ...
                                    <div key={index2}>
                                      <p>
                                        ...
                                        <a
                                          ...
                                          onClick={() => handleEditBinLocation(x, binLocation, index, index2)}
                                        >
                                        ...
                                        <a
                                          ...
                                          onClick={() => handleDeleteBinLocation(x, binLocation, index, index2)}
                                        >
                                          <i className='material-icons'></i>
                                        </a>
                                      </p>
                              ...
                              <td>
                                <p
                                  ...
                                  className='btn btn-success'
                                  style={{ marginLeft: '10px' }}
                                  onClick={() => handleArrangeLot(x, index, 1)}
                                >
                                  {'xếp hàng'}
                                </p>
                              </td>
