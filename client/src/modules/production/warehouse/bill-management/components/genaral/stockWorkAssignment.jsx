...
function StockWorkAssignment(props) {
  ...
  const getEmployees = () => {
    ...
    if (userdepartments) {
      ...
      if (managers[keyManagers[0]]) {
        list = managers[keyManagers[0]].members.map((category) => {
          return { value: category._id, text: category.name }
        })
      }
      if (deputyManagers[keyDeputyManagers[0]]) {
        list = list.concat(
          deputyManagers[keyDeputyManagers[0]].members.map((category) => {
            return { value: category._id, text: category.name }
          })
        )
      }
      if (employees[keyEmployees[0]]) {
        list = list.concat(
          employees[keyEmployees[0]].members.map((category) => {
            return { value: category._id, text: category.name }
          })
        )
      }
  ...
  const validateTask = () => {
    ...
    workAssignment.forEach((item, index) => {
      if (
        !validateTaskStartDate(item.startDate, index, false) ||
  ...
  const isFormValidated = () => {
    ...
    let result =
      validateAccountables(accountables, false) &&
  ...
  const handleOpenCalendarChart = () => {
    if (isFormValidated()) {
      ...
      let data = [
        {
          nameField: 'Công việc ' + groupText + ' phiếu: ' + props.code,
          workAssignmentStaffs: state.peopleInCharge,
        ...
        {
          nameField: 'Giám sát quá trình ' + groupText + ' phiếu: ' + props.code,
          workAssignmentStaffs: state.accountables,
          startDate: state.startDate,
          startTime: state.startTime,
          endDate: state.endDate,
          endTime: state.endTime
        ...
        {
          nameField: 'Kế toán ' + groupText + ' phiếu: ' + props.code,
          workAssignmentStaffs: state.accountants,
          startDate: state.startDate,
          startTime: state.startTime,
          endDate: state.endDate,
          endTime: state.endTime
    ...
    <React.Fragment>
        ...
        <fieldset className='scheduler-border'>
          ...
          <div className={'row'}>
            ...
            <div className='col-lg-4 col-md-4 col-ms-12 col-xs-12 form-group'>
              ...
              <select
                style={{ borderRadius: '5px', width: '97%', marginLeft: '3.5%', border: '1px solid #d2d6de' }}
          ...
          <div className={`col-xs-12 col-sm-6 col-md-6 col-lg-6 ${!errorOnStartDateAll ? '' : 'has-error'}`}>
            <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
              ...
              <DatePicker id={`startDatePicker`} dateFormat='year-month-day' value={startDate} onChange={handleChangeStartDate} />
            </div>
            <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
              <label>
                {'Thời gian bắt đầu'}
              ...
              <TimePicker id={`startTimePicker`} refs={`startTimePicker`} value={startTime} onChange={handleStartTimeChange} />
            </div>
          ...
          <div className={`col-xs-12 col-sm-6 col-md-6 col-lg-6 ${!errorOnEndDateAll ? '' : 'has-error'}`}>
            <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
              ...
              <DatePicker id={`endDatePicker`} dateFormat='year-month-day' value={endDate} onChange={handleChangeEndDate} />
            </div>
            <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
              <label>
                {'Thời gian kết thúc'}
              ...
              <TimePicker id={`endTimePicker`} refs={`endTimePicker`} value={endTime} onChange={handleEndTimeChange} />
            </div>
        ...
        <fieldset className='scheduler-border'>
          ...
          <div className='col-xs-12'>
            <div className='form-group'>
              <p type='button' className='btn btn-success' onClick={handleAddWorkAssignment}>
                {'Thêm công việc mới'}
              </p>
              <p style={{ float: 'right' }} type='button' className='btn btn-primary'>
                {'Phân công tự động'}
              </p>
            </div>
      ...
      <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
        <fieldset className='scheduler-border'>
          ...
          <button type='button' disabled={!isFormValidated()} className='btn btn-info' onClick={handleOpenCalendarChart}>
            {!isOpenCalendarChart ? 'Xem biểu đồ công việc' : 'Làm mới biểu đồ'}
          </button>
