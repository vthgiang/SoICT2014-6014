...
function QuantityLotGoodReceipt(props) {
  ...
  const isLotsValidated = () => {
    let result =
      ...
      state.lot.rfidCode &&
      state.lot.rfidCode.length > 0
    return result
  }

  const isRfidCodeValidate = () => {
    let result = validateQuantity(state.lot.quantity, false) && validateRfidQuantity(state.lot.rfidQuantity, false)
    return result
  }
        ...
        <fieldset className='scheduler-border'>
          ...
          {quantity ? (
            different !== 0 ? (
              ...
              >{`Bạn cần phải đánh lô cho ${different} số lượng hàng nhập`}</div>
            ) : (
              <div className='form-group' style={{ color: 'green', textAlign: 'center' }}>
                Bạn đã đánh xong lô cho {quantity} số lượng hàng nhập
              </div>
            )
          ) : (
            []
          ...
          <div className={`form-group ${!errorRfidQuantity ? '' : 'has-error'}`}>
            ...
            <div>
              <input
                type='number'
                className='form-control'
                placeholder={translate('manage_warehouse.bill_management.rfid_quantity')}
                value={lot.rfidQuantity}
                onChange={handleRfidQuantityChange}
              />
            </div>
          ...
          <div className={`form-group ${!errorExpirationDate ? '' : 'has-error'}`}>
            ...
            <DatePicker id={`expirationDate-lot`} value={lot.expirationDate} onChange={handleExpirationDateChange} />
          ...
          <div className={`form-group`}>
            ...
            <div>
              <input
                type='text'
                className='form-control'
                placeholder={translate('manage_warehouse.bill_management.description')}
                value={lot.note}
          ...
          <div className='pull-right' style={{ marginBottom: '10px' }}>
            <p
              ...
              disabled={!isRfidCodeValidate()}
              onClick={handleGenerateRfid}
            >
              {translate('manage_warehouse.bill_management.create_rfid_code')}
            </p>
            {state.editInfo ? (
              <React.Fragment>
                <p type='button' className='btn btn-success' onClick={handleCancelEditLot} style={{ marginLeft: '10px' }}>
                  {translate('task_template.cancel_editing')}
                </p>
                <p
                  type='button'
                  className='btn btn-success'
                  disabled={!isLotsValidated()}
            ...
            <p type='button' className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearLot}>
          ...
          <table className='table table-bordered'>
            <thead>
              <tr>
                ...
                <th title={translate('manage_warehouse.bill_management.expiration_date')}>
                  {translate('manage_warehouse.bill_management.expiration_date')}
                </th>
                <th title={translate('manage_warehouse.bill_management.rfid_code')}>
                  {translate('manage_warehouse.bill_management.rfid_code')}
                </th>
                ...
                lots.map((x, index) => (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <a href='#abc' className='delete' title={translate('general.delete')} onClick={() => handleDeleteLot(x.lot, index)}>
                        <i className='material-icons'></i>
                      </a>
                    </td>
                  </tr>
          ...
          <div className='pull-right' style={{ marginTop: '10px' }}>
            <p type='button' className='btn btn-info' style={{ marginLeft: '10px' }}>
