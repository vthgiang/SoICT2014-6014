      ...
      >
        <form id='good-receipt-work-flow-modal'>
            ...
            <div className='timeline-items'>
              {steps.map((item, index) => (
                <div className={`timeline-item ${item.active ? 'active' : ''}`} key={index}>
                  <div className={`timeline-contain`} onClick={(e) => setCurrentStep(e, index)}>
                    {item.label}
                  </div>
                </div>
              ))}
          ...
          <div>
            ...
            {step === 1 && billInfor && (
              <AddLotComponent billId={billInfor._id} billInfor={billInfor} statusLot={statusLot} onDataChange={handleAddLotDataChange} />
            )}
            {step === 2 && (
