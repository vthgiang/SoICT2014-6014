...
function GoodReceiptWorkFlowModal(props) {
    ...
    if (props.billId !== state.billId) {
        setState({
            ...state,
            statusQuality: props.billInfor.statusArray[0],
            statusLot: props.billInfor.statusArray[1],
            statusInventory: props.billInfor.statusArray[2],
            billId: props.billId,
        })
    }
    ...
    const save = async () => {
        if (isFormValidated()) {
            ...
            let statusArray = [state.statusQuality, state.statusLot, state.statusInventory];
    ...
    const { billInfor } = props;
    console.log("billInfor", billInfor);
    return (
        <React.Fragment>
            <DialogModal
