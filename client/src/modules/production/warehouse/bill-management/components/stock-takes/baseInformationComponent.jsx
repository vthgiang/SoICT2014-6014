...
function BaseInformationComponent(props) {
  ...
  const validateGoods = () => {
    ...
    else if (listGood.length > 0) return true
    return false
  }

  const isFormValidated = () => {
    let { fromStock, listGood, type } = state
    let result = validateStock(fromStock, false) && validateBillTypeProduct(state.type, false) && validateGoods()
    return result
  }
                ...
                <table className='table'>
                  <thead>
                    <tr>
                      ...
                      {createType === 1 && <th>{translate('task_template.action')}</th>}
                    </tr>
                  </thead>
