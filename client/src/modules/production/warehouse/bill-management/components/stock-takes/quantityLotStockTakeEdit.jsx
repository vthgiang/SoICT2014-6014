...
function QuantityLotStockTakeEdit(props) {
    const EMPTY_LOT = {
        lot: '',
        quantity: 0,
        realQuantity: '',
    ...
    const validateQuantity = (value, willUpdateState = true) => {
        ...
        if (willUpdateState) {
            state.lot.realQuantity = value;
            state.lot.damagedQuantity = Number(state.lot.realQuantity) - Number(state.lot.quantity);
            setState({
                ...state,
                errorQuantity: msg,
            });
                        ...
                        {state.editInfo &&
                            <div>
                                ...
                                <div className={`form-group ${!errorQuantity ? "" : "has-error"}`}>
                                    ...
                                    <div>
                                        <input type="number" className="form-control" placeholder={translate('manage_warehouse.bill_management.real_quantity')} value={lot.realQuantity ? lot.realQuantity : ''} onChange={handleQuantityChange} />
                                    </div>
