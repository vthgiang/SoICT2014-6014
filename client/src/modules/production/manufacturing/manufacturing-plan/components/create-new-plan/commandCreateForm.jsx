...
class CommandCreateForm extends Component {
    ...
    getRemainingQuantityOfGood = () => {
        ...
        if (manufacturingCommands.length) { // Nếu mà trước đó đã có lệnh thì phải lấy listGoods trừ đi list commands
        ...
        } else {// Nếu chưa có lệnh nào thì lấy theo listGoods
            listGoods.map(x => {
                listRemainingGoods.push({
                    good: x.good,
                    remainingQuantity: x.quantity
                            ...
                            <div className="row">
                                ...
                                <div className="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                    <div className={`form-group ${!errorGood ? "" : "has-error"}`}>
                                        <label>{translate('manufacturing.plan.choose_good')}<span className="attention"> * </span></label>
                                        <SelectBox
                            ...
                            <div className="row">
                                ...
                                <div className="col-xs-12 col-sm-6 col-md-6 col-lg-6">
                                    <div className={`form-group ${!errorQuantity ? "" : "has-error"}`}>
                                        <label>{translate('manufacturing.plan.command_quantity')}<span className="attention"> * </span></label>
                            ...
                            <div className="pull-right" style={{ marginBottom: "10px" }}>
                                {this.state.editCommand ?
                                    <React.Fragment>
                                        ...
                                        <button className="btn btn-success" onClick={this.handleSaveEditCommand} disabled={!this.validateCommand()} style={{ marginLeft: "10px" }}>{translate('manufacturing.purchasing_request.save_good')}</button>
                                    </React.Fragment> :
                                    <button className="btn btn-success" style={{ marginLeft: "10px" }} disabled={!this.validateCommand()} onClick={this.handleAddCommand}>{translate('manufacturing.purchasing_request.add_good')}</button>
                                }
                                    ...
                                    manufacturingCommands && manufacturingCommands.length === 0
                                        ...
                                        manufacturingCommands.map((command, index) => (
                                            <tr key={index}>
                                                ...
                                                <td>
                                                    <a href="#abc" className="edit" title={translate('general.edit')} onClick={() => this.handleEditCommand(command, index)}><i className="material-icons"></i></a>
                                                    <a href="#abc" className="delete" title={translate('general.delete')} onClick={() => this.handleDeleteCommand(command, index)}><i className="material-icons"></i></a>
                                                </td>
                                            </tr>
                                        ))
                                }
                    ...
                    <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                        {
                            ...
                            <div className="pull-left form-group has-error" style={{ marginBottom: "10px" }}>
                                <label>{translate('manufacturing.plan.created_all_command')}<span className="attention"> * </span></label>
                            </div>
                        }
                    </div>
                </div >
