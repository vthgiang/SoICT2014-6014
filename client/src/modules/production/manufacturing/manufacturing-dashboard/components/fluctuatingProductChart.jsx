...
class FluctuatingProductLineChart extends Component {
  ...
  barAndChart = (translate, fluactuatingProduct) => {
    ...
    let chart = c3.generate({
      ...
      data: {
        x: 'x',
        columns: [arrayMonth, arrayTotal, arrayFinished, arraySub],
      ...
      <React.Fragment>
        <div className='box'>
          <div className='box-header with-border'>
            ...
            <h3 className='box-title'>{translate('manufacturing.dashboard.product_quantity_change')}</h3>
            ...
            <div className='form-inline'>
              ...
              <div className='form-group'>
                ...
                <DatePicker id='from-month-fluctuating' dateFormat='month-year' value={fromMonth} onChange={this.handleFromDateChange} />
              </div>
              <div className='form-group'>
                <label className='form-control-static'>{translate('manufacturing.dashboard.to')}</label>
                <DatePicker id='to-month-fluctuating' dateFormat='month-year' value={toMonth} onChange={this.handleToDateChange} />
              </div>
              <div className='box-tools pull-right'>
                <div className='btn-group pull-rigth'>
                  <button
                    type='button'
                    className={`btn btn-xs ${this.state.barChart ? 'btn-danger' : 'active'}`}
                    onClick={() => this.handleChangeViewChart(true)}
                  >
                    {translate('manufacturing.dashboard.line')}
                  </button>
                  <button
                    type='button'
                    className={`btn btn-xs ${this.state.barChart ? 'active' : 'btn-danger'}`}
                    onClick={() => this.handleChangeViewChart(false)}
                  >
                    {translate('manufacturing.dashboard.bar')}
                  </button>
                </div>
              </div>

              <div className='form-group'>
                <button disabled={errorTime} onClick={this.handleSubmitSearch} className='btn btn-success'>
                  {translate('manufacturing.dashboard.filter')}
                </button>
              </div>
            </div>
