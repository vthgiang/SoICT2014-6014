    ...
    <div id={id} className='tab-pane'>
      <div className='form-group pull-right' style={{ padding: '6px 12px', margin: '5px', width: '100%' }}>
        <ExportExcel
          ...
          buttonName='Download hồ sơ nhân sự chủ chốt'
        />
        <a
          className='btn btn-success'
          style={{ paddingLeft: '15px', marginLeft: '15px' }}
          onClick={() => props.downLoadDocument(props._id)}
          title='Tải xuống file minh chứng'
        >
      ...
      <div className='box-body qlcv'>
        {state.keyPeople &&
          ...
          state.keyPeopleRequires.map((y, index) => {
                ...
                <div className='box'>
                  <div className='box-header with-border'>
                    ...
                    >
                      ...
                      Vị trí công việc: {`${listCareer?.find((x) => x._id == y.careerPosition)?.name}`}
                    </p>
                  </div>
                                  ...
                                  {x.careerPositions?.length > 0 ? (
                                    x.careerPositions?.map((e, key) => {
                                      return (
                                        <li key={key}>
                                          {' '}
                                          {listCareer?.filter((x) => x._id == item.careerPosition)[0]?.name}{' '}
                                          {e?.startDate ? '- Ngày bắt đầu: ' + formatDate(e?.startDate) : ''}{' '}
                                  ...
                                  {x.degrees.length > 0 ? (
                                    x.degrees?.map((e, key) => {
                                      ...
                                      return (
                                        <li key={key}>
                                          {' '}
                                          {formatDate(e?.year)} - {e?.name} - Loại: {e?.degreeType} - Chuyên ngành: {e.major?.name} - Bậc:{' '}
                                          {degreeQualification}
                                        </li>
                                      )
                                    })
