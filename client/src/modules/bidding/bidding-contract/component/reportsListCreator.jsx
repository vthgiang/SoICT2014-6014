...
export const taskReportDocxCreate = (tasksOfTag, contract) => { // taskOfTag: {tag: String, tasks: []}
        ...
        sections: [
            {
                children: [
                    ...
                    createText(`Trong thời gian từ ngày _____ đến ngày _____ chuyên gia thực hiện dịch vụ ${tagName} cho ${contract?.companyA}. Các công việc đã thực hiện bao gồm:`),
...
const createTaskReportHeading = (tasks, headingIndex = 2) => {
    ...
    for (let x = 0; x < tasks?.length; x++) {
        ...
        res.push(
            createHeading(`${headingIndex}.${x+1}. Công việc ${taskItem.taskName}`, 4, 'l')
        );
        res.push(createText(``));
        res.push(createText(``));
        res.push(createText(``));
