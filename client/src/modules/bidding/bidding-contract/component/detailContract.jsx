...
const ViewBiddingContract = (props) => {
  ...
  const formatTimeOfEffection = (unitOfTime, effectiveDate, endDate) => {
    let result = 100
    // if (unitOfTime === "days") {
    //   result = (endDate.getTime() - effectiveDate.getTime()) / (24 * 60 * 60 * 1000)
    // }
    // else if (unitOfTime === "hours") {
    //   result = (endDate.getTime() - effectiveDate.getTime()) / (60 * 60 * 1000)
    // }
    // else result = (endDate.getTime() - effectiveDate.getTime()) / (30 * 24 * 60 * 60) // tính trung bình 1 tháng 30 ngày
          ...
          <div className="tab-content">
            <div className={state.selectedTab === "general" ? "active tab-pane" : "tab-pane"} id="general">
              ...
              <div>
                {state.files?.map(x => {
                  return <span>
                    {!x.urlFile ? translate('human_resource.profile.no_files') :
                      <a className='intable'
                        style={{ cursor: "pointer" }}
                        onClick={(e) => requestDownloadFile(e, `.${x.urlFile}`, x.name)}>
                        <i className="fa fa-download"> &nbsp;{x.name}</i>
                      </a>
                    }</span>
                })}
              </div>
