    ...
    sections: [
      {
        children: [
          ...
          createBulletHeader(
            `Căn cứ Quyết định số _____ ngày ___ tháng ___ năm _____ của ____ về việc phê duyệt kết quả lựa chọn nhà thầu gói thầu ${contract?.biddingPackage.name} và thông báo kết quả lựa chọn nhà thầu số ___ ngày ___ tháng ___ năm _____ của bên mời thầu`
          ),
          ...
          createText(
            `Giao cho Ông/Bà ${contract?.decideToImplement.projectManager?.map((userItem) => convertUserIdToUserName(listUsers, userItem)).join(', ')} phụ trách triển khai thực hiện hợp đồng "${contract?.name}" số ${contract?.code} ký ngày ${contract?.createdDate} giữa ${contract?.companyB} và ${contract?.companyA}.`
          ),
          createText(
            `Ông/Bà ${contract?.decideToImplement.projectManager?.map((userItem) => convertUserIdToUserName(listUsers, userItem)).join(', ')} có trách nhiệm thực hiện đầy đủ các công việc sau: `
          ),
          createText(
            `(1) Chủ trì thực hiện các nhiệm vụ sau đây để đảm bảo thực hiện triển khai dự án đúng nội dung, phạm vi, tiến độ và chất lượng như hợp đồng ký kết với khách hàng: `
          ),
          createBulletNumber(
            `Lập kế hoạch và phân công công việc chi tiết cho từng thành viên trong nhóm để đảm bảo chất lượng và tiến độ dự án theo đúng hợp đồng ký kết; đề xuất và thực hiện phương án thuê chuyên gia, thiết bị chuyên dùng thực hiện hợp đồng, đảm bảo hiệu quả, đúng quy định pháp luật.`
          ),
          createBulletNumber(
            `Chủ trì theo dõi, cùng với các thành viên thực hiện và hoàn thành các nhiệm vụ theo đúng yêu cầu hợp đồng, phù hợp với kế hoạch, tiến độ hợp đồng ký kết với khách hàng.`
          ),
          createBulletNumber(
            `Giám sát chất lượng, kiểm tra đánh giá chất lượng sản phẩm trước khi bàn giao, chịu trách nhiệm về chất lượng sản phẩm bàn giao cho khách hàng.`
          ),
          createText(
            `(2) Báo cáo Giám đốc khi có các vấn đề phát sinh ảnh hưởng đến chất lượng, tiến độ của việc thực hiện hợp đồng. Đề xuất và thực hiện các điều chỉnh cần thiết phù hợp với thực tế triển khai. `
          ),
          ...
          createText(
            `Quyết định có hiệu lực từ ngày ký. Ban Giám đốc, Ông/Bà ${contract?.decideToImplement.projectManager?.map((userItem) => convertUserIdToUserName(listUsers, userItem)).join(', ')} và các thành viên có tên trong danh sách tham gia thực hiện dự án có trách nhiệm thi hành Quyết định này`
          ),
...
const renderTableMember = (contract, listUsers) => {
  ...
  const pmArr = contract?.decideToImplement?.projectManager?.map((x, idx) => {
    return new TableRow({
      children: [
        ...
        new TableCell({
          children: [createText(`${convertUserIdToUserName(listUsers, x)}`)],
          verticalAlign: VerticalAlign.CENTER,
          width: { size: 45, type: WidthType.PERCENTAGE }
        }),
        new TableCell({
          children: [createText(`Quản lý dự án`)],
          verticalAlign: VerticalAlign.CENTER,
          width: { size: 45, type: WidthType.PERCENTAGE }
        })
      ]
  ...
  const resArr = contract?.decideToImplement?.responsibleEmployees?.map((x, idx) => {
    ...
    return new TableRow({
      children: [
        new TableCell({
          children: [createText(`${numOfManager + idx + 1}`)],
          verticalAlign: VerticalAlign.CENTER,
          width: { size: 10, type: WidthType.PERCENTAGE }
        }),
        new TableCell({
        ...
        new TableCell({
          children: [createText(`Thành viên dự án`)],
          verticalAlign: VerticalAlign.CENTER,
          width: { size: 45, type: WidthType.PERCENTAGE }
        })
      ]
