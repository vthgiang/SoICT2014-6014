...
const ViewTaskProjectTemplateInGantt = (props) => {
  const { translate, taskList, allEmployee, unitOfTime } = props
  ...
  const getDataTask = (zoom = currentZoom) => {
    ...
    if (taskList && taskList.length > 0) {
      for (let taskItem of taskList) {
        ...
        let end = moment(start).add(Number(taskItem.estimateNormalTime), unitOfTime).toDate()
        ...
        if (taskItem.preceedingTasks && taskItem.preceedingTasks.length > 0) {
          ...
          if (preTasks[0]?.end) {
            ...
            end = moment(start).add(Number(taskItem.estimateNormalTime), unitOfTime).toDate()
          }
        }
...
const connectedComponent = connect(mapStateToProps, mapDispatchToProps)(withTranslate(ViewTaskProjectTemplateInGantt))
export { connectedComponent as ViewTaskProjectTemplateInGantt }
