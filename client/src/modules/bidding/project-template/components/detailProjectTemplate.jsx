...
const DetailProjectTemplate = (props) => {
      ...
      <div style={{ lineHeight: 2.3 }}>
        <div className='row'>
          <div className='col-md-6'>
            <div>
              <strong>Tên dự án: </strong> {projectData?.name}
            </div>
          </div>
          <div className='col-md-6'>
            <div>
              <strong>Hình thức quản lý dự án: </strong> {projectData ? renderProjectTemplateTypeText(projectData?.projectType) : null}
            </div>
          </div>
        </div>
        ...
        <div className='row'>
          <div className='col-md-6'>
            <div>
              <strong>Đơn vị chi phí: </strong> {projectData && projectData?.currenceUnit ? projectData?.currenceUnit : 'Không xác định'}
            </div>
          </div>
          <div className='col-md-6'>
            <div>
              <strong>Đơn vị thời gian: </strong>
              {fakeUnitTimeList.find((x) => x.value === projectData?.unitOfTime)?.text || 'Không xác định'}
            </div>
          </div>
        </div>
        ...
        <div className='row'>
          <div className='col-md-6'>
            <div>
              <strong>Quản lý dự án: </strong> {projectData?.projectManager?.map((m) => m.name).join(', ') || 'Không xác định'}
            </div>
          </div>
          <div className='col-md-6'>
            <div>
              <strong>Thành viên dự án: </strong>
              {projectData?.responsibleEmployees?.map((r) => r.name).join(', ') || 'Không xác định'}
            </div>
          </div>
        </div>
      </div>
    ...
    return (
      <>
        {projectData?.tasks?.length === 0 && (
          <span style={{ display: 'flex', justifyContent: 'center' }}>Không có thông tin công việc</span>
        )}
                ...
                <div className='box-body collapse' data-toggle='collapse' id={`task-proposal-${index}`} style={{ lineHeight: 2.3 }}>
                  <div className='row'>
                    <div className='col-md-6'>
                      ...
                      <div>
                        <strong>Công việc tiền nhiệm: </strong>
                        <span>{item.preceedingTasks ?? 'Không có'}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        <strong>Thời gian ước lượng: </strong>
                        <span>
                          {item.estimateNormalTime} ({fakeUnitTimeList.find((x) => x.value === projectData?.unitOfTime)?.text})
                        </span>
                      </div>
                      <div>
                        <strong>Thời lượng thỏa hiệp: </strong>
                        <span>
                          {item.estimateOptimisticTime} ({fakeUnitTimeList.find((x) => x.value === projectData?.unitOfTime)?.text})
                        </span>
                      </div>
                    </div>
                    <div className='col-md-6'>
                      <div>
                        ...
                        <span>{item.accountableEmployees?.map((x) => convertUserIdToUserName(listUsers, x)).join(', ')}</span>
                      </div>
                      <div>
                        <strong>Người thực hiện: </strong>
                        <span>{item.responsibleEmployees?.map((x) => convertUserIdToUserName(listUsers, x)).join(', ')}</span>
                      </div>
                      <div>
                        <strong>Tổng trọng số người thực hiện: </strong>
                        <span>{item.totalResWeight} (%)</span>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-detail-project-template-${id}`}
        isLoading={false}
        ...
        <form id='form-detail-project-template'>
          <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
            ...
            <ul className='nav nav-tabs'>
              ...
              <li className='active'>
                <a href={`#general-detail-${id}`} onClick={() => handleChangeContent(`general-detail-${id}`)} data-toggle='tab'>
                  Thông số và nhân sự mẫu dự án
                </a>
              </li>
              {/* Nút tab các bên tgia */}
              <li>
                <a href={`#tasks-detail-${id}`} onClick={() => handleChangeContent(`tasks-detail-${id}`)} data-toggle='tab'>
                  Công việc trong mẫu dự án
                </a>
              </li>
            </ul>
