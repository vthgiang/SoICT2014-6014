...
const DetailTag = (props) => {
    ...
    return (
        <React.Fragment>
            <DialogModal
                modalID={`modal-detail-tag-${id}`}
                formID="form-detail-tag"
                title="Chi tiết tag"
                hasSaveButton={false}
                size={50}
            >
                <form id="form-detail-tag">
                    <div style={{ lineHeight: 2 }}>
                        <div><strong>Tên tag: </strong>{name}</div>
                        <div><strong>Mô tả tag: </strong>{description}</div>
                        <div><strong>Danh sách nhân viên và độ phù hợp tương ứng: </strong>
                            <ul>
                                {employeeWithSuitability.sort((a, b) => b.suitability - a.suitability)?.map((x, idx) => {
                                    return <li key={idx}>
