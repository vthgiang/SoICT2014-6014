...
function PurchaseRequest(props) {
    ...
    var pageTotal = ((purchaseRequest.totalList % limit) === 0) ?
        parseInt(purchaseRequest.totalList / limit) :
        parseInt((purchaseRequest.totalList / limit) + 1);
            ...
            <div className="box-body qlcv">
                ...
                <div className="form-inline">
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" className="form-control" name="recommendNumber" onChange={handleRecommendNumberChange} placeholder={translate('supplies.purchase_request.recommendNumber')} autoComplete="off" />
                    </div>
                ...
                <table id={tableId} className="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        {(listPurchaseRequests && listPurchaseRequests.length !== 0) ?
                            listPurchaseRequests.filter(item => item.proponent && item.proponent._id === auth.user._id).map((x, index) => (
                                <tr key={index}>
                                    ...
                                    <td style={{ textAlign: "center" }}>
                                        <a onClick={() => handleView(x)} style={{ width: '5px' }} title={translate('supplies.general_information.view_recommend_card')}><i className="material-icons">view_list</i></a>
                                        {
                                            (x.status === 'waiting_for_approval' || x.status === 'disapproved') &&
                                            <a onClick={() => handleEdit(x)} className="edit text-yellow" style={{ width: '5px' }} title={translate('supplies.general_information.edit_recommend_card')}><i className="material-icons">edit</i></a>
                                        }
                                        <DeleteNotification
                ...
                {purchaseRequest.isLoading ?
                    ...
                    (!listPurchaseRequests || listPurchaseRequests.length === 0) && <div className="table-info-panel">{translate('confirm.no_data')}</div>
                }
