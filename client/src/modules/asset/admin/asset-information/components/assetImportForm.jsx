...
function AssetImportForm(props) {
  ...
  const isFormValidated = () => {
    ...
    return (
      rowErrorGeneralInformationData.length === 0 &&
  ...
  const convertAssetTemplate = (dataTemplate) => {
    ...
    for (let i = 0; i < dataTemplate.dataSheets.length; i++) {
      switch (dataTemplate.dataSheets[i].type) {
        case 'General':
          dataTemplate.dataSheets[i] = convertImportGeneralInformationOfAssetTemplate(
            dataTemplate.dataSheets[i],
            userList,
            assetTypes,
            roles,
            assetLocations,
        ...
        case 'Usage':
          ...
          break
        case 'Incident':
          dataTemplate.dataSheets[i] = convertImportIncidentInformationOfAssetTemplate(
            dataTemplate.dataSheets[i],
            userList,
            incidentType,
            incidentStatus
          ...
          break
        case 'Maintainance':
          dataTemplate.dataSheets[i] = convertImportMaintainanceInformationOfAssetTemplate(
            dataTemplate.dataSheets[i],
            maintainanceType,
            maintainanceStatus
          )
    ...
    let length = Math.max(
      ...
      assetGroups.length,
      status.length,
      typeRegisterForUse.length,
      depreciationType.length,
      incidentType.length,
      incidentStatus.length,
      maintainanceType.length,
      maintainanceStatus.length,
      disposalType.length
    )
  ...
  const convertImportGeneralInformationOfAssetTemplate = (
    dataTemplate,
    userList,
    assetTypes,
    roles,
  ...
  const handleImportExcelGeneralInformation = (value, checkFileImport) => {
    ...
    if (assetsManager) {
      assetsManager.buildingAssets &&
        assetsManager.buildingAssets.list.map((item) => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (valueTemporary.code) {
          ...
          if (type === 'add') {
            ...
            if (
              !valueTemporary.code ||
              !valueTemporary.assetName ||
              !valueTemporary.assetType ||
              !valueTemporary.status ||
              !valueTemporary.typeRegisterForUse ||
              ...
              (valueTemporary.readByRoles &&
                valueTemporary?.readByRoles?.trim().toLowerCase() !== 'all' &&
                !roles[valueTemporary.readByRoles]) ||
          ...
          valueTemporary = {
            ...valueTemporary,
            purchaseDate:
              purchaseDate && typeof purchaseDate === 'string' ? purchaseDate : convertExcelDateToJSDate(purchaseDate, 'dd-mm-yy'),
            warrantyExpirationDate:
              warrantyExpirationDate && typeof warrantyExpirationDate === 'string'
                ? warrantyExpirationDate
                : convertExcelDateToJSDate(warrantyExpirationDate, 'dd-mm-yy'),
            errorAlert: errorAlert
          }

          let count = 0
          valueTemporary?.assetInfo?.length &&
            valueTemporary.assetInfo.forEach((x) => {
          ...
          importGeneralInformationData = [
            ...importGeneralInformationData,
            {
              ...
              readByRoles:
                valueTemporary?.readByRoles?.trim().toLowerCase() === 'all'
                  ? allRoleId
                  : roles[valueTemporary.readByRoles] && [roles[valueTemporary.readByRoles]],
              ...
              purchaseDate:
                purchaseDate && typeof purchaseDate === 'string'
                  ...
                  : convertExcelDateToJSDate(purchaseDate, 'yy-mm-dd'),
              warrantyExpirationDate:
                warrantyExpirationDate && typeof warrantyExpirationDate === 'string'
                  ? convertStringToDate(warrantyExpirationDate)
                  : convertExcelDateToJSDate(warrantyExpirationDate, 'yy-mm-dd'),
              detailInfo
            }
          ]
          ...
          if (importGeneralInformationData.length !== 0) {
            ...
            if (importGeneralInformationData && importGeneralInformationData.length - 1 >= 0) {
              let assetInfo = {}
              let countInfo = 0
              valueTemporary?.assetInfo?.length === 2 &&
                valueTemporary.assetInfo.forEach((x) => {
                  if (x === null) countInfo = countInfo + 1
                })
              ...
              importGeneralInformationData[importGeneralInformationData.length - 1] = {
                ...
                readByRoles:
                  ...
                  !importGeneralInformationData[importGeneralInformationData.length - 1].readByRoles.includes(
                    roles[valueTemporary.readByRoles]
                  )
                    ? [
                        ...importGeneralInformationData[importGeneralInformationData.length - 1].readByRoles,
                    ...
                    : importGeneralInformationData[importGeneralInformationData.length - 1].readByRoles,
                detailInfo:
                  Object.keys(assetInfo).length && importGeneralInformationData[importGeneralInformationData.length - 1].detailInfo
    ...
    if (checkFileImport) {
      value = value.map((item, index) => {
        ...
        item = {
          ...item,
          startDepreciation:
            startDepreciation && typeof startDepreciation === 'string'
              ? startDepreciation
              : convertExcelDateToJSDate(startDepreciation, 'dd-mm-yy'),
          errorAlert: errorAlert
        }
        importDepreciationInformationData = [
          ...importDepreciationInformationData,
          {
            ...
            startDepreciation:
              startDepreciation && typeof startDepreciation === 'string'
                ? convertStringToDate(startDepreciation)
                : convertExcelDateToJSDate(startDepreciation, 'yy-mm-dd')
          }
        ]
  ...
  const handleImportExcelUsageInformation = (value, checkFileImport) => {
    ...
    if (checkFileImport) {
      value = value.map((item, index) => {
        ...
        if (
          (item.usedByUser && !userList[item.usedByUser]) ||
          (item.usedByOrganizationalUnit && !departmentList[item.usedByOrganizationalUnit])
        ) {
          rowError = [...rowError, index + 1]
          item = { ...item, error: true }
        }
        ...
        importUsageInformationData = [
          ...importUsageInformationData,
          {
            ...
            startDate:
              startDate && typeof startDate === 'string' ? convertStringToDate(startDate) : convertExcelDateToJSDate(startDate, 'yy-mm-dd'),
  ...
  const handleImportExcelIncidentInformation = (value, checkFileImport) => {
    ...
    if (checkFileImport) {
      value = value.map((item, index) => {
        ...
        if (
          (item.type && !incidentType.includes(item.type)) ||
          (item.reportedBy && !userList[item.reportedBy]) ||
          (item.statusIncident && !status.includes(item.statusIncident))
        ) {
          rowError = [...rowError, index + 1]
          item = { ...item, error: true }
        ...
        item = {
          ...item,
          dateOfIncident:
            dateOfIncident && typeof dateOfIncident === 'string' ? dateOfIncident : convertExcelDateToJSDate(dateOfIncident, 'dd-mm-yy'),
          errorAlert: errorAlert
        }
        importIncidentInformationData = [
          ...importIncidentInformationData,
          {
            ...
            dateOfIncident:
              dateOfIncident && typeof dateOfIncident === 'string'
                ? convertStringToDate(dateOfIncident)
                : convertExcelDateToJSDate(dateOfIncident, 'yy-mm-dd')
          }
        ]
      ...
      value = value.map((item, index) => {
        ...
        importMaintainanceInformationData = [
          ...importMaintainanceInformationData,
          {
            ...item,
            createDate:
              createDate && typeof createDate === 'string'
                ? convertStringToDate(createDate)
                : convertExcelDateToJSDate(createDate, 'yy-mm-dd'),
            startDate:
              startDate && typeof startDate === 'string' ? convertStringToDate(startDate) : convertExcelDateToJSDate(startDate, 'yy-mm-dd'),
            endDate: endDate && typeof endDate === 'string' ? convertStringToDate(endDate) : convertExcelDateToJSDate(endDate, 'yy-mm-dd')
          }
    ...
    if (checkFileImport) {
      value = value.map((item, index) => {
        ...
        item = {
          ...item,
          disposalDate:
            disposalDate && typeof disposalDate === 'string' ? disposalDate : convertExcelDateToJSDate(disposalDate, 'dd-mm-yy'),
          errorAlert: errorAlert
        }
        importDisposalInformationData = [
          ...importDisposalInformationData,
          {
            ...item,
            disposalDate:
              disposalDate && typeof disposalDate === 'string'
                ? convertStringToDate(disposalDate)
                : convertExcelDateToJSDate(disposalDate, 'yy-mm-dd')
          }
        ]
        return item
      })
      ...
      >
        <div className='box-body row'>
          <div className='form-group col-md-6 col-xs-12'>
            ...
            <UploadFile importFile={handleImportExcel} />
          </div>
          <div className='form-group col-md-6 col-xs-12'>
            <ExportExcel
              id={`download_asset_file`}
              type='link'
              exportData={importAssetTemplateData}
        ...
        <div className='nav-tabs-custom row' style={{ marginTop: '-10px' }}>
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a title='Thông tin chung' data-toggle='tab' href={`#import_create_general${id}`}>
                Thông tin chung
              </a>
            </li>
            <li>
              <a title='Thông tin khấu hao' data-toggle='tab' href={`#import_depreciation${id}`}>
                Thông tin khấu hao
              </a>
            </li>
            <li>
              <a title='Thông tin sử dụng' data-toggle='tab' href={`#import_usage${id}`}>
                Thông tin sử dụng
              </a>
            </li>
            <li>
              <a title='Thông tin sự cố' data-toggle='tab' href={`#import_incident${id}`}>
                Thông tin sự cố
              </a>
            </li>
            <li>
              <a title='Thông tin bảo trì' data-toggle='tab' href={`#import_maintainance${id}`}>
                Thông tin bảo trì
              </a>
            </li>
            <li>
              <a title='Thông tin thanh lý' data-toggle='tab' href={`#import_disposal${id}`}>
                Thông tin thanh lý
              </a>
            </li>
          </ul>
