...
function ComponentInfor(props) {
    ...
    useEffect(() => {
        ...
        /* Lấy dữ liệu tăng ca của nhân viên trong công ty */
        props.getTimesheets({ organizationalUnits: organizationalUnits, startDate: monthNew, endDate: monthNew, trendOvertime: true, callApiByDashboardPersional: "curent_unit" });
    ...
    useEffect(() => {
        ...
        let taskInCurrentUnit = [], taskInCompany = [], totalListCommendationInUnit = [], totalListCommendationInCompany = [];
        ...
        if (taskListByStatus && organizationalUnits) {
            ...
            taskListByStatus.forEach(x => {
                if (x?.organizationalUnit?._id === orgUnit)
                    taskInCurrentUnit = [...taskInCurrentUnit, x]
            })
        }
        ...
        let maxTask = 1;
        let employeeTaskInCurrentUnit = [], employeeTasksInCompany = [], employeeOvertime = [], employeeHoursOff = [];
        ...
        for (let i in listEmployee) {
            let accountableTaskInCurrentUnit = [], consultedTaskInCurrentUnit = [], responsibleTaskInCurrentUnit = [], informedTaskInCurrentUnit = [];
            let accountableTaskInCompany = [], consultedTaskInCompany = [], responsibleTaskInCompany = [], informedTaskInCompany = [];

            // xử lý danh sách công việc đơn vị hiện tại của người dùng
            taskInCurrentUnit && taskInCurrentUnit.forEach(x => {
                if (x?.accountableEmployees) {
                    x.accountableEmployees.forEach(k => {
                        if (typeof k === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                accountableTaskInCurrentUnit = [...accountableTaskInCurrentUnit, x]
                        }
                        if (typeof k === 'string') {
                ...
                if (x?.responsibleEmployees) {
                    x.responsibleEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                responsibleTaskInCurrentUnit = [...responsibleTaskInCurrentUnit, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                responsibleTaskInCurrentUnit = [...responsibleTaskInCurrentUnit, x]
                ...
                if (x?.consultedEmployees) {
                    x.consultedEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                consultedTaskInCurrentUnit = [...consultedTaskInCurrentUnit, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                consultedTaskInCurrentUnit = [...consultedTaskInCurrentUnit, x]
                ...
                if (x?.informedEmployees) {
                    x.informedEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                informedTaskInCurrentUnit = [...informedTaskInCurrentUnit, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                informedTaskInCurrentUnit = [...informedTaskInCurrentUnit, x]
            ...
            taskListByStatus && taskListByStatus.forEach(x => {
                if (x?.accountableEmployees) {
                    x.accountableEmployees.forEach(k => {
                        if (typeof k === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                accountableTaskInCompany = [...accountableTaskInCompany, x]
                        }
                        if (typeof k === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                accountableTaskInCompany = [...accountableTaskInCompany, x]
                ...
                if (x?.responsibleEmployees) {
                    x.responsibleEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                responsibleTaskInCompany = [...responsibleTaskInCompany, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                responsibleTaskInCompany = [...responsibleTaskInCompany, x]
                ...
                if (x?.consultedEmployees) {
                    x.consultedEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                consultedTaskInCompany = [...consultedTaskInCompany, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                consultedTaskInCompany = [...consultedTaskInCompany, x]
                ...
                if (x?.informedEmployees) {
                    x.informedEmployees.forEach(k => {
                        if (typeof (k) === 'object') {
                            if (k?._id === listEmployee?.[i]?.userId?._id)
                                informedTaskInCompany = [...informedTaskInCompany, x]
                        }
                        if (typeof (k) === 'string') {
                            if (k === listEmployee?.[i]?.userId?._id)
                                informedTaskInCompany = [...informedTaskInCompany, x]
            ...
            taskInCurrentUnit = taskInCurrentUnit.concat(accountableTaskInCurrentUnit).concat(consultedTaskInCurrentUnit).concat(responsibleTaskInCurrentUnit).concat(informedTaskInCurrentUnit);
            taskInCompany = taskInCompany.concat(accountableTaskInCompany).concat(consultedTaskInCompany).concat(responsibleTaskInCompany).concat(informedTaskInCompany);
            ...
            employeeTaskInCurrentUnit = [...employeeTaskInCurrentUnit, {
            ...
            employeeTasksInCompany = [...employeeTasksInCompany, {
                _id: listEmployee[i]?.userId?._id,
        ...
        if (employeeTasksInCompany.length) {
            ...
            taskMin = [...taskMin, employeeTasksInCompany[employeeTasksInCompany.length - 1]?.totalTask ? employeeTasksInCompany[employeeTasksInCompany.length - 1].totalTask : 0]
        ...
        if (employeeTaskInCurrentUnit.length !== 0) {
            ...
            taskMin = [...taskMin, employeeTaskInCurrentUnit[employeeTaskInCurrentUnit.length - 1]?.totalTask ? employeeTaskInCurrentUnit[employeeTaskInCurrentUnit.length - 1].totalTask : 0]
            if (taskPersonal) {
        ...
        if (props?.discipline?.totalListCommendation?.length) {
            ...
            totalListCommendationInCompany.forEach(x => {
                if (x.organizationalUnit === orgUnit)
                    totalListCommendationInCurrentUnit = [...totalListCommendationInCurrentUnit, x]
            })
            ...
            if (totalListCommendationInCompany) {
                ...
                totalListCommendationInCompany.forEach(x => {
                    if (!listCommendationInCompany[x?.employee?._id])
                        listCommendationInCompany[x?.employee?._id] = 1
                    else
                        listCommendationInCompany[x?.employee?._id] = listCommendationInCompany[x?.employee?._id] + 1
                })
                ...
                const currentCommendation = totalListCommendationInCompany.filter(x => x?.employee?.emailInCompany === currentEmployee?.userId?.email);

                if (currentCommendation)
                    commendationCurrent = [...commendationCurrent, currentCommendation?.length];
                else
            ...
            if (totalListCommendationInCurrentUnit?.length) {
                ...
                totalListCommendationInCurrentUnit.forEach(x => {
                    if (!listDisciplineInUnits[x?.employee?._id])
                        listDisciplineInUnits[x?.employee?._id] = 1
                    else
                        listDisciplineInUnits[x?.employee?._id] = listDisciplineInUnits[x?.employee?._id] + 1
                })
                ...
                const currentCommendation = totalListCommendationInCurrentUnit.filter(x => x?.employee?.emailInCompany === currentEmployee?.userId?.email);
        ...
        if (props?.discipline?.totalListDiscipline?.length) {
            ...
            totalListDisciplineInCompany.forEach(x => {
                if (x.organizationalUnit === orgUnit)
                    totalListDisciplineInCurrentUnit = [...totalListDisciplineInCurrentUnit, x]
            })
            ...
            if (totalListDisciplineInCompany) {
                ...
                totalListDisciplineInCompany.forEach(x => {
                    if (!listDisciplineInCompany[x?.employee?._id])
                        listDisciplineInCompany[x?.employee?._id] = 1
                    else
                        listDisciplineInCompany[x?.employee?._id] = listDisciplineInCompany[x?.employee?._id] + 1
                })
                ...
                const currentCommendation = totalListDisciplineInCompany.filter(x => x?.employee?.emailInCompany === currentEmployee?.userId?.email);

                if (currentCommendation)
                    disciplineCurrent = [...disciplineCurrent, currentCommendation?.length];
            } else {
            ...
            if (totalListDisciplineInCurrentUnit?.length) {
                ...
                totalListDisciplineInCurrentUnit.forEach(x => {
                    if (!listCommendationInUnits[x?.employee?._id])
                        listCommendationInUnits[x?.employee?._id] = 1
                    else
                        listCommendationInUnits[x?.employee?._id] = listCommendationInUnits[x?.employee?._id] + 1
                })
                ...
                const currentCommendation = totalListDisciplineInCurrentUnit.filter(x => x?.employee?.emailInCompany === currentEmployee?.userId?.email);
        ...
        if (timesheets?.listOvertimeOfUnitsByStartDateAndEndDate?.length) {
            ...
            if (listOvertimeInCompany?.length) {
                ...
                listOvertimeInCompany = listOvertimeInCompany.map(x => ({ employeeId: x?.employee?._id, emailInCompany: x?.employee?.emailInCompany, totalHoursOff: x?.totalHoursOff, totalOvertime: x?.totalOvertime }));
        ...
        if (timesheets?.listOvertimeOfCurrentUnitByStartDateAndEndDate?.length) {
            ...
            if (listOvertimeInCurrentUnits?.length) {
                ...
                listOvertimeInCurrentUnits = listOvertimeInCurrentUnits.map(x => ({ employeeId: x?.employee?._id, emailInCompany: x?.employee?.emailInCompany, totalHoursOff: x?.totalHoursOff, totalOvertime: x?.totalOvertime }));
    ...
    }, [JSON.stringify(props?.user?.employees), JSON.stringify(props?.tasks?.organizationUnitTasks?.tasks), JSON.stringify(props?.discipline?.totalListCommendation), JSON.stringify(props?.discipline.totalListDiscipline),
    ...
    return (
        <React.Fragment>
                ...
                ? <div className="qlcv">
                    ...
                    <div className="row">
                        ...
                        <div className="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <h3 className="box-title">Thông tin cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} {monthShow}</h3>
                                </div>
                                <div className="box-body" style={{ paddingBottom: 53 }}>
                                    <ul className="todo-list">
                                        ...
                                        <li>
                                            ...
                                            <span className="text"><a href='/task-management-dashboard' target="_blank" >Tổng số công việc theo từng vai trò</a></span>
                        ...
                        <div className="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <div className="box-title">Tương quan công việc cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} và công ty {monthShow}</div>
                                </div>
                    ...
                    <div className='row'>
                        <div className="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <div className="box-title">Tương quan số lượt khen thưởng của cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} và công ty {monthShow}</div>
                                </div>
                                <div className="box-body">
                                    {
                        ...
                        <div className="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <div className="box-title"> Tương quan số lượt kỷ luật của cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} và công ty {monthShow}</div>
                                </div>
                                <div className="box-body">
                                    {
                    ...
                    <div className='row'>
                        <div className="col-lg-16 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <div className="box-title">Tương quan số giờ nghỉ phép cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} và công ty {monthShow}</div>
                                </div>
                                <div className="box-body">
                                    {
                        ...
                        <div className="col-lg-16 col-md-6 col-sm-12 col-xs-12">
                            <div className="box box-solid">
                                <div className="box-header with-border">
                                    <div className="box-title"> Tương quan số giờ tăng ca cá nhân trong {getUnitName(organizationalUnitsOfUser, organizationalUnits[0])} và công ty {monthShow}</div>
                                </div>
                                <div className="box-body">
                                    {
                    ...
                    <div className="row">
                        ...
                        <LazyLoadComponent>
                            <TrendWorkChart nameChart={`Thống kê tình hình làm việc`} nameData1={`${translate('human_resource.dashboard_personal.total_hours_works')}`} nameData2={`${translate('human_resource.dashboard_personal.overtime_total')}`} />
                        </LazyLoadComponent>
                    </div>
            ...
            <ViewAllTasks employeeTasks={employeeTaskInCurrentUnit ? employeeTaskInCurrentUnit : []} title={`${translate('human_resource.dashboard_personal.general_task')} ${monthShow}`} showCheck={true} />
