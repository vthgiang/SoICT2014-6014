...
import { DialogModal, LazyLoadComponent, forceCheckOrVisible } from '../../../common-components'
        ...
        <div className='qlcv'>
          <div className='nav-tabs-custom'>
            <ul className='alarm-tabs nav nav-tabs'>
              <li className='active'>
                <a className='alarm-tabs-type' href='#allGeneralNoneUpdate' data-toggle='tab' onClick={() => handleNavTabs()}>
                  {`${translate('task.task_dashboard.none_update_recently')} `}{' '}
                  <span>{`(${noneUpdateTask ? noneUpdateTask.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a className='alarm-tabs-type' href='#allGeneralUnconfirm' data-toggle='tab' onClick={() => handleNavTabs()}>
                  Chưa xác nhận thực hiện<span>{`(${unconfirmedTask ? unconfirmedTask.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a className='alarm-tabs-type' href='#allGeneralTaskNotKpi' data-toggle='tab' onClick={() => handleNavTabs()}>
                  Chưa liên kết với KPI tháng này<span>{`(${notLinkedTasks ? notLinkedTasks.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a
                  className='alarm-tabs-type'
                  href='#allGeneralTaskHasActionsResponsible'
                  data-toggle='tab'
                  onClick={() => handleNavTabs()}
                >
                  Chưa được đánh giá hoạt động<span>{`(${taskHasActionsResponsible ? taskHasActionsResponsible.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a
                  className='alarm-tabs-type'
                  href='#allGeneralTaskHasActionsAccountable'
                  data-toggle='tab'
                  onClick={() => handleNavTabs()}
                >
                  Cần đánh giá công việc<span>{`(${taskHasActionsAccountable ? taskHasActionsAccountable.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a className='alarm-tabs-type' href='#allGeneralTaskHasEvaluationInMonth' data-toggle='tab' onClick={() => handleNavTabs()}>
                  Chưa có kết quả đánh giá tháng hiện tại
                  <span>{`(${taskHasNotEvaluationResultIncurrentMonth ? taskHasNotEvaluationResultIncurrentMonth.length : 0})`}</span>
                </a>
              </li>
              <li>
                <a
                  className='alarm-tabs-type'
                  href='#allGeneralTaskHasNotApproveRequestToClose'
                  data-toggle='tab'
                  onClick={() => handleNavTabs()}
                >
                  Chưa phê duyệt kết thúc
                  <span>{`(${taskHasNotApproveResquestToClose ? taskHasNotApproveResquestToClose.length : 0})`}</span>
                </a>
              </li>
            </ul>
                          ...
                          noneUpdateTask.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                data-parent='#accordion-noneupdate'
                                href={`#collapse-noneupdate${index}`}
                                aria-expanded='true'
                              ...
                              <div id={`collapse-noneupdate${index}`} className='panel-collapse collapse' role='tabpanel'>
                                <div className='panel-body'>
                                  <div className='time-todo-range'>
                                    <span style={{ marginRight: '10px' }}>Thời gian thực hiện công việc: </span>{' '}
                                  ...
                                  <div className='priority-task-wraper'>
                                    ...
                                    <span style={{ color: checkPrioritySetColor(obj.priority) }}>
                                      {formatPriority(obj.priority, translate)}
                                    </span>
                                  </div>
                                  ...
                                  <a href={`/task?taskId=${obj._id}`} target='_blank' className='seemore-task'>
                                    Xem chi tiết
                                  </a>
                                </div>
                              </div>
                          ...
                          unconfirmedTask.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                className='item-question collapsed'
                                data-toggle='collapse'
                                data-parent='#accordion-unconfirm'
                                href={`#collapse-unconfirm${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
                                ...
                                <small className='label label-primary' style={{ fontSize: '9px', marginLeft: '5px', borderRadius: '.5em' }}>
                                  {obj.createdToNow} {translate('task.task_dashboard.day_ago')}
                                </small>
                              </span>
                          ...
                          notLinkedTasks.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                className='item-question collapsed'
                                data-toggle='collapse'
                                data-parent='#accordion-notlinkkpi'
                                href={`#collapse-notlinkkpi${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
                          ...
                          taskHasActionsResponsible.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                className='item-question collapsed'
                                data-toggle='collapse'
                                data-parent='#accordion-hasactionres'
                                href={`#collapse-hasactionres${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
                          ...
                          taskHasActionsAccountable.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                className='item-question collapsed'
                                data-toggle='collapse'
                                data-parent='#accordion-hasactionacc'
                                href={`#collapse-hasactionacc${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
                          ...
                          taskHasNotEvaluationResultIncurrentMonth.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                className='item-question collapsed'
                                data-toggle='collapse'
                                data-parent='#accordion-notevaluation'
                                href={`#collapse-notevaluation${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
                          ...
                          taskHasNotApproveResquestToClose.map((obj, index) => (
                            <div className='panel panel-default' key={index}>
                              <span
                                ...
                                data-toggle='collapse'
                                data-parent='#accordion-notevaluation'
                                href={`#collapse-requested-to-close${index}`}
                                aria-expanded='true'
                                aria-controls='collapse1a'
                              >
                                <span className='index'>{index + 1}</span>
