...
function SystemApiUpdateModal(props) {
  ...
  const checkedCategory = (category) => {
    if (category === apiUpdateCategory.ADD) {
      return addApiChecked.filter((el) => el === addApiChecked[0]).length === addApiChecked.length ? addApiChecked[0] : false
    } else if (category === apiUpdateCategory.REMOVE) {
      return removeApiChecked.filter((el) => el === removeApiChecked[0]).length === removeApiChecked.length ? removeApiChecked[0] : false
  ...
  const handleSubmit = () => {
    if (updateApiLog.add.apis.length > 0) {
      updateApiLog.add.apis.map((api, index) => {
        if (addApiChecked[index]) props.createSystemApi(api)
      })
    }

    if (updateApiLog.remove.apis.length > 0) {
      updateApiLog.remove.apis.map((api, index) => {
        if (removeApiChecked[index]) props.deleteSystemApi(api._id)
      })
    }
  ...
  const renderSystemApiUpdateTable = () => (
    <table id={tableId} className='table table-hover table-striped table-bordered'>
      ...
      <tbody>
        {updateApiLog.add.apis.length <= 0 && updateApiLog.remove.apis.length <= 0 && updateApiLog.modified.apis.length <= 0 && (
          <tr key={'no_data'}>
        ...
        {updateApiLog.add.apis.length > 0 && (
          <>
            ...
            {updateApiLog.add.apis.map((api, index) => (
              <tr key={index} className='api-update-add'>
                <td>
                  <input
                    type='checkbox'
                    value={index}
        ...
        {updateApiLog.remove.apis.length > 0 && (
          <>
            ...
            {updateApiLog.remove.apis.map((api, index) => (
              <tr key={index} className='api-update-remove'>
                <td>
                  <input
                    type='checkbox'
                    value={index}
        ...
        {updateApiLog.modified.apis.length > 0 && (
          <>
            ...
            {updateApiLog.modified.apis.map((api, index) => (
              <tr key={index} className='api-update-modified'>
                <td>
                  <input
                    type='checkbox'
                    value={index}
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID='update-system-api-modal'
        isLoading={false}
        size='75'
