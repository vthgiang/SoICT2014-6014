...
const AnnualLeaveTrendsChart = (props) => {
  ...
  const [state, setState] = useState({
    ...
    data1: employeeDashboardData.annualLeaveTrendChartData.data1
      ? {
          ratioX: employeeDashboardData.annualLeaveTrendChartData.data1.ratioX,
          data: employeeDashboardData.annualLeaveTrendChartData.data1.data
        }
      : { ratioX: [], data: [] },
    data2: employeeDashboardData.annualLeaveTrendChartData.data2
      ? {
          ratioX: employeeDashboardData.annualLeaveTrendChartData.data2.ratioX,
          data: employeeDashboardData.annualLeaveTrendChartData.data2.data
        }
      : { ratioX: [], data: [] }
  })
            ...
            <div className='dashboard_box_body'>
              ...
              <div className='box-tools pull-right' style={{ marginBottom: '10px' }}>
                <div className='btn-group pull-rigth'>
                  <button
                    type='button'
                    className={`btn btn-xs ${countAnnuaLeave ? 'btn-danger' : 'active'}`}
                    onClick={() => handleChangeDataChart(true)}
                  >
                    {nameData1}
                  </button>
                  <button
                    type='button'
                    className={`btn btn-xs ${countAnnuaLeave ? 'active' : 'btn-danger'}`}
                    onClick={() => handleChangeDataChart(false)}
                  >
                    {nameData2}
                  </button>
                </div>
