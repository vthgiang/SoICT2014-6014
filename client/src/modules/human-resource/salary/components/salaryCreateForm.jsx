...
const SalaryCreateForm = (props) => {
  ...
  const handleAddBonus = () => {
    ...
    if (bonus.length !== 0) {
      let result
      for (let n in bonus) {
        result = validateNameSalary(bonus[n].nameBonus, n) && validateMoreMoneySalary(bonus[n].number, n)
        if (result === false) {
  ...
  const isFormValidated = () => {
    ...
    let result =
      validateEmployeeNumber(employee, false) && validateOrganizationalUnit(organizationalUnit, false) && validateMonthSalary(month, false)
    ...
    if (result === true) {
      if (bonus !== []) {
        let test = bonus
        for (let n in test) {
          result = validateNameSalary(test[n].nameBonus, n, false) && validateMoreMoneySalary(test[n].number, n, false)
      ...
      >
        <form className='form-group' id='form-create-salary'>
          ...
          <div className={`form-group ${errorOnEmployee && 'has-error'}`}>
            ...
            <SelectBox
              ...
              items={listAllEmployees
                .map((y) => {
                  return { value: y.emailInCompany, text: `${y.employeeNumber} - ${y.fullName}` }
                })
          ...
          <div className={`form-group ${errorOnMonthSalary && 'has-error'}`}>
            ...
            <DatePicker id='create_month' dateFormat='month-year' value={month} onChange={handleMonthChange} />
          ...
          <div className={`form-group`}>
            ...
            <div>
              <input
                ...
                autoComplete='off'
                placeholder={translate('human_resource.salary.table.main_salary')}
                autoComplete='off'
              />
          ...
          <div className={`form-group ${(errorOnNameSalary || errorOnMoreMoneySalary) && 'has-error'}`}>
            <label>
              {translate('human_resource.salary.table.other_salary')}
              <a title={translate('human_resource.salary.table.add_more_salary')}>
                <i className='fa fa-plus-square' style={{ color: '#28A745', marginLeft: 5 }} onClick={handleAddBonus} />
              </a>
            </label>
                    ...
                    <tr key={index}>
                      <td>
                        <input
                          ...
                          name='nameBonus'
                          style={{ width: '100%' }}
                          onChange={handleChangeNameBonus}
                        />
                      </td>
                      <td>
                        <input
                          ...
                          name='number'
                          style={{ width: '100%' }}
                          onChange={handleChangeMoneyBonus}
                        />
                      </td>
                      <td style={{ textAlign: 'center' }}>
                        <a className='delete' title='Delete' data-toggle='tooltip' onClick={() => Delete(index)}>
