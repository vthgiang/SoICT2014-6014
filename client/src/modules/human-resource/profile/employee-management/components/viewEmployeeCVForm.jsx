...
const ViewEmployeeCVForm = (props) => {
    ...
    // }
    useEffect(() => {
        const shouldComponentUpdate = async () => {
            ...
            if(state.suggestItem?.suggested !== props.suggestItem?.suggested){
                setState(state => ({
                    ...state,
                    suggestItem: props.suggestItem
                }));
    ...
    }, [props._id, state._id, props.employeesInfo.isLoading, state.dataStatus, state.suggestItem])
    ...
    const addSuggest = async () => {
        ...
        window.$(`#suggest-modal-view-cv-form-employee${state.emp._id}`).modal('show');
    }

    const onChangeSuggest = async (x) => {
        setState(state => ({
            ...state,
            suggestItem: x
        }));
    ...
    const deleteSuggest = async (x) => {
        ...
        console.log('gọi2', state.suggestItem);
        props.deleteSuggest(x)
    }

    const { employeesInfo, translate } = props;

    let { _id, employees, roles = [], career, major, company, experiences, suggestItem, emp, currentItem, position } = state;
    return (
        <React.Fragment>
            <DialogModal
                ...
                <form className="form-group" id={`modal-view-cv-form-employee${_id}`} style={{ marginTop: "-15px" }}>
                    {
                        suggestItem?.suggested ?
                            <a onClick={() => deleteSuggest(_id)} className={`pull-right btn btn-danger`}> Bỏ đề xuất
                            </a> :
                            <a onClick={() => addSuggest()} className={`pull-right btn btn-success`}> Thêm đề xuất nhân sự
                            </a>
                    }
                                        ...
                                        <div className="pull-right col-lg-8 col-md-8 col-ms-12 col-xs-12">
                                            ...
                                            <div className="row">
                                                ...
                                                <div className="form-group ">
                                                    ...
                                                    {formatSkill(x.professionalSkill)}
                                                </div>
                                            </div>
                                            <div className="row">
                                                {/* Chuyên ngành */}
                                            ...
                                            <div className="row">
                                                ...
                                                <div className="form-group" style={{ lineHeight: 2 }}>
                                                    ...
                                                    <li>Số năm làm việc cho người sử dụng lao động hiện tại: {getYear(experiences[0].startDate)} năm</li>
