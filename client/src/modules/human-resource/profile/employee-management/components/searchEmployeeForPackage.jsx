...
const SearchEmployeeForPackage = (props) => {
  ...
  useEffect(() => {
    let value = []
    if (state.keyPeople) state.keyPeople.forEach((item) => item.employees.forEach((x) => value.push(x._id)))
    setState({
      ...state,
      listPeople: value
    })
    ...
    if (keyPeople) {
      ...
      keyPeople.map((o, index) => {
        return o.employees?.map((item) => {
          ...
          professionalSkill = professionalSkillArr.find((item) => item.value == professionalSkill).text
            ? professionalSkillArr.find((item) => item.value == professionalSkill).text
            : 'Không có'
          stt2 = stt2 + 1
        ...
        return o.employees?.map((item) => {
          return item.careerPositions?.map((x) => {
            let professionalExperience =
              'Công ty ' +
              `${x.company ? x.company : 'vnist'}` +
              '; Dự án ' +
              `${x.project ? x.project : ''}` +
              '; Vị trí công việc ' +
              `${x.careerPosition.name}` +
              '; Kinh nghiệm ' +
              `${x.professionalExperience ? x.professionalExperience : ''}`
          ...
          tables: [
            {
              ...
              columns: [
                ...
                { key: 'endDate', value: 'Đến ngày' },
                {
                  ...
                  value: 'Công ty/Dự án/Vị trí công việc/Kinh nghiệm chuyên môn và quản lý có liên quan',
                  width: 56
                }
              ],
  ...
  return (
    <div className='box'>
      <div className='box-body qlcv'>
        <div className='form-inline' style={{ marginBottom: 15 }}>
          ...
          <div className='form-group pull-right' style={{ padding: '6px 12px', margin: '5px' }}>
            <ExportExcel
              id='download_template_search_package'
              type='link'
              exportData={convertDataExport()}
        ...
        {state.keyPeople &&
          state.keyPeopleRequires &&
          employeesManager.isSearchComplete == 1 &&
          state.keyPeople.map((item, index) => {
            return (
                ...
                <div className='box'>
                  <div className='box-header with-border'>
                    <p
                      ...
                      data-target={`#employee-table-${index}`}
                      aria-expanded='false'
                    ...
                    >
                      ...
                      Vị trí công việc: {`${career?.listPosition?.filter((x) => x._id == item.careerPosition)[0]?.name}`}
                    </p>
                  </div>
                  <div className='box-body collapse' data-toggle='collapse' id={`employee-table-${index}`}>
                    <div className='box-header with-border'>
                      <p
                        data-toggle='collapse'
                        data-target={`#employee-require-${index}`}
                        aria-expanded='false'
                        style={{ display: 'flex', alignItems: 'center', fontWeight: '600', cursor: 'pointer' }}
                        onClick={() => {
                    ...
                    <div
                      className='box-body collapse'
                      data-toggle='collapse'
                      id={`employee-require-${index}`}
                    ...
                    >
                      <div className='row' style={{ marginTop: '15px' }}>
                        <div className='form-group col-md-6'>
                          ...
                          {career?.listPosition
                            ?.filter((x) => x._id == state.keyPeopleRequires?.[index]?.careerPosition)
                            .map((y) => y.name)}
                        </div>
                        <div className='form-group col-md-6'>
                      ...
                      <div className='row' style={{ marginTop: '15px' }}>
                        ...
                        <div className='form-group col-md-6'>
                          ...
                          {state.keyPeopleRequires?.[index]?.professionalSkill
                            ? professionalSkillArr
                                .filter((x) => x.value == state.keyPeopleRequires?.[index]?.professionalSkill)
                                .map((y) => y.text)
                            : 'Không có'}
                        </div>
                      </div>
                        ...
                        <div className='row'>
                          ...
                          <div className='form-group col-md-12'>
                            ...
                            <input
                              ...
                              name='count'
                              value={state.keyPeopleRequires?.[index]?.certificateRequirements?.count}
                              autoComplete='off'
                              disabled={true}
                            />
                          </div>
                          ...
                          item.employees.map((x, index1) => {
                            ...
                            if (x.degrees) {
                              for (let y of x.degrees) {
                                if (state.keyPeopleRequires?.[index]?.majors && y.majors) {
                                  if (
                                    state.keyPeopleRequires?.[index]?.majors?.includes(y.majors) &&
                                    y.degreeQualification > professionalSkill
                                  )
                                    professionalSkill = y.degreeQualification
                                } else if (y.major) {
                                  if (y.degreeQualification > professionalSkill) professionalSkill = y.degreeQualification
                            ...
                            return (
                              <tr key={index1}>
                                  ...
                                  {x.careerPositions?.length > 0 ? (
                                    x.careerPositions?.map((e, key) => {
                                      return (
                                        <li key={key}>
                                          {' '}
                                          {e?.careerPosition?.name} {e?.startDate ? '- Ngày bắt đầu: ' + formatDate(e?.startDate) : ''}{' '}
                                          {e?.endDate ? '- Ngày kết thúc: ' + formatDate(e?.endDate) : ''}{' '}
                                        </li>
                                      )
                                    })
                                  ) : (
                                    ...
                                    x.certificates?.map((e, key) => {
                                      return (
                                        <li key={key}>
                                          {' '}
                                          {e.certificate?.name}
                                          {e.certificate?.abbreviation ? '(' + e.certificate?.abbreviation + ')' : ''} - {e?.issuedBy} -
                                          hiệu lực: {formatDate(e?.endDate)}{' '}
                                        </li>
                                      )
                                    })
                                  ) : (
                                  ...
                                  {x.degrees.length > 0 ? (
                                    x.degrees?.map((e, key) => {
                                      ...
                                      return (
                                        <li key={key}>
                                          {' '}
                                          {formatDate(e?.year)} - {e?.name} - Loại: {translate(`human_resource.profile.${e?.degreeType}`)} -
                                ...
                                <td>
                                  ...
                                  <a
                                    onClick={() => handleChangeEmployee(x, index)}
      ...
      {/** modal import - export */ state.importSearch && <SearchDataImportForm updateSearchData={updateSearchData} />}
    </div>
  )
}
