...
import { configurationEmployee } from './fileConfigurationImportEmployee'
...
const EmployeeImportForm = (props) => {
    ...
    toastId.current = toast.warn(
      <div>
        <ServerResponseAlert
          ...
          content={[translate('human_resource.profile.employee_management.importing_employee')]}
        />
        <Loading />
      </div>,
      {
  ...
  const handleCheckImportDataOfEmployeeInfor = (value) => {
    ...
    value = value.map((x) => {
      ...
      let healthInsuranceStartDate = !x.healthInsuranceStartDate
        ? x.healthInsuranceStartDate
        : convertExcelDateToJSDate(x.healthInsuranceStartDate)
      ...
      let maritalStatus = x.maritalStatus
        ? x.maritalStatus.trim() === translate('human_resource.profile.single')
          ? 'single'
          : 'married'
        : null
    ...
    value = value.map((x, index) => {
      ...
      if (
        ...
        x.fullName === null ||
        checkImportData.filter((y) => y.employeeNumber === x.employeeNumber).length > 1 ||
      ...
      if (x.employeeTimesheetId && checkImportData.filter((y) => y.employeeTimesheetId === x.employeeTimesheetId).length > 1) {
        errorAlert = [
          ...errorAlert,
          `${translate('human_resource.profile.attendance_code')} ${translate('human_resource.value_duplicate')}`
        ]
      }
  ...
  const handleCheckImportDataOfExperience = (value) => {
    ...
    value = value.map((x, index) => {
      ...
      if (x.startDate === null) {
        errorAlert = [
          ...errorAlert,
          `${translate('human_resource.profile.from_month_year')} ${translate('human_resource.cannot_be_empty')}`
        ]
      }
      if (x.endDate === null) {
  ...
  const handleCheckImportDataOfCertificate = (value) => {
    ...
    value = value.map((x, index) => {
      ...
      if (x.name === null) {
        errorAlert = [
          ...errorAlert,
          `${translate('human_resource.profile.name_certificate')} ${translate('human_resource.cannot_be_empty')}`
        ]
      }
      // if (x.issuedBy === null) {
  ...
  const handleCheckImportDataOfFamily = (value) => {
    ...
    value = value.map((x, index) => {
      ...
      if (x.name === null) {
        errorAlert = [
          ...errorAlert,
          `${translate('human_resource.profile.house_hold.members.name_member')} ${translate('human_resource.cannot_be_empty')}`
        ]
      }
  ...
  const handleImportEmployeeInfor = () => {
    ...
    notify()
    console.log(
      'importDataOfEmployeeInfor',
      importDataOfEmployeeInfor.map((x) => x.birthdate)
    )
        ...
        <form className='form-group' id={`form_import_file`}>
          <div className='nav-tabs-custom row' style={{ marginTop: '-15px' }}>
            <ul className='nav nav-tabs'>
              <li className='active'>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_general_infor_title`)}
                  data-toggle='tab'
                  href='#import_employee_general_infor'
                >
                  {translate(`human_resource.profile.employee_management.import.import_general_infor`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_work_process_title`)}
                  data-toggle='tab'
                  href='#import_employee_workProcess'
                >
                  {translate(`human_resource.profile.employee_management.import.import_work_process`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_experience_title`)}
                  data-toggle='tab'
                  href='#import_employee_experience'
                >
                  {translate(`human_resource.profile.employee_management.import.import_experience`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_degree_title`)}
                  data-toggle='tab'
                  href='#import_employee_degree'
                >
                  {translate(`human_resource.profile.employee_management.import.import_degree`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_certificate_title`)}
                  data-toggle='tab'
                  href='#import_employee_certificate'
                >
                  {translate(`human_resource.profile.employee_management.import.import_certificate`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_contract_title`)}
                  data-toggle='tab'
                  href='#import_employee_contract'
                >
                  {translate(`human_resource.profile.employee_management.import.import_contract`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_socialInsurance_details_title`)}
                  data-toggle='tab'
                  href='#import_employee_socialInsurance_details'
                >
                  {translate(`human_resource.profile.employee_management.import.import_socialInsurance_details`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_file_title`)}
                  data-toggle='tab'
                  href='#import_employee_file'
                >
                  {translate(`human_resource.profile.employee_management.import.import_file`)}
                </a>
              </li>
              <li>
                <a
                  title={translate(`human_resource.profile.employee_management.import.import_file_family`)}
                  data-toggle='tab'
                  href='#import_employee_family'
                >
                  {translate(`human_resource.profile.employee_management.import.import_family`)}
                </a>
              </li>
            </ul>
