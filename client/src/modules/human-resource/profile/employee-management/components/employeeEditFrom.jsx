...
const EmployeeEditFrom = (props) => {
    ...
    const shouldUpdate = async () => {
      ...
      if (state.dataStatus === DATA_STATUS.QUERYING && !props.employeesInfo.isLoading) {
        setState({
          ...
          organizationalUnits:
            props.employeesInfo?.organizationalUnits?.length > 0 && props.employeesInfo?.organizationalUnits.map((x) => x._id),
  ...
  const isFormValidated = () => {
    ...
    let result = true
    if (employee) {
      result = validatorInput(employee.employeeNumber) && validatorInput(employee.fullName)
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='75'
        modalID={`modal-edit-employee${props._id}`}
        ...
        {employee && (
          <div className='nav-tabs-custom row' style={{ marginTop: '-15px' }}>
            <ul className='nav nav-tabs'>
              ...
              <li>
                <a
                  ...
                  data-toggle='tab'
                  href={`#edit_reward${_id}`}
                >
                  {translate('human_resource.profile.tab_name.menu_reward_discipline')}
                </a>
              </li>
              <li>
                <a title={translate('menu.annual_leave_personal')} data-toggle='tab' href={`#edit_salary${_id}`}>
                  {translate('menu.annual_leave_personal')}
                </a>
              </li>
              <li>
                <a title={'Thành viên hộ gia đình'} data-toggle='tab' href={`#edit_family_member${_id}`}>
            ...
            <div className='tab-content'>
              ...
              <ContactTab id={`edit_contact${_id}`} handleChange={handleChange} employee={employee} />
              ...
              />
              {/* Tab Tài khoản - thuế */}
              <TaxTab id={`edit_account${_id}`} employee={employee} handleChange={handleChange} />
