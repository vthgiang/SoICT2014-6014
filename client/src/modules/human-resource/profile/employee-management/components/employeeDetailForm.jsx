...
const EmployeeDetailForm = (props) => {
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='75'
        modalID={`modal-detail-employee${props?._id}`}
            ...
            employees.map((x, index) => (
              <div className='nav-tabs-custom row' key={index}>
                <ul className='nav nav-tabs'>
                  ...
                  <li>
                    <a
                      title={translate('human_resource.profile.tab_name.menu_contact_infor_title')}
                      data-toggle='tab'
                      href={`#view_contact${_id}`}
                    >
                  ...
                  <li>
                    <a
                      title={translate('human_resource.profile.tab_name.menu_account_tax_title')}
                      data-toggle='tab'
                      href={`#view_account${_id}`}
                    >
                  ...
                  <li>
                    <a
                      ...
                      data-toggle='tab'
                      href={`#view_reward${_id}`}
                    >
                      {translate('human_resource.profile.tab_name.menu_reward_discipline')}
                    </a>
                  </li>
                  <li>
                    <a title={translate('menu.annual_leave_personal')} data-toggle='tab' href={`#view_salary${_id}`}>
                      {translate('menu.annual_leave_personal')}
                    </a>
                  </li>
                  <li>
                    <a title={'Thành viên hộ gia đình'} data-toggle='tab' href={`#view_family_member${_id}`}>
                ...
                <div className='tab-content'>
                  {/* Thông tin chung */}
                  <GeneralTab id={`view_general${_id}`} employee={x} roles={roles} />
                  {/* Thông tin liên hệ */}
                  <ContactTab id={`view_contact${_id}`} employee={x} />
                  ...
                  <TaxTab id={`view_account${_id}`} employee={x} />
                  {/* Bằng cấp - Chúng chỉ */}
                  <CertificateTab
                    id={`view_diploma${_id}`}
                    degrees={x.degrees}
                  ...
                  />
                  {/* bảo hiểm y tế */}
                  <InsurranceTab id={`view_insurrance${_id}`} employee={x} socialInsuranceDetails={x.socialInsuranceDetails} />
                  {/* Hợp đồng lao động */}
                  <ContractTab id={`view_contract${_id}`} employee={x} courses={courses} contracts={x.contracts} />
                  {/* thông tin khen thương - kỷ luật */}
                  <DisciplineTab id={`view_reward${_id}`} commendations={commendations} disciplines={disciplines} />
                  {/* Thông tin lương thưởng */}
                  <SalaryTab id={`view_salary${_id}`} annualLeaves={annualLeaves} />
                  {/* Tài liệu đính kèm*/}
                  <AttachmentTab id={`view_attachments${_id}`} employee={x} files={x.files} />
                  <FamilyMemberTabInfo id={`view_family_member${_id}`} houseHold={houseHold} />
                </div>
              </div>
            ))}
        </form>
