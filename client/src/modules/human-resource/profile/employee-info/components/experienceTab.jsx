...
function ExperiencTab(props) {
  ...
  if (professionalSkill) {
    professionalSkill.map((item) => {
      professionalSkills =
        professionalSkills +
        professionalSkillArr.find((x) => x.value == item.degreeQualification).text +
        ' (' +
        listMajor.find((y) => item.major == y._id)?.name +
        ', ' +
        item.issuedBy +
        ', ' +
  ...
  return (
    <div id={id} className='tab-pane'>
      <div className='box-body'>
        ...
        <fieldset className='scheduler-border'>
          <legend className='scheduler-border'>
            <h4 className='box-title'>{translate('human_resource.profile.Working_process')}</h4>
          </legend>
        ...
        <fieldset className='scheduler-border'>
          <legend className='scheduler-border'>
            <h4 className='box-title'>Dự án từng tham gia</h4>
          </legend>
          <table className='table table-striped table-bordered table-hover' style={{ marginBottom: 0 }}>
            <thead>
              <tr>
