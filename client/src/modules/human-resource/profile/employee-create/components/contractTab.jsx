...
function ContractTab(props) {
  ...
  const handleEditContract = async (data) => {
    ...
    let checkData = checkForDuplicate(
      data,
      contractsNew.filter((x, index) => index !== data.index)
    )
    if (checkData) {
  ...
  const handleAddCourse = async (data) => {
    ...
    if (check === false) {
      ...
      toast.error(<ServerResponseAlert type='error' title={'general.error'} content={['Khoá đào tạo đã tồn tại']} />, {
        containerId: 'toast-notification'
      })
    }
  }
  ...
  return (
    <div id={id} className='tab-pane'>
      <div className='box-body'>
        ...
        <fieldset className='scheduler-border'>
          <legend className='scheduler-border'>
            <h4 className='box-title'>{translate('human_resource.profile.training_process')}</h4>
          </legend>
          {pageCreate && (
            <a
                  ...
                  if (courseInfo) {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <a
                            onClick={() => handleCourseEdit(x, index)}
                            className='edit text-yellow'
                            style={{ width: '5px' }}
                            title='Chỉnh sửa thông tin khoá đào tạo'
                          >
                            <i className='material-icons'>edit</i>
                          </a>
                          <a className='delete' title='Delete' data-toggle='tooltip' onClick={() => deleteCourse(index)}>
                            <i className='material-icons'></i>
                          </a>
                        </td>
                      </tr>
