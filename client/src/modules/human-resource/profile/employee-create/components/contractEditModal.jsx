...
function ContractEditModal(props) {
  ...
  const isFormValidated = () => {
    ...
    let result =
      validateNameContract(name, false) && validateTypeContract(contractType, false) && validateContractNumber(contractNumber, false)
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='50'
        modalID={`modal-edit-contract-${id}`}
        isLoading={false}
        ...
        <form className='form-group' id={`form-edit-contract-${id}`}>
          ...
          <div className='row'>
            ...
            <div className={`form-group col-sm-6 col-xs-12 ${errorOnStartDate && 'has-error'}`}>
              ...
              <DatePicker id={`edit-start-date-${id}`} deleteValue={false} value={startDate} onChange={handleStartDateChange} />
              <ErrorLabel content={errorOnStartDate} />
            </div>
            {/* Ngày hết hiệu lực*/}
            <div className={`form-group col-sm-6 col-xs-12 ${errorOnEndDate && 'has-error'}`}>
              <label>{translate('human_resource.profile.end_date_certificate')}</label>
              <DatePicker id={`edit-end-date-${id}`} deleteValue={true} value={endDate} onChange={handleEndDateChange} />
