...
function CommendationAddModal(props) {
  /**
  ...
  const isFormValidated = () => {
    const { startDate, decisionNumber, organizationalUnit, type, reason } = state
    let result =
  ...
  return (
    <React.Fragment>
      <ButtonModal
        modalID={`modal-create-praise-${id}`}
        ...
        <form className='form-group' id={`form-create-praise-${id}`}>
          ...
          <div className='row'>
            ...
            <div className={`col-sm-6 col-xs-12 form-group ${errorOnStartDate && 'has-error'}`}>
              ...
              <DatePicker id={`add_praise_start_date${id}`} value={startDate} onChange={handleStartDateChange} />
            ...
            <div className={`col-sm-6 col-xs-12 form-group ${errorOnType && 'has-error'}`}>
              ...
              <input
                ...
                autoComplete='off'
                placeholder={translate('human_resource.commendation_discipline.commendation.table.reward_forms')}
              />
