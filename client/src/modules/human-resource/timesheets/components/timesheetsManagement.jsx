...
const TimesheetsManagement = (props) => {
    ...
    const { month, limit, page, allDayOfMonth, dayNow, organizationalUnits, currentRowViewChart, currentRowView, currentRow, importExcel } = state;
                ...
                <div className="form-inline">
                    ...
                    <div className="dropdown pull-right" style={{ marginBottom: 15 }}>
                        ...
                        <ul className="dropdown-menu pull-right" style={{ marginTop: 0 }} >
                            <li><a style={{ cursor: 'pointer' }} title={translate('human_resource.timesheets.add_timesheets_title')} onClick={createTimesheets}>{translate('human_resource.timesheets.add_by_hand')}</a></li>
                            <li><a style={{ cursor: 'pointer' }} title={translate('human_resource.add_data_by_excel')} onClick={handleImport}>{translate('human_resource.timesheets.add_import')}</a></li>
                        </ul>
                    </div>
                                ...
                                <tbody>
                                    {
                                        listTimesheets.length !== 0 && listTimesheets.map((x, index) => (
                                            <React.Fragment key={index}>
                                                <tr>
                                                    <td rowSpan="3" style={{ paddingTop: 22 }}><a style={{ cursor: 'pointer' }} onClick={() => handleView(x.employee)}>{x.employee ? x.employee.employeeNumber : null}</a></td>
                                                    ...
                                                    <td rowSpan="3" style={{ paddingTop: 22, textAlign: "center" }}>
                                                        <a onClick={() => handleViewChart(x.employee)} style={{ width: '5px' }} title={`Báo cáo ngày công của ${x.employee ? x.employee.fullName : null}`}><i className="material-icons">insert_chart_outlined</i></a>
                                                        <a onClick={() => handleEdit(x)} className="edit text-yellow" style={{ width: '5px' }} title={translate('human_resource.timesheets.edit_timesheets')}><i className="material-icons">edit</i></a>
                                                        <DeleteNotification
                                                            ...
                                                            return (
                                                                <td className="tooltip-timesheet" key={indexs} style={{ backgroundColor: check ? "#e0e0e0" : "none", borderTopColor: check ? "#e0e0e0" : "none" }} >
                                                                    ...
                                                                    {check &&
                                                                        <span className="tooltiptext">
                                                                            {data[0].reason}
                                                                            <p>{"Từ " + (data[0].startTime ? data[0].startTime : "") + " " + formatDate(data[0].startDate)
                                                                                + ' đến '
                                                                                + (data[0].endTime ? data[0].endTime : "") + " " + formatDate(data[0].endDate)
                                                                            }
                                                                            </p>
                                                                        </span>
                                                                    }
                            ...
                            <tbody>
                                {
                                    listTimesheets.length !== 0 && listTimesheets.map((x, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                <td><a style={{ cursor: 'pointer' }} onClick={() => handleView(x.employee)}>{x.employee ? x.employee.employeeNumber : null}</a></td>
