        ...
        if(dataTree.length > 0){
            return dataTree.map(node => {
                        if(node.children === undefined)
                            return <li className="domain-tree" key={node.id} style={{paddingLeft: '0px', marginLeft: '0px'}}><b className="domain-tree-content"> <i className="fa fa-file-text text-gray"></i> {node.title} </b></li>
                        return <li className="domain-tree" key={node.id} style={{paddingLeft: '0px', marginLeft: '0px'}}>
                            <b className="domain-tree-content"> <i className="fa fa-plus-square text-yellow" onClick={()=>this.slideTreeElement(node.id)}></i> {node.title} </b>
