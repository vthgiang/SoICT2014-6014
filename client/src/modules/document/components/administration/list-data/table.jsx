...
function Table(props) {
  ...
  useEffect(() => {
    ...
    props.getAllDocuments({
      ...
      limit: state.limit,
      calledId: 'paginate',
      by: currentPage === '/documents/organizational-unit' ? 'organizational-unit' : undefined
    })

    props.getAllDocuments({
      page: state.page,
      limit: state.limit,
      calledId: 'relationshipDocs',
      by: currentPage === '/documents/organizational-unit' ? 'organizational-unit' : undefined
    })
    props.getAllRoles()
  ...
  return (
    <div className='qlcv'>
      <CreateForm />
      {currentRow && <ListView docs={currentRow} />}
      {currentRow && <ListDownload docs={currentRow} />}
      ...
      {currentRow && (
        <EditForm
          ...
          documentRelationshipDocuments={
            currentRow.relationshipDocuments
              ? currentRow.relationshipDocuments.map((relationshipDocument) => relationshipDocument._id || relationshipDocument)
              : []
          }
      ...
      <div className='form-inline'>
        ...
        {state?.results?.length > 0 && (
          <button
            ...
            title={'Tải xuống các tệp đính kèm trong tài liệu'}
            onClick={handleDownloadAllFileOfDocument}
          >
            <i className='fa fa-download' style={{ marginRight: 5 }} aria-hidden='true'></i>Tải file đính kèm
          </button>
        )}
      </div>
      ...
      <SmartTable
        ...
        tableBodyData={
          ...
          paginate.map((doc) => ({
            ...
            upload_file: (
              <td>
                <a href='#' onClick={() => requestDownloadDocumentFile(doc._id, doc.name, doc.versions.length - 1)}>
                  <u>
                    {doc?.versions?.length && doc?.versions?.[doc?.versions?.length - 1]?.file ? <i className='fa fa-download'></i> : ''}
                  </u>
                </a>
                ...
                <a href='#' onClick={() => showFilePreview(doc.versions.length && doc.versions[doc.versions.length - 1].file)}>
                  <u>
                    {doc?.versions?.length &&
                    doc?.versions?.[doc?.versions?.length - 1]?.file &&
                    checkTypeFile(doc?.versions?.[doc?.versions?.length - 1]?.file) ? (
            ...
            upload_file_scan: (
              <td>
                ...
                <a
                  href='#'
                  onClick={() =>
                    showFilePreview(doc?.versions?.length && doc?.versions?.[doc?.versions?.length - 1]?.scannedFileOfSignedDocument)
                  }
                >
                  <u>
                    {doc?.versions?.length &&
                    doc?.versions?.[doc?.versions?.length - 1]?.scannedFileOfSignedDocument &&
                    checkTypeFile(doc?.versions?.[doc?.versions?.length - 1]?.scannedFileOfSignedDocument) ? (
                      <i className='fa fa-eye'></i>
                    ) : (
                      ''
                    )}
            ...
            views: (
              <td>
                <a href='#modal-file-preview`' onClick={() => showDetailListView(doc)}>
                  {doc?.numberOfView}
                </a>
              </td>
            ),
            downloads: (
              <td>
                <a href='#modal-list-download' onClick={() => showDetailListDownload(doc)}>
                  {doc?.numberOfDownload}
                </a>
              </td>
            ),
            action: (
