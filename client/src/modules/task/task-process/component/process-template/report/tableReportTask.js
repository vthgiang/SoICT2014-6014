  ...
  if (props.info && props.info.length !== 0) {
    for (let i in props.info) {
      ...
      if (props.dataTaskProcess && props.dataTaskProcess.length !== 0) {
        for (let j in props.dataTaskProcess) {
          if (props.dataTaskProcess[j].tasks.find((value) => value.codeInProcess === props.info[i].code)) {
            ...
            if (currentMemberCurrentTaskPoint !== 0 && value1.evaluations.length !== 0)
              currentMemberCurrentTaskPoint = currentMemberCurrentTaskPoint / value1.evaluations.length
      ...
      dataShow[i] = {
        ...
        dunghan: dungHan,
        trehan: treHan,
        maxPoint: minMaxPoint[1],
        minPoint: minMaxPoint[0],
        avgPoint: minMaxPoint[2]
      }
  ...
  return (
    <React.Fragment>
      <div class='box-header'>
        <div class='box-title'>Bảng phân tích thời gian thực hiện và điểm số công việc</div>
      </div>
