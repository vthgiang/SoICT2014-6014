...
function EvaluateByResponsibleEmployee(props) {
    ...
    const { isEval, startDate, endDate, startTime, endTime, info, task, point, oldAutoPoint, autoPoint, evaluatingMonth, date, unit, kpi, showAutoPointInfo, dentaDate, prevDate, indexReRender, evaluation } = state;
    ...
    const isFormValidated = () => {
        const { evaluatingMonth, errorOnDate, errorOnMonth, errorOnStartDate, errorOnEndDate, errorOnPoint, errorOnInfoDate, errorOnTextInfo, errorOnNumberInfo } = state;
        var { info } = state;
                                            ...
                                            actionsNotRating?.map((item, index) => (
                                                <div>
                                                    <span key={index}>
                                                        ({index + 1})&nbsp;&nbsp;
                                                        <QuillEditor
                                                            id={`evaluateByRes${item._id}${props.id}`}
