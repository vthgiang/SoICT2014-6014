...
function ActionForm(props) {
  ...
  const handleAddAction = (event) => {
    ...
    if (!taskActions) taskActions = []
    const newTaskActions = [...taskActions, action]
    ...
    <fieldset className='scheduler-border'>
      ...
      <div className={`form-group ${state.action.errorOnName === undefined ? '' : 'has-error'}`}>
        ...
        <input
          ...
          placeholder={translate('task_template.action_name')}
          value={action.name}
      ...
      <div className='pull-right' style={{ marginBottom: '10px' }}>
        {state.editAction ? (
          <React.Fragment>
            <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleCancelEditAction}>
              {translate('task_template.cancel_editing')}
            </button>
            <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleSaveEditedAction}>
              {translate('task_template.save')}
            </button>
          </React.Fragment>
        ) : (
          <button className='btn btn-success' style={{ marginLeft: '10px' }} onClick={handleAddAction}>
            {translate('task_template.add')}
          </button>
        )}
        <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={handleClearAction}>
      ...
      <table className='table table-hover table-striped table-bordered'>
        <thead>
          <tr>
            ...
            <th style={{ width: '60px' }} title='Bắt buộc'>
              {translate('task_template.mandatory')}
            </th>
            <th style={{ width: '60px' }} title='Hành động'>
              {translate('task_template.action')}
            </th>
          </tr>
        </thead>
        ...
        {typeof taskActions === 'undefined' || taskActions.length === 0 ? (
          ...
          <ReactSortable
            ...
            setList={(newState) => {
              ...
              setState({ ...state, taskActions: newState })
            }}
          >
