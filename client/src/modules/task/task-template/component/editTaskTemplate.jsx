        ...
        if (props.isProcess && props.id !== state.id) {
            ...
            setState(state => {
                return {
                    ...
                    id: props.id,
                    editingTemplate: {
                        organizationalUnit: (info && info.organizationalUnit) ? info.organizationalUnit._id : info.organizationalUnit,
                        collaboratedWithOrganizationalUnits: (info && info.collaboratedWithOrganizationalUnits) ? info.collaboratedWithOrganizationalUnits.map(item => { if (typeof(item)==='object') {return item._id} else {return item}}) : [],
                        name: (info && info.name) ? info.name : '',
                        readByEmployees: info.readByEmployees,
                        responsibleEmployees: (info && info.responsibleEmployees) ? info.responsibleEmployees.map(item => { if (typeof(item)==='object') {return item._id} else {return item} }) : [],
                        accountableEmployees: (info && info.accountableEmployees) ? info.accountableEmployees.map(item => { if (typeof(item)==='object') {return item._id} else {return item} }) : [],
                        consultedEmployees: (info && info.consultedEmployees) ? info.consultedEmployees.map(item => { if (typeof(item)==='object') {return item._id} else {return item} }) : [],
                        informedEmployees: (info && info.informedEmployees) ? info.informedEmployees.map(item => { if (typeof(item)==='object') {return item._id} else {return item} }) : [],
