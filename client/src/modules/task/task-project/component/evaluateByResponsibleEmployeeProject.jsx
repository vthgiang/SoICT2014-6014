...
const EvaluateByResponsibleEmployeeProject = (props) => {
  ...
  const currentUserAutomaticPointInDB = task.overallEvaluation?.responsibleEmployees?.find(
    (item) => String(item.employee?._id) === userId
  )?.automaticPoint
  const currentUserHandPointInDb = task.overallEvaluation?.responsibleEmployees?.find(
    (item) => String(item.employee?._id) === userId
  )?.employeePoint
  ...
  return (
    <React.Fragment>
      <div className='body-evaluation'>
        <ModalShowAutoPointInfoProjectTask task={task} progress={currentProgress} projectDetail={projectDetail} />
          ...
          <div className='row'>
            <div className='col-md-12'>
              ...
              <input
                ...
                type='number'
                value={currentMemberActualCost}
                onChange={(e) => setCurrentMemberActualCost(e.target.value)}
              />
            </div>
          </div>
          <div className='row'>
          ...
          <div className='row'>
            <div className='col-md-12'>
              ...
              <input
                className='form-control'
                type='number'
                value={currentUserHandPoint}
                onChange={(e) => setCurrentUserHandPoint(e.target.value)}
              />
            </div>
          </div>
        </fieldset>
