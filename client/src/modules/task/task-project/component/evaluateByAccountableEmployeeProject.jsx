...
const EvaluateByAccountableEmployeeProject = (props) => {
  ...
  const currentAccUserAutomaticPointInDB = task.overallEvaluation?.accountableEmployees?.find(
    (item) => String(item.employee?._id) === userId
  )?.automaticPoint
  const currentAccUserHandPointInDb = task.overallEvaluation?.accountableEmployees?.find(
    (item) => String(item.employee?._id) === userId
  )?.employeePoint
  ...
  const [currentResHandPointFromAccArr, setCurrentResHandPointFromAccArr] = useState(
    resEvalArr?.map((resItem) => resItem?.accountablePoint || '')
  )
  ...
  const [autoResMemPointArr, setAutoResMemPointArr] = useState(resEvalArr.map((resEvalItem) => resEvalItem.automaticPoint))
    ...
    <React.Fragment>
      <div className='body-evaluation'>
          ...
          <div className='row'>
            ...
            <div className={`col-md-6 col-xs-6 form-group ${error.costInput === undefined ? '' : 'has-error'}`}>
              ...
              <input
                ...
                type='number'
                value={currentActualCostTask}
                onChange={(e) => setCurrentActualCostTask(e.target.value)}
              />
        ...
        <fieldset className='scheduler-border'>
          ...
          <div className='row'>
            <div className={`col-md-12 form-group ${error.costInput === undefined ? '' : 'has-error'}`}>
              ...
              <input
                className='form-control'
                type='number'
                value={currentActualAccMemberCost}
                onChange={(e) => setCurrentActualAccMemberCost(e.target.value)}
              />
              <ErrorLabel content={error.costInput} />
            </div>
          </div>
          <div className='row'>
            <div className='col-md-12'>
              <strong>Điểm tự động đã được lưu:</strong>
              {'  '}
              {checkIsNullUndefined(currentAccUserAutomaticPointInDB)
                ? 'Chưa tính được'
                : numberWithCommas(currentAccUserAutomaticPointInDB)}
            </div>
          </div>
          <div className='row'>
            <div className='col-md-12'>
              ...
              <a style={{ cursor: 'pointer' }} onClick={openModalMemberCalculation}>
                {checkIsNullUndefined(currentAccUserAutomaticPoint) ? 'Chưa tính được' : numberWithCommas(currentAccUserAutomaticPoint)}
              </a>
            </div>
          </div>
          <div className='row'>
            <div className='col-md-12'>
              ...
              <input
                className='form-control'
                type='number'
                value={currentAccUserHandPoint}
                onChange={(e) => setCurrentAccUserHandPoint(e.target.value)}
              />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className='scheduler-border'>
          ...
          <h4>
            <strong>Danh sách người thực hiện</strong>
          </h4>
                ...
                <tr key={resEvalIndex}>
                  ...
                  <td>
                    ...
                    <ErrorLabel
                      style={{ color: 'red' }}
                      content={
                        currentResHandPointFromAccArr[resEvalIndex] > 100 || currentResHandPointFromAccArr[resEvalIndex] < 0
                          ? 'Số phải >= 0 và <= 100'
                          : null
                      }
                    />
