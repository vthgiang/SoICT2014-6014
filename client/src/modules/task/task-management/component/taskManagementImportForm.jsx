...
function TaskManagementImportForm(props) {
    ...
    if (serial && typeof serial === 'string') {
      if (serial.includes('/')) {
        ...
        if (date?.length) {
          let month = date[1],
            day = date[0]
          if (date[1]?.toString()?.length < 2) month = '0' + date[1]
          if (date[0]?.toString()?.length < 2) day = '0' + date[0]
    ...
    value.forEach((o, index) => {
      ...
      let priority = o?.priority ? convertPriority(o.priority) : 3,
        ...
        startDate = o?.startDate
          ? convertDateTime(convertExcelDateToJSDate(o.startDate), o?.startTime ? convertTimeExcelToJSDate(o.startTime) : '08:00 AM')
          : null,
        endDate = o?.endDate
          ? convertDateTime(convertExcelDateToJSDate(o.endDate), o?.endTime ? convertTimeExcelToJSDate(o.endTime) : '05:30 PM')
          : null,
        ...
        collaboratedWithOrganizationalUnits = o?.collaboratedWithOrganizationalUnits
          ? getDataCollaboratedWithUnits(o.collaboratedWithOrganizationalUnits)
          : [],
      ...
      if (
        o?.code === null ||
        o.name === null ||
        o.organizationalUnit === null ||
        (o.organizationalUnit && getIdUnitFromName(o.organizationalUnit) === -1) ||
        convertTaskStatus(o.status) === -1 ||
        o.priority === null ||
        (o.priority && convertPriority(o.priority) === -1) ||
        o.startDate === null ||
        o.endDate === null ||
        o.responsibleEmployees === null ||
        (o.responsibleEmployees &&
          (getUserIdFromEmail(o.responsibleEmployees) === -1 || getUserIdFromEmail(o.responsibleEmployees).indexOf(-1) !== -1)) ||
        o.accountableEmployees === null ||
        (o.accountableEmployees &&
          (getUserIdFromEmail(o.accountableEmployees) === -1 || getUserIdFromEmail(o.accountableEmployees).indexOf(-1) !== -1)) ||
        (o.consultedEmployees &&
          (getUserIdFromEmail(o.consultedEmployees) === -1 || getUserIdFromEmail(o.consultedEmployees).indexOf(-1) !== -1)) ||
        (o.informedEmployees &&
          (getUserIdFromEmail(o.informedEmployees) === -1 || getUserIdFromEmail(o.informedEmployees).indexOf(-1) !== -1)) ||
        (o.collaboratedWithOrganizationalUnits &&
          (getDataCollaboratedWithUnits(o.collaboratedWithOrganizationalUnits) === -1 ||
            getDataCollaboratedWithUnits(o.collaboratedWithOrganizationalUnits).indexOf(-1) !== -1)) ||
        (o.taskProject && getProjectId(o.taskProject) === -1) ||
        checkCodeDuplicateInFileExcell?.length > 1
      ) {
      ...
      if (o.organizationalUnit === null) errorAlert = [...errorAlert, 'Đơn vị quản lý công việc không được để trống']
      ...
      if (o.priority === null) errorAlert = [...errorAlert, 'Độ ưu tiên không được để trống']
      if (o.priority && convertPriority(o.priority) === -1) errorAlert = [...errorAlert, 'Độ ưu tiên không hợp lệ']

      if (o.startDate === null) errorAlert = [...errorAlert, 'Ngày bắt đầu công việc không được để trống']
      if (o.endDate === null) errorAlert = [...errorAlert, 'Ngày kết thúc công việc không được để trống']

      if (o.responsibleEmployees === null) errorAlert = [...errorAlert, 'Người thực hiện công việc không được để trống']
      if (
        o.responsibleEmployees &&
        (getUserIdFromEmail(o.responsibleEmployees) === -1 || getUserIdFromEmail(o.responsibleEmployees).indexOf(-1) !== -1)
      )
      ...
      if (o.accountableEmployees === null) errorAlert = [...errorAlert, 'Người phê duyệt công việc không được để trống']
      if (
        o.accountableEmployees &&
        (getUserIdFromEmail(o.accountableEmployees) === -1 || getUserIdFromEmail(o.accountableEmployees).indexOf(-1) !== -1)
      )
      ...
      if (o.taskProject && getProjectId(o.taskProject) === -1) errorAlert = [...errorAlert, 'Dự án không hợp lệ']
    ...
    if (value) {
      value.forEach((o, index) => {
        ...
        if (
          ...
          o?.description === null ||
          o?.createBy === null ||
          (o.createBy && getUserIdFromEmail(o.createBy) === -1) ||
          (o.createBy && getUserIdFromEmail(o.createBy).indexOf(-1) !== -1)
        ) {
          rowErrorTaskActions = [...rowErrorTaskActions, index + 1]
          o = { ...o, error: true }
        }

        if (o?.taskName === null) errorAlert = [...errorAlert, 'Tên công việc không được để trống']

        if (o?.code === null) errorAlert = [...errorAlert, 'Code không được để trống']

        if (o?.description === null) errorAlert = [...errorAlert, 'Tên hoạt động không được để trống']
        if (o?.createBy === null) {
  ...
  const handleImportTaskTimesheetLogsExcell = (value, checkFileImport = true) => {
    ...
    let valueImportTaskTimesheetLog = [],
    ...
    if (value) {
      value.forEach((o, index) => {
        ...
        if (o?.code === null) errorAlert = [...errorAlert, 'TaskId công việc không được để trống']

        if (o?.taskName === null) errorAlert = [...errorAlert, 'Tên công việc không được để trống']

        if (o?.createdBy === null) errorAlert = [...errorAlert, 'Người bấm giờ không được để trống']
  ...
  const handleImportUpdate = () => {
    ...
    if (valueImport?.length) props.importTasks({ importType: 'import_update_task_info', importData: { valueImport } })
  }

  const handleImportTaskActions = () => {
    const { valueImportTaskActions } = state
    if (valueImportTaskActions?.length) props.importTasks({ importType: 'import_task_actions', importData: valueImportTaskActions })
  }
  ...
  const note = (
    <p className='text-left'>
      <span className='text-red'>Sau khi tiến hành import xong, F5 lại trang để xem dữ liệu vừa cập nhật</span>
    </p>
  )
  ...
  const isFormValidate = () => {
    ...
    return rowErrorTaskTimesheetLogs?.length === 0 && rowErrorTaskActions?.length === 0 && rowError?.length === 0
  }
  ...
  if (tasks?.error?.generalInfo?.rowError?.length) {
    ...
    let data = [],
      dataLength = showValueImportGeneralInfo?.length
    for (let i = 0; i < dataLength; i++) {
      data = [
        ...data,
        {
          ...showValueImportGeneralInfo[i],
          errorAlert: tasks?.error?.generalInfo?.data
            ? tasks.error.generalInfo.data[i].errorAlert
            : showValueImportGeneralInfo[i].errorAlert,
    ...
    >
      <div className='box-body row'>
        <div className='row'>
          ...
          <div className='form-group col-md-6 col-xs-12'>
            ...
            <UploadFile importFile={handleUploadFile} />
            <a style={{ cursor: 'pointer' }} onClick={handleDownloadFileImport}>
              <u>
                Tải xuống file mẫu<i className='fa fa-fw fa-file-excel-o'> </i>
              </u>{' '}
            </a>
          </div>
          <div className='form-group col-md-6 col-xs-12'>
      ...
      <div className='nav-tabs-custom row'>
        <ul className='nav nav-tabs'>
          <li className='active'>
            <a data-toggle='tab' href='#import_task_general'>
              Thông tin chung
            </a>
          </li>
          <li>
            <a data-toggle='tab' href='#import_task_actions'>
              {' '}
              Thông tin hoạt động
            </a>
          </li>
          <li>
            <a data-toggle='tab' href='#import_timesheetlogs'>
              {' '}
              Thông tin bấm giờ
            </a>
          </li>
