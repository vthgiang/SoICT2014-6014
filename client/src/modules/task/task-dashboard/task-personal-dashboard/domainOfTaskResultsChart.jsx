...
function DomainOfTaskResultsChart(props) {
  ...
  useEffect(() => {
    if (TaskOrganizationUnitDashboard) {
      ...
      let month = [],
        maxResults = [translate('task.task_management.dashboard_max')],
        minResults = [translate('task.task_management.dashboard_min')]
    ...
    state.role,
    state.typePoint,
    JSON.stringify(taskDashboardCharts?.['task-results-domain-chart']),
  ...
  useEffect(() => {
    if (state?.dataChart) domainChart()
  }, [state.dataChart])
  ...
  const handleSearchData = () => {
    ...
    //dashboard cv cá nhân
      ...
      setState({
        //setState để component render lại, vẽ ra biểu đồ mới
        ...state,
        role: DATA_SEARCH.role,
        typePoint: DATA_SEARCH.typePoint
      })
