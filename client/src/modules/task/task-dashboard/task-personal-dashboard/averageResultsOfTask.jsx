...
function AverageResultsOfTask(props) {
      ...
      listTask.map((task) => {
        task.evaluations
          .filter((evaluation) => {
            ...
            if (dayjs(evaluatingMonth).isBetween(startMonth, endMonth, null, '[]')) {
              // '[]': tham số này check cho phép evaluatingMonth = startMonth hoặ startMonth = endMOnth, ko muốn thì set '()'
              return 1
            }
              ...
              .map((result) => {
                if (criteria === CRITERIA.COEFFICIENT) {
                  ...
                  if (result.automaticPoint && result?.taskImportanceLevel) {
                    sumAutomaticPointCoefficient =
                      sumAutomaticPointCoefficient + result.automaticPoint * result.taskImportanceLevel * totalDay
  ...
  useEffect(() => {
    ...
    let months = [],
      averageAutomatic = [translate('task.task_management.detail_auto_point')],
      averageEmployee = [translate('task.task_management.detail_emp_point')],
      averageApproved = [translate('task.task_management.detail_acc_point')]
    ...
    if (months?.length)
      setState({
        ...state,
        dataChart: [months, averageAutomatic, averageEmployee, averageApproved]
      })
  }, [
    JSON.stringify(props?.tasks?.responsibleTasks),
    JSON.stringify(props?.tasks?.accountableTasks),
    JSON.stringify(props?.tasks?.consultedTasks),
    state.role,
    state.criteria,
    props?.startMonth,
    props?.endMonth
  ...
  useEffect(() => {
    if (state?.dataChart) averageChart()
  }, [state.dataChart])
