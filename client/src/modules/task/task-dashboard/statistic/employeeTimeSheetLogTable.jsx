...
function EmployeeTimeSheetLogTable(props)
{
    ...
    useEffect(() => {
        ...
        if (allUnit.length > 0) { // Kiểm tra đã lấy được danh sách đơn vị chưa, nếu chưa lấy được thì chưa gửi request
                            ...
                            allTimeSheetLogs?.docs?.map((tsl, index) => {
                                ...
                                return (
                                    <tr key={index}>
                                        ...
                                        <td style={{ whiteSpace: 'pre-wrap'}} className="col-sort">{parse(str)}</td>
