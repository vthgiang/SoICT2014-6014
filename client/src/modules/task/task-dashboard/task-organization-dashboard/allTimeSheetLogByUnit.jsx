...
function AllTimeSheetLogsByUnit(props) {
  ...
  const convertDataExport = (data) => {
    ...
    const listUnitSelect = props.unitsSelected.map((o, index) => ({ STT: index + 1, name: o }))
    ...
    if (exportData) props.handleDataTimeSheetsExport(exportData)
  }
  ...
  return (
    <React.Fragment>
      <DataTableSetting tableId={'all-time-sheet-logs'} setLimit={setLimitAllTimeSheetLogs} />
      ...
      <React.Fragment>
        <table className='table table-hover table-striped table-bordered' id='table-user-timesheetlogs'>
          ...
          <tbody>
            {allTimeSheet?.length > 0 &&
              allTimeSheet.map((tsl, index) => {
                return (
                  <tr key={index}>
                    <td>{index + 1}</td>
                    <td>
                      <a onClick={() => handleInforTimeSheet(tsl, filterTimeSheetLogs)}>{tsl.name}</a>
                    </td>
      ...
      <PaginateBar display={display} total={total} pageTotal={pageTotal} currentPage={page} func={handlePaginationAllTimeSheetLogs} />
    </React.Fragment>
  )
}
