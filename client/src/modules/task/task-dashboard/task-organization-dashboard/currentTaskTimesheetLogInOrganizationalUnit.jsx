        ...
        <div className='box-title'>
          ...
          {organizationalUnitIds?.length < 2 ? (
            <>
              ...
              <span>{` ${props
                .getUnitName(listUnitSelect, organizationalUnitIds)
                .map((o) => o)
                .join(', ')}`}</span>
            </>
          ) : (
      ...
      <div className='box-body' style={{ marginBottom: 15 }}>
        <div className='qlcv'>
          <table className='table table-bordered table-striped table-hover'>
            <thead>
              <tr>
                <th title={translate('kpi.organizational_unit.create_organizational_unit_kpi_set.no_')} style={{ width: '60px' }}>
                  {translate('kpi.organizational_unit.create_organizational_unit_kpi_set.no_')}
                </th>
                <th title={translate('kpi.organizational_unit.kpi_organizational_unit_manager.employee_name')}>
                  {translate('kpi.organizational_unit.kpi_organizational_unit_manager.employee_name')}
                </th>
              ...
              {performtasks.loading ? (
                ...
                currentTaskTimesheetLog.map((item, index) => (
                  <tr key={item._id}>
                    ...
                    <td title={item?.task?.name}>
                      <a href={`/task?taskId=${item?.task?._id}`} target='_blank'>
                        {item?.task?.name}
                      </a>
                    </td>
...
const connectedCurrentTaskTimesheetLogInOrganizationalUnit = connect(
  mapState,
  actions
)(withTranslate(CurrentTaskTimesheetLogInOrganizationalUnit))
