...
function ResultsOfAllOrganizationalUnitKpiChart(props) {
    ...
    useEffect(() => {
        ...
        if (createKpiUnit?.organizationalUnitKpiSetsOfChildUnit?.length > 0 &&
            !createKpiUnit.loading &&
            childOrganizationalUnit?.length) {
            ...
            if (singleUnit) {
                ...
                for (let x = 0; x < organizationalUnitKpiSetsOfChildUnitLength; x++) {
                    if (
                        childOrganizationalUnit[0].name ===
                    ...
                    createKpiUnit.organizationalUnitKpiSetsOfChildUnit.forEach((o, index) => {
                        if (kindOfPoint === 1) {
                            if (o.length > 1) {
                                for (let i = 1; i < o.length; i++) {
                                    if (fullnameUnit.includes(o[0]?.name))
                                        dataChart = [...dataChart, o[i]?.automaticPoint]
                                }
                            } else {
                                if (fullnameUnit.includes(o[0]?.name))
                        ...
                        if (kindOfPoint === 2) {
                            if (o.length > 1) {
                                for (let i = 1; i < o.length; i++) {
                                    if (fullnameUnit.includes(o[0]?.name))
                                        dataChart = [...dataChart, o[i]?.employeePoint]
                                }
                            } else {
                                if (fullnameUnit.includes(o[0]?.name))
                                    dataChart = [...dataChart, null]
                        ...
                        if (kindOfPoint === 3) {
                            if (o.length > 1) {
                                for (let i = 1; i < o.length; i++) {
                                    if (fullnameUnit.includes(o[0]?.name))
                                        dataChart = [...dataChart, o[i]?.approvedPoint]
                                }
                            } else {
                                if (fullnameUnit.includes(o[0]?.name))
                                    dataChart = [...dataChart, null]
    ...
    return (
        <div className="box box-solid">
            <div className="box-header with-border">
                    ...
                    {translate('kpi.organizational_unit.dashboard.result_kpi_unit')}
                    {
                        singleUnit ?
                            <>
                                ...
                                {date?.startDate}<i className="fa fa-fw fa-caret-right"></i>{date?.endDate}
                            </>
                            :
                ...
                {
                    !singleUnit &&
                    <a title={advancedMode ? "Tắt chế độ nâng cao" : "Bật chế độ nâng cao"}><i className="fa fa-gear" onClick={handleToggleAdvanceMode} style={{ fontSize: 19, float: 'right', cursor: 'pointer' }}></i></a>
                }
            </div>
...
const connectedResultsOfAllOrganizationalUnitKpiChart = React.memo(connect(mapState, actions)(withTranslate(ResultsOfAllOrganizationalUnitKpiChart)));
