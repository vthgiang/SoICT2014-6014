...
function StatisticsTaskUnits(props) {
  ...
  const showNodeContent = (translate, data) => {
    ...
    return (
      <div
        key={data.organizationalUnitName}
      ...
      >
        <div>
          ...
          <div className='progress-weigth-kpi' style={{ backgroundColor: data.parent_id ? '#8a8a8a' : '#676767' }}>
            <div
              className='fillmult'
              data-width={`${pointShow}%`}
              style={{ width: `${pointShow}%`, backgroundColor: pointShow < 50 ? '#dc0000' : 'rgb(76 179 99)' }}
            ></div>
            <a
              title={titleShow}
              className='perc'
          ...
          <a
            className='kpi-number-target'
            data-toggle='modal'
            data-target='#modal-task-detail'
            data-backdrop='static'
            data-keyboard='false'
            onClick={() => handleShowTask(data.tasks)}
            style={{ fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}
          >
            {translate('kpi.organizational_unit.dashboard.trend_chart.amount_tasks')}: {data?.tasks?.length}
          </a>
        </div>
      </div>
    )
  ...
  const displayTreeView = (translate, data) => {
    if (data) {
      if (!data.children) {
        return <li key={data.id}>{showNodeContent(translate, data)}</li>
      }

      return (
        <li key={data.id}>
          {showNodeContent(translate, data)}
          <ul>{data.children.map((tag) => displayTreeView(translate, tag))}</ul>
        </li>
      )
    } else {
      return null
      ...
      {chartData?.isLoading ? (
        ...
        treeTaskUnits.map((tree, index) => (
          <div key={index} className='tf-tree example' style={{ display: 'flex', justifyContent: 'flex-start', marginTop: '50px' }}>
            <ul>{displayTreeView(translate, tree)}</ul>
          </div>
        ))
      ) : (
