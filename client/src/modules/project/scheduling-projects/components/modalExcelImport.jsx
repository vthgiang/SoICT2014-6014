...
const ModalExcelImport = (props) => {
  ...
  const handleCheckDataSuitable = (data) => {
    ...
    for (let dataItem of data) {
        ...
        for (let preItem of dataItem.preceedingTasks) {
          if (dataItem?.preceedingTasks?.filter((dataItemPreceedingItem) => preItem === dataItemPreceedingItem).length > 1) {
            setCurrentMessageError(
              `Danh sách công việc tiền nhiệm của công việc ${dataItem.name} đang có sự trùng nhau: ${dataItem.preceedingTasks.join(', ')}`
            )
            return
          }
        }
      ...
      if (nonExistentCode.length > 0) {
        setCurrentMessageError(
          `Mã công việc tiền nhiệm của công việc ${dataItem.name} không hợp lệ : Không tồn tại (các) công việc với mã công việc : ${nonExistentCode.join(', ')}`
        )
        return
      }
    }
  ...
  const getTaskDataImportCPM = (data) => {
    let resultData = []
    let formatPreceedingTasks = [],
      formatEmailResponsibleEmployees = [],
      formatEmailAccountableEmployees = []
    ...
    while (dataIndex < data.length) {
      ...
      if (!code && !name && !projectPhase && !estimateOptimisticTime && !estimateNormalTime && !totalResWeight) {
        ...
        while (dataAdditionRowIndex < data.length) {
          // Nếu các thành phần trên rỗng thì mảng nào push mảng nấy
          if (
            data[dataAdditionRowIndex].code &&
            data[dataAdditionRowIndex].name &&
            data[dataAdditionRowIndex].estimateOptimisticTime &&
            data[dataAdditionRowIndex].estimateNormalTime &&
        ...
        formatEmailResponsibleEmployees =
          emailResponsibleEmployees === null || emailResponsibleEmployees === '' || emailResponsibleEmployees === undefined
            ? []
            : [emailResponsibleEmployees]
        formatEmailAccountableEmployees =
          emailAccountableEmployees === null || emailAccountableEmployees === '' || emailAccountableEmployees === undefined
            ? []
            : [emailAccountableEmployees]
        resultData.push({
  ...
  const getPhaseDataImportCPM = (data) => {
    let resultData = []
    let formatEmailResponsibleEmployees = [],
      formatEmailAccountableEmployees = []
    ...
    <React.Fragment>
      ...
      >
        <div className='description-box without-border'>
          <h4>
            <strong>Các bước upload file excel danh sách công việc dự án:</strong>
          </h4>
          <div>
            ...
            <ExportExcel
              ...
              type='link'
              id='downloadTemplateImport-cpm'
              buttonName={`Tải file template excel danh sách công việc dự án`}
              exportData={convertDataCPMExport(dataImportTemplate)}
            />
          </div>
          ...
          <div>
            ...
            <div style={{ marginTop: 10 }}>
              <div className='form-group'>
                <UploadFile callFunctionDeleteFile={handleResetModal} importFile={handleUploadFile} />
              </div>
