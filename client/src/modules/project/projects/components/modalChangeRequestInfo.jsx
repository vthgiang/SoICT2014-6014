...
import {
    convertDepartmentIdToDepartmentName, convertUserIdToUserName, getEndDateOfProject, getEstimateCostOfProject,
    getListDepartments, getNewTasksListAfterCR, MILISECS_TO_DAYS, MILISECS_TO_HOURS, processAffectedTasksChangeRequest,
    processDataTasksStartEnd, renderItemLabelContent
} from './functionHelper';
                    ...
                    <tbody>
                        {
                            ...
                            changeRequest?.affectedTasksList?.map((affectedItem, affectedIndex) => {
                                const isEditTaskAndCurrentTask = type === 'edit_task' && String(changeRequest?.currentTask?.task) === String(affectedItem?.task);
                                return (
                                    <>
                                        <tr style={{ backgroundColor: isEditTaskAndCurrentTask  ? 'green' : 'f1f1f1' }}>
                                            ...
                                            <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.task ? 'Chỉnh sửa' : 'Thêm mới'}</td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                        </tr>
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.preceedingTasks && affectedItem?.old?.preceedingTasks?.map((preItem) => getTaskName(currentProjectTasks, preItem.task)).join(', ')}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.new?.preceedingTasks?.map((preItem) => getTaskName(currentProjectTasks, preItem.task)).join(', ')}</td>
                                            </tr>
                                        }
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                <td></td>
                                                <td></td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>Thời gian ước lượng ({translate(`project.unit.${projectDetail?.unitTime}`)})</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.estimateNormalTime && Number(affectedItem?.old?.estimateNormalTime) / (projectDetail?.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{Number(affectedItem?.new?.estimateNormalTime) / (projectDetail?.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)}</td>
                                            </tr>
                                        }
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.startDate && moment(affectedItem?.old?.startDate).format('HH:mm DD/MM/YYYY')}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{moment(affectedItem?.new?.startDate).format('HH:mm DD/MM/YYYY')}</td>
                                            </tr>
                                        }
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.endDate && moment(affectedItem?.old?.endDate).format('HH:mm DD/MM/YYYY')}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{moment(affectedItem?.new?.endDate).format('HH:mm DD/MM/YYYY')}</td>
                                            </tr>
                                        }
                                        {/* {
                                                    ...
                                                    } */}
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.estimateNormalCost && numberWithCommas(Number(affectedItem?.old?.estimateNormalCost))}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{numberWithCommas(Number(affectedItem?.new?.estimateNormalCost))}</td>
                                            </tr>
                                        }
                                        {/* {
                                                    ...
                                                    } */}
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.responsibleEmployees && affectedItem?.old?.responsibleEmployees?.map((resItem) => convertUserIdToUserName(listUsers, resItem)).join(', ')}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.new?.responsibleEmployees?.map((resItem) => convertUserIdToUserName(listUsers, resItem)).join(', ')}</td>
                                            </tr>
                                        }
                                        {
                                            ...
                                            <tr style={{ backgroundColor: isEditTaskAndCurrentTask ? 'green' : 'white' }}>
                                                ...
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.old?.accountableEmployees && affectedItem?.old?.accountableEmployees?.map((accItem) => convertUserIdToUserName(listUsers, accItem)).join(', ')}</td>
                                                <td style={{ color: isEditTaskAndCurrentTask ? 'white' : 'black' }}>{affectedItem?.new?.accountableEmployees?.map((accItem) => convertUserIdToUserName(listUsers, accItem)).join(', ')}</td>
                                            </tr>
                                        }
                                    </>
                                )
                ...
                <div>
                    ...
                    <div className="box">
                        <div className="box-body qlcv">
                            ...
                            {changeRequest?.type === 'edit_task' && <span style={{ color: 'green' }}>*Vùng tô màu xanh là Công việc mà người dùng muốn sửa</span>}
