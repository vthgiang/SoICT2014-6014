...
function ProjectTreeTable(props) {
    const { translate, column, data, actions = true, tableId = 'project-tree-table', tableSetting = false, rowPerPage = true, allowSelectAll = false, viewWhenClickName = false, funcEditTask, funcEditPhase, titleAction, performtasks } = props;
    ...
    const showActionColumn = (actionData, id, type) => {
        ...
        if (type === 'phase') {
            switch (actionData) {
                case "edit":
                    return <a style={{ cursor: 'pointer' }} onClick={() => props.funcEditPhase(id)} className="edit" data-toggle="modal" title={titleAction.edit}>
                        <i className="material-icons"></i>
                    </a>
                case "view":
                    return <a style={{ cursor: 'pointer' }} onClick={() => props.funcViewPhase(id)} data-toggle="modal" title={titleAction.view}>
                                            ...
                                            rows.action && rows.action.map((x, index) => Array.isArray(x) ?
                                                <React.Fragment key={index}>
                                                    ...
                                                    <div id={`actionTask${rows._id}`} className="collapse">
                                                        {x.map((y, index) => (
                                                            <React.Fragment key={index}>
                                                                {showActionColumn(y, rows._id, rows.type)}
                                                            </React.Fragment>
                                                        ))}
                                                    </div>
                                                </React.Fragment> :
                                                <React.Fragment key={index}>
                                                    {showActionColumn(x, rows._id, rows.type)}
                                                </React.Fragment>
                                            )
                                        }
                                    </td>
