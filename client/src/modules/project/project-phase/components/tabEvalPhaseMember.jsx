...
const TabEvalPhaseMember = (props) => {
    ...
    for (let memberItem of responsibleEmployees) {
      ...
      if (tasksWithMemberArr.length === 0) continue
        ...
        for (let tasksWithMemberItem of tasksWithMemberArr) {
          ...
          const currentMemberEvalRes = tasksWithMemberItem?.overallEvaluation?.responsibleEmployees.find(
            (resEval) => String(resEval.employee) === String(memberItem.id)
          )
          const currentMemberEvalAcc = tasksWithMemberItem?.overallEvaluation?.accountableEmployees.find(
            (accEval) => String(accEval.employee) === String(memberItem.id)
          )
          if (currentMemberEvalRes) {
            ...
            let curAccountablePoint = checkIsNullUndefined(currentMemberEvalRes.accountablePoint)
              ? 0
              : currentMemberEvalRes.accountablePoint
          ...
          // Check currentRole
          if (tasksWithMemberItem.responsibleEmployees.find((resItem) => String(resItem.id) === String(memberItem.id)))
  ...
  return (
    <React.Fragment>
      <div className='box-body qlcv'>
        <h4>
          <strong>Điểm số của các thành viên trong tháng</strong>
        </h4>
        {/* Chọn tháng để lọc đánh giá */}
        <div className='form-group'>
          <label style={{ marginRight: 20 }}>Chọn tháng</label>
          <DatePicker
            id='start-date-eval-phase-members-statistical'
        ...
        {isLoading ? (
          ...
          <table id='eval-phase-members-statistical-table' className='table table-bordered table-hover'>
            <thead>
              <tr>
                <th>Họ và tên</th>
                    ...
                    return (
                      <tbody key={`member-summary-${memberItem.id}`}>
                        <tr key={`${memberItem.id}-${memberTaskItem?.tasksWithMemberItem.name}-${memberIndex}-${memberTaskIndex}-0`}>
                          <td>
                            <strong>{memberTaskIndex === 0 ? memberItem?.name : ''}</strong>
                          </td>
                          <td style={{ color: renderStatusColor(memberTaskItem?.tasksWithMemberItem) }}>
                            {formatTaskStatus(translate, memberTaskItem?.tasksWithMemberItem?.status)}
                          </td>
                          ...
                          <td>
                            {memberTaskItem?.tasksWithMemberItem?.actualEndDate &&
                              memberTaskItem?.tasksWithMemberItem?.status === 'finished' &&
                              moment(memberTaskItem?.tasksWithMemberItem?.actualEndDate).format('HH:mm DD/MM/YYYY')}
                          </td>
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslate(TabEvalPhaseMember))
