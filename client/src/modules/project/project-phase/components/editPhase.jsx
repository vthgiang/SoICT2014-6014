...
const PhaseEditForm = (props) => {
  ...
  const isFormValidated = () => {
    if (
      !ValidationHelper.validateName(translate, phaseName, 6, 255).status ||
      errorOnStartDate ||
      errorOnEndDate ||
      errorOnStatus ||
      errorOnProgress ||
  ...
  const freeTask = currentProjectTasks
    ?.filter((task) => !task.taskPhase || task.taskPhase === phaseEditId)
    .map((task) => {
      return {
        value: task._id,
        text: task.name
          ...
          <div className={`col-sm-6 col-md-6 col-xs-12`}>
            <fieldset className='scheduler-border'>
              ...
              <div className='pull-right'>
                <a onClick={calculateProgress} style={{ cursor: 'pointer', fontWeight: 'normal' }}>
                  Tự động tính mức độ hoàn thành dựa trên các công việc và cột mốc thành phần
                </a>
              </div>
            </fieldset>
          </div>
