            ...
            >
                <div className="nav-tabs-custom" style={{ marginTop: '-15px' }}>
                    ...
                    <ul className="nav nav-tabs">
                        ...
                        <li><a title={translate('manage_delegation.log_activity_task')} data-toggle="tab" href={`#detail_log_activity_task_actions`}>{translate('manage_delegation.log_activity_task')}</a></li>
                        <li><a title={translate('manage_delegation.log_timesheet_task')} data-toggle="tab" href={`#detail_log_activity_task_timesheet`}>{translate('manage_delegation.log_timesheet_task')}</a></li>
                        <li><a title={translate('manage_delegation.log_history_tab')} data-toggle="tab" href={`#detail_log_activity_task_history`}>{translate('manage_delegation.log_history_task')}</a></li>
                    </ul>
                    ...
                    <div className="tab-content">
                                        ...
                                        getTaskActions(delegateTask.taskActions).map((item, index) =>
                                            <div key={item._id} className={index > 3 ? "hide-component" : ""}>
                                                ...
                                                <ul className="list-inline tool-level1">
                                                    ...
                                                    {item.creator &&
                                                        <React.Fragment>
                                                            {item.evaluations && <li><a style={{ cursor: "pointer", pointerEvents: item.evaluations.length > 0 ? "" : "none" }} className="link-black text-sm show-evaluation" onClick={() => { handleShowEvaluations(item._id) }}><i className="fa fa-thumbs-o-up margin-r-5"></i>{translate("task.task_perform.evaluation")} ({item.evaluations && item.evaluations.length})</a></li>}
                                                        </React.Fragment>
                                                    }
                                                </ul>
                                                        ...
                                                        <div style={{ marginBottom: "10px" }}>
                                                                        ...
                                                                        item.evaluations.map((element, index) => {
                                                                            return (
                                                                                <div key={index}>
                                                                                    ...
                                                                                    <span> {translate('task.task_management.evaluation_score')}:<span className="text-red"> {element?.rating}/10</span> - {translate('kpi.evaluation.employee_evaluation.importance_level')}:<span className="text-red"> {element?.actionImportanceLevel}/10</span></span>
                                                                                    &ensp;
                                                            ...
                                                            {Array.isArray(item?.evaluations) && item?.evaluations?.filter(element => element.role === "accountable").length > 0 &&
                                                                <p>
                                                                    ...
                                                                    <span> {translate('task.task_management.evaluation_score')}:<span className="text-red"> {item?.rating}/10</span> - {translate('kpi.evaluation.employee_evaluation.importance_level')}:<span className="text-red"> {item?.actionImportanceLevel}/10</span></span>
                                                                </p>
                                                            }
                                                        </div>
                                                    }
                        ...
                        <div id={`detail_log_activity_task_timesheet`} className="tab-pane">
                            {delegateTask && delegateTask.timesheetLogs.filter(a => a.creator._id == delegatee._id).length > 0 ?
                                ...
                                >
                                    {
                                        delegateTask.timesheetLogs.filter(a => a.creator._id == delegatee._id).map((item, index) =>
                        ...
                        <div id={`detail_log_activity_task_history`} className="tab-pane">
                            {delegateTask && delegateTask.logs.filter(a => a.creator._id == delegatee._id).length > 0 ?
                                ...
                                >
                                    {
                                        delegateTask.logs.filter(a => a.creator._id == delegatee._id).map((item, index) =>
                                            <React.Fragment key={index}>
                                                {item.createdAt &&
                                                    <div key={item._id} className={`item-box ${index > 3 ? "hide-component" : ""}`}>
                                                        <a style={{ fontWeight: 700, cursor: "pointer" }}>{item.creator?.name} </a>
