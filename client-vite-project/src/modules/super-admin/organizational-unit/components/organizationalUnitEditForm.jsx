...
function DepartmentEditForm(props) {
  ...
  const isFormValidated = () => {
    ...
    let { translate } = props
    if (
      !ValidationHelper.validateName(translate, departmentName).status ||
      !ValidationHelper.validateDescription(translate, departmentDescription).status
    )
      return false
    return true
  }
        ...
        <div id='form-edit-department'>
          ...
          <fieldset className='scheduler-border'>
            <legend className='scheduler-border'>
              <span>{translate('manage_department.info')}</span>
            </legend>

            {/* Tên đơn vị */}
            <div className={`form-group ${!departmentNameError ? '' : 'has-error'}`}>
              <label>
                {translate('manage_department.name')}
            ...
            <div className={`form-group ${!departmentDescriptionError ? '' : 'has-error'}`}>
              <label>
                {translate('manage_department.description')}
                <span className='attention'> * </span>
              </label>
            ...
            <div className='form-group'>
              ...
              <SelectBox
                ...
                items={[
                  ...
                  ...department.list
                    .filter((department) => department && department._id !== departmentId)
                    .map((department) => {
                      return { value: department ? department._id : null, text: department ? department.name : '' }
                    })
                ]}
          ...
          <fieldset className='scheduler-border'>
            <legend className='scheduler-border'>
              <span>{translate('manage_department.roles_of_department')}</span>
            </legend>
            ...
            <div className='form-group'>
              <table className='table table-hover table-striped table-bordered'>
                <thead>
                  <tr>
                    <th>
                      <label>{translate('manage_department.manager_name')}</label>
                    </th>
                    <th style={{ width: '40px' }} className='text-center'>
                      <a href='#add-manager' className='text-green' onClick={handleAddManager}>
            ...
            <div className='form-group'>
              <table className='table table-hover table-striped table-bordered'>
                <thead>
                  <tr>
                    <th>
                      <label>{translate('manage_department.deputy_manager_name')}</label>
                    </th>
                    <th style={{ width: '40px' }} className='text-center'>
                      <a href='#add-vicemanager' className='text-green' onClick={handleAddDeputyManager}>
                        <i className='material-icons'>add_box</i>
                      </a>
                    </th>
                  </tr>
                    ...
                    deputyManagers.map((vicemanager, index) => {
                      return (
                        <tr key={`vicemanager-add-${index}`}>
                          ...
                          <td>
                            <a
                              href='#delete-vice-manager'
                              className='text-red'
                              style={{ border: 'none' }}
                              onClick={() => handleRemoveDeputyManager(index)}
                            >
                              <i className='fa fa-trash'></i>
                            </a>
                          </td>
              ...
              <table className='table table-hover table-striped table-bordered'>
                <thead>
                  <tr>
                    ...
                    <th style={{ width: '40px' }} className='text-center'>
                      <a href='#add-employee' className='text-green' onClick={handleAddEmployee}>
                        <i className='material-icons'>add_box</i>
                      </a>
                    </th>
                  </tr>
