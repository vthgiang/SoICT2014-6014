...
function RoleTable(props) {
  ...
  const handleCheck = () => {
    ...
    roleDuplicateName = [...new Set(allRoleNames.filter((value, index, self) => self.indexOf(value) !== index))]
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ display: 'flex', marginBottom: 6, float: 'right' }}>
        <button style={{ marginRight: 10 }} type='button' className='pull-right btn btn-info' onClick={handleCheck}>
          Kiểm tra
        </button>
      </div>
      ...
      {/* Thanh tìm kiếm */}
      <SearchBar
        columns={[{ title: translate('manage_role.name'), value: 'name' }]}
        option={option}
        setOption={setOption}
          ...
          <tr>
            ...
            <th style={{ width: '120px', textAlign: 'center' }}>
              ...
              <DataTableSetting
                ...
                columnArr={[translate('manage_role.name'), translate('manage_role.extends'), translate('manage_role.users')]}
                setLimit={setLimit}
                tableId={tableId}
              />
            </th>
        ...
        <tbody>
          {role.listPaginate &&
            ...
            role.listPaginate.map((role) => (
              <tr
                key={`roleList${role._id}`}
                style={
                  roleDuplicateName && roleDuplicateName.includes(role.name.trim().toLowerCase().replaceAll(' ', ''))
              ...
              >
                <td> {role.name} </td>
                <td>
                  <ToolTip dataTooltip={role?.parents?.length ? role.parents.map((parent) => (parent ? parent.name : '')) : []} />
                </td>
                <td>
                  <ToolTip
                    dataTooltip={
                      role?.users?.length
                        ? role.users.filter((user) => user && user.userId && !user.delegation).map((user) => user.userId.name)
                        : []
                    }
                  />
                </td>
                <td style={{ textAlign: 'center' }}>
                  <a className='edit' href={`#${role._id}`} onClick={() => handleEdit(role)}>
      ...
      {/* PaginateBar */}
      <PaginateBar
        display={role.listPaginate.length}
        total={role.totalDocs}
        pageTotal={role.totalPages}
        currentPage={role.page}
        func={setPage}
      />
    </React.Fragment>
  )
