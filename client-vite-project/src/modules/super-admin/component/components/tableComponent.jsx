    ...
    <React.Fragment>
      ...
      {currentRow && (
        <ComponentInfoForm
          ...
          componentRoles={currentRow.roles
          ...
          {component.listPaginate &&
            ...
            component.listPaginate.map((component) => (
              <tr key={component._id}>
                ...
                <td>
                  <ToolTip
                    dataTooltip={component.roles
                      .filter((role) => role && role.roleId && (!role.policies || role.policies.length == 0))
                      .map((role) => role.roleId.name)}
                  />
                </td>
                <td style={{ textAlign: 'center' }}>
                  <a className='edit' onClick={() => handleEdit(component)}>
      ...
      {component.isLoading ? (
        ...
        component.listPaginate &&
        component.listPaginate.length === 0 && <div className='table-info-panel'>{translate('confirm.no_data')}</div>
      )}
      {/* PaginateBar */}
      <PaginateBar
        display={component.listPaginate.length}
        total={component.totalDocs}
        pageTotal={component.totalPages}
