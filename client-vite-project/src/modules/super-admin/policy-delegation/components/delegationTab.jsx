  ...
  return (
    <div id={props.id} className='tab-pane'>
      ...
      <AttributeAddForm
        ...
        translation={
          state.delegateType == 'Role' ? 'manage_delegation_policy.delegatedObject' : 'manage_delegation_policy.delegatedObjectTask'
        }
        policyID={state.policyID}
      ...
      <div className='form-group row'>
        <div className={`col-lg-12 col-md-12 col-ms-12 col-xs-12`}>
          <label style={{ paddingLeft: '9px' }} className={`col-lg-3 col-md-3 col-ms-4 col-xs-4`}>
            {translate('manage_delegation_policy.delegateType')}:
          </label>
          <div style={{ display: 'inline-block' }} className={`col-lg-9 col-md-9 col-ms-8 col-xs-8`}>
            <div className='radio-inline '>
              <label style={{ fontWeight: 'normal' }}>
                <input
                  type='radio'
                  name={`delegateType${state?.policyID}`}
                  value='Role'
                  onChange={handleDelegateTypeChange}
                  checked={state.delegateType == 'Role' ? true : false}
                />
                &nbsp;&nbsp;{translate('manage_delegation_policy.delegateTypeRole')}
              </label>
            </div>
            <div className='radio-inline ' style={{ marginLeft: '38px' }}>
              <label style={{ fontWeight: 'normal' }}>
                <input
                  ...
                  value='Task'
                  onChange={handleDelegateTypeChange}
                  checked={state.delegateType === 'Task' ? true : false}
                />
                &nbsp;&nbsp;{translate('manage_delegation_policy.delegateTypeTask')}
              </label>
            </div>
          </div>
        </div>
      ...
      <table className='table table-bordered policy-attribute-table not-sort'>
        ...
        <tbody>
            ...
            <td rowSpan={!delegatedObjectAttributes || delegatedObjectAttributes.length == 0 ? 1 : delegatedObjectAttributes.length}>
              {!delegatedObjectAttributes || delegatedObjectAttributes.length == 0 ? (
                <a
                  href='#add-attributes'
                  className='text-green'
                  onClick={() => window.$(`#modal-add-attribute-${props.id}-delegatedObject`).modal('show')}
                  title={
                    delegateType == 'Role'
                      ? translate('manage_delegation_policy.add_delegatedObject_attribute')
                      : translate('manage_delegation_policy.add_delegatedObject_attributeTask')
                  }
                >
                  <i className='material-icons'>add_box</i>
                </a>
                ...
                <a
                  ...
                  title={
                    delegateType == 'Role'
                      ? translate('manage_delegation_policy.edit_delegatedObject_attribute')
                      : translate('manage_delegation_policy.edit_delegatedObject_attributeTask')
                  }
                >
                  <i className='material-icons'>edit</i>
                </a>
          ...
          {!delegatedObjectAttributes || delegatedObjectAttributes.length <= 1
            ? null
              ...
              <td rowSpan={!resourceAttributes || resourceAttributes.length == 0 ? 1 : resourceAttributes.length}>
                {!resourceAttributes || resourceAttributes.length == 0 ? (
                  <a
                    ...
                    title={translate('manage_delegation_policy.add_resource_attribute')}
                  >
                    <i className='material-icons'>add_box</i>
                  </a>
                ) : (
                  <a
                    ...
                    className='edit text-yellow'
                    style={{ width: '5px' }}
                    title={translate('manage_delegation_policy.edit_resource_attribute')}
                  >
                    <i className='material-icons'>edit</i>
                  </a>
                )}
