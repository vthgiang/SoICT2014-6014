...
function LinkAttributeCreateForm(props) {
  ...
  const validateAttributes = () => {
    ...
    if (linkAttributes.length !== 0) {
      for (let n in linkAttributes) {
        if (
          !ValidationHelper.validateEmpty(props.translate, linkAttributes[n].attributeId).status ||
          !ValidationHelper.validateEmpty(props.translate, linkAttributes[n].value).status
        ) {
          result = false
          break
        }
  ...
  return (
    <React.Fragment>
      ...
      <div style={{ display: 'flex', marginBottom: 6, float: 'right' }}>
        {
          <div className='dropdown'>
            <button
              ...
              aria-expanded='true'
            >
              {translate('manage_link.add_attribute')}
            </button>
            <ul className='dropdown-menu pull-right'>
              <li>
                <a href='#' onClick={handleOpenModalCreateLinkAttribute}>
                  {translate('manage_link.add_link_attribute')}
                </a>
              </li>
      ...
      <DialogModal
        modalID='modal-create-link-attribute'
      ...
      >
        ...
        <form id='form-create-link-attribute'>
          ...
          <div className='form-group'>
            <label>
              {translate('manage_link.links_add_attribute')}
