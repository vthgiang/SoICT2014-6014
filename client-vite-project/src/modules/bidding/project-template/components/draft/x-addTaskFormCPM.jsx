...
const AddTaskFormCPM = (props) => {
  ...
  const initTasksToChoose = currentProjectTasks
    ? currentProjectTasks?.map((item) => ({
        value: item._id,
        text: item.name
      }))
    : []
  ...
  useEffect(() => {
    ...
    newTask.preceedingTasks.length > 0 &&
      setState({
        ...state,
        newTask: {
          ...state.newTask,
  ...
  useEffect(() => {
    ...
    if (
      newTask.currentLatestStartDate &&
      newTask.preceedingTasks.length > 0 &&
        ...
        <div className={`${isProcess ? 'col-lg-12' : 'col-sm-4 col-md-4 col-xs-12'}`}>
          ...
          <fieldset className='scheduler-border'>
            ...
            <div className={'row'}>
              ...
              <div className='col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group'>
                <label className='control-label'>
                  {translate('task.task_management.detail_priority')}
                  <span className='text-red'>*</span>
                </label>
          ...
          {/* Chi phí */}
          {newTask.startDate.trim().length > 0 &&
            newTask.endDate.trim().length > 0 &&
            newTask.responsibleEmployees.length > 0 &&
            newTask.accountableEmployees.length > 0 && (
              <fieldset className='scheduler-border'>
                <legend className='scheduler-border'>Ước lượng chi phí</legend>
                <div className={'row'}>
                  {/* Chi phí ước lượng tài sản */}
                  <div
                    className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group ${newTask.errorOnAssetCode === undefined ? '' : 'has-error'}`}
                  >
                    <label className='control-label'>
                      Chi phí ước lượng tài sản<span className='text-red'>*</span> ({getCurrentProjectDetails(project)?.unitCost})
                    </label>
                    <input
                      type='text'
                      className='form-control'
                  ...
                  <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group`}>
                    <label className='control-label'>Chi phí ước lượng nhân lực ({getCurrentProjectDetails(project)?.unitCost})</label>
                ...
                <div className={'row'}>
                  {/* Chi phí ước lượng */}
                  <div
                    className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group ${Number(estimateNormalCost.replace(/,/g, '')) > Number(newTask.estimateMaxCost.replace(/,/g, '')) ? 'has-error' : ''}`}
                  >
                    ...
                    <ErrorLabel
                      content={
                        Number(estimateNormalCost.replace(/,/g, '')) > Number(newTask.estimateMaxCost.replace(/,/g, '')) &&
                        'Ngân sách đang thấp hơn chi phí ước lượng'
                      }
                    />
                  </div>
                  {/* Ngân sách cho công việc */}
                  <div
                    className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 form-group ${newTask.errorOnBudget === undefined ? '' : 'has-error'}`}
                  >
                    <label>
                      Chi phí thoả hiệp<span className='text-red'>*</span> ({getCurrentProjectDetails(project)?.unitCost})
                    </label>
                    <input
                      type='text'
                      className='form-control'
        ...
        <div className={`${isProcess ? 'col-lg-12' : 'col-sm-8 col-md-8 col-xs-12'}`}>
          ...
          <fieldset className='scheduler-border'>
            ...
            <div className='row form-group'>
              <div className={`col-lg-3 col-md-3 col-ms-12 col-xs-12 ${newTask.errorOnStartDate === undefined ? '' : 'has-error'}`}>
                ...
                />
                <TimePicker id={`time-picker-project-add-task`} value={startTime} onChange={handleStartTimeChange} />
              ...
              <div className='row form-group'>
                <div className={`col-lg-3 col-md-3 col-ms-12 col-xs-12 ${newTask.errorOnEndDate === undefined ? '' : 'has-error'}`}>
                  ...
                  <div>{state.newTask.endDate && state.newTask.estimateNormalTime && `${state.newTask.endDate} ${endTime}`}</div>
          ...
          <fieldset className='scheduler-border'>
            ...
            <div className='row form-group'>
              ...
              <div
                className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${newTask.errorOnResponsibleEmployees === undefined ? '' : 'has-error'}`}
              >
                <label className='control-label'>
              ...
              <div
                className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${newTask.errorOnAccountableEmployees === undefined ? '' : 'has-error'}`}
              >
                <label className='control-label'>
          ...
          <fieldset className='scheduler-border' style={{ lineHeight: 1.5 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              ...
              <div
                className={`col-md-12 ${newTask.errorOnTotalWeight === undefined ? '' : 'has-error'}`}
...
const connectedAddTaskForm = connect(mapState, actionCreators)(withTranslate(AddTaskFormCPM))
export { connectedAddTaskForm as AddTaskFormCPM }
