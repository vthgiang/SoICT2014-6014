                ...
                {bid ? (
                  <div style={{ lineHeight: 2 }}>
                    <div>
                      ...
                      {bid?.proposals?.executionTime} ({bid?.proposals?.unitOfTime})
                    </div>
                    <div>
                  ...
                  <span>Không có thông tin đề xuất liên quan đến gói thầu này</span>
                )}
              </fieldset>
            </div>
            <div className='col-md-6'>
                ...
                {projectBP ? (
                  <div style={{ lineHeight: 2 }}>
                    <div>
                      ...
                      <span
                        style={
                          formatTimeOfEffection(projectBP?.unitTime, projectBP?.startDate, projectBP?.endDate) <
                          Number(bid?.proposals?.executionTime)
                    ...
                    <div>
                      ...
                      <span
                        style={
                          Number(projectBP?.budget) < Number(bid?.price)
                            ? { color: 'green', fontWeight: 600 }
                            : { color: 'red', fontWeight: 600 }
                        }
                      >
                        ...
                        {projectBP?.projectManager?.map((x, idx) => {
                          return (
                            <li
                              key={`prj-pm-${idx}`}
                              style={
                                checkInArr(x._id, contract?.decideToImplement?.projectManager) ? {} : { color: 'red', fontWeight: 600 }
                              }
                            >
                              <span>{x.name}</span>
                            </li>
                        ...
                        {projectBP?.responsibleEmployees?.map((x, idx) => {
                          return (
                            <li
                              key={`prj-res-${idx}`}
                              style={
                                checkInArr(x._id, contract?.decideToImplement?.responsibleEmployees)
                                  ? {}
                                  : { color: 'red', fontWeight: 600 }
                              }
                            >
                    ...
                    <div>
                      ...
                      (projectBP?.endDateRequest &&
                        formatTimeOfEffection(projectBP?.unitTime, projectBP?.startDate, projectBP?.endDateRequest) !==
                          formatTimeOfEffection(projectBP?.unitTime, projectBP?.startDate, projectBP?.endDate)) ? (
                        <ul>
                          {projectBP?.budgetChangeRequest && projectBP?.budgetChangeRequest !== projectBP?.budget && (
                            <li key={`budgetChangeRequest}`}>
                              Ngân sách ước lượng hiện tại:{' '}
                              <span
                                style={
                                  Number(projectBP?.budgetChangeRequest) < Number(bid?.price)
                                    ? { color: 'green', fontWeight: 600 }
                                    : { color: 'red', fontWeight: 600 }
                                }
                              >
                          ...
                          {projectBP?.endDateRequest &&
                            ...
                            formatTimeOfEffection(projectBP?.unitTime, projectBP?.startDate, projectBP?.endDateRequest) !==
                              ...
                              <li key={`endDateRequest}`}>
                                Ngày kết thúc thay đổi từ <span style={{ fontWeight: 600 }}>{formatDate(projectBP?.endDate)}</span> thành{' '}
                                <span
                                  style={
                                    formatTimeOfEffection(projectBP?.unitTime, projectBP?.startDate, projectBP?.endDateRequest) <
                                    Number(bid?.proposals?.executionTime)
                                ...
                                >
                                  {formatDate(projectBP?.endDateRequest)}
                                </span>
                                <br />
                  ...
                  <span>Không có thông tin dự án liên quan đến gói thầu này</span>
                )}
              </fieldset>
            </div>
          </div>
