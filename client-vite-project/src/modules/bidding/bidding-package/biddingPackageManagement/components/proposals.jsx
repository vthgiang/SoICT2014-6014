...
import getAllEmployeeSelectBoxItems, {
  convertTagIdToTagName,
  getAllEmployeeWithTaskSelectBoxItems,
  getEmployeeInfoWithTask
} from './employeeHelper'
...
function Proposals(props) {
      ...
      for (let c of currentTasks) {
        for (let o of oldTasks) {
          if (c?.code?.trim() === o?.code?.trim()) {
            ...
            if (
              filteredBackupEmp.length !== c.backupEmployees?.length ||
              filteredDirectEmp.length !== c.directEmployees?.length ||
              filteredBackupEmp.length !== o.backupEmployees?.length ||
              filteredDirectEmp.length !== o.directEmployees?.length
            ) {
        ...
        for (let p of proposedTasks) {
          if (c?.code?.trim() === p?.code?.trim()) {
            ...
            if (
              filteredBackupEmp.length !== c.backupEmployees?.length ||
              filteredDirectEmp.length !== c.directEmployees?.length ||
              filteredBackupEmp.length !== p.backupEmployees?.length ||
              filteredDirectEmp.length !== p.directEmployees?.length
            ) {
  ...
  const generateLog = () => {
    ...
    if (isChange) {
      if (satisfied) {
        // đã dùng đề xuất tự động và hài lòng, thay đổi bằng cách đề xuất tự động
        return {
          oldVersion: {
            isProposal: false,
            tasks: oldTasks // có cả trường hợp tạo mới và chỉnh sửa
  ...
  }, [props.id, JSON.stringify(props.listCareer)])
  ...
  let listEmpInfoFormated = getEmployeeInfoWithTask(
    allUsers,
    allEmployee,
    tasks?.tasks ?? [],
    proposals?.executionTime ?? 0,
    proposals?.unitOfTime,
    biddingPackage
  )
  useEffect(() => {
    listEmpInfoFormated = getEmployeeInfoWithTask(
      allUsers,
      allEmployee,
      tasks?.tasks ?? [],
      proposals?.executionTime ?? 0,
  ...
  const getListSuitableEmpSelectBox = (listSuitableEmp) => {
    ...
    if (allEmployee) {
      res = allEmployee
        ?.filter((item) => listSuitableEmp?.indexOf(String(item?._id)) !== -1)
        ?.map((o) => {
      ...
      {currentStep === 0 && (
        <div>
          <fieldset className='scheduler-border'>
            ...
            <div className='form-group'>
              ...
              <input
                ...
                name={`executeTime`}
                onChange={(value) => handleChangeData('executionTime', value)}
                value={proposals.executionTime}
          ...
          <fieldset className='scheduler-border'>
                    ...
                    <div className='col-md-6'>
                      <div className='form-group'>
                        ...
                        <input
                          type='text'
                          className='form-control'
                          name={`taskName-${currentIndex}`}
                          onChange={(value) => handleChangeForm('taskName', value)}
                      ...
                      <div className='form-group'>
                        ...
                        <input
                          type='text'
                          className='form-control'
                          name={`code-${currentIndex}`}
                          onChange={(value) => handleChangeForm('code', value)}
                    ...
                    <div className='col-md-6'>
                      <div className='form-group'>
                        ...
                        <input
                          type='number'
                          className='form-control'
                          name={`estimateTime-${currentIndex}`}
                          onChange={(value) => handleChangeForm('estimateTime', value)}
                          value={currentTask?.estimateTime}
                          placeholder='Thời gian thực hiện'
                          autoComplete='off'
                        />
                  ...
                  <div>
                    <div className={`form-group`}>
                      <label className='control-label'>
                        Tags công việc<span className='text-red'>*</span>({' '}
                        <a style={{ cursor: 'pointer' }} onClick={() => handleShowCreateTag(`${currentIndex}-${id}`)}>
                          Quản lý
                        </a>{' '}
                        )
                      </label>
                    ...
                    <div className={`form-group`}>
                      ...
                      <div className='pull-right'>
                        <ModalViewEmployee id={id ?? ''} listEmployee={listEmpInfoFormated} />
                        <a style={{ cursor: 'pointer' }} onClick={() => handleShowViewEmployee(id)}>
                          Xem thông tin nhân viên
                        </a>
                      </div>
                      <label className='control-label'>
                        Nhân sự phù hợp<span className='text-red'>*</span>({' '}
                        <a style={{ cursor: 'pointer' }} onClick={() => handleGetSuitableEmpByTag()}>
                          Lấy danh sách nhân sự phù hợp theo tag
                        </a>{' '}
                        )
                      </label>
                      {
                    ...
                    <div className='form-group'>
                      <label>
                        Số người thực hiện<span className='text-red'>*</span>
                      </label>
                      <input
                        type='number'
                        className='form-control'
                        name={`numberOfEmployees-${currentIndex}`}
                        onChange={(value) => handleChangeForm('numberOfEmployees', value)}
                        value={currentTask?.numberOfEmployees}
                        placeholder='Số người thực hiện'
                        autoComplete='off'
                      />
                    </div>
                  </div>
            ...
            <br />
            {!isTable ? (
              <ViewTaskInGantt taskList={proposals?.tasks} allEmployee={allEmployee} />
            ) : (
        ...
        <div className='box-body'>
            ...
            {proposals?.tasks?.length ? (
              <div>
                ...
                <span>
                  ...
                  <a
                    style={{ margin: '0 0 5px 5px', textDecoration: 'underline', fontWeight: '600', cursor: 'pointer' }}
                    onClick={() => calcPropose()}
                  >
                  ...
                  <i
                    ...
                    title='Xem cơ ché đề xuất'
                    onClick={() => {
                      handelProposeModal(id)
                    }}
          ...
          {showMessage && isLoading === false && proposedData.isComplete === 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', color: 'green' }}>
              Đã tính toán xong - Thông tin đề xuất phân công nhân sự hiển thị ở bảng bên dưới!
            </div>
          )}
          {showMessage && isLoading === false && proposedData.isComplete === 0 && (
            <div style={{ display: 'flex', justifyContent: 'center', color: 'red' }}>
              Hãy kiểm tra lại thông tin nhân sự phù hợp và số lượng nhân sự để phân công nhân sự tối ưu hơn!
            </div>
          )}
          {!isTable ? (
            <ViewTaskInGantt taskList={proposals?.tasks} allEmployee={allEmployee} />
