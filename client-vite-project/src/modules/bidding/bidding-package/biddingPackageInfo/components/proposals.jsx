  ...
  return (
    <div id={id} className='tab-pane'>
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        <div className='btn btn-success' onClick={() => generateProposalDocx(proposals)}>
          Tải file Đề xuất
        </div>
      </div>
      {proposals?.executionTime && (
        <div style={{ marginLeft: '20px' }}>
          <strong>Thời gian thực hiện hợp đồng: </strong>
          <span>
            {proposals?.executionTime} ({arrUnitTimeList.find((x) => x.value === proposals?.unitOfTime)?.text})
          </span>{' '}
      ...
      <div className='nav-tabs-custom'>
        {
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a href={`#proposal_info_${id}`} data-toggle='tab' onClick={() => forceCheckOrVisible(true, false)}>
                Thông tin công việc đề xuất
              </a>
            </li>
            <li>
              <a href={`#proposal_log_${id}`} data-toggle='tab' onClick={() => forceCheckOrVisible(true, false)}>
                Lịch sử phân công nhân sự
              </a>
            </li>
          </ul>
        }
        <div className='tab-content'>
            ...
            {proposals?.tasks?.length > 0 ? (
              <div>
                ...
                <p style={{ color: 'green', marginLeft: '20px' }}>
                  *Thông tin đề xuất công việc sẽ được hiển thị bên dưới - click để xem chi tiết*
                </p>
              </div>
            ) : (
              <span style={{ marginLeft: '20px' }}>Chưa có thông tin đề xuất!</span>
            )}
                    ...
                    <div className='box'>
                      <div className='box-header with-border'>
                        <p
                          ...
                          data-target={`#task-proposal-${index}`}
                          aria-expanded='false'
                          style={{ display: 'flex', alignItems: 'center', fontWeight: '600', cursor: 'pointer' }}
                          onClick={() => {
                        ...
                        >
                          ...
                          <span
                            ...
                            style={{ display: 'none', fontWeight: 'bold', marginRight: '10px' }}
                          >
                            {`keyboard_arrow_down`}
                          </span>
                          Công việc: {item.taskName}
                        </p>
                      </div>
                      <div className='box-body collapse' data-toggle='collapse' id={`task-proposal-${index}`} style={{ lineHeight: 2 }}>
                        ...
                        <div>
                          <strong>Tags: </strong>
                          <span>{item?.tag?.map((x) => convertTagIdToTagName(alltag, x))?.join(', ')}</span>
                        </div>
                        <div>
                        ...
                        <div>
                          <strong>Nhân viên trực tiếp: </strong>
                          <span>{item.directEmployees.map((x) => x.fullName).join(', ')}</span>
                        </div>
                        <div>
                          <strong>Nhân viên dự phòng: </strong>
                          <span>{item.backupEmployees.map((x) => x.fullName).join(', ')}</span>
                        </div>
                      </div>
                    </div>
                  </section>
          ...
          <div className='tab-pane' id={`proposal_log_${id}`}>
            {!(proposals?.logs?.length > 0) ? (
              <div style={{ textAlign: 'center' }}>Chưa có thông tin lịch sử phân công nhân sự</div>
            ) : (
              <div className='row'>
                  ...
                  <div className='box box-solid' style={{ border: '1px solid #ecf0f6', borderBottom: 'none' }}>
                    <div className='box-header with-border'>
                      <h3 className='box-title' style={{ fontWeight: 800 }}>
                        Các phiên bản phân công nhân sự
                      </h3>
                    </div>
                ...
                {currentLog && (
                  <div className='col-xs-12 col-sm-8' style={{ lineHeight: 2 }}>
                    <h4>Thông tin chi tiết phân công nhân sự tự động lần {currentLogIndex + 1}</h4>
                    {!currentLog?.isSatisfied ? (
                      <div style={{ lineHeight: 2 }}>
                        <div>
                          <strong>Hình thức: </strong>&nbsp;{' '}
                          {currentLog.type === 'create' ? (
                            <span style={{ color: 'green', fontWeight: 'bold' }}>Khởi tạo phân công nhân sự vào công việc gói thầu</span>
                          ) : (
                            <span style={{ color: 'red', fontWeight: 'bold' }}>Chỉnh sửa phân công nhân sự vào công việc gói thầu</span>
                          )}
                        </div>
                        <div>
                          <strong>Người thực hiện: </strong> {currentLog.createdBy.name}
                        </div>
                        <div>
                          <strong>Thời gian: </strong> <DateTimeConverter dateTime={currentLog.createdAt} />
                        </div>
                        <div>
                          <strong>Ghi chú: </strong>{' '}
                          <span style={{ color: `${currentLog.isSatisfied ? 'green' : 'red'}`, fontWeight: 'bold' }}>
                            {currentLog?.note}
                          </span>
                        </div>

                        <div>
                          <a style={{ cursor: 'pointer' }} onClick={() => setShowDetailLog(!showDetailLog)}>
                            {showDetailLog ? 'Ẩn chi tiết' : 'Hiển thị chi tiết'}
                          </a>{' '}
                        </div>
                        <br />
                        {!showDetailLog ? null : (
                                    ...
                                    <td>
                                      ...
                                      <div>
                                        ...
                                        {item?.old?.backupEmployees?.length > 0
                                          ? item?.old?.backupEmployees
                                              ?.map((userItem) => convertEmpIdToName(allEmployee, userItem))
                                              .join(', ')
                                          : 'N/A'}
                                      </div>
                                    </td>
                      ...
                      <div>
                        {currentLog?.type === 'edit' ? (
                          <div style={{ lineHeight: 2 }}>
                              ...
                              <span>
                                ...
                                {currentLog.type === 'create' ? (
                                  <span style={{ color: 'green', fontWeight: 'bold' }}>
                                    Khởi tạo phân công nhân sự vào công việc gói thầu
                                  </span>
                                ) : (
                                  <span style={{ color: 'red', fontWeight: 'bold' }}>
                                    Chỉnh sửa phân công nhân sự vào công việc gói thầu
                                  </span>
                                )}
                              </span>
                            </div>
                            ...
                            {!showDetailLog ? null : (
                              <table
                                id='proposal-result-show-data-2'
                                className='table not-has-action table-striped table-bordered table-hover'
                              >
                                <thead>
                                  <tr>
                                    <th>STT</th>
                          ...
                          <div style={{ lineHeight: 2 }}>
                            ...
                            {!showDetailLog ? null : (
                              <table
                                id='proposal-result-show-data-3'
                                className='table not-has-action table-striped table-bordered table-hover'
                              >
                                <thead>
                                  <tr>
                                  ...
                                  {currentLog.newVersion.tasks?.map((item, listIndex) => {
                                    return (
                                      <tr key={`tag-${listIndex}`}>
                                        ...
                                        <td>{item?.taskName}</td>
                                        <td>
                                          {item?.directEmployees.map((userItem) => convertEmpIdToName(allEmployee, userItem)).join(', ')}
                                        </td>
                                        <td>
                                          {item?.backupEmployees?.length > 0
                                            ? item?.backupEmployees?.map((userItem) => convertEmpIdToName(allEmployee, userItem)).join(', ')
                                            : 'N/A'}
                                        </td>
                                      </tr>
                                    )
