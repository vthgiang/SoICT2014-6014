...
function ModalEditTaskByAccountableEmployee(props) {
  ...
  const handleChangeActiveAccountable = async (e, id) => {
    ...
    let numOfInactiveResp = 0,
      numOfInactiveAcc = 0,
      listInactive = state.listInactive
  ...
  const validateTaskStartDate = (value, willUpdateState = true) => {
    ...
    if (value === '') {
      ...
      if (startDate > endDate) {
        ...
        if (state.errorOnEndDate) state.errorOnEndDate = undefined
      }
    }
    if (willUpdateState) {
      setState({
  ...
  const validateTaskEndDate = (value, willUpdateState = true) => {
    ...
    if (value === '') {
      ...
      if (startDate > endDate) {
        ...
        if (state.errorOnStartDate) state.errorOnStartDate = undefined
      }
    }
    if (willUpdateState) {
      state.endDate = value
  ...
  const isFormValidated = () => {
    ...
    // check &&
    return (
      validateTaskName(state.taskName, false) &&
      validateTaskDescription(state.taskDescription, false) &&
      state.errorOnProgress === undefined &&
  ...
  const save = () => {
    let listInactive = state.listInactive,
      taskId,
      inactiveEmployees = []
    taskId = props.id
    for (let i in listInactive) {
            ...
            <fieldset className='scheduler-border'>
              ...
              <div>
                <div className={`form-group ${errorTaskName === undefined ? '' : 'has-error'}`}>
                  ...
                  <input type='text' value={taskName} className='form-control' onChange={handleTaskNameChange} />
            ...
            <fieldset className='scheduler-border'>
                ...
                <div className='form-group'>
                  ...
                  <SelectBox
                    ...
                    items={listDepartment
                      .filter((item) => item._id !== organizationalUnit)
                      .map((x) => {
              ...
              <div className='row form-group'>
                <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${errorOnStartDate === undefined ? '' : 'has-error'}`}>
                  ...
                  <DatePicker id={`datepicker2-startdate-${id}`} value={startDate} onChange={handleChangeTaskStartDate} />
                  <TimePicker id={`time-picker-1-start-time${id}`} value={startTime} onChange={handleStartTimeChange} />
                ...
                <div className={`col-lg-6 col-md-6 col-ms-12 col-xs-12 ${errorOnEndDate === undefined ? '' : 'has-error'}`}>
                  ...
                  <DatePicker id={`datepicker2-enddate-${id}`} value={endDate} onChange={handleChangeTaskEndDate} />
                  <TimePicker id={`time-picker-2-end-time-${id}`} value={endTime} onChange={handleEndTimeChange} />
              ...
              <div className={` form-group ${errorOnFormula === undefined ? '' : 'has-error'}`}>
                ...
                <input
                  ...
                  value={formula}
                  onChange={handleChangeTaskFormula}
                />
                <ErrorLabel content={errorOnFormula} />

                <br />
                <div>
                  <span style={{ fontWeight: 800 }}>Ví dụ 1: </span>progress / (daysUsed / totalDays) - (numberOfFailedActions /
                  (numberOfFailedActions + numberOfPassedActions)) * 100
                </div>
                <div>
                  <span style={{ fontWeight: 800 }}>Ví dụ 2: </span>progress / (daysUsed / totalDays) - (10 - averageActionRating) * 10
                </div>
                <br />
                <div>
                ...
                <div>
                  <span style={{ fontWeight: 600 }}>p1, p2,...</span> - Thông tin công việc kiểu số (Chỉ có với các công việc theo mẫu)
                </div>
              </div>
            </fieldset>
                        ...
                        <div key={index} style={{ paddingLeft: 20 }}>
                          <label style={{ fontWeight: 'normal', margin: '7px 0px' }}>
                            <input
                              ...
                              value={elem._id}
                              name='accountable'
                              onChange={(e) => handleChangeActiveAccountable(e, elem._id)}
                            />
                            &nbsp;&nbsp;&nbsp;{elem.name}
                          </label>
                        </div>
                        ...
                        <div key={index} style={{ paddingLeft: 20 }}>
                          <label style={{ fontWeight: 'normal', margin: '7px 0px' }}>
                            <input
                              ...
                              value={elem._id}
                              name='responsible'
                              onChange={(e) => handleChangeActiveResponsible(e, elem._id)}
                            />
                            &nbsp;&nbsp;&nbsp;{elem.name}
                          </label>
                          <br />
                          ...
                          <div key={key} style={{ paddingLeft: 20 }}>
                            <label style={{ fontWeight: 'normal', margin: '7px 0px' }}>
                              <input
                                ...
                                value={elem._id}
                                name='consulted'
                                onChange={(e) => handleChangeActiveConsulted(e, elem._id)}
                              />
                              &nbsp;&nbsp;&nbsp;{elem.name}
                            </label>
                            <br />
