...
function calcTaskEVMPoint(data) {
  ...
  const plannedValue =
    (diffFromStartToNow / diffFromStartToEnd > 1 ? 1 : diffFromStartToNow / diffFromStartToEnd) * Number(estimateNormalCost)
  // Actual Cost
...
function calcMemberStatisticEvalPoint(data) {
  ...
  // Estimate duration
  const estDuration =
    ((currentMemberWithSalary?.weight / 100) * estimateNormalTime) /
    (projectDetail?.unitTime === 'days' ? MILISECS_TO_DAYS : MILISECS_TO_HOURS)
  // Estimate cost
