...
function DetailTaskTab(props) {
  ...
  function initState() {
    ...
    const currentRoleId = getStorage('currentRole')
  ...
  const ROLE = {
    RESPONSIBLE: {
      name: translate('task.task_management.responsible'),
      value: 'responsible'
    },
    ACCOUNTABLE: {
      name: translate('task.task_management.accountable'),
      value: 'accountable'
    },
    CONSULTED: {
      name: translate('task.task_management.consulted'),
      value: 'consulted'
    },
    CREATOR: {
      name: translate('task.task_management.creator'),
      value: 'creator'
    },
    INFORMED: {
      name: translate('task.task_management.informed'),
      value: 'informed'
    }
  }
  ...
  useEffect(() => {
    if (performtasks?.task) {
      const { task } = performtasks /// chú ý: cần check thêm trường hợp quy trình có lấy dữ liệu ở performTasks hay ko
      ...
      if (task) {
        ...
        if (task.creator?._id) if (userId === task.creator._id) roles.push(ROLE.CREATOR)
        if (!task.creator?._id) if (userId === task.creator) roles.push(ROLE.CREATOR)
      }
      ...
      if (roles.length > 0) {
        ...
        } else if (!roles.some((e) => e.value === currentRole)) {
          setCurrentRole(roles[0].value)
          if (props.onChangeTaskRole) {
            props.onChangeTaskRole(roles[0].value)
          }
        } else if (props.onChangeTaskRole) {
  ...
  const remindEvaluateTaskOnThisMonth = (task) => {
    const endOfMonth = new Date(dayjs().endOf('month').format()) // lấy ngày cuối cũng của tháng hiện tại
    ...
    if (task) {
      ...
      if (evaluations && evaluations.length > 0) {
        for (let i = 0; i < evaluations.length; i++) {
          ...
          const splitter = formatDate(evaluations[i].evaluatingMonth).split('-')
          ...
          prevEval = evaluations.find(
            (e) =>
              monthOfPrevEval === new Date(e.evaluatingMonth).getMonth() && yearOfPrevEval === new Date(e.evaluatingMonth).getFullYear()
          )
          if (prevEval) {
            ...
            if (
              ...
              prevDate = new Date(
                dayjs(`${yearOfPrevEval}-${monthOfPrevEval + 1}`)
                  .endOf('month')
                  .format()
              )
            }
  ...
  return (
    <>
        ...
        <div style={{ marginLeft: '-10px' }}>
          ...
          {(currentRole === 'responsible' || currentRole === 'accountable') && checkInactive && (
            <a
              ...
              onClick={() => handleShowEdit(id, currentRole, checkHasAccountable)}
          ...
          {performtasks?.task?.status !== 'finished' && (
            <>
              {(currentRole === 'consulted' || currentRole === 'responsible' || currentRole === 'accountable') && checkInactive && (
                <a
                  className='btn btn-app'
                  onClick={() => !performtasks.currentTimer && startTimer(task._id, currentUser)}
          ...
          {(currentRole === 'consulted' || currentRole === 'responsible' || currentRole === 'accountable') && checkInactive && (
            <a
              className='btn btn-app'
              onClick={() => handleShowEvaluate(id, currentRole)}
              title={translate('task.task_management.detail_evaluate')}
            >
              <i className='fa fa-calendar-check-o' style={{ fontSize: '16px' }} />
              {translate('task.task_management.detail_evaluate')}
            </a>
          )}
          {(currentRole === 'responsible' || currentRole === 'accountable') && checkInactive && (
            <a
              className='btn btn-app'
              onClick={() => handleCopyTask(id, currentRole)}
              title={translate('task.task_management.detail_copy_task')}
            >
              <i className='fa fa-clone' style={{ fontSize: '16px' }} />
              {translate('task.task_management.detail_copy_task')}
            </a>
          )}
          {(currentRole === 'accountable' || currentRole === 'responsible') &&
            checkInactive &&
            checkCurrentRoleIsManager &&
            checkCurrentRoleIsManager.length > 0 && (
              <a
                className='btn btn-app'
                onClick={() => handleSaveAsTemplate(id, currentRole)}
                title={translate('task.task_management.detail_save_as_template')}
              >
                <i className='fa fa-floppy-o' style={{ fontSize: '16px' }} />
                {translate('task.task_management.detail_save_as_template')}
              </a>
            )}

          {task &&
            statusTask !== 'finished' &&
            ((currentRole === 'responsible' && task?.requestToCloseTask?.requestStatus !== 3) ||
              (currentRole === 'accountable' && task?.requestToCloseTask?.requestStatus === 1)) &&
            checkInactive &&
            checkHasAccountable && (
              <a
                className='btn btn-app'
                onClick={() => handleShowRequestCloseTask(id)}
                title={
                  currentRole === 'responsible'
                    ? translate('task.task_perform.request_close_task')
                    : translate('task.task_perform.approval_close_task')
                }
              >
          ...
          {collapseInfo === false ? (
            <a
              ...
              href='#info'
              onClick={handleChangeCollapseInfo}
              role='button'
              aria-expanded='false'
              aria-controls='info'
            >
                ...
                {roles.map((item, index) => {
                  return (
                    <li className={item.value === currentRole ? 'active' : undefined} key={index}>
                      <a href='#' onClick={() => changeRole(item.value)}>
                        {item.name}
                      </a>
                    </li>
                  )
          ...
          {task && warning && (
            <div className='description-box warning'>
              ...
              {checkConfirmTask && checkConfirmTask.checkConfirmCurrentUser && (
                <div>
                  <strong>
                    ...
                    <a style={{ cursor: 'pointer' }} onClick={() => confirmTask(task)}>
              ...
              {task.status === 'inprocess' && checkEvaluationTaskAndKpiLink && checkEvaluationTaskAndKpiLink.checkEvaluationTask && (
                <div>
                  <strong>{translate('task.task_management.not_have_evaluation')}</strong>
                </div>
              )}

              {
                ...
                task.status === 'inprocess' &&
                  checkEvaluationTaskAndKpiLink &&
                  checkEvaluationTaskAndKpiLink.checkEvaluationTask &&
                  checkEvaluate && (
                    <div>
                      <strong>{translate('task.task_management.warning_evaluate')}</strong>
                    </div>
                  )
              }
          ...
          {task && (
            <div className='description-box'>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <i style={{ fontSize: '17px', marginRight: '5px' }} className='fa fa-info-circle' aria-hidden='true' />
              ...
              <div>
                <strong>{translate('task.task_management.detail_time')}:</strong> {formatTime(task && task.startDate)}{' '}
                <i className='fa fa-fw fa-caret-right' /> {formatTime(task && task.endDate)}{' '}
              </div>
              <div>
                <strong>{translate('task.task_management.unit_manage_task')}:</strong>{' '}
                {task && task.organizationalUnit ? task.organizationalUnit.name : translate('task.task_management.err_organizational_unit')}
              </div>
              <div>
                    ...
                    <span>
                      {task?.collaboratedWithOrganizationalUnits?.map((item, index) => {
                        ...
                        return (
                          <span key={index}>
                            {seperator}
                            {item.organizationalUnit && item.organizationalUnit.name}
                          </span>
                        )
                      })}
                    </span>
              ...
              <div>
                <strong>{translate('task.task_management.detail_status')}:</strong>{' '}
                <span style={{ color: taskStatusColor(task?.status) }}>{task && formatStatus(task.status)}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <strong>{translate('task.task_management.detail_progress')}:</strong>
                <div className='progress-task' style={{ width: '30%' }}>
                  <div
                    className='fillmult'
                    data-width={`${task && task.progress}%`}
                    style={{
                      width: `${task && task.progress}%`,
                      backgroundColor: task && task.progress < 50 ? '#dc0000' : '#28a745'
                    }}
                  />
                  ...
                  if (info.type === 'date') {
                    return (
                      <div key={key}>
                        ...
                        {info.value ? formatDate(info.value) : translate('task.task_management.detail_not_hasinfo')}
                      </div>
                    )
                  }
                  return (
                  ...
                  {task?.responsibleEmployees?.length > 0 &&
                    task.responsibleEmployees.map((item, index) => {
                      ...
                      if (task?.inactiveEmployees?.some((o) => o._id === item._id)) {
                        return (
                          <a key={index} title='đã rời khỏi công việc' className='raci-style' style={{ opacity: 0.5 }}>
                            <img
                              src={process.env.REACT_APP_SERVER + item.avatar}
                ...
                evalList.map((eva, keyEva) => {
                  return (
                    <div key={keyEva} className='description-box'>
                      <h4>
                        {translate('task.task_management.detail_eval')}&nbsp;
                        {formatDate(eva.startDate)} <i className='fa fa-fw fa-caret-right' /> {formatDate(eva.endDate)}
                      </h4>
                      <a style={{ cursor: 'pointer' }} onClick={() => handleChangeShowMoreEvalItem(eva._id)}>
                        {showMore[eva._id] ? (
                          <p>
                      ...
                      {showMore[eva._id] && (
                        <div>
                          {eva.results.length !== 0 && (
                            <div>
                              <div>
                                ...
                                {translate('task.task_management.detail_auto_point')} - {translate('task.task_management.detail_emp_point')}{' '}
                              ...
                              {eva.taskInformations.map((info, key) => {
                                if (info.type === 'date') {
                                  return (
                                    <li key={key}>
                                      {info.name}: &nbsp;&nbsp;{' '}
                                      {info.value ? formatDate(info.value) : translate('task.task_management.detail_not_eval_on_month')}
                                    </li>
                                  )
                                }
                                return (
                          ...
                          {eva.results.length !== 0 ? (
                            ...
                            <div>
                              <strong>{translate('task.task_management.detail_all_not_kpi')}</strong>
                            </div>
                          )}
      ...
      {id && showRequestClose === id && (currentRole === 'responsible' || currentRole === 'accountable') && checkHasAccountable && (
        <RequestToCloseTaskModal id={id} task={task && task} role={currentRole} />
      )}
    </>
  )
}
