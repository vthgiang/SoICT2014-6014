...
function RequestToCloseProjectTaskModal(props) {
    ...
    return (
      <div>
        {role === 'responsible' && requestToCloseTask ? (
          <div style={{ textAlign: 'right' }}>
            {requestToCloseTask.requestStatus === 0 || requestToCloseTask.requestStatus === 2 ? (
              <button className='btn btn-success' onClick={() => sendRequestCloseTask()}>
                {translate('task.task_perform.send_request_close_task')}
              </button>
            ) : (
              requestToCloseTask &&
              requestToCloseTask.requestStatus === 1 && (
                <button className='btn btn-danger' onClick={() => cancelRequestCloseTask()}>
                  {translate('task.task_perform.cancel_request_close_task')}
                </button>
              )
            )}
          </div>
          ...
          requestToCloseTask.requestStatus === 1 && (
            <div style={{ textAlign: 'right' }}>
              <button className='btn btn-success' onClick={() => approvalRequestCloseTask()}>
                {translate('task.task_perform.approval_request_close_task')}
              </button>
              <button className='btn btn-danger' onClick={() => declineRequestCloseTask()} style={{ marginLeft: '5px' }}>
                {translate('task.task_perform.decline_request_close_task')}
              </button>
            </div>
          )
        )}
        <div className='form-group'>
          ...
          <SelectBox
            id='multiSelectStatusRequestClose'
            style={{ width: '100%' }}
            value={status}
        ...
        <div className='form-group'>
          ...
          {role === 'responsible' ? (
            ...
            <span>{parse(requestToCloseTask.description)}</span>
          ) : (
            <span>{translate('task.task_perform.none_description')}</span>
          )}
        </div>
  ...
  const renderNotRequestClostTaskYet = () => {
    return <div>Chưa có yêu cầu công việc nào được gửi</div>
  }
  ...
  console.log(
    'project?.data?.listbyuser?.find(item => String(item._id) === String(task?.taskProject))',
    project?.data?.listbyuser?.find((item) => String(item._id) === String(task?.taskProject))
  )
  ...
  const renderStatusItemsArr = () => {
    if (task?.taskProject && project?.data?.listbyuser?.find((item) => String(item._id) === String(task?.taskProject))?.projectType === 2) {
      return [{ value: 'finished', text: translate('task.task_management.finished') }]
    }
    return [
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-request-close-task-${id}`}
        isLoading={false}
      ...
      >
        ...
        {role === 'accountable' && isUserInCurrentTask(userId, task) && (
          <EvaluateByAccountableEmployeeProject
            role={role}
            task={task}
            handleSaveAccountableData={handleSaveAccountableData}
            handleSaveResponsibleData={handleSaveResponsibleData}
          />
        )}

        <fieldset className='scheduler-border'>
          <legend className='scheduler-border'>Yêu cầu kết thúc công việc</legend>
          {role === 'accountable' && requestToCloseTask?.requestStatus !== 1 ? renderNotRequestClostTaskYet() : renderRequestCloseTask()}
        </fieldset>
      </DialogModal>
    </React.Fragment>
  )
