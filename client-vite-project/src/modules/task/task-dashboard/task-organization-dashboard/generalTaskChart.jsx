...
const GeneralTaskChart = (props) => {
  ...
  const convertDataExport = (data) => {
    ...
    if (exportData) props.handleDataExport(exportData)
  }
  ...
  const bindTextToEvent = (key, text, data, name = '', index = 0, bold = 'bold') => {
    ...
    return (
      <a onClick={() => handleShowGeneralTask(data, name, index, key)} style={{ color: textColor, fontWeight: bold }}>
        {text}
      </a>
    )
  }
  ...
  for (let i in dataTemp) {
    ...
    data[i] = {
      ...
      taskInprocess: bindTextToEvent(
        'taskInprocess',
        dataTemp[i].taskInprocess.length,
        dataTemp[i].taskInprocess,
        dataTemp[i].name,
        i,
        bold
      ),
      taskFinished: bindTextToEvent('taskFinished', dataTemp[i].taskFinished.length, dataTemp[i].taskFinished, dataTemp[i].name, i, bold),
      confirmedTask: bindTextToEvent(
        'confirmedTask',
        dataTemp[i].confirmedTask.length,
        dataTemp[i].confirmedTask,
        dataTemp[i].name,
        i,
        bold
      ),
      noneUpdateTask: bindTextToEvent(
        'noneUpdateTask',
        dataTemp[i].noneUpdateTask.length,
        dataTemp[i].noneUpdateTask,
        dataTemp[i].name,
        i,
        bold
      ),
