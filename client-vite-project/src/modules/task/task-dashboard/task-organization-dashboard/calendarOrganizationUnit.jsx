        ...
        for (let i = 1; i <= tasksByStatus.length; i++) {
          ...
          if (multi) {
            if (tasksByStatus[i - 1].status !== 'inprocess') {
              ...
              titleTask =
                tasksByStatus[i - 1].name + ' - ' + responsibleEmployeeNames.join(' - ') + ' - ' + tasksByStatus[i - 1].progress + '%'
    ...
    if (tasks) {
      ...
      if (tasksByStatus1) {
        for (let i = 1; i <= tasksByStatus1.length; i++) {
          ...
          if (responsibleEmployeeIds.length === 1) {
            // Nếu công việc chỉ có 1 người thực hiện
            groupName.push({
            ...
            // Nếu công việc có nhiều người làm chung
