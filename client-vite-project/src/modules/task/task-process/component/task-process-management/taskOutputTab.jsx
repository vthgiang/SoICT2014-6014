...
function TaskOutputTab(props) {
  ...
  const taskOutputsLabel = [{ value: '0', text: 'Chọn kết quả cần giao nộp' }]

  for (const i in taskOutputs) {
    taskOutputsLabel.push({
      value: taskOutputs[i]._id,
      text: taskOutputs[i].title
    })
  }
  ...
  return (
    <div style={{ overFlow: 'auto' }}>
      ...
      {getTaskOutputsApproved(taskOutputs) != '0' && (
        <div>
      ...
      {taskOutput && (
        <div>
          <div>
            <strong style={{ fontWeight: 600, paddingRight: '10px' }}>Kiểu dữ liệu:</strong> {formatTypeInfo(taskOutput.type)}
          </div>
          <div>
            ...
            <div>
              <span style={{ fontWeight: 600 }}>
                {taskOutput.submissionResults?.description
                  ? `Kết quả giao nộp lần ${taskOutput.versions.length + 1}`
          ...
          <div>
            ...
            {taskOutput?.submissionResults.description && (
              ...
              >
                ...
                <li>
                  <a style={{ cursor: 'pointer' }} className='text-sm' onClick={() => handleShowFile(taskOutput.submissionResults._id)}>
                ...
                <li>
                  <a style={{ cursor: 'pointer' }} className='text-sm' onClick={() => handleShowComment(taskOutput._id)}>
                    <i className='fa fa-comments-o margin-r-5' aria-hidden='true' /> Trao đổi (
                    {taskOutput.comments && taskOutput.comments.length})
                  </a>
                </li>
              </ul>
            )}
            {showFile.some((obj) => obj === taskOutput.submissionResults._id) && taskOutput?.submissionResults.files.length > 0 && (
