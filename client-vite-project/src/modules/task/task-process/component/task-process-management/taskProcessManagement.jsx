  ...
  const handleFinished = async (item) => {
    ...
    message =
      lengthProcess == 0
        ? message
        : length == 0
          ? `Còn ${lengthProcess} quy trình chưa hoàn thành`
          : `${message} và ${lengthProcess} quy trình chưa hoàn thành`
    if (message) {
      Swal.fire({
        title: `${message}`,
        icon: 'warning',
      ...
      <div className='box-body qlcv'>
        ...
        <div className='pull-right' style={{ marginTop: 5 }}>
          <button
            ...
            title='Thêm'
            onClick={(event) => {
              showModalCreateTaskByProcess(event)
            }}
          >
        ...
        {showmodalCreateTaskByProcess && (
          <ModalCreateTaskByProcess title={'Quy trình không theo mẫu'} listOrganizationalUnit={listOrganizationalUnit} />
        )}
        ...
        <table className='table table-bordered table-striped table-hover' id={tableId}>
          ...
          <tbody className='task-table'>
            {listTaskProcess && listTaskProcess.length !== 0
              ? listTaskProcess.map((item, key) => {
                  return (
                    <tr key={key}>
                          ...
                          <React.Fragment>
                            ...
                            {item.status === 'not_initialized' && (
                              <a
                                className='edit'
                                onClick={() => {
                                  showEditProcessNoInit(item)
                                }}
                                title={'Khởi tạo công việc quy trình'}
                              >
                                <i className='material-icons'>edit</i>
                              </a>
                            )}
