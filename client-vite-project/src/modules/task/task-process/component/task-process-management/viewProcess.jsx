...
function ViewProcess(props) {
      ...
      modeler.importXML(props.data.xmlDiagram, function (err) {
        ...
        if (infoProcess) {
          for (let i in infoProcess) {
            ...
            let element1 =
              Object.keys(modeler.get('elementRegistry')).length > 0 && modeler.get('elementRegistry').get(infoProcess[i].codeInProcess)
            element1 &&
              modeling.updateProperties(element1, {
  ...
  const { id, info, startDate, endDate, status, processDescription, processName, showProcessChild, processChilds, showInfo, selectedView } =
    state
    ...
    <React.Fragment>
      <div>
        ...
        {id !== undefined && showProcessChild && (
          <ViewProcessChild id={id} processChild={processChilds && processChilds[`${id}`] && processChilds[`${id}`]} />
        )}
        <div className='nav-tabs-custom' style={{ boxShadow: 'none', MozBoxShadow: 'none', WebkitBoxShadow: 'none', marginBottom: 0 }}>
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a href='#info-view' onClick={() => handleChangeContent('info')} data-toggle='tab'>
                {translate('task.task_process.process_information')}
              </a>
            </li>
            <li>
              <a className='viewReport' href='#view-task-output' onClick={() => handleChangeContent('viewTaskOutput')} data-toggle='tab'>
                Kết quả công việc
              </a>
            </li>
            {props.checkManager && (
              <React.Fragment>
                <li>
                  <a className='viewReport' href='#report-view' onClick={() => handleChangeContent('report')} data-toggle='tab'>
                    Báo cáo
                  </a>
                </li>
                <li>
                  <a
                    className='viewReportHuman'
                    href='#report-human-view'
                    onClick={() => handleChangeContent('reportHuman')}
                    data-toggle='tab'
                  >
                    Báo cáo thành viên
                  </a>
              ...
              <div className={`${isTabPane ? '' : 'col-md-4'}`}>
                <div className='description-box without-border'>
                  ...
                  <div>
                    ...
                    <span>
                      {formatDate(startDate)} <i className='fa fa-fw fa-caret-right'></i> {formatDate(endDate)}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    ></div>
                    {translate('task.task_process.wait_for_approval')}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <div
                      style={{
                        backgroundColor: '#84ffb8',
                        height: '30px',
                        width: '40px',
                        border: '2px solid #14984c',
                        borderRadius: '3px',
                        marginRight: '5px',
                        marginTop: 4
                      }}
                    ></div>
                    {translate('task.task_process.inprocess')}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    <div
                      style={{
                    ...
                    ></div>
                    {translate('task.task_process.finished')}
                  </div>
                  <div>
        ...
        <React.Fragment>
          <div className='tab-content'>
            <div className={selectedView === 'report' ? 'active tab-pane' : 'tab-pane'} id='info-report'>
              <ReportProcess
                officeHours={props.data.officeHours}
                convertDayToHour={props.data.convertDayToHour}
                listTask={props.data.tasks}
                processTemplate={props.data.processTemplate}
              />
            </div>
          </div>
          <div className='tab-content'>
            <div className={selectedView === 'reportHuman' ? 'active tab-pane' : 'tab-pane'} id='info-report-human'>
              <ReportHumanOfProcess
                officeHours={props.data.officeHours}
                convertDayToHour={props.data.convertDayToHour}
                listTask={props.data.tasks}
              />
