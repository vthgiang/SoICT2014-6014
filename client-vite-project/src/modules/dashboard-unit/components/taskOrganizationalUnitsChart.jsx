...
class TaskOrganizationalUnitsChart extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      nextProps.tasks?.organizationUnitTasks &&
      !this.isEqual(nextProps.tasks?.organizationUnitTasks?.tasks, this.state.taskOfUnists) &&
    ...
    if (
      JSON.stringify(nextProps.childOrganizationalUnit) !== JSON.stringify(this.props.childOrganizationalUnit) ||
  ...
  componentDidUpdate = (prevProps, prevState) => {
    ...
    if (prevState.advancedMode === this.state.advancedMode) this.renderChart()
  }
  ...
  setDataLineChart = () => {
    ...
    if (!totalTask) {
      childOrganizationalUnit &&
        childOrganizationalUnit.forEach((unit) => {
      ...
      childOrganizationalUnit.forEach((x, index) => {
        ...
        row = row.map((r) => {
          let taskOfUnistInMonth = taskOfUnist.filter((t) => {
            ...
            if (
              (new Date(r).getTime() <= new Date(t.startDate).getTime() && new Date(t.startDate).getTime() <= new Date(endDate)) ||
  ...
  setSingleDataChart = () => {
    ...
    let totalTask = ['Số công việc'],
      taskPerEmployeeOfUnit = ['Công việc trên đầu người']
    ...
    return [title, totalTask, taskPerEmployeeOfUnit]
  }
  ...
  setMultiDataChart = () => {
    ...
    childOrganizationalUnit &&
      childOrganizationalUnit.forEach((unit, index) => {
        let taskOfUnist = []
        if (listTask.length !== 0) {
    ...
    return {
      data: [shortTitle, totalTask, taskPerEmployeeOfUnit],
      title
    }
  }
  ...
  renderChart = () => {
    ...
    if (singleUnit) {
      // chế độ 1 unit
      if (barChart || !barChart) dataChart = this.setSingleDataChart()
    } else {
          ...
          {!singleUnit && (
            <a title={advancedMode ? 'Tắt chế độ nâng cao' : 'Bật chế độ nâng cao'}>
              <i
                ...
                onClick={this.handleToggleAdvanceMode}
          ...
          {!advancedMode && (
            <div className='box-tools pull-right'>
              <div className='btn-group pull-rigth'>
                <button
                  type='button'
                  className={`btn btn-xs ${barChart ? 'btn-danger' : 'active'}`}
                  onClick={() => this.handleChangeTypeChart(true)}
                >
                  Bar Chart
                </button>
                <button
                  type='button'
                  className={`btn btn-xs ${barChart ? 'active' : 'btn-danger'}`}
                  onClick={() => this.handleChangeTypeChart(false)}
                >
                  Line Chart
                </button>
              </div>
              ...
              {!singleUnit && advancedMode && searchAdvanceMode && (
                <div className='box-tools pull-right'>
                  <div className='btn-group pull-rigth'>
                    <button
                      type='button'
                      className={`btn btn-xs ${totalTask ? 'btn-danger' : 'active'}`}
                    ...
                    <button
                      type='button'
                      className={`btn btn-xs ${totalTask ? 'active' : 'btn-danger'}`}
                      onClick={() => this.handleChangeViewChart(false)}
                    >
                      Công việc trên đầu người
                    </button>
                  </div>
                </div>
              )}
              <section id={'taskUnitsChart'} className='c3-chart-container'>
                ...
                {!singleUnit && advancedMode && searchAdvanceMode && (
                  <CustomLegendC3js
                    ...
                    title={
                      this.props?.childOrganizationalUnit &&
                      `${translate('general.list_unit')} (${this.props?.childOrganizationalUnit?.length})`
                    }
                    dataChartLegend={
                      this.props?.childOrganizationalUnit?.length && this.props.childOrganizationalUnit.map((item) => item.name)
                    }
                  />
                )}
              </section>
