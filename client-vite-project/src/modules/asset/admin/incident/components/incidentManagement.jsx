...
function IncidentManagement(props) {
  ...
  var pageTotal =
    incidentManager.incidentLength % limit === 0
      ...
      data = data.forEach((asset, dataIndex) => {
        ...
        if (item && item.incidentLogs && item.incidentLogs.length !== 0) {
          let assetLog = item.incidentLogs.map((x, index) => {
            ...
            let reporter =
              x.reportedBy && userlist.length && userlist.filter((item) => item._id === x.reportedBy).pop()
                ? userlist.filter((item) => item._id === x.reportedBy).pop().email
                : ''
      ...
      <div className='box-body qlcv'>
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            <SelectMulti
              id={`multiSelectIncidentType`}
              multiple='multiple'
              options={{
                nonSelectedText: translate('asset.general_information.select_incident_type'),
                allSelectedText: translate('asset.general_information.select_all_incident_type')
              }}
          ...
          tableBodyData={
            ...
            lists.map((x, index) => {
              return {
                ...
                assetCode: (
                  <td>
                    <a onClick={() => handleEditAsset(x.asset)}>{x.asset && x.asset.code}</a>
                  </td>
                ),
      ...
      {currentRowEditAsset && (
        <AssetEditForm
          ...
          unitsProducedDuringTheYears={
            currentRowEditAsset.unitsProducedDuringTheYears &&
            currentRowEditAsset.unitsProducedDuringTheYears.map((x) => ({
