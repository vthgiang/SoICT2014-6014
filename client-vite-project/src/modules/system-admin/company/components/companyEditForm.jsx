...
function CompanyEditForm(props) {
  ...
  const isFormValidated = () => {
    const { companyName, companyShortName, companyDescription, companyEmail } =
      state;
    const { translate } = props;
    if (
      ...
      !ValidationHelper.validateDescription(translate, companyDescription)
        .status
    )
      return false;
    return true;
  ...
  return (
    ...
    >
      <form id="form-edit-company">
        <div className="row">
          <div
            className={`form-group col-sm-9 ${nameError === undefined ? "" : "has-error"}`}
          >
            <label>
          ...
          <div className="form-group col-sm-3">
            <label>
              {translate("system_admin.company.table.service")}
              <span className="text-red"> * </span>
            </label>
            <select
              className="form-control"
            ...
            >
              <option key="1" value>
                {translate("system_admin.company.on")}
              </option>
              <option key="2" value={false}>
                {translate("system_admin.company.off")}
              </option>
            </select>
          </div>

          <div
            className={`form-group col-sm-9 ${shortNameError === undefined ? "" : "has-error"}`}
          >
          ...
          <div className="form-group col-sm-3">
            <label>
              {translate("system_admin.company.table.log")}
              <span className="text-red"> * </span>
            </label>
            <select
              className="form-control"
        ...
        <div
          className={`form-group ${emailError === undefined ? "" : "has-error"}`}
        >
          <label>
        ...
        <div
          className={`form-group ${descriptionError === undefined ? "" : "has-error"}`}
        >
          <label>
...
const connectedCompanyEditForm = connect(
  mapState,
  action,
)(withTranslate(CompanyEditForm));
