...
const FieldManagement = (props) => {
  ...
  pageTotal = field.totalList % limit === 0 ? parseInt(field.totalList / limit) : parseInt(field.totalList / limit + 1)
  ...
  return (
    <div className='box'>
      <div className='box-body qlcv'>
        ...
        <div className='form-inline' style={{ marginBottom: 10 }}>
          ...
          <div className='form-group'>
            ...
            <input
              ...
              placeholder={translate('human_resource.field.table.name')}
              autoComplete='off'
            />
            <button type='submit' className='btn btn-success' title={translate('general.search')} onClick={() => handleSunmitSearch()}>
        ...
        <table id={tableId} className='table table-striped table-bordered table-hover'>
          <thead>
            <tr>
              ...
              <th style={{ width: '120px', textAlign: 'center' }}>
                ...
                <DataTableSetting
                  tableId={tableId}
                  columnArr={[translate('human_resource.field.table.name'), translate('human_resource.field.table.description')]}
                  setLimit={setLimit}
                />
              </th>
            </tr>
          ...
          <tbody>
            {listFields &&
              listFields.length !== 0 &&
              listFields.map((x, index) => {
                return (
                  <tr key={index}>
                    <td>{x.name}</td>
                    <td>{x.description}</td>
                    <td style={{ textAlign: 'center' }}>
                      <a onClick={() => handleEdit(x)} className='edit text-yellow' style={{ width: '5px' }}>
      ...
      {currentRow && <FieldEditForm _id={currentRow._id} name={currentRow.name} description={currentRow.description} />}
    </div>
  )
}
