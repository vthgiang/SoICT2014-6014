                    ...
                    for (let n in users) {
                      if (userlist?.length > 0) {
                        userlist.forEach((item) => {
                          if (item._id === users[n]) infoManager = [...infoManager, item]
                        })
                      }
                    }
                  }
                  ...
                  return (
                    <fieldset key={index} className='scheduler-border' style={{ marginBottom: 10, paddingBottom: 10 }}>
                      <legend className='scheduler-border' style={{ marginBottom: 0 }}>
                        <h4 className='box-title'>{obj.name}</h4>
                      </legend>
                      <div className='form-group' key={index} style={{ marginBottom: 0 }}>
                        <div className='employeeBox'>
                          <SelectBox
                            ...
                            style={{ width: '100%' }}
                            multiple={true}
                            items={user.list.map((y) => {
                              return { value: y._id, text: `${y.name} (${y.email})` }
                            })}
                          />
                        </div>
                        <button
                          ...
                          onClick={() => this.handleAddEmployeesManager(obj.id)}
                        >
                          {translate('human_resource.manage_department.add_employee_unit')}
                        </button>
                      </div>
                            ...
                            infoManager.map((user, index) => (
                              <tr key={index}>
                                ...
                                <td style={{ textAlign: 'left' }}>
                                  {user.allRolePosition ? user.allRolePosition.map((o) => o).join(', ') : null}
                                </td>
                                <td>
                                  <a className='delete' title='Delete' onClick={() => this.handleDeleteEmployeesManager(user._id, obj.id)}>
                ...
                {roleDeputyManagers.map((obj, index) => {
                  ...
                  if (obj.users && obj.users.length > 0) {
                    ...
                    for (let n in users) {
                      if (userlist?.length > 0) {
                        userlist.forEach((item) => {
                          if (item._id === users[n]) infoDeputyManagers = [...infoDeputyManagers, item]
                        })
                      }
                    }
                  }
                    ...
                    <fieldset key={index} className='scheduler-border' style={{ marginBottom: 10, paddingBottom: 10 }}>
                      ...
                      <div className='form-group' key={index} style={{ marginBottom: 0 }}>
                        ...
                        <button
                          ...
                          className='btn btn-success pull-right'
                          style={{ marginBottom: 5 }}
                          onClick={() => this.handleAddEmployeesDeputyManager(obj.id)}
                        >
                          {translate('human_resource.manage_department.add_employee_unit')}
                        </button>
                      </div>
                              ...
                              <tr key={index}>
                                ...
                                <td>
                                  <a
                                    className='delete'
                                    title='Delete'
                                    onClick={() => this.handleDeleteEmployeesDeputyManager(user._id, obj.id)}
                                  >
                                    <i className='material-icons'></i>
                                  </a>
                                </td>
                  ...
                  roleEmployees.map((x, index) => {
                    ...
                    if (x.users && x.users.length > 0) {
                      ...
                      for (let n in users) {
                        if (userlist?.length > 0) {
                          userlist.forEach((item) => {
                            if (item._id === users[n]) infoEmployee = [...infoEmployee, item]
                          })
                        }
                      }
                    }
                    ...
                    return (
                      <fieldset key={index} className='scheduler-border' style={{ marginBottom: 10, paddingBottom: 10 }}>
                        <legend className='scheduler-border' style={{ marginBottom: 0 }}>
                          <h4 className='box-title'>{x.name}</h4>
                        </legend>
                        <div className='form-group' key={index} style={{ marginBottom: 0 }}>
                          <div className='employeeBox'>
                            <SelectBox
                            ...
                            {infoEmployee.length > 0 &&
                              infoEmployee.map((user, index) => (
                                <tr key={index}>
                                  ...
                                  <td>
                                    <a className='delete' title='Delete' onClick={() => this.handleDeleteEmployees(user._id, x.id)}>
                                      <i className='material-icons'></i>
                                    </a>
                                  </td>
                                </tr>
