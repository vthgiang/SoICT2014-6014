...
function areEqual(prevProps, nextProps) {
  if (
    ...
    JSON.stringify(prevProps.createEmployeeKpiSet.employeeKpiSetByMonth) ===
      JSON.stringify(nextProps.createEmployeeKpiSet.employeeKpiSetByMonth)
  ) {
    return true
  } else {
    return false
...
function KpiUser(props) {
  ...
  const renderChart = (data) => {
    ...
    setTimeout(function () {
      chart.load({
        columns: [data.ratioX, ['data1', ...data.approvedPoint], ['data2', ...data.automaticPoint], ['data3', ...data.employeePoint]]
      })
    }, 100)
    setTimeout(function () {
      chart.load({
