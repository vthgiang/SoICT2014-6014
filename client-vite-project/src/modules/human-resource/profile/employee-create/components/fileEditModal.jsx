...
function FileEditModal(props) {
  ...
  const { name, description, number, status, file, urlFile, fileUpload, errorOnNameFile, errorOnDiscFile, errorOnNumberFile } = state
  ...
  const isFormValidated = () => {
    ...
    let result = validateNameFile(name, false) && validateDiscFile(description, false) && validateNumberFile(number, false)
    return result
  }
      ...
      >
        <form className='form-group' id={`form-create-file-${id}`}>
          ...
          <div className={`form-group ${errorOnNameFile && 'has-error'}`}>
            <label>
              {translate('human_resource.profile.file_name')}
          ...
          <div className='row'>
            ...
            <div className={`form-group col-sm-6 col-xs-12 ${errorOnNumberFile && 'has-error'}`}>
              <label>
                {translate('human_resource.profile.number')}
                <span className='text-red'>*</span>
              </label>
