      ...
      <div className='box-body'>
        <div className=' row col-md-12'>
          ...
          <fieldset className='scheduler-border'>
            ...
            {(pageCreate || employeesInfo?.organizationalUnits?.length === 0) && (
              <a
                style={{ marginBottom: '10px', marginTop: '2px', cursor: 'pointer' }}
                className='btn btn-success pull-right'
                title={translate('human_resource.profile.employee_management.staff_no_unit_title')}
                disabled
              >
                {translate('modal.create')}
              </a>
            )}
            {!pageCreate && employeesInfo?.organizationalUnits?.length !== 0 && (
              <AnnualLeaveAddModal handleChange={handleAddAnnualLeave} id={`addSabbatical${id}`} />
            )}
                  ...
                  annualLeaves.map((x, index) => {
                    ...
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          <a
                            onClick={() => handleViewEdit(x, index)}
                          ...
                          <a
                            ...
                            title='Delete'
                            data-toggle='tooltip'
                            onClick={() => handleDeleteAnnualLeave(index)}
