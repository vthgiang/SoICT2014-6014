...
function ModalAddCareerPosition(props) {
  ...
  const validatePosition = (value, willUpdateState = true) => {
    ...
    if (!value || value?.toString()?.replace(/\s/g, '') === '0') message = translate('general.validate.empty_error')
  ...
  return (
    <React.Fragment>
      <ButtonModal
        modalID={`modal-create-career-position-${id}`}
      ...
      >
        <form className='form-group' id={`modal-create-career-position-${id}`}>
          ...
          <div className={`form-group ${errorOnPosition && 'has-error'}`}>
            <label>
              {translate('human_resource.profile.project_employee_position')}
              <span className='text-red'>*</span>
              <a href='/hr-list-career-position' target='_blank'>
                {' '}
                (Quản lý){' '}
              </a>
            </label>
            <SelectBox
              ...
              items={[
                { value: '0', text: 'Chọn vị trí công việc' },
                ...listPosition.map((y) => {
                  return { value: y._id, text: y.name }
                }),
                { value: '', text: 'Chọn vị trí công việc' }
              ]}
          ...
          <div className='form-group'>
            <label>
              {translate('human_resource.profile.project')}
              <span className='text-red'>*</span>
            </label>
            <input
              type='text'
          ...
          <div className='form-group'>
            ...
            <textarea
              ...
              type='text'
              value={professionalExperience ? professionalExperience : ''}
              className='form-control'
