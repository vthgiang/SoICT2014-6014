...
function ManageLeaveApplication(props) {
  ...
  const handleAcceptApplication = (value) => {
    ...
    Swal.fire({
      ...
      if (result.value) {
        props.updateAnnualLeave(value._id, {
          ...value,
  ...
  let pageTotal =
    annualLeave.totalList % limit === 0 ? parseInt(annualLeave.totalList / limit) : parseInt(annualLeave.totalList / limit + 1)
      ...
      <div className='box-body qlcv'>
        <div className='form-inline'>
          ...
          <div className='dropdown pull-right' style={{ marginBottom: 15 }}>
            ...
            <ul className='dropdown-menu pull-right' style={{ marginTop: 0 }}>
              <li>
                <a style={{ cursor: 'pointer' }} onClick={() => window.$(`#modal-create-annual-leave`).modal('show')}>
                  {translate('human_resource.salary.add_by_hand')}
                </a>
              </li>
              <li>
                <a style={{ cursor: 'pointer' }} onClick={() => window.$(`#modal_import_file`).modal('show')}>
        ...
        <div className='form-inline' style={{ marginBottom: 10 }}>
          ...
          <div className='form-group'>
            ...
            <DatePicker id='month' dateFormat='month-year' value={formatDate(month, true)} onChange={handleMonthChange} />
          </div>
          {/* Trạng thái */}
          <div className='form-group'>
        ...
        <AnnualLeaveCreateForm typeView='manager' />
                ...
                <tr key={index}>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    {(x.status === 'waiting_for_approval' || x.status === 'disapproved') && (
                      <a
                        onClick={() => handleAcceptApplication(x)}
                        className='add_circle'
                        style={{ width: '5px' }}
                        title={translate('human_resource.work_plan.accept_application')}
                      >
                    ...
                    {(x.status === 'waiting_for_approval' || x.status === 'approved') && (
                      <a
                        onClick={() => handleRefuseApplication(x)}
                        className='delete'
                        style={{ width: '5px' }}
                        title={translate('human_resource.work_plan.refuse_application')}
                      >
