...
function AnnualLeaveManagement(props) {
    ...
    if (data) {
      data = data.map((x, index) => {
        ...
        return {
          ...
          status:
            x.status === 'approved'
              ? translate('human_resource.annual_leave.status.approved')
              : x.status === 'waiting_for_approval'
                ? translate('human_resource.annual_leave.status.waiting_for_approval')
                : translate('human_resource.annual_leave.status.disapproved')
        }
      })
    }
    let exportData = {
  ...
  pageTotal = annualLeave.totalList % limit === 0 ? parseInt(annualLeave.totalList / limit) : parseInt(annualLeave.totalList / limit + 1)
    ...
    <div className='box'>
      <div className='box-body qlcv'>
        <div className='dropdown pull-right' style={{ marginBottom: 15 }}>
          ...
          >
            {translate('human_resource.annual_leave.add_annual_leave')}
          </button>
          <ul className='dropdown-menu pull-right' style={{ marginTop: 0 }}>
            <li>
              <a style={{ cursor: 'pointer' }} onClick={createAnnualLeave}>
                {translate('human_resource.salary.add_by_hand')}
              </a>
            </li>
            <li>
              <a style={{ cursor: 'pointer' }} onClick={_importAnnualLeave}>
        ...
        <AnnualLeaveCreateForm typeView='admin' />
        {importAnnualLeave && <AnnualLeaveImportForm />}
        <ExportExcel
          id='export-annual_leave'
        ...
        <div className='form-inline'>
          ...
          <div className='form-group'>
            ...
            <DatePicker id='month' dateFormat='month-year' value={formatDate(month ? month : '', true)} onChange={handleMonthChange} />
          </div>
        </div>
        ...
        <div className='form-group col-md-12 row'>
          {(Number(annualLeave.numberWaitForApproval) > 0 ||
            ...
            <React.Fragment>
              ...
              >{`${Number(annualLeave.numberWaitForApproval) + Number(annualLeave.numberApproved) + Number(annualLeave.numberNotApproved)} ${translate('human_resource.annual_leave.leaveOfAbsenceLetter')}`}</span>
            </React.Fragment>
          )}
          ...
          {Number(annualLeave.numberWaitForApproval) > 0 && (
            <React.Fragment>
              <span className='text-warning' style={{ fontWeight: 'bold' }}>
                ,&nbsp;{`${annualLeave.numberWaitForApproval} ${translate('human_resource.annual_leave.waiting_for_approval_letter')}`}
              </span>
            </React.Fragment>
          )}
          {Number(annualLeave.numberApproved) > 0 && (
            <span className='text-success' style={{ fontWeight: 'bold' }}>
              ,&nbsp;{`${annualLeave.numberApproved} ${translate('human_resource.annual_leave.approved_letter')}`}
            </span>
          )}
          {Number(annualLeave.numberNotApproved) > 0 && (
            <span className='text-danger' style={{ fontWeight: 'bold' }}>
              ,&nbsp;{`${annualLeave.numberNotApproved} ${translate('human_resource.annual_leave.not_approved_letter')}`}
            </span>
          )}
          {(Number(annualLeave.numberWaitForApproval) > 0 ||
            Number(annualLeave.numberApproved) > 0 ||
          ...
          <thead>
            <tr>
              ...
              <th style={{ width: '120px', textAlign: 'center' }}>
                {translate('human_resource.annual_leave.table.action')}
                <DataTableSetting
                  tableId={tableId}
                  columnArr={[
              ...
              listAnnualLeaves.map((x, index) => {
                ...
                return (
                  <tr key={index}>
                    ...
                    <td>
                      <p>{formatDate(x.startDate)}</p>
                      {x.startTime ? x.startTime : null}
                    </td>
                    <td>
                      <p>{formatDate(x.endDate)}</p>
                      {x.endTime ? x.endTime : null}
                    </td>
                    <td>{organizationalUnit ? organizationalUnit.name : null}</td>
                    <td>{x.reason}</td>
                    <td
                      style={{ color: x.status === 'approved' ? '#28A745' : x.status === 'waiting_for_approval' ? '#ff7f0e' : '#dd4b39' }}
                    >
                      {translate(`human_resource.annual_leave.status.${x.status}`)}
                    </td>
...

      {
        ...
        state?.requestToChange && (
          <AnnualLeaveEditForm
            ...
            employeeNumber={
              state?.requestToChange?.employee
                ? `${state?.requestToChange?.employee?.employeeNumber} - ${state?.requestToChange.employee.fullName}`
                : ''
            }
