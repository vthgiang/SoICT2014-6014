...
class CrmCustomerInformation extends Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <DialogModal
          modalID={`modal-crm-customer-info-${customerId}`}
            ...
            <div className='row'>
              <div className='col-xs-12 col-sm-5 col-md-3 col-lg-3'>
                ...
                >
                  ...
                  <div>
                    <strong>
                      <i className='fa fa-user margin-r-5' /> {translate('crm.customer.name')}
                    </strong>
                    <p className='text-muted'>{customerInfomation.name ? customerInfomation.name : ''}</p>

                    <strong>
                      <i className='fa fa-barcode margin-r-5' /> {translate('crm.customer.code')}
                    </strong>
                    <p className='text-muted'>{customerInfomation.code ? customerInfomation.code : ''}</p>

                    <strong>
                      <i className='fa fa-phone margin-r-5' /> {translate('crm.customer.mobilephoneNumber')}
                    </strong>
                    <p className='text-muted'>{customerInfomation.mobilephoneNumber ? customerInfomation.mobilephoneNumber : ''}</p>

                    <strong>
                      <i className='fa fa-envelope margin-r-5' /> {translate('crm.customer.email')}
                    </strong>
                    <p className='text-muted'>{customerInfomation.email ? customerInfomation.email : ''}</p>

                    <strong>
                      <i className='fa fa-map-marker margin-r-5' /> {translate('crm.customer.address')}
                    </strong>
              ...
              <div className='col-xs-12 col-sm-8 col-md-9 col-lg-9'>
                <div className='nav-tabs-custom'>
                  <ul className='nav nav-tabs'>
                    <li className='active'>
                      <a href={`#generalInfo-${customerId}`} data-toggle='tab'>
                        Thông tin chi tiết
                      </a>
                    </li>
                    <li>
                      <a href={`#purchaseHistories-${customerId}`} data-toggle='tab'>
                        Lịch sử mua hàng
                      </a>
                    </li>
                    <li>
                      <a href={`#careHistories-${customerId}`} data-toggle='tab'>
                        Lịch sử chăm sóc khách hàng
                      </a>
                    </li>
                    <li>
                      <a href={`#historyOfStateTransitions-${customerId}`} data-toggle='tab'>
                        Lịch sử thay đổi trạng thái
                      </a>
                    </li>
                    <li>
                      <a href={`#fileAttachment-${customerId}`} data-toggle='tab'>
                        File đính kèm
                      </a>
                    </li>
                  </ul>
                  <div className='tab-content'>
                    ...
                    <PurchaseHistoriesInfoForm id={`purchaseHistories-${customerId}`} customerId={customerId} />
                    ...
                    {customerInfomation && dataStatus === 3 && (
                      <FileTabInfoForm id={`fileAttachment-${customerId}`} files={customerInfomation.files} customerId={customerId} />
                    )}
                  </div>
                </div>
              </div>
