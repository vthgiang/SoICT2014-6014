...
function CustomerPromotionInfoForm(props) {
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID='modal-customer-promotion-info'
        isLoading={false}
      ...
      >
        ...
        <table className='table table-hover table-striped table-bordered' id='customer-promotion-info' style={{ marginTop: '10px' }}>
          ...
          <tbody>
            {customer &&
              customer.promotions.map((o, index) => (
                <tr key={o.code}>
                  <td>{index + 1}</td>
                  <td>{o.code}</td>
                  <td>{o.value}</td>
                  ...
                  <td style={{ textAlign: 'center' }}>
                    <a className='text-yellow' title='Chỉnh sửa khuyến mãi khách hàng' onClick={() => handleEditPromotion(o)}>
        ...
        <h4 style={{ textAlign: 'center' }}>
          <b>Danh sách khuyến mãi nhóm có thể sử dụng</b>
        </h4>
