...
function CreateForm(props) {
  ...
  const isValidateForm = () => {
    return (
      validateName(state.documentName, false) &&
  ...
  function deleteDocumentVersion(i) {
    let { documentVersions, documentFile, documentFileScan, versionName, documentIssuingDate, documentEffectiveDate, documentExpiredDate } =
      state
  ...
  const editVersion = async (data) => {
    let { documentFile, documentFileScan, versionName, documentIssuingDate, documentEffectiveDate, documentExpiredDate } = state
    const file = {
      fileName: data.file,
      urlFile: data.urlFile,
      fileUpload: data.fileUpload
  ...
  const documentRoles = role.list.map((role) => {
    return { value: role._id, text: role.name }
  })
  const relationshipDocs = documents.administration.relationshipDocs.paginate.map((doc) => {
      ...
      <div className='form-inline'>
        <div className='dropdown pull-right' style={{ marginBottom: 15 }}>
          ...
          <ul className='dropdown-menu pull-right'>
            ...
            <li>
              <a
                href='#modal_import_file_document'
                title='ImportForm'
                onClick={(event) => {
        ...
        <form id='form-create-document'>
          <div className='nav-tabs-custom'>
            <ul className='nav nav-tabs'>
              <li className='active'>
                <a href='#doc-info' data-toggle='tab'>
                  {translate('document.infomation_docs')}
                </a>
              </li>
              <li>
                <a href='#doc-sub-info' data-toggle='tab'>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    ...
                    <div className={`form-group ${!errorOfficialNumber ? '' : 'has-error'}`}>
                      ...
                      <input
                        ...
                        onChange={handleOfficialNumber}
                        placeholder={translate('document.doc_version.exp_official_number')}
                      />
                    ...
                    <div className='form-group'>
                      ...
                      <input
                        type='text'
                        className='form-control'
                        onChange={handleIssuingBody}
                        placeholder={translate('document.doc_version.exp_issuing_body')}
                      />
                    </div>
                    ...
                    <div className='form-group'>
                      ...
                      <input
                        ...
                        placeholder={translate('document.doc_version.exp_signer')}
                      />
                    </div>
                  </div>
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                            ...
                            return (
                              <tr key={i}>
                                ...
                                <td>
                                  <a href='#'>
                                    <u>
                                      {version.documentFile && version.documentFile.length && version.documentFile[0].fileName
                                        ? version.documentFile[0].fileName
                                        : ''}
                                    </u>
                                  </a>
                                </td>
                                <td>
                                  <a href='#'>
                                    <u>
                                      {version.documentFileScan && version.documentFileScan.length && version.documentFileScan[0].fileName
                                        ? version.documentFileScan[0].fileName
                                        : ''}
                                    </u>
                                  </a>
                                </td>
                                <td>
                                  <a
                                    ...
                                    onClick={() => toggleEditVersion(version, i)}
                                  >
                ...
                <div className='row'>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    ...
                    <div className='form-group'>
                      ...
                      {path && path.length ? path.map((y) => <div>{y}</div>) : null}
                    </div>
                  </div>
                </div>
              </div>
