...
function QualityControlComponent(props) {
  ...
  const { dataLots, statusQuality, content, code, listGoods, errorOnStatus, passedQuantityErrorPosition, passedQuantityErrorOnTable } =
    state
  return (
    <React.Fragment>
                  ...
                  {typeof dataLots === 'undefined' || dataLots.length === 0 ? (
                    ...
                    dataLots.map((x, index) => (
                      <tr key={index}>
                        ...
                        <td>
                          <div
                            className={`form-group ${parseInt(passedQuantityErrorPosition) === index && passedQuantityErrorOnTable ? 'has-error' : ''} `}
                          >
                            <input
