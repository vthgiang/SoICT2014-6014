...
function GoodTakesWorkFlowModal(props) {
  ...
  useEffect(() => {
    ...
    const listLotsByGood =
      props.billInfor.goods.length == 0 || props.billInfor.goods[0].lots.length == 0
        ? props.lots.listLotsByGood
        : formatGoodDataToListLotsByGood(props.billInfor.goods)
    setState({
      ...state,
      billId: props.billId,
  ...
  const sortData = (data, type) => {
    if (type == 2) {
      data.forEach((item) => {
        item.binLocations.sort((a, b) => a.binLocation.name.localeCompare(b.binLocation.name))
      })
    }
    return data
  }
  ...
  const getSubArray = (array, passedQualityControl) => {
    let data = []
    array.forEach((item) => {
      if (item.passedQualityControl == passedQualityControl) data.push(item)
    })
    return data
  }
  ...
  const {
    ...
    displayType,
    listData,
    quantityErrorPosition1,
    quantityErrorPosition2,
    quantityErrorOnTable
  } = state
  ...
  const tbodies = listData.map((data, index) => {
      ...
      if (displayType == 1) {
        return (
          <tr key={index2}>
            ...
            <td>
              <div
                className={`form-group ${parseInt(quantityErrorPosition1) === index && parseInt(quantityErrorPosition2) === index2 && quantityErrorOnTable ? 'has-error' : ''} `}
              >
                <input
    ...
    return <tbody key={index}>{binRows}</tbody>
  })
