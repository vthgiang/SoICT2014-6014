...
class goodIssueCreateForm extends Component {
  ...
  getRemainingQuantityOfGood = () => {
    ...
    if (listBills.length) {
      // lấy trong materials trừ đi trong listBills là ra
      let listGoodCopy = []
      materials.map((x) => {
  ...
  static getDerivedStateFromProps = (props, state) => {
    if (props.commandIssueId !== state.commandIssueId) {
      if (state.listRemainingGoods.length === 0) {
        // Nếu chưa lên đơn nào thì tính toán listRemainingGoods
  ...
  isGoodsValidated = () => {
    if (this.validateQuantityChange(this.state.good.quantity, false) || this.validateGoodChange(this.state.good.goodId, false)) {
      return false
    }
    return true
  }
  ...
  isFormValidated = () => {
    ...
    if (!this.state.listBills.length || !result) {
      return false
    }
    return true
  }
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <DialogModal
          modalID={`modal-create-bill-issue-material`}
              ...
              <fieldset className='scheduler-border'>
                ...
                <div className={`form-group`}>
                  <table className='table'>
                    ...
                    <tbody>
                      {commandIssue && commandIssue.good && commandIssue.good.materials && commandIssue.good.materials.length ? (
            ...
            <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
              <fieldset className='scheduler-border'>
                ...
                <div className='row'>
                  ...
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    <div className={`form-group ${!errorQualityControlStaffs ? '' : 'has-error'}`}>
                      <label>
                        {translate('manage_warehouse.bill_management.quality_control_staffs')}
                        <span className='attention'> * </span>
                      </label>
                      <SelectBox
                ...
                <div className='row'>
                  <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                    <div className={`form-group ${!errorName ? '' : 'has-error'}`}>
                      <label>
                        {translate('manage_warehouse.bill_management.name_receiver')}
                        <span className='attention'> * </span>
                      </label>
                    ...
                    <div className={`form-group ${!errorPhone ? '' : 'has-error'}`}>
                      <label>
                        {translate('manage_warehouse.bill_management.phone_receiver')}
                        <span className='attention'> * </span>
                      </label>
                        ...
                        <div className='row'>
                          <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                            <div className={`form-group ${!errorGood ? '' : 'has-error'}`}>
                              <label>
                                {translate('manage_warehouse.bill_management.choose_good')}
                                <span className='attention'> * </span>
                              </label>
                              <SelectBox
                          ...
                          <div className='col-xs-12 col-sm-6 col-md-6 col-lg-6'>
                            <div className={`form-group ${!errorQuantity ? '' : 'has-error'}`}>
                              <label className='control-label'>
                                {translate('manage_warehouse.bill_management.quantity')} <span className='attention'> * </span>
                              </label>
                              <div>
                                <input
                                  type='number'
                ...
                <div className='pull-right' style={{ marginBottom: '10px' }}>
                  {this.state.editBill ? (
                    <React.Fragment>
                      <button className='btn btn-success' onClick={this.handleCancelEditBill} style={{ marginLeft: '10px' }}>
                  ...
                  <button className='btn btn-primary' style={{ marginLeft: '10px' }} onClick={this.handleClearBill}>
                        ...
                        <tr key={index}>
                          ...
                          <td>
                            <a
                              ...
                              className='edit'
                              title={translate('general.edit')}
                              onClick={() => this.handleEditBill(bill, index)}
                            >
                              <i className='material-icons'></i>
                            </a>
                            <a
                              ...
                              className='delete'
                              title={translate('general.delete')}
                              onClick={() => this.handleDeleteBill(bill, index)}
                            >
                              <i className='material-icons'></i>
                            </a>
                          </td>
            ...
            <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12'>
              {!this.isFormValidated() && (
                <div className='pull-left form-group has-error' style={{ marginBottom: '10px' }}>
                  <label>
                    {translate('manage_warehouse.bill_management.quantity_needed_true')}
                    <span className='attention'> * </span>
                  </label>
                </div>
              )}
