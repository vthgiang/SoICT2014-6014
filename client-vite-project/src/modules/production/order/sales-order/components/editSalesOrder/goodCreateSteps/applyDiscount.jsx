    ...
    <React.Fragment>
      <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12' style={{ padding: '10px 10px 20px 10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline' }}>
          <span>Giá: &ensp;</span>
          <h4 className='text-red'>{pricePerBaseUnit ? formatCurrency(pricePerBaseUnit) + ' (vnđ) ' : '0 (vnđ) '}</h4> <h6>/ {baseUnit}</h6>
        </div>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <input type='number' value={quantity} className='input-add-quantity-for-quote' onChange={props.handleQuantityChange} />
            <button className='button-add-quantity-for-quote' onClick={(e) => handleQuantity(parseInt(quantity) + 1, e)}>
              +
            </button>
          </div>
      ...
      <div className='col-xs-12 col-sm-12 col-md-12 col-lg-12' style={{ padding: 10, height: '100%', borderTop: 'solid 0.3px #c5c5c5' }}>
        <div className='form-group apply-discount-for-good-element'>
          ...
          <div className='apply-discounts-for-good-tag'>
            <div>{countOfSlasChecked ? `Đã chọn ${countOfSlasChecked} cam kết chất lượng` : 'Hãy kiểm tra và chọn cam kết chất lượng'}</div>
          </div>
          <CreateSlaForGood
            goodId={good}
