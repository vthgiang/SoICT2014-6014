...
function StatisticsOfOrganizationalUnitKpi(props) {
          ...
          unit.map((kpi) => {
            if (kpi._id) {
              ...
              treeData.push({
                ...
                state: { opened: true },
                parent:
                  kpi?.employeeKpi?.[0]?.parentOfUnitKpi && organizationalUnit && organizationalUnit.text !== kpi.organizationalUnit
                    ? kpi.employeeKpi[0].parentOfUnitKpi.toString()
                    : '#',
  ...
  const setTreeData = () => {
    ...
    if (
      dashboardOrganizationalUnitKpi?.tasksOfChildrenOrganizationalUnit &&
      dashboardOrganizationalUnitKpi.tasksOfChildrenOrganizationalUnit !== []
    ) {
    ...
    let arrayListChildTargetSameOrganizationUnitKpi = FunctionHelperStatisticKpi.getArrayListChildTargetOrganizationUnitKpi(
      listOrganizationalUnitKpi,
      listChildTarget
    )
    let arrayListTaskSameOrganizationUnitKpi = FunctionHelperStatisticKpi.getArrayListTaskSameOrganizationUnitKpi(
      listOrganizationalUnitKpi,
      listTask,
      listChildTarget
    )
    ...
    if (
      arrayListChildTargetSameOrganizationUnitKpi &&
      arrayListChildTargetSameOrganizationUnitKpi.length !== 0 &&
      organizationalUnit &&
      arrayListTaskSameOrganizationUnitKpi
    ) {
      for (let i = arrayListChildTargetSameOrganizationUnitKpi.length - 1; i >= 0; i--) {
        let listTaskSameParent = arrayListTaskSameOrganizationUnitKpi.filter(
          (item) =>
            arrayListChildTargetSameOrganizationUnitKpi?.[i]?.[0]?.[0]?.[0] &&
            item?.[0]?.name === arrayListChildTargetSameOrganizationUnitKpi?.[i]?.[0]?.[0]?.[0]?._id
        )
        let treeData = []
        ...
        if (arrayListChildTargetSameOrganizationUnitKpi[i]) {
          for (let j = arrayListChildTargetSameOrganizationUnitKpi[i].length - 1; j >= 0; j--) {
            treeData[j] = []
            treeData[j] = traversesListChildTargetSameParent(
              arrayListChildTargetSameOrganizationUnitKpi?.[i]?.[j],
              treeData?.[j + 1],
              listTaskSameParent,
        ...
        {childrenOrganizationalUnit ? (
          <div className='box-body qlcv'>
            ...
            <div className='row row-equal-height'>
                  ...
                  {currentKPI ? (
                    <React.Fragment>
                      <h4 className='box-title'>
                        ...
                        <a
                          className='text-red'
                          title={translate('kpi.organizational_unit.statistics.unit_not_initial_kpi')}
                          onClick={() =>
                            showListInSwal(
                              organizationalUnitNotInitialKpi?.map((item) => item?.[0]?.name),
                              translate('kpi.organizational_unit.statistics.unit_not_initial_kpi')
                            )
                          }
                        >
                      ...
                      <Tree id='tree-qlcv-document' onChanged={onChanged} data={dataTree} plugins={false} />
                    </React.Fragment>
                  ) : (
                    organizationalUnitKpiLoading && (
                      <h4>
                        {translate('kpi.organizational_unit.create_organizational_unit_kpi_set.not_initialize')}{' '}
                        {month.slice(5, 7) + '-' + month.slice(0, 4)}
                      </h4>
                    )
                  )}
                  <SlimScroll
                    outerComponentId='details-tree'
                    innerComponentId='tree-qlcv-document'
              ...
              <div className='col-xs-12 col-sm-12 col-md-6 col-lg-6' style={{ padding: 10, maxHeight: '400px' }}>
                <DetailsOfOrganizationalUnitKpiForm details={details ? details : dataTree?.[0]} />
              </div>
            </div>
            ...
            <div className='row'>
              <div className='col-xs-12' style={{ padding: 10 }}>
                <div className='description-box' style={{ height: '100%' }}>
                  <h4 className='box-title'>
                    <span>
                      Tính hợp lý khi phân bố mục tiêu (KPI) đơn vị cho các nhân viên trong đơn vị, tháng{' '}
                      {month.slice(5, 7) + '-' + month.slice(0, 4)}
                    </span>
                    <a
                      className='text-red'
                      title={translate('task.task_management.explain')}
                      onClick={() => showDistributionOfEmployeeKpiDoc()}
                    >
                      <i className='fa fa-question-circle' style={{ color: '#dd4b39', cursor: 'pointer', marginLeft: '5px' }} />
                    </a>
                  </h4>

                  <DistributionOfEmployeeKpiChart organizationalUnitKPI={dataTree.filter((item) => item.parent === '#')} />
                </div>
              </div>
            </div>
            ...
            <div className='row'>
              <div className='col-xs-12' style={{ padding: 10 }}>
                <div className='description-box' style={{ height: '100%' }}>
                  <h4 className='box-title'>
                    <span>
                      Tính hợp lý khi phân bố mục tiêu (KPI) đơn vị cho các đơn vị con, tháng {month.slice(5, 7) + '-' + month.slice(0, 4)}
                    </span>
                    <a
                      className='text-red'
                      title={translate('task.task_management.explain')}
