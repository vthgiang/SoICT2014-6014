...
function ModalCreateEmployeeKpi(props) {
  ...
  const handleCreateEmployeeKpi = async () => {
    ...
    if (state.parent === null) {
      if (!currentOrganizationalKpiSet) {
        ...
        items = currentOrganizationalKpiSet.kpis
  ...
  if (currentOrganizationalUnitKpiSet === undefined) {
    ...
    items = currentOrganizationalUnitKpiSet.kpis
      .filter((item) => item.type === 0)
      .map((x) => {
        //type !==0 thì đc. cái này để loại những mục tiêu mặc định?
        return { value: x._id, text: x.name }
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <form
          id='formCreateEmployeeKpi'
          onSubmit={() => handleCreateEmployeeKpi(translate('kpi.employee.employee_kpi_set.create_employee_kpi_modal.success'))}
        >
          {/**Tên của mục tiêu */}
          <div className={`form-group ${errorOnName === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.create_employee_kpi_modal.name')}
          ...
          {createKpiUnit?.currentKPI && //unit.parent === null này!!! kiểm tra xem đây là đơn vị gốc hay không!
            items?.length > 0 && (
              <div className='form-group'>
                <label>
                  {translate('kpi.employee.employee_kpi_set.create_employee_kpi_modal.parents')}
                  <span className='text-red'>*</span>
                </label>
          ...
          <div className={`form-group ${errorOnCriteria === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.create_employee_kpi_modal.evaluation_criteria')}
              <span className='text-red'>*</span>
            </label>
            <QuillEditor id={'create-employee-kpi'} getTextData={handleCriteriaChange} toolbar={false} />
          ...
          <div className={`form-group ${errorOnWeight === undefined ? '' : 'has-error'}`}>
            <label>
              {translate('kpi.employee.employee_kpi_set.create_employee_kpi_modal.weight')}
              <span className='text-red'>*</span>
            </label>
