    ...
    <React.Fragment>
      <DialogModal
        ...
        title={
          ...
          `KPI ${employeeKpiSet.creator.name}, ${translate('general.month')} ${formatMonth(employeeKpiSet.date)}`
        }
        hasSaveButton={false}
        size={100}
      >
          ...
          <ul className='nav nav-tabs'>
            <li className='active'>
              <a href='#overview' data-toggle='tab'>
                {translate('menu.kpi_personal_overview')}
              </a>
            </li>
            <li>
              <a href='#detail' data-toggle='tab'>
                {translate('menu.kpi_member_detail')}
              </a>
            </li>
          </ul>
                        ...
                        list.map((item, index) => (
                          <li key={index} className={state.content === item._id ? 'active' : ''}>
                            <a
                              ...
                              onClick={() => handleChangeContent(item._id, employeeKpiSet.creator._id, item.type)}
                            >
                              {item.name}&nbsp;
                            </a>
                          </li>
                    ...
                    if (item._id === state.content)
                      return (
                        <React.Fragment key={item._id}>
                          ...
                          {exportData && (
                            <ExportExcel id='export-employee-kpi-management-detail-kpi' exportData={exportData} style={{ marginTop: 5 }} />
                          )}
                          ...
                          <div
                            ...
                            style={{ height: 'calc(80vh - 160px)', overflow: 'auto' }}
                          >
                            {/**Table danh sách công việc của mục tiêu */}
                            <table
                              id='employeeKpiEvaluate'
                            ...
                            >
                              <thead>
                                <tr>
                                  <th
                                    title={translate('kpi.evaluation.employee_evaluation.index')}
                                    style={{ width: '40px' }}
                                    className='col-fixed'
                                  >
                                    {translate('kpi.evaluation.employee_evaluation.index')}
                                  ...
                                  <th title={translate('kpi.evaluation.employee_evaluation.work_duration_time')}>
                                    {translate('kpi.evaluation.employee_evaluation.work_duration_time')}
                                  </th>
                                  <th title={translate('kpi.evaluation.employee_evaluation.evaluate_time')}>
                                    {translate('kpi.evaluation.employee_evaluation.evaluate_time')}
                                  </th>
                                  <th title={translate('kpi.evaluation.employee_evaluation.status')}>
                                    {translate('kpi.evaluation.employee_evaluation.status')}
                                  </th>
                                  <th title={translate('kpi.evaluation.employee_evaluation.contribution')}>
                                    {translate('kpi.evaluation.employee_evaluation.contribution')} (%)
                                  </th>
                                  <th title={translate('kpi.evaluation.employee_evaluation.point')}>
                                    {translate('kpi.evaluation.employee_evaluation.point')}
                                  </th>
                                  <th title={translate('kpi.evaluation.employee_evaluation.importance_level')}>
                                    {translate('kpi.evaluation.employee_evaluation.importance_level')}
                                  </th>
                                </tr>
                              </thead>
                                ...
                                {kpimembers?.tasks?.length > 0 &&
                                  ...
                                  kpimembers.tasks.map((itemTask, index) => (
                                    <tr key={index}>
                                      ...
                                      <td>
                                        <div>
                                          {translate('kpi.evaluation.employee_evaluation.evaluated_value')}:{' '}
