...
const ModalViewUsedLog = (props) => {
  ...
  return (
    <React.Fragment>
      <DialogModal
        size='50'
        modalID='modal-view-used-log-kpi-template'
        isLoading={false}
      ...
      >
        <div className='row' style={{ padding: '0 20px' }}>
          {kpiTemplate?.kpiSet.length === 0 ? (
            <h4>Mẫu KPI này chưa được sử dụng lần nào</h4>
          ) : (
            <div>
                ...
                <tbody>
                  {kpiTemplate?.kpiSet.map((item, index) => (
                    <tr className='' key={`_${index}`}>
                      ...
                      <td>
                        <ToolTip type={'text_tooltip'} dataTooltip={`Điểm tự động - Điểm tự đánh giá - Điểm người phê duyệt đánh giá`}>
                          <span>
                            <span>
                              {item?.automaticPoint !== null && item?.automaticPoint >= 0 ? item.automaticPoint : 'Chưa đánh giá'} -{' '}
                            </span>
                            <span>
                              {item?.employeePoint !== null && item?.employeePoint >= 0 ? item.employeePoint : 'Chưa đánh giá'} -{' '}
                            </span>
