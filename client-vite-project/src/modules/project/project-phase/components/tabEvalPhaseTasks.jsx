...
const TabEvalPhaseTasks = (props) => {
  const { currentTasks, translate, listTasksEval, project, currentMonth, projectDetailId, projectDetail, isLoading, handleChangeMonth } =
    props
  ...
  return (
    <React.Fragment>
      <div className='box-body qlcv'>
        <h4>
          <strong>Điểm số các công việc của giai đoạn theo tháng</strong>
        </h4>
        {/* Chọn tháng để lọc đánh giá */}
        <div className='form-group'>
          <label style={{ marginRight: 20 }}>Chọn tháng</label>
          <DatePicker
            id='start-date-eval-phase-statistical'
        ...
        {isLoading ? (
          ...
          <table id='eval-phase-statistical-table' className='table table-bordered table-hover'>
            <thead>
              <tr>
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      {taskItem?.actualEndDate &&
                        ...
                        moment(taskItem?.actualEndDate).format('HH:mm DD/MM/YYYY')}
                    </td>
                    ...
                    <td>
                      {checkUndefinedNull(taskItem?.overallEvaluation?.automaticPoint)
                        ? 'Chưa tính được'
                        : `${numberWithCommas(taskItem?.overallEvaluation?.automaticPoint)} / 100`}
                    </td>
                  </tr>
                )
              })}
...
export default connect(mapStateToProps, mapDispatchToProps)(withTranslate(TabEvalPhaseTasks))
