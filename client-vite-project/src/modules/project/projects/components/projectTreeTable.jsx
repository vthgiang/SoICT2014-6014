...
import './projectTreeTable.css'
import _isEqual from 'lodash/isEqual'
import { DataTableSetting } from '../../../../common-components'

function ProjectTreeTable(props) {
  const {
    translate,
    column,
    data,
    actions = true,
    tableId = 'project-tree-table',
    tableSetting = false,
    rowPerPage = true,
   ...
   * @param {*} actionData : Array tên các action của từng dòng
   * @param {*} id : Id dữ liệu tương ứng từng dòng, dùng để gọi server lấy dữ liệu
   * @param {*} type : xác định loại dữ liệu của dòng này
   */
  const showActionColumn = (actionData, id, type) => {
    if (type === 'task') {
      switch (actionData) {
        case 'edit':
          return (
            <a
              ...
              onClick={() => props.funcEditTask(id)}
              className='edit'
              data-toggle='modal'
              title={titleAction.edit}
            >
        ...
        case 'view':
          return (
            <a style={{ cursor: 'pointer' }} onClick={() => props.funcViewTask(id)} data-toggle='modal' title={titleAction.view}>
      ...
      switch (actionData) {
        case 'edit':
          return (
            <a
              style={{ cursor: 'pointer' }}
              onClick={() => props.funcEditPhase(id)}
              className='edit'
              data-toggle='modal'
              title={titleAction.edit}
            >
        ...
        case 'view':
          return (
            <a style={{ cursor: 'pointer' }} onClick={() => props.funcViewPhase(id)} data-toggle='modal' title={titleAction.view}>
              <i className='material-icons'>view_list</i>
            </a>
          )
        case 'delete':
          return (
            <a style={{ cursor: 'pointer' }} onClick={() => props.funcDeletePhase(id)} className='delete' title={titleAction.delete}>
              <i className='material-icons'></i>
            </a>
          )
        default:
      ...
      switch (actionData) {
        case 'edit':
          return (
            <a
              style={{ cursor: 'pointer' }}
              onClick={() => props.funcEditMilestone(id)}
              className='edit'
              data-toggle='modal'
              title={titleAction.edit}
            >
        ...
        case 'view':
          return (
            <a style={{ cursor: 'pointer' }} onClick={() => props.funcViewMilestone(id)} data-toggle='modal' title={titleAction.view}>
              <i className='material-icons'>view_list</i>
            </a>
          )
        case 'delete':
          return (
            <a style={{ cursor: 'pointer' }} onClick={() => props.funcDeleteMilestone(id)} className='delete' title={titleAction.delete}>
              <i className='material-icons'></i>
            </a>
          )
        default:
        ...
        <tbody id='treeTable' className='tree-table-body'>
          {dataTreeTable(column, data).length > 0
            ? dataTreeTable(column, data).map((rows, index) => (
                <tr key={index} data-id={rows._id} data-parent={rows.parent} data-level={rows.level} data-type={rows.type}>
                  {allowSelectAll && (
                    <td>
                          ...
                          Array.isArray(x) ? (
                            <React.Fragment key={index}>
                              ...
                              <div id={`actionTask${rows._id}`} className='collapse'>
                                {x.map((y, index) => (
                                  <React.Fragment key={index}>{showActionColumn(y, rows._id, rows.type)}</React.Fragment>
                                ))}
                              </div>
                            </React.Fragment>
                          ) : (
                            <React.Fragment key={index}>{showActionColumn(x, rows._id, rows.type)}</React.Fragment>
                          )
                        )}
                    </td>
                  )}
...
const projectTreeTable = React.memo(connect(mapState, null)(withTranslate(ProjectTreeTable)), areEqual)

export { projectTreeTable as ProjectTreeTable }
