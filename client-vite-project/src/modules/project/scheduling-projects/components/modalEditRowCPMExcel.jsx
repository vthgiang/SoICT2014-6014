...
const ModalEditRowCPMExcel = (props) => {
  ...
  if (currentRow.code !== currentRowCode) {
    ...
    setCurrentResWeightArr(
      currentRow?.currentResWeightArr ||
        currentRow?.currentResponsibleEmployees?.map((resItem) => {
          return {
            userId: resItem,
    ...
    setCurrentAccWeightArr(
      currentRow?.currentAccWeightArr ||
        currentRow?.currentAccountableEmployees?.map((accItem) => {
          return {
            userId: accItem,
  ...
  const isFormValidated = useMemo(() => {
    return (
      currentAccountableEmployees.length > 0 &&
      currentResponsibleEmployees.length > 0 &&
  ...
  useEffect(() => {
    ...
    if (
      currentEstimateNormalTime.toString()?.length === 0 ||
  ...
  return (
    <React.Fragment>
      <DialogModal
        modalID={`modal-edit-row-cpm-excel-${currentRow.code}`}
        isLoading={false}
            ...
            <fieldset className='scheduler-border' style={{ lineHeight: 1.5 }}>
                ...
                <div className='col-md-12'>
                  <div className='form-horizontal'>
                    <div className={`form-group  ${error.errorOnNormalTime === undefined ? '' : 'has-error'}`}>
                      ...
                      <div className='col-sm-8'>
                        <input type='number' className='form-control' value={currentEstimateNormalTime} onChange={handleChangeNormalTime} />
              ...
              <div className='row'>
                ...
                <div className={`col-md-12 form-group ${error.errorOnAssetCode === undefined ? '' : 'has-error'}`}>
                  <label className='control-label'>
                    Chi phí ước lượng tài sản<span className='text-red'>*</span> (VND)
                  </label>
                  <input
                    type='text'
                    className='form-control'
              ...
              <div className='row'>
                <div className='col-md-12 form-group'>
                  <label className='control-label'>
                    Chi phí thoả hiệp tối đa<span className='text-red'>*</span> (VND)
                  </label>
                  <input
                    type='text'
                    className='form-control'
              ...
              <div className='row'>
                ...
                <div className={`col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group`}>
                  <label className='control-label'>
                    Chi phí ước lượng nhân sự <span className='text-red'>*</span> (VND)
                  </label>
                  <div className='form-control'>{currentHumanCost}</div>
                </div>
                {/* Chi phí ước lượng tổng quan */}
                <div
                  className={`col-lg-6 col-md-6 col-sm-12 col-xs-12 form-group ${Number(currentEstimateNormalCost.replace(/,/g, '')) > Number(currentEstimateMaxCost.replace(/,/g, '')) ? 'has-error' : ''}`}
                >
                  <label className='control-label'>
                    Chi phí ước lượng tổng quan <span className='text-red'>*</span> (VND)
                  </label>
                  <div className='form-control'>{currentEstimateNormalCost}</div>
                  <ErrorLabel
                    content={
                      Number(currentEstimateNormalCost.replace(/,/g, '')) > Number(currentEstimateMaxCost.replace(/,/g, '')) &&
          ...
          <div className='col-xs-12 col-sm-6 col-md-6'>
            <fieldset className='scheduler-border' style={{ lineHeight: 1.5 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
                ...
                <div
                  className={`col-md-12 ${error.errorOnTotalWeight === undefined ? '' : 'has-error'}`}
