...
class Tree extends Component {
  ...
  componentDidMount() {
    ...
    window.$(`#${id}`).on('check_node.jstree', function (e, data) {
      if (checkNode) checkNode(e, data)
    })

    window.$(`#${id}`).on('uncheck_node.jstree', function (e, data) {
      if (unCheckNode) unCheckNode(e, data)
    })
  }
  ...
  render() {
    ...
    return <React.Fragment>{id !== undefined ? <div id={`${id}`} className={className} style={style} /> : null}</React.Fragment>
  }
}
