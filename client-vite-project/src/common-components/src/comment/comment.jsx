...
import { ContentMaker, ApiImage } from "../../index";
...
class Comment extends Component {
  ...
  editComment = async (e, description, commentId, dataId) => {
    e.preventDefault();
    const { newCommentEdited } = this.state;
  ...
  editChildComment = async (
    ...
    e.preventDefault();
    const { newChildCommentEdited } = this.state;
    const data = new FormData();
  ...
  submitComment = async (dataId) => {
    ...
    this.props.currentTask &&
      data.append("currentTask", this.props.currentTask);
    comment.files &&
      comment.files.forEach((x) => {
        data.append("files", x);
      });
  ...
  submitChildComment = async (dataId, commentId) => {
    ...
    childComment[`${commentId}`].files &&
      childComment[`${commentId}`].files.forEach((x) => {
        data.append("files", x);
      });
    if (
      childComment[`${commentId}`].creator &&
      childComment[`${commentId}`].description
    ) {
  ...
  save = () => {
    ...
    if (deleteFile.type === "comment") {
      this.props.deleteFileComment(
        deleteFile.fileId,
        deleteFile.commentId,
        deleteFile.dataId,
      );
    } else if (deleteFile.type === "childComment") {
      this.props.deleteFileChildComment(
        deleteFile.fileId,
        deleteFile.childCommentId,
        deleteFile.commentId,
        deleteFile.dataId,
      );
    }
      ...
      <>
        {comments && comments
          ? // Hiển thị bình luận của công việc
            comments.map((item) => {
              const listImage = item.files
                .map((elem) => (this.isImage(elem.name) ? elem.url : -1))
                .filter((url) => url !== -1);
              return (
                    ...
                    <>
                      <div className="content-level1">
                        ...
                        {item.description.split("\n").map((x, idx) => {
                          return <span key={item._id + idx}>{parse(x)}</span>;
                        })}
                          ...
                          <div className="btn-group pull-right">
                            ...
                            <ul className="dropdown-menu">
                              ...
                              <li>
                                ...
                                >
                                  {translate(
                                    "task.task_perform.delete_comment",
                                  )}
                                </a>
                              </li>
                            </ul>
                        ...
                        {childrenComments && (
                          <li>
                            <a
                              ...
                              onClick={() =>
                                this.handleShowChildComment(item._id)
                              }
                            >
                              <i className="fa fa-comments-o margin-r-5" />{" "}
                              {translate("task.task_perform.comment")} (
                            ...
                            <li style={{ display: "inline-table" }}>
                              <div>
                                <a
                                  style={{ cursor: "pointer" }}
                                  className="link-black text-sm"
                                  onClick={() => this.handleShowFile(item._id)}
                                >
                                  <b>
                                    <i
                    ...
                    <div className="comment-content-child">
                      {item?.comments?.length > 0 &&
                        item.comments.map((child) => {
                          const listImage = child.files
                            .map((elem) =>
                              this.isImage(elem.name) ? elem.url : -1,
                            )
                            .filter((url) => url !== -1);
                          return (
                            <div key={child._id}>
                              ...
                              {editChildComment === child._id && (
                                <div>
                                  <ContentMaker
                                    ...
                                    text={
                                      newChildCommentEdited.descriptionDefault
                                    }
                                    submitButtonText={translate(
                                      "task.task_perform.save_edit",
                                    )}
                                    cancelButtonText={translate(
                                      "task.task_perform.cancel",
                                    )}
                                    handleEdit={(e) =>
                                      this.handleEditChildComment(e)
                                    }
                                    onTextChange={(value, imgs) => {
                                      this.setState((state) => {
                                        return {
                      ...
                      <div>
                        <img
                          className="user-img-level2"
                          src={process.env.REACT_APP_SERVER + auth.user.avatar}
                          alt="user avatar"
                        />
                        <ContentMaker
                          idQuill={`add-child-comment-${item._id}`}
                          inputCssClass="text-input-level2"
                          controlCssClass="tool-level2"
                          onFilesChange={(files) =>
                            this.onCommentFilesChange(files, item._id)
                          }
                          onFilesError={this.onFilesError}
                          files={childComment[`${item._id}`]?.files}
                          text={childComment[`${item._id}`]?.descriptionDefault}
                          placeholder={translate(
                            "task.task_perform.enter_comment",
                          )}
                          submitButtonText={translate(
                            "task.task_perform.create_comment",
                          )}
                          onTextChange={(value, imgs) => {
                            this.setState((state) => {
                          ...
                          onSubmit={() =>
                            this.submitChildComment(data._id, item._id)
                          }
                        />
                      </div>
                    </div>
