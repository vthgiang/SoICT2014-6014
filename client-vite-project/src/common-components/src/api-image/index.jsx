...
class ApiImage extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.src && nextProps.src !== prevState.src) {
      if (nextProps.src.search(";base64,") < 0) {
        // Cần download ảnh để hiển thị
      ...
      // Ảnh đã ở dạng base64, sẵn sàng hiển thị
      return {
        ...prevState,
        src: nextProps.src,
        image: nextProps.src,
  ...
  shouldComponentUpdate(nextProps, nextState) {
    if (
      this.state.dataStatus === ApiImage.DATA_STATUS.QUERYING &&
      nextProps.auth.showFiles &&
      nextProps.auth.showFiles.find((x) => x.fileName === nextState.src)
    ) {
      // Dữ liệu đã về
      this.setState((state) => {
        return {
          ...state,
          dataStatus: ApiImage.DATA_STATUS.AVAILABLE,
          image: nextProps.auth.showFiles.find(
            (x) => x.fileName === nextState.src,
          ).file,
        };
      });
      return false;
  ...
  showImage = (index, image) => {
    ...
    const haveNextImage = listImage ? index < listImage.length - 1 : false;
    const havePreviousImage = index > 0;
    ...
    if (showImg) {
      Swal.fire({
        html:
          ...
          '<i class="fa fa-long-arrow-left" style="font-size:17px;color:white">' +
          "</i>" +
          "</button>" +
          "&emsp;" +
          '<button class="btn" role="button" style="padding: 3px 15px;background-color: rgb(48, 133, 214);visibility:hidden" id ="showNext">' +
          '<i class="fa fa-long-arrow-right" style="font-size:17px;color: white">' +
          "</i>" +
          "</button>" +
          "<br/>" +
          "<br/>" +
  ...
  handleKeyDown = (e, index) => {
    ...
    if (e.key === "ArrowRight" && index < listImage.length - 1)
      this.showNextImage(index);
    if (
      e.key === "Enter" ||
      e.key === "Escape" ||
  ...
  showNextImage = async (i) => {
    ...
    if (i < listImage.length - 1) {
      const src = listImage[i + 1];
      if (
        src.search(";base64,") < 0 &&
        !this.props.auth.showFiles.find((x) => x.fileName === src).file
      ) {
        await this.props.downloadFile(src, `${src}`, false);
      }
      const image = await this.props.auth.showFiles.find(
        (x) => x.fileName === src,
      ).file;
    ...
    return (
      <>
        <img
          ...
          src={image}
          alt={alt}
          onClick={() => this.showImage(index, this.state.image)}
        />
        {file && (
          <a
