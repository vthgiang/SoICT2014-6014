...
function SmartTable(props) {
  ...
  return (
    <React.Fragment>
      <DataTableSetting tableId={tableId} columnArr={columnArr} setLimit={props.onSetNumberOfRowsPerpage} />
      <table id={tableId} className='table table-striped table-bordered table-hover smart-table' data-toggle='checkboxes' data-range='true'>
        <thead>
          <tr key={`smart-table-head-${tableId}`}>
            ...
            {keys?.length > 0 && keys.map((item) => tableHeaderData?.[item])}
          </tr>
        </thead>
          ...
          {tableBodyData?.length > 0 &&
            tableBodyData.map((data, index) => (
              <tr key={`smart-table-${tableId}${index}`}>
                {!disableCheckbox && (
                  <td>
                    <input type='checkbox' defaultChecked={false} value={data?.id}></input>
                  </td>
                )}
                {keys?.length > 0 && keys.map((item) => data?.[item])}
              </tr>
            ))}
        </tbody>
      </table>
