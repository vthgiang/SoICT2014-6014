...
class Header extends Component {
  ...
  render() {
    ...
    const { oldPasswordError, newPasswordError, confirmPasswordError, password2Error } = this.state
    const { user } = auth
    ...
    return (
      <React.Fragment>
        <header className='main-header'>
          <Link to='/' className='logo'>
            ...
            <span className='logo-lg'>
              <b className='text-white'>DX workplace</b>
            </span>
          </Link>
        ...
        {auth.user.name !== undefined && (
          <ModalChangeUserInformation userId={auth.user._id} userName={auth.user.name} userEmail={auth.user.email} />
        )}
        <ResetPassword2 />
        <CreatePassword2 />
        <DeletePassword2 />
        ...
        >
          <form id='form-security' style={{ padding: '10px 20px 10px 20px' }}>
            ...
            <div className={`form-group ${!newPasswordError ? '' : 'has-error'}`}>
              <label>
                {translate('auth.security.new_password')}
                <span className='text-red'>*</span>
              </label>
            ...
            <div className={`form-group ${!confirmPasswordError ? '' : 'has-error'}`}>
              <label>
                {translate('auth.security.re_enter_new_password')}
                <span className='text-red'>*</span>
              </label>
              <input
                ...
                placeholder={translate('auth.security.re_enter_new_password')}
              />
            ...
            {user && Object.keys(user).length > 0 && user.password2Exists === true ? (
              <div className={`form-group ${!password2Error ? '' : 'has-error'}`}>
                <label style={{ display: 'flex', marginTop: '5px' }}>
                  Mật khẩu cấp 2
                  <span className='text-red' style={{ marginLeft: '5px' }}>
                    *
                  </span>
                  <a onClick={this.handleResetPassword2} style={{ marginLeft: '5px', cursor: 'pointer' }}>
                    Đổi mật khẩu cấp 2
                  </a>
                  <a onClick={this.handleDeletePassword2} style={{ marginLeft: '5px', cursor: 'pointer' }}>
                    Xóa mật khẩu cấp 2
                  </a>
                </label>
              ...
              <label style={{ display: 'flex', marginTop: '5px' }}>
                ...
                <div style={{ display: 'flex' }}>
                  <span>
                    <a onClick={this.handleCreatePassword2} style={{ fontSize: '13px', cursor: 'pointer' }}>
                      Thiết lập
                    </a>
                  </span>
                </div>
